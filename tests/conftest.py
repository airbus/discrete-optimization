        ...
        if populate_nonregression_database:
            ...
            if status in [StatusSolver.OPTIMAL, StatusSolver.SATISFIED]:
                with open(nonregression_database_filepath, "at") as f:
                    f.write(
                        f'{test_id},{status.value},{mode_optim.value},"{objective}"\n'
                    )
            else:
            ...
            if previous_status == StatusSolver.OPTIMAL:
                ...
                assert status in [StatusSolver.OPTIMAL, StatusSolver.SATISFIED]
                ...
                if isinstance(objective, tuple):
                    ...
                    i = 0
                    while (
                        objective[i] == previous_objective[i] and i < len(objective) - 1
                    ):
                        i += 1
                    ...
                    if objective[i] < previous_objective[
                        i
                    ] - nonregression_margin * abs(previous_objective[i]):
                        warnings.warn(
