...
def test_parsing_sm():
    ...
    assert rcpsp_problem.n_jobs == 32
    assert rcpsp_problem.resources_list == ["R1", "R2", "R3", "R4"]
    assert rcpsp_problem.successors == {
        1: [2, 3, 4],
        2: [6, 11, 15],
        3: [7, 8, 13],
        4: [5, 9, 10],
    ...
    assert rcpsp_problem.mode_details == {
    ...
    assert rcpsp_problem.resources == {"R1": 12, "R2": 13, "R3": 4, "R4": 12}
    assert rcpsp_problem.non_renewable_resources == []
    assert rcpsp_problem.horizon == 158
...
def test_parsing_mm():
    ...
    rcpsp_problem = parse_file(file_path)
    assert rcpsp_problem.n_jobs == 12
    assert rcpsp_problem.resources_list == ["R1", "R2", "N1", "N2"]
    assert rcpsp_problem.successors == {
        1: [2, 3, 4],
        2: [10, 11],
        3: [5, 9],
    ...
    assert rcpsp_problem.resources == {"R1": 10, "R2": 13, "N1": 29, "N2": 30}
    assert rcpsp_problem.non_renewable_resources == ["N1", "N2"]
    assert rcpsp_problem.horizon == 77
...
def test_parsing_rcp():
    ...
    assert len(rcpsp_problem.resources_list) == 4
    assert all(rcpsp_problem.resources[r] == 10 for r in rcpsp_problem.resources_list)
    assert rcpsp_problem.source_task == 1
    assert rcpsp_problem.sink_task == 32
    assert len(rcpsp_problem.successors[rcpsp_problem.source_task]) == 18
    successors_source = [
        2,
        3,
        4,
    ...
    assert all(
        s in rcpsp_problem.successors[rcpsp_problem.source_task]
        for s in successors_source
    )
