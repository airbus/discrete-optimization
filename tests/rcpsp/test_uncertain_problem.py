...
def test_uncertain_durations_sm():
    ...
    worst_model = uncertain_model.create_rcpsp_problem(
        MethodRobustification(
            method_base=MethodBaseRobustification.WORST_CASE, percentile=0
        )
    )
    best_model = uncertain_model.create_rcpsp_problem(
        MethodRobustification(
            method_base=MethodBaseRobustification.BEST_CASE, percentile=0
        )
    )
    average_model = uncertain_model.create_rcpsp_problem(
        MethodRobustification(
            method_base=MethodBaseRobustification.AVERAGE, percentile=0
        )
    )
    sample_model = uncertain_model.create_rcpsp_problem(
        MethodRobustification(
            method_base=MethodBaseRobustification.SAMPLE, percentile=0
        )
    )
    percentile_model = uncertain_model.create_rcpsp_problem(
        MethodRobustification(
...
def test_uncertain_resources_sm():
    ...
    file_path = files[0]
    rcpsp_problem: RcpspProblem = parse_file(file_path)
    poisson_laws = create_poisson_laws_resource(rcpsp_problem)
    uncertain_model: UncertainRcpspProblem = UncertainRcpspProblem(
