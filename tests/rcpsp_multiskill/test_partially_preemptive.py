...
def test_partial_preemptive(model):
    model_variant: VariantMultiskillRcpspProblem = model.to_variant_model()
    ...
    plot_resource_individual_gantt_preemptive(
        rcpsp_problem=model, rcpsp_sol=dummy_solution
    )
    plot_ressource_view(rcpsp_problem=model, rcpsp_sol=dummy_solution)
    cp_solver = CpPartialPreemptiveMultiskillRcpspSolver(problem=model)
    ...
    plot_ressource_view(rcpsp_problem=model, rcpsp_sol=rcpsp_sol)


def test_ls(model):
    model_variant: VariantMultiskillRcpspProblem = model.to_variant_model()
    solver = LsGenericRcpspSolver(problem=model_variant, ls_solver=LsSolverType.SA)
