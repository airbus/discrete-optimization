...
def run_lexico():
    ...
    from discrete_optimization.generic_tools.callbacks.sequential_solvers_callback import RetrieveSubRes
    from discrete_optimization.generic_tools.callbacks.stats_retrievers import StatsCpsatCallback
    retrieve_sub_res = RetrieveSubRes()
    stats_cb = StatsCpsatCallback()
    res = lexico.solve(callbacks=[retrieve_sub_res],
                       objectives=["nb_teams", 'duration'],
                       subsolver_callbacks=[stats_cb],
                       time_limit=5)
    fig, ax = plt.subplots(2)
    fits = [[allocation_problem.evaluate(s) for s in res_]
