...
def script():
    ...
    for method in methods:
        ...
        for submethod in solvers[method]:
            print(submethod[0])
            t = time.time()
            solution = solve(submethod[0], rcpsp_problem, **submethod[1])
...
def script_choose_solver():
    ...
    file_path = files[0]
    rcpsp_problem: RcpspProblem = parse_file(file_path)
    solvers_choice = look_for_solver(rcpsp_problem)
    while True:
        ...
        if choice == 0:  # the user wants to quit
            ...
            solution = solve(
                selected_solver, rcpsp_problem, **solvers_map[selected_solver][1]
            )
            print("Solver ran ! ")
