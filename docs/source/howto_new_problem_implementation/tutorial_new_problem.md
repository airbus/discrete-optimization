...
To be able to [apply existing local search algorithms](#apply-a-generic-solver-local-search) from the library to our problem,
...
The complete resulting python module with creation of problem, solutions and evaluation and satisfiability checks can be found here: <path:tutorial_new_problem.py>.
...
The simulated annealing needs a mutation to apply at each step.
...
and should be run in the same directory as the previous module <path:tutorial_new_problem.py> so that you can import your new classes.
