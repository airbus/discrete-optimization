...
Note that we implement also `lazy_copy()` which defaults to `copy` but here avoid a deep copy of `list_taken` to improve
...
class MyKnapsackSolution(Solution):
    ...
    def __init__(self, problem: MyKnapsackProblem, list_taken: list[bool]):
        ...
        self._total_weight: Optional[int] = (
            None  # cache total weight to avoid computing it twice (for problem.satisfy and problem.evaluate)
        )
    ...
    def change_problem(self, new_problem: Problem) -> None:
        """Change problem instance (e.g. to be evaluated against it)"""
        if not isinstance(new_problem, MyKnapsackProblem):
            raise ValueError("new_problem must be of type `MyKnapsackProblem`")
        self.problem = new_problem
        self._total_weight = None  # reset total weight which depends on chosen problem

    def compute_total_weight(self) -> int:
        ...
        if self._total_weight is None:
            self._total_weight = sum(
                taken * weight
                for taken, (value, weight) in zip(self.list_taken, self.problem.items)
            )
        return self._total_weight

    def compute_total_value(self) -> int:
...
class MyKnapsackProblem(Problem):
    ...
    def get_attribute_register(self) -> EncodingRegister:
        ...
        To be used by evoluationary solvers to choose the appropriate mutations
    ...
    def evaluate(self, variable: Solution) -> dict[str, float]:
        ...
        value = variable.compute_total_value()
        weight = variable.compute_total_weight()
    ...
    def satisfy(self, variable: Solution) -> bool:
        ...
        if not isinstance(variable, MyKnapsackSolution):
            return False
        return variable.compute_total_weight() <= self.max_capacity
```
