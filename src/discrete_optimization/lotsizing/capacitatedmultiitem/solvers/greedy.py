...
class GreedyLotSizingSolver(SolverDO):
    ...
    These solutions can be used as starting points for local search or other metaheuristic methods.
    ...
    def _earliest_demand_first(self) -> CapacitatedMultiItemSolution:
        ...
        """
        list_item_per_time = [self.problem.nb_items] * self.problem.horizon
        ...
        for item in self.problem.items_list:
            ...
            total_demand[item] = sum(demands)
            cumul_demand[item] = np.cumsum(demands)

        # Track cumulative production for each item
        cumul_prod = {item: 0 for item in self.problem.items_list}

        # Get capacity per period
        capacity = int(self.problem.get_available_production_time(0))
    ...
    def _minimum_changeover(self) -> CapacitatedMultiItemSolution:
        ...
        required_time = {
            item: int(np.ceil(total_demand[item] / capacity))
            for item in self.problem.items_list
        }
        ...
        for item in self.problem.items_list:
            for t in range(self.problem.horizon):
                if self.problem.get_demand(item, t) > 0:
    ...
    def _just_in_time(self) -> CapacitatedMultiItemSolution:
        ...
        remaining_to_produce = {
            item: total_demand[item] for item in self.problem.items_list
        }
        ...
        for t in range(self.problem.horizon - 1, -1, -1):
            ...
            if best_item is not None and max_remaining > 0:
                ...
                remaining_to_produce[best_item] -= min(
                    capacity, remaining_to_produce[best_item]
                )
    ...
    def _balanced(self) -> CapacitatedMultiItemSolution:
        ...
        # Calculate average stock cost rate for each item
        stock_cost_rate = {}
        for item in self.problem.items_list:
            costs = [
            ...
            stock_cost_rate[item] = np.mean(costs)
        ...
        for t in range(self.problem.horizon):
            ...
            for item in self.problem.items_list:
                ...
                required_periods = int(np.ceil(remaining_demand[item] / capacity))
            ...
            if best_item is not None:
                ...
                remaining_demand[best_item] -= min(
                    capacity, remaining_demand[best_item]
                )
                last_item = best_item
            else:
...
def greedy_best(problem: CapacitatedMultiItemLSP) -> CapacitatedMultiItemSolution:
