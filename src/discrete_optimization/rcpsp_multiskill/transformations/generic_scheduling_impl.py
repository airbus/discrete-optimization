...
def transform_solution_from_raw_generic_to_rcpsp_ms(
    raw_sol: RawSolution[Task, UnaryResource, Skill], problem: MultiskillRcpspProblem
) -> MultiskillRcpspSolution:
...
class MultiskillRcpspToGenericSchedulingTransformation(
    ToGenericSchedulingImpl[MultiskillRcpspProblem, MultiskillRcpspSolution]
):
    ...
    def transform_solution_from_raw_generic_to_specific(
        ...
        source_problem: MultiskillRcpspProblem,
    ) -> MultiskillRcpspSolution:
        return transform_solution_from_raw_generic_to_rcpsp_ms(
...
class GenericSchedulingToMultiskillRcpspTransformation(
    FromGenericSchedulingImpl[MultiskillRcpspProblem, MultiskillRcpspSolution]
):
    ...
    def transform_problem(
        ...
        resources_availability: dict[str, list[int]] = {
            str(resource): [
                source_problem.get_non_renewable_resource_capacity(resource=resource)
            ]
        ...
        resources_set: set[str] = set(resources_availability)
        skills_set: set[str] = {str(skill) for skill in source_problem.skills_list}
        employees: dict[UnaryResource, Employee] = {
            unary_resource: Employee(
                dict_skill={
                    str(skill): SkillDetail(
                        skill_value=value, efficiency_ratio=1, experience=0
                    )
                ...
                calendar_employee=[
                    bool(value)
                    for value in source_problem.get_resource_calendar(
        ...
        return MultiskillRcpspProblem(
            ...
            only_one_skill_per_task=source_problem.only_one_skill_per_task,
        )
