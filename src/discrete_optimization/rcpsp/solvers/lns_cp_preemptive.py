...
class PostProLeftShift(PostProcessSolution):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(problem, SpecialConstraintsPreemptiveRcpspProblem):
    ...
    def build_other_solution(self, result_storage: ResultStorage) -> ResultStorage:
        ...
        if self.do_ls:
            solver = LsGenericRcpspSolver(
                problem=self.problem, ls_solver=LsSolverType.SA
            )
...
def sgs_variant(
    ...
    while True:
        ...
        if isinstance(problem, SpecialConstraintsPreemptiveRcpspProblem):
            ...
            times_predecessors += [
                new_proposed_schedule[t]["starts"][0]
                + problem.special_constraints.dict_start_to_start_min_time_lag_reverse.get(
                    task
                )[t]
                if t in new_proposed_schedule
                else solution.rcpsp_schedule[t]["starts"][0]
        ...
        if len(sorted_tasks) == 0:
            break
    new_solution = SpecialPreemptiveRcpspSolution(
        problem=problem,
