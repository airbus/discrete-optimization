...
% upper bound: total load must fit in Freq trips
&sum{S*load(F, T, TR, P) : flow(F, T, TR, P),productSizeSmall(P,S)} <= TotalCap :-
routeFreq(F, T, TR, Freq, TotalCap).

% lower bound: total load must NOT fit in (Freq-1) trips
% This ensures we use the minimum number of trips needed
...
class ClingconMultibatchingSolver(SolverDO):
    ...
    def processModelAtoms(self, model: clingo.Model):
        ...
        for name in [next(iter(d.values())) for d in self.modelAtomTemplates]:
            path_symbols += list(
                filter(lambda s: s.name == name, model.symbols(atoms=True))
            )
    ...
    def solve(
        ...
        **kwargs: Any,
        ...
        def on_model_wrapper(model: clingo.Model):
            self.modelIndex += 1
            logger.info(
                f"Model [{self.modelIndex}] found after {round((time.process_time() - self.start_solving) / 60, 2)} minutes of solving."
            )
    ...
    def _generate_facts(
        ...
        if valid_links_map is not None:
            ...
            possibleFlow_file = os.path.join(
                os.path.dirname(__file__), "possibleFlows_shortestPath.lp"
            )
    ...
    def compute_active_flows(self):
        ...
        print(
            f"Total active flows including frequencies for model: {route_freq_df['freq'].sum()}"
        )
    ...
    def compute_costs(self):
        ...
        transportCost_df["transportationCost"] = transportCost_df[
            "transportationCost"
        ].values
        transportCost_df["transportationCost"] = transportCost_df[
            "transportationCost"
        ].apply(self.extract_clingo_number)
        ...
        merged_df1 = pd.merge(
            route_freq_df, route_df, on=["from", "to", "transportresource"], how="inner"
        )

        merged_df1["total_transportation_costs"] = (
            merged_df1["freq"] * merged_df1["cost"]
        )

        merged_df2 = pd.merge(
            merged_df1, co2Cost_df, on=["transportresource"], how="inner"
        )

        merged_df2["total_co2_costs"] = (
            merged_df2["freq"] * merged_df2["CO2"] * merged_df2["distance"]
        )
        ...
        print(
            "total costs:",
            total_costs,
            f"({total_costs:.2e}) [transport: {transport_costs} ({transport_costs:.2e}), emission: {co2_costs}, ({co2_costs:.2e})]",
        )
