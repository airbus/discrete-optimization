...
class MisKamisSolver(MisSolver):
    ...
    def init_model(self):
        ...
        if not os.path.exists(tmp_dir):
            os.makedirs(tmp_dir)
        lines_to_write = []
        lines_to_write += [f"{self.problem.number_nodes} {len(self.problem.edges)} {11} \n"]
        nb_edge = 0
        for i in range(self.problem.number_nodes):
            node = self.problem.index_to_nodes[i]
            neighs = sorted(self.problem.graph.neighbors(node), key=lambda x: self.problem.nodes_to_index[x])
            s = " ".join([f"{int(self.problem.attr_list[i])} " + f"{self.problem.nodes_to_index[x] + 1}"
                          for x in neighs])
            nb_edge += len(neighs)
            lines_to_write += [s + "\n"]
    ...
    def solve(self, **kwargs) -> ResultStorage:
        ...
        if folder_ is None:
            warnings.warn('The environment variable "KAMIS_DEPLOY" is not define, you need to define it as the path to the folder where are Kamis executable')

        time_limit = kwargs.get("time_limit", 60)
        method = kwargs.get("method", "weighted")

        if method not in methods:
            logger.warning(f"{method} is not available, please use one of : {list(methods.keys())}")
        ...
        command = (f"{methods[method]} {self.current_file} "
                   f"--output {output} --time_limit={time_limit} --console_log")

        # Launch the command and wait for it to finish
        print("Launching command")
        try:
            result = subprocess.run(command, shell=True, check=True, timeout=time_limit + 50)
            chosen = []
            with open(output, "r") as file:
                for line in file:
                    # Convert each line to an integer and append to the list
                    number = int(line.strip())  # Remove leading/trailing whitespaces and convert to int
            ...
            print("A problem has been encountered, exceptionally Kamis ignored time-out")
