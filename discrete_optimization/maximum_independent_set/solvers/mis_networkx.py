...
class MisNetworkXSolver(MisSolver):
    ...
    def solve(self, callbacks: Optional[List[Callback]] = None, **kwargs: Any) -> ResultStorage:
        ...
        chosen = [1 if self.problem.nodes[i] in sol else 0 for i in range(self.problem.number_nodes)]
        solution = MisSolution(problem=self.problem, chosen=chosen)
        fit = self.aggreg_from_sol(solution)
        return ResultStorage([(solution, fit)], mode_optim=self.params_objective_function.sense_function)