...
from discrete_optimization.knapsack.knapsack_model import KnapsackModel, KnapsackSolution, \
    MultidimensionalKnapsack, KnapsackSolutionMultidimensional, MultiScenarioMultidimensionalKnapsack
...
class CPMultidimensionalSolver(CPSolver):
    def __init__(self, knapsack_model: MultidimensionalKnapsack,
                 cp_solver_name: CPSolverName = CPSolverName.CHUFFED,
                 params_objective_function: ParamsObjectiveFunction = None):
        self.knapsack_model = knapsack_model
        self.instance = None
    ...
    def init_model(self, **args):
        model = Model(os.path.join(this_path, "../minizinc/multidimension_knapsack.mzn"))
        solver = Solver.lookup(map_cp_solver_name[self.cp_solver_name])
        ...
        instance["values"] = [int(self.knapsack_model.list_items[i].value)
                              for i in range(self.knapsack_model.nb_items)]
        instance["weights"] = [[self.knapsack_model.list_items[i].weights[j] for j in range(instance["nb_dimension"])]
                               for i in range(self.knapsack_model.nb_items)]
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP) -> ResultStorage:
        ...
        for taken, objective in zip(l_taken, objectives):
            print(objective)
            sol = KnapsackSolutionMultidimensional(problem=self.knapsack_model,
                                                   list_taken=taken)
            fit = self.aggreg_sol(sol)
            print(fit)
            list_solutions_fit += [(sol, fit)]
        return ResultStorage(list_solution_fits=list_solutions_fit,
                             best_solution=None,
                             mode_optim=self.params_objective_function.sense_function)
...
class CPMultidimensionalMultiScenarioSolver(CPSolver):
    def __init__(self, knapsack_model: MultiScenarioMultidimensionalKnapsack,
                 cp_solver_name: CPSolverName = CPSolverName.CHUFFED,
                 params_objective_function: ParamsObjectiveFunction = None):
        self.knapsack_model = knapsack_model
        self.instance = None
    ...
    def init_model(self, **args):
        model = Model(os.path.join(this_path, "../minizinc/multidim_multiscenario_knapsack.mzn"))
        solver = Solver.lookup(map_cp_solver_name[self.cp_solver_name])
        custom_output_type = args.get("output_type", False)
        if custom_output_type:
            model.output_type = KnapsackSol
        ...
        instance["values"] = [[int(list_problems[j].list_items[i].value) for j in range(instance["nb_scenario"])]
                              for i in range(instance["nb_items"])]
        instance["weights"] = [[[list_problems[k].list_items[i].weights[j] for k in range(instance["nb_scenario"])]
                                for j in range(instance["nb_dimension"])]
                               for i in range(instance["nb_items"])]
        instance["max_capacity"] = [[list_problems[s].max_capacities[k] for s in range(instance["nb_scenario"])]
...
from discrete_optimization.generic_tools.lns_cp import ConstraintHandler, LNS_CP, InitialSolution
...
class KnapConstraintHandler(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: CPMultidimensionalMultiScenarioSolver, child_instance,
        ...
        for i in range_item:
            if i in subpart_item:
                strings += ["constraint taken[" + str(i + 1) + "] == "
                             + str(1 if current_best_solution.list_taken[i] else 0) + ";\n"]
                child_instance.add_string(strings[-1])
        return strings

    def remove_constraints_from_previous_iteration(self, cp_solver: MultiScenarioMultidimensionalKnapsack,
                                                   child_instance,
