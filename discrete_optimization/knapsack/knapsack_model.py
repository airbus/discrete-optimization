...
from discrete_optimization.generic_tools.do_problem import Problem, Solution, EncodingRegister, \
    TypeAttribute, ObjectiveRegister, TypeObjective, ObjectiveHandling, TupleFitness, ModeOptim, RobustProblem, \
    MethodAggregating
...
class KnapsackSolutionMultidimensional(Solution):
    ...
    def copy(self):
        return KnapsackSolutionMultidimensional(problem=self.problem,
                                                value=self.value,
    ...
    def lazy_copy(self):
        return KnapsackSolutionMultidimensional(problem=self.problem,
                                                value=self.value, weights=self.weights,
                                                list_taken=self.list_taken)
...
class ItemMultidimensional(NamedTuple):
    ...
    def __str__(self):
        return "ind: "+str(self.index)+" weights: "+str(self.weights)+" value: "+str(self.value)
...
class MultidimensionalKnapsack(Problem):
    ...
    def evaluate_from_encoding(self, int_vector, encoding_name):
        if encoding_name == 'list_taken':
            kp_sol = KnapsackSolutionMultidimensional(problem=self, list_taken=int_vector)
        elif encoding_name == 'custom':
            kwargs = {encoding_name: int_vector, 'problem': self}
    ...
    def evaluate_value(self, knapsack_solution: KnapsackSolutionMultidimensional):
        s = sum([knapsack_solution.list_taken[i] * self.list_items[i].value
                 for i in range(self.nb_items)])
        w = [sum([knapsack_solution.list_taken[i] * self.list_items[i].weights[j]
                 for i in range(self.nb_items)])
    ...
    def evaluate_weight_violation(self, knapsack_solution: KnapsackSolutionMultidimensional):
        return sum([max(0., knapsack_solution.weights[j] - self.max_capacities[j])
                    for j in range(len(self.max_capacities))])

    def satisfy(self, knapsack_solution: KnapsackSolutionMultidimensional):
        if knapsack_solution.value is None:
            self.evaluate(knapsack_solution)
        return all(knapsack_solution.weights[j] <= self.max_capacities[j] for j in range(len(self.max_capacities)))

    def __str__(self):
        s = "Knapsack model with " + str(self.nb_items) + " items and capacity " + str(self.max_capacities) + '\n'
        s += "\n".join([str(item) for item in self.list_items])
        return s

    def get_dummy_solution(self):
        kp_sol = KnapsackSolutionMultidimensional(problem=self, list_taken=[0] * self.nb_items)
        self.evaluate(kp_sol)
        return kp_sol
    ...
    def copy(self):
        return MultidimensionalKnapsack(list_items=[deepcopy(x) for x in self.list_items],
...
def from_kp_to_multi(knapsack_model: KnapsackModel):
    return MultidimensionalKnapsack(list_items=[ItemMultidimensional(index=x.index, value=x.value,
                                                                     weights=[x.weight])
...
def create_noised_scenario(problem: MultidimensionalKnapsack, nb_scenarios: int = 20):
    ...
    for p in scenarios:
        ...
        for item in p.list_items:
            litem += [ItemMultidimensional(index=item.index,
                                           value=np.random.randint(max(0, int(item.value*0.9)),
                                                                   int(item.value*1.1)),
