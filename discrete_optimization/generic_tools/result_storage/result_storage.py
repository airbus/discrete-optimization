from discrete_optimization.generic_tools.do_problem import Problem, Solution, TupleFitness, \
    ModeOptim, ParamsObjectiveFunction, build_aggreg_function_and_params_objective
...
def merge_results_storage(result_1: ResultStorage, result_2: ResultStorage):
    return ResultStorage(result_1.list_solution_fits+result_2.list_solution_fits,
                         mode_optim=result_1.mode_optim)


def from_solutions_to_result_storage(list_solution: List[Solution], problem: Problem,
                                     params_objective_function: ParamsObjectiveFunction = None):
    aggreg_sol, aggreg_dict, params_objective_function = \
        build_aggreg_function_and_params_objective(problem=problem,
                                                   params_objective_function=params_objective_function)
    list_solution_fit = []
    for s in list_solution:
        list_solution_fit += [(s, aggreg_sol(s))]
    return ResultStorage(list_solution_fits=list_solution_fit,
