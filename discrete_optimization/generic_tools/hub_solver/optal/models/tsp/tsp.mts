...
function defineModelAndVarsJson(filename:string): ModelWithVariables {
  ...
  const nodeCount = data.node_count as number;
  const startIndex = data.start_index as number;
  const endIndex = data.end_index as number;
  const distanceMatrix = data.distance_matrix as number[][];

  // 2. Create an interval variable for each node visit
  const visitIntervals = Array.from({ length: nodeCount }, (_, i) =>
    model.intervalVar({ length: 0, name: `Visit_${i}` }) // Standard TSP has zero visit duration
  );
  ...
  visitIntervals[startIndex].setStart(0);
  ...
  for (let i = 0; i < nodesInSequence.length; i++) {
    model.endBeforeStart(visitIntervals[startIndex], visitIntervals[nodesInSequence[i]],
                         distanceMatrix[startIndex][nodesInSequence[i]]);
    model.endBeforeStart(visitIntervals[nodesInSequence[i]], tourEnd, distanceMatrix[nodesInSequence[i]][endIndex]);
  }
  // 5. Set the objective
