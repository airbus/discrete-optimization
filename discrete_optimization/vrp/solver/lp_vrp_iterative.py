...
from discrete_optimization.vrp.vrp_toolbox import Customer
...
def build_matrice_distance_np(customer_count: int, 
    ...
    print("matrix init")
    # id_x = np.zeros((customer_count, customer_count), dtype=np.int32)
    # id_y = np.zeros((customer_count, customer_count), dtype=np.int32)
...
def build_graph_pruned_vrp(vrp_problem: VrpProblem):
    ...
    g = nx.DiGraph()
    g.add_nodes_from([(v, i) for i in range(customer_count)
    ...
    for i in range(shape):
        ...
        for n in nodes_to_add:
            for v in range(vehicle_count):
                ...
                edges_out_customers[n].add((node_2, node_1))
        for v in range(vehicle_count):
            nodes_to_add = [vrp_problem.start_indexes[v],
    ...
    g_empty = nx.DiGraph()
    g_empty.add_nodes_from([(v, i) for i in range(customer_count)
                            for v in range(vehicle_count)])
    return g, g_empty, edges_in_customers, edges_out_customers, edges_in_merged_graph, edges_out_merged_graph


# def build_graph_pruned_v2(customers: List[Customer],
#                           vehicle_count: int,
...
#     g.add_nodes_from([(v, i) for i in range(customer_count)
#                      for v in range(vehicle_count)])
#     g.add_nodes_from([(v, 0) for i in range(customer_count)
...
#                            weight=length(customers[i], customers[n]),
#                            demand=customers[n].demand)
#                 g.add_edge(node_2, node_1, weight=length(customers[i], customers[n]),
#                            demand=customers[i].demand)
#                 edges_in_merged_graph[node_2].add((node_1, node_2))
#                 edges_out_merged_graph[node_1].add((node_1, node_2))
#                 edges_in_merged_graph[node_1].add((node_2, node_1))
...
def compute_start_end_flows_info(start_indexes: List[int],
...
def init_model_lp(g,
    ...
    for e in edges:
        ...
        if e in edges_to_constraint:
            constraint_on_edge[iedge] = tsp_model.addConstr(x_var[e] == val,
                                                            name=str((e, iedge)))
            iedge += 1
        ...
        for edge in edges:
            if (edge[1], edge[0]) in edges:
                ...
                if edge[0][1] == end_indexes[edge[0][0]] or edge[1][1] == end_indexes[edge[0][0]]:
                    continue
                constraint_tour_2length[cnt_tour] = tsp_model.addConstr(x_var[edge]+x_var[(edge[1],
                                                                                           edge[0])] <= 1,
                                                                        name="tour_"+str(edge))
                cnt_tour += 1
    if include_triangle:
    ...
    for s in start_to_i:
        for vehicle in start_to_i[s]["vehicle"]:
            constraint_flow_out["start_v_"+str(vehicle)] = \
                tsp_model.addConstr(quicksum([x_var[e]
    ...
    for customer in edges_in_customers:
        if customer in end_to_i or customer in start_to_i:
            ...
            constraint_flow_in[customer] = tsp_model.addConstr(quicksum([x_var[e]
    ...
    for n in edges_in_merged_graph:
        if start_indexes[n[0]] == end_indexes[n[0]] \
            ...
            c_flow[n] = tsp_model.addConstr(quicksum([x_var[e] for e in edges_in_merged_graph[n]]
                                                     + [-x_var[e] for e in edges_out_merged_graph[n]]) == 0,
                                            name="flow_"+str(n))
    for v in range(vehicle_count):
        tsp_model.addConstr(quicksum([g[e[0]][e[1]]["demand"]*x_var[e]
...
class VRPIterativeLP(SolverDO):
    def __init__(self, problem: VrpProblem, params_objective_function: ParamsObjectiveFunction):
        ...
        self.aggreg_sol, self.aggreg_fit, self.params_objective_function = \
    ...
    def solve(self, **kwargs):
        ...
        while not finished:
            ...
            obj, components, components_global, component_all, component_global_all = \
                reevaluate_solutions(solutions_ll, vehicle_count, g, vrp_problem=self.problem)
            ...
            if len(component_global_all[0]) > 1 or True:
                ...
                if all((e in edges) for e in edges_to_add):
                    ...
                    for e in x_var:
                        ...
                        if e in edges_to_constraint:
                            if do_lns:
                                self.constraint_on_edge[iedge] = self.model.addConstr(x_var[e] == val,
                                                                                      name=str((e, iedge)))
                                iedge += 1
                else:
                    pass
            ...
            # json.dump({v: [int(x[1]) for x in rebuilt_solution[best_solution_rebuilt_index][v]]
            #            for v in rebuilt_solution[best_solution_rebuilt_index]}, open(name_json, "w"))
