from discrete_optimization.vrp.vrp_model import VrpProblem, VrpSolution, VrpProblem2D
from discrete_optimization.generic_tools.do_mutation import Mutation, LocalMoveDefault, LocalMove
...
class RelocateMove(LocalMove):
    ...
    def apply_local_move(self, solution: VrpSolution) -> Solution:
        self.index_from = min([self.index_from, len(solution.list_paths[self.index_vehicle_from])-1])
        self.index_to = min([self.index_to, len(solution.list_paths[self.index_vehicle_to])])
        city = solution.list_paths[self.index_vehicle_from][self.index_from]
        solution.capacities[self.index_vehicle_from] -= solution.problem.customers[city].demand
        solution.capacities[self.index_vehicle_to] += solution.problem.customers[city].demand
        solution.list_paths[self.index_vehicle_to].insert(self.index_to, city)
        previous_from_length = (solution.lengths[self.index_vehicle_from][self.index_from],
        ...
        new_length_vehicle_from = solution.problem.evaluate_function_indexes(index_1=
                                                                             get_index_previous(self.index_vehicle_from,
                                                                                                self.index_from),
                                                                             index_2=get_index_next(self.index_vehicle_from,
                                                                                                    self.index_from))
        ...
        solution.lengths[self.index_vehicle_from].insert(self.index_from, new_length_vehicle_from)
        # include new length

        previous_length_vehicle_to = solution.lengths[self.index_vehicle_to][self.index_to]
        solution.lengths[self.index_vehicle_to].pop(self.index_to)

        new_length_vehicle_to_1 = solution.problem.evaluate_function_indexes(index_1=
                                                                             get_index_previous(self.index_vehicle_to,
                                                                                                self.index_to),
                                                                             index_2=city)
        solution.lengths[self.index_vehicle_to].insert(self.index_to, new_length_vehicle_to_1)
        new_length_vehicle_to_2 = solution.problem.evaluate_function_indexes(index_1=
                                                                             city,
                                                                             index_2=get_index_next(
                                                                                 self.index_vehicle_to,
                                                                                 self.index_to))
        solution.lengths[self.index_vehicle_to].insert(self.index_to+1, new_length_vehicle_to_2)
        delta = new_length_vehicle_from+new_length_vehicle_to_1+new_length_vehicle_to_2\
            - previous_length_vehicle_to - sum(previous_from_length)
        solution.length = solution.length + delta

        return solution

    def backtrack_local_move(self, solution: VrpSolution) -> Solution:
        move = RelocateMove(index_vehicle_from=self.index_vehicle_to,
...
class MutationRelocate(Mutation):
    ...
    def mutate(self, solution: VrpSolution) -> Tuple[Solution, LocalMove]:
        vehicles_used = [v for v in range(self.vrp_model.vehicle_count)
                         if len(solution.list_paths[v]) > 0]
        some_vehicle = random.choice(vehicles_used)
        some_other_vehicle = random.choice([v for v in range(self.vrp_model.vehicle_count)
                                            if v != some_vehicle])
        index_from = random.choice(range(len(solution.list_paths[some_vehicle])))
        index_to = random.choice(range(max(1, len(solution.list_paths[some_other_vehicle]))))
        move = RelocateMove(index_vehicle_from=some_vehicle,
                            index_vehicle_to=some_other_vehicle,
                            index_from=index_from, index_to=index_to)
        sol = move.apply_local_move(solution)
        return sol, move

    def mutate_and_compute_obj(self, solution: VrpSolution) -> Tuple[Solution, LocalMove, Dict[str, float]]:
...
class SwapMove(LocalMove):
    ...
    def apply_local_move(self, solution: VrpSolution) -> Solution:
        self.index_from = min([self.index_from, len(solution.list_paths[self.index_vehicle_from])-1])
        self.index_to = min([self.index_to, len(solution.list_paths[self.index_vehicle_to])-1])
        ...
        previous_from_length = (solution.lengths[self.index_vehicle_from][self.index_from],
                                solution.lengths[self.index_vehicle_from][self.index_from+1])
        previous_to_length = (solution.lengths[self.index_vehicle_to][self.index_to],
        ...
        new_length_vehicle_from_1 = solution.problem.evaluate_function_indexes(index_1=get_index_previous(self.index_vehicle_from,
                                                                                                          self.index_from),
                                                                               index_2=city_to)
        ...
        new_length_vehicle_to_2 = solution.problem.evaluate_function_indexes(index_1=
                                                                             city_from,
                                                                             index_2=get_index_next(self.index_vehicle_to,
                                                                                                    self.index_to))
        ...
        solution.lengths[self.index_vehicle_from][self.index_from] = new_length_vehicle_from_1
        solution.lengths[self.index_vehicle_from][self.index_from+1] = new_length_vehicle_from_2
        solution.lengths[self.index_vehicle_to][self.index_to] = new_length_vehicle_to_1
        solution.lengths[self.index_vehicle_to][self.index_to+1] = new_length_vehicle_to_2
        solution.capacities[self.index_vehicle_from] += (solution.problem.customers[city_to].demand-
                                                         solution.problem.customers[city_from].demand)

        solution.capacities[self.index_vehicle_to] += (solution.problem.customers[city_from].demand-
                                                       solution.problem.customers[city_to].demand)
        delta = new_length_vehicle_to_1+new_length_vehicle_to_2+new_length_vehicle_from_1+new_length_vehicle_from_2-\
            sum(previous_to_length)-sum(previous_from_length)
        solution.length = solution.length + delta
        return solution

    def backtrack_local_move(self, solution: VrpSolution) -> Solution:
        move = SwapMove(index_vehicle_from=self.index_vehicle_from,
...
class MutationSwap(Mutation):
    ...
    def mutate(self, solution: VrpSolution) -> Tuple[Solution, LocalMove]:
        ...
        if len(vehicles_used) > 1:
            swap = True
            some_other_vehicle = random.choice([v for v in vehicles_used
                                                if v != some_vehicle])
        else:
            some_other_vehicle = random.choice([v for v in range(self.vrp_model.vehicle_count)
                                                if v != some_vehicle])
        ...
        index_to = random.choice(range(max(1, len(solution.list_paths[some_other_vehicle]))))
        if swap:
            move = SwapMove(index_vehicle_from=some_vehicle,
                            index_vehicle_to=some_other_vehicle,
                            index_from=index_from, index_to=index_to)
        else:
            move = RelocateMove(index_vehicle_from=some_vehicle,
...
class MutationTwoOptVRP(Mutation):
    ...
    def mutate_and_compute_obj(self, variable: VrpSolution):
        ...
        range_its = range(length_permut) if self.test_all \
            else random.sample(range(length_permut), min(self.nb_test, length_permut))
        for i in range_its:
            if i == length_permut - 1:
                range_jts = []
            else:
                range_jts = range(i + 1, length_permut) if self.test_all \
                    else random.sample(range(i + 1,
                                             length_permut),
                                       min(1, self.nb_test, length_permut - i - 1))
            for j in range_jts:
                i_before, i_, j_, j_after = self.get_points_index(some_vehicle, i, j, variable)
        ...
        i_before, i_, j_, j_after = self.get_points_index(some_vehicle, it, jt, variable)
        permut = variable.list_paths[some_vehicle][:(it)] + variable.list_paths[some_vehicle][it:jt + 1][::-1] \
        ...
        if min_change < 0 or not self.return_only_improvement:
            v = VrpSolution(list_start_index=variable.list_start_index,
                            list_end_index=variable.list_end_index,
                            list_paths=[permut if j == some_vehicle else
                                        variable.list_paths[j] for j in range(len(variable.list_paths))],
                            lengths=[lengths if j == some_vehicle else
                                     variable.lengths[j] for j in range(len(variable.lengths))],
                            length=fitness,
                            capacities=variable.capacities,
                            problem=self.vrp_model)
            return v, LocalMoveDefault(variable, v), self.vrp_model.evaluate(v)
        else:
            return variable, LocalMoveDefault(variable, variable), self.vrp_model.evaluate(variable)
