...
from discrete_optimization.rcpsp_multiskill.solvers.cp_solvers \
import PrecomputeEmployeesForTasks, ParametersCP, CPSolverName, MS_RCPSPModel
import numpy as np
...
class MultiSkillToRCPSP:
    ...
    def is_compatible(self, task_requirements: Dict[str, int],
                      ressource_availability: Dict[str, np.array],
                      duration_task,
                      horizon):
        non_zeros_res = [r for r in task_requirements if task_requirements[r] >= 0]
        p = np.multiply.reduce([ressource_availability[non_zeros_res[j]][:horizon] >= task_requirements[non_zeros_res[j]]
                               for j in range(len(non_zeros_res))])
        s = np.sum(p)
        return s >= duration_task

    def construct_rcpsp_by_worker_type(self,
        ...
        params_cp = ParametersCP(time_limit=30,
                                 pool_solutions=10000,
                                 intermediate_solution=True,
                                 all_solutions=False,
                                 nr_solutions=100)
        solver = PrecomputeEmployeesForTasks(ms_rcpsp_model=self.multiskill_model,
        ...
        map_names_to_understandable = {worker_type_name[i]:
                                       "WorkerType-"+str(i)
                                       for i in range(len(worker_type_name))}
        self.map_names_to_understandable = map_names_to_understandable
        self.worker_type_to_worker = {self.map_names_to_understandable[k]:
                                      solver.skills_representation_str[k]
        ...
        for worker_type in worker_type_name:
            ...
            calend = np.array(self.multiskill_model.employees[employees[0]].calendar_employee,
                              dtype=int)
            for j in range(1, len(employees)):
                calend += np.array(self.multiskill_model.employees[employees[j]].calendar_employee,
                                   dtype=int)
        ...
        for task in self.multiskill_model.tasks_list:
            solver.init_model(tasks_of_interest=[task],
            ...
            best_overskill_results = min(results,
                                         key=lambda x: x.overskill_type).overskill_type
            pruned_results = [r for r in results
                              if r.overskill_type == best_overskill_results]
            ...
            for i in range(len(pruned_results)):
                ...
                ddd = {key: ddd[key] for key in ddd if key not in self.multiskill_model.skills_set}  # remove the skills
                wtype_used = [(j, pruned_results[i].worker_type_used[j][0])
            ...
            if check_resource_compliance:
                ...
                for t in task_requirement_list:
                    b = self.is_compatible(task_requirements={r: t[r] for r in t if r != "duration"},
            ...
            tt = sorted(tt, key=lambda x: min([usage_worker_in_chosen_modes[y]
                                              for y in x if y in usage_worker_in_chosen_modes], default=0))
        ...
        rcpsp_model = RCPSPModel(resources=resources_dict,
