...
from discrete_optimization.rcpsp_multiskill.rcpsp_multiskill import MS_RCPSPModel,\
    MS_RCPSPSolution, MS_RCPSPSolution_Preemptive
...
def compute_schedule_per_resource_individual_preemptive(rcpsp_model: MS_RCPSPModel,
...
def plot_resource_individual_gantt(rcpsp_model: MS_RCPSPModel,
    ...
    for i in range(len(sorted_employees)):
        ...
        for activity in position_label:
            ax.annotate(name_task[activity],
...
def plot_resource_individual_gantt_preemptive(rcpsp_model: MS_RCPSPModel,
                                              rcpsp_sol: MS_RCPSPSolution_Preemptive,
                                              title_figure="",
                                              name_task=None,
                                              subtasks=None,
                                              ...
                                              ax=None,
                                              current_t=None):
    array_ressource_usage = compute_schedule_per_resource_individual_preemptive(rcpsp_model,
                                                                                rcpsp_sol)
    sorted_task_by_start = sorted(rcpsp_sol.schedule,
                                  key=lambda x: 100000 * rcpsp_sol.get_start_time(x) + rcpsp_model.index_task[x])
    sorted_task_by_end = sorted(rcpsp_sol.schedule,
...
def plot_task_gantt(rcpsp_model: MS_RCPSPModel,
                    rcpsp_sol: MS_RCPSPSolution,
                    subtasks=None,
                    fig=None,
                    ax=None,
    ...
    nb_task = len(tasks)
    sorted_task_by_start = sorted(tasks,
                                  key=lambda x:
                                  100000 * rcpsp_sol.get_start_time(x) + rcpsp_model.index_task[x])
    sorted_task_by_end = sorted(tasks,
                                key=lambda x: 100000 * rcpsp_sol.get_end_time(x) + rcpsp_model.index_task[x])
    max_time = rcpsp_sol.get_end_time(sorted_task_by_end[-1])
    ...
    for j in range(nb_task):
        ...
        colors = plt.cm.get_cmap("hsv", nb_colors)
        for start, end in zip(rcpsp_sol.get_start_times_list(tasks[j]),
        ...
        ax.annotate(tasks[j],
                    xy=((rcpsp_sol.get_start_times_list(tasks[j])[0] +
                         +rcpsp_sol.get_end_times_list(tasks[j])[0])/2, j),
                    # textcoords="offset points",
                    font_properties=FontProperties(size=7,
                                                   weight="bold"),
                    verticalalignment='center',
