...
class MultiskillRcpspPostProcessSolution(PostProcessSolution):
    def __init__(
        self,
...
class PreemptiveMultiskillRcpspPostProcessSolution(PostProcessSolution):
    def __init__(
        self,
        problem: MultiskillRcpspProblem,
        params_objective_function: ParamsObjectiveFunction = None,
...
def sgs_variant(
    ...
    new_solution = MultiskillRcpspSolution(
        problem=problem,
...
def sgs_variant_preemptive(
    ...
    while True:
        ...
        if isinstance(problem, VariantPreemptiveMultiskillRcpspSolution):
            times_predecessors += [
    ...
    new_solution = PreemptiveMultiskillRcpspSolution(
        problem=problem,
        schedule=new_proposed_schedule,
        modes=solution.modes,
        employee_usage=solution.employee_usage,
...
def compute_schedule_per_employees(
    solution: Union[PreemptiveMultiskillRcpspSolution, MultiskillRcpspSolution],
    problem: MultiskillRcpspProblem,
    predecessors_dict,
):
...
def shift_left_method(
    ...
    while True:
        ...
        if task not in new_proposed_schedule:
            ...
            if isinstance(solution, VariantPreemptiveMultiskillRcpspSolution):
                times_predecessors += [
                    new_proposed_schedule[t]["starts"][0]
                    if t in new_proposed_schedule
                    else solution.schedule[t]["starts"][0]
        ...
        if len(sorted_piece_to_schedule) == 0:
            break
    if isinstance(solution, PreemptiveMultiskillRcpspSolution):
        employee_usage = {}
