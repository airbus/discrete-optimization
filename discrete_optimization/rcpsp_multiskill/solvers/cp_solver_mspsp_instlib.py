...
files_mzn = {"mspsp": os.path.join(this_path,
                                   "../minizinc/mspsp.mzn"),
             "mspsp_two_obj": os.path.join(this_path, "../minizinc/mspsp_wip.mzn"),
             "mspsp_compatible": os.path.join(this_path, "../minizinc/mspsp_compatible_all_solvers.mzn")}
...
def create_usefull_res_data(rcpsp_model: MS_RCPSPModel):
    ...
    employees_position = {rcpsp_model.employees_list[i]: i + 1 for i in range(rcpsp_model.nb_employees)}
    non_zero_skills = {rcpsp_model.employees_list[i]:
                       set(rcpsp_model.employees[rcpsp_model.employees_list[i]].get_non_zero_skills())
    ...
    for task_number in range(rcpsp_model.n_jobs):
        ...
        skills_required = [sk for m in rcpsp_model.mode_details[task]
                           for sk in rcpsp_model.skills_set
                           if rcpsp_model.mode_details[task][m].get(sk, 0) > 0]
        possibly_interested_employee = [emp for emp in non_zero_skills if any(sk in non_zero_skills[emp]
                                                                              for sk in skills_required)]
        useful_res[task_number].update(set([employees_position[emp] for emp in possibly_interested_employee]))
...
class CP_MSPSP_MZN(CPSolver):
    ...
    def init_from_model(self, **args):
        ...
        dict_data["mint"] = 10 # here put a better number (from critical path method for example)
        dict_data["nActs"] = self.rcpsp_model.n_jobs
        dict_data["dur"] = [self.rcpsp_model.mode_details[t][1]["duration"] for t in self.rcpsp_model.tasks_list]
        dict_data["nSkills"] = len(self.rcpsp_model.skills_set)
        dict_data["sreq"] = [[self.rcpsp_model.mode_details[t][1].get(sk, 0) for sk in list_skills]
                             for t in self.rcpsp_model.tasks_list]
        dict_data["nResources"] = self.rcpsp_model.nb_employees
        dict_data["mastery"] = [[self.rcpsp_model.employees[self.rcpsp_model.employees_list[i]].dict_skill.
                                     get(s, SkillDetail(0, 0, 0)).skill_value > 0
                                 for s in list_skills]
                                for i in range(self.rcpsp_model.nb_employees)]
        dict_data["nPrecs"] = sum([len(self.rcpsp_model.successors[n]) for n in self.rcpsp_model.successors])
        self.index_in_minizinc = {task: self.rcpsp_model.return_index_task(task, offset=1)
                                  for task in self.rcpsp_model.tasks_list}
        pred = []
        succ = []
        for task in self.rcpsp_model.tasks_list:
            pred += [self.index_in_minizinc[task]]*len(self.rcpsp_model.successors[task])
            succ += [self.index_in_minizinc[successor] for successor in self.rcpsp_model.successors[task]]
        dict_data["pred"] = pred
        dict_data["succ"] = succ
        from discrete_optimization.generic_rcpsp_tools.graph_tools_rcpsp import build_unrelated_task, build_graph_rcpsp_object
    ...
    def constraint_end_time_string(self, task, end_time, sign: SignEnum = SignEnum.EQUAL) -> str:
        return "constraint start[" + str(self.index_in_minizinc[task]) + \
               "]+dur[" + str(self.index_in_minizinc[task]) + "]" + str(sign.value) + str(end_time) + ";\n"
    ...
    def constraint_used_employee(self, task, employee, indicator: bool = False):
        ...
        return ["constraint assign[" + str(id_task) + "," + str(id_employee) + "]==" + str(tag) + ";\n"]
    ...
    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        ...
        for j in set_of_jobs:
            s += ["constraint start[" + str(self.index_in_minizinc[j]) +
                  "]+dur[" + str(self.index_in_minizinc[j]) + "]<=objective;\n"]
        return s
    ...
    def constraint_sum_of_ending_time(self, set_subtasks: Set[Hashable]):
        ...
        s = """int: nb_indexes="""+str(len(indexes))+""";\n
               ...
               array[1..nb_indexes] of ACT: index_tasks="""+str(indexes)+""";\n
                    ...
                    if unit_used[task][w] == 1:
                        if isinstance(result[index_solution], MS_RCPSPSolCP):
                            if "contrib" in result[index_solution].dict:
                                intersection = [skills_list[i]
                                                for i in range(len(result[index_solution].dict["contrib"][task][w]))
