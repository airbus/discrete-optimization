...
from typing import Union, List, Tuple, Hashable, Dict, Set

from minizinc import Instance, Model, Solver, Status

from discrete_optimization.generic_rcpsp_tools.graph_tools_rcpsp import build_graph_rcpsp_object, build_unrelated_task
from discrete_optimization.generic_tools.cp_tools import CPSolver, ParametersCP, CPSolverName, map_cp_solver_name, \
    SignEnum
from discrete_optimization.generic_tools.do_problem import ParamsObjectiveFunction, \
    build_aggreg_function_and_params_objective
from discrete_optimization.generic_tools.result_storage.result_storage import ResultStorage
from discrete_optimization.rcpsp.rcpsp_model import RCPSPModelCalendar, PartialSolution, RCPSPSolution
...
from discrete_optimization.rcpsp_multiskill.rcpsp_multiskill import MS_RCPSPModel, \
    MS_RCPSPSolution, create_fake_tasks_multiskills, cluster_employees_to_resource_types, MS_RCPSPSolution_Preemptive

this_path = os.path.dirname(os.path.abspath(__file__))

files_mzn = {"multi-calendar": os.path.join(this_path, "../minizinc/ms_rcpsp_multi_mode_mzn_calendar.mzn"),
             "multi-calendar-overlap": os.path.join(this_path, "../minizinc/ms_rcpsp_with_overlap_variable.mzn"),
             "multi-calendar-no-ressource": os.path.join(this_path,
                                                         "../minizinc/ms_rcpsp_multi_mode_mzn_calendar_no_ressource.mzn"),
             "ms_no_multitasking": os.path.join(this_path,
                                                "../minizinc/ms_rcpsp_multi_mode_mzn_calendar_no_ressource_nomultitasking.mzn"),
             "ms_rcpsp_preemptive": os.path.join(this_path, "../minizinc/ms_rcpsp_preemptive.mzn"),
             "ms_rcpsp_partial_preemptive": os.path.join(this_path, "../minizinc/ms_rcpsp_preemptive_partially_preemptive.mzn"),
             "compute_worker_for_tasks": os.path.join(this_path,
                                                      "../minizinc/ms_rcpsp_compute_workers_for_tasks.mzn")}


def add_fake_task_cp_data(rcpsp_model: Union[MS_RCPSPModel],
                          ignore_fake_task: bool = True,
                          max_time_to_consider: int = None):
    if not ignore_fake_task:
        fake_tasks_res, fake_tasks_unit = create_fake_tasks_multiskills(rcpsp_problem=rcpsp_model)
        if len(fake_tasks_res) > 0:
            ...
            fake_tasks_res = [f for f in fake_tasks_res if f["start"] <= max_time_to_consider]
            n_fake_tasks_res = len(fake_tasks_res)
            fakestart_res = [fake_tasks_res[i]["start"] for i in range(n_fake_tasks_res)]
            fake_dur_res = [fake_tasks_res[i]["duration"] for i in range(n_fake_tasks_res)]
            max_duration_fake_task_res = max(fake_dur_res)
            fake_req_res = [[fake_tasks_res[i].get(res, 0) for i in range(n_fake_tasks_res)]
                            for res in rcpsp_model.resources_list]
            dict_to_add_in_instance = {}
            dict_to_add_in_instance["max_duration_fake_task_resource"] = max_duration_fake_task_res
            ...
            dict_to_add_in_instance["fakereq_resource"] = [[] for req in rcpsp_model.resources_list]
            dict_to_add_in_instance["include_fake_tasks_resource"] = False
        if len(fake_tasks_unit) > 0:
            fake_tasks_unit = [f for f in fake_tasks_unit if f["start"] <= max_time_to_consider]
            n_fake_tasks_unit = len(fake_tasks_unit)
            fakestart_unit = [fake_tasks_unit[i]["start"] for i in range(n_fake_tasks_unit)]
            fake_dur_unit = [fake_tasks_unit[i]["duration"] for i in range(n_fake_tasks_unit)]
            max_duration_fake_task_unit = max(fake_dur_unit)
            fake_req_unit = [[fake_tasks_unit[i].get(res, 0) for i in range(n_fake_tasks_unit)]
                             for res in rcpsp_model.employees_list]
            dict_to_add_in_instance["max_duration_fake_task_unit"] = max_duration_fake_task_unit
            ...
            dict_to_add_in_instance["fakereq_unit"] = [[] for e in rcpsp_model.employees_list]
        ...
        dict_to_add_in_instance["fakestart_resource"] = []
        dict_to_add_in_instance["fakedur_resource"] = []
        dict_to_add_in_instance["fakereq_resource"] = [[] for r in rcpsp_model.resources_list]
        dict_to_add_in_instance["include_fake_tasks_resource"] = False
        dict_to_add_in_instance["max_duration_fake_task_unit"] = 0
        dict_to_add_in_instance["n_fake_task_unit"] = 0
        dict_to_add_in_instance["fakestart_unit"] = []
...
class CP_MS_MRCPSP_MZN(CPSolver):
    ...
    def manual_cumulative_resource_constraints(self, instance):
        blocking_per_resource = {r: {"blocking": [],
                                     "merged": set(),
                                     "not_blocked": set()}
                                 for r in self.rcpsp_model.resources_list}
        array = [[0 for i in range(self.rcpsp_model.n_jobs)] for r in self.rcpsp_model.resources_list]
        for l in self.rcpsp_model.resource_blocking_data:
            ...
            for res in set_res:
                ...
                for l in list_task[1:]:
                    array[index_res][self.rcpsp_model.index_task[l]] = \
                        -1
        for r in blocking_per_resource:
            blocking_per_resource[r]['not_blocked'] = [t for t in self.rcpsp_model.tasks_list
    ...
    def manual_starting_time(self, instance):
        ...
        for i in range(self.rcpsp_model.nb_tasks):
            ...
            if t in self.rcpsp_model.special_constraints.start_times_window:
                st = []
                if self.rcpsp_model.special_constraints.start_times_window[t][0] is not None:
                    st += [self.rcpsp_model.special_constraints.start_times_window[t][0]]
                if self.rcpsp_model.special_constraints.start_times_window[t][1] is not None:
                    st += [self.rcpsp_model.special_constraints.start_times_window[t][1]]
                s = """constraint (member([start[j]+adur[j]|j in Act]++[fakestart_resource[j]+fakedur_resource[j]|
                                                                        j in FakeActRes]++
                                          [fakestart_unit[j]+fakedur_unit[j]|j in FakeActUnit]++"""+str(st)+""", 
                                           start["""+str(i+1)+"""]));\n"""
            else:
                s = """constraint (member([start[j]+adur[j]|j in Act]++[fakestart_resource[j]+fakedur_resource[j]|
                                                                        j in FakeActRes]++
    ...
    def constraint_used_employee(self, task, employee, indicator: bool = False):
        ...
        return ["constraint unit_used["+str(id_employee)+","+str(id_task)+"]=="+str(tag)+";\n"]

    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        s = []
        for j in set_of_jobs:
            s += ["constraint s["+str(self.index_in_minizinc[j])+
    ...
    def init_model(self, **args):
        no_ressource = False
        model_type = args.get("model_type", "multi-calendar" if not no_ressource else "multi-calendar-no-ressource")
        ...
        include_constraint_on_start_value = args.get("include_constraint_on_start_value", False)
        include_constraint_on_start_value_manual = args.get("include_constraint_on_start_value_manual", False)
        search_strategy = args.get("search_strategy", SearchStrategyMS_MRCPSP.START_THEN_USED_UNIT)
        ...
        if search_strategy.value != "none":
            ...
            instance.add_string("my_search="+str(SearchStrategyMS_MRCPSP.START_THEN_USED_UNIT.value)+";\n") # we have to put one by default.
        ...
        instance["include_constraint_on_start_value"] = include_constraint_on_start_value
        ...
        if model_type == "multi-calendar-overlap":
            ...
            instance["unpred"] = [self.rcpsp_model.index_task[x[0]]+1 for x in unrelated]
            instance["unsucc"] = [self.rcpsp_model.index_task[x[1]]+1 for x in unrelated]
        if not include_cumulative_resource and len(self.rcpsp_model.resource_blocking_data) > 0:
        ...
        skill_required = [[int(all_modes[i][2].get(s, 0)) for i in range(len(all_modes))] for s in skills_set]
        if "max_time" in args:
        ...
        instance["source"] = self.rcpsp_model.index_task[self.rcpsp_model.source_task]+1
        ...
        self.index_employees_in_minizinc = {self.rcpsp_model.employees_list[i]: i+1
    ...
    def retrieve_solutions(self, result,
        ...
        for start_times, mrun, unit_used in zip(starts, mruns, units_used):
            ...
            for w in range(len(unit_used)):
                for task in range(len(unit_used[w])):
                    ...
                    if unit_used[w][task] == 1:
                        if isinstance(result[index_solution], MS_RCPSPSolCP) and\
                                "contrib" in result[index_solution].dict:  # model="ms_no_multitasking"
                                intersection = [skills_list[i] for i in
                                                range(len(result[index_solution].dict["contrib"][task][w]))
                                                if result[index_solution].dict["contrib"][task][w][i] == 1]
                        else:
                            ...
                            skills_worker = set([s
                                                 ...
                                                 if self.rcpsp_model.employees[self.employees_position[w]].dict_skill[s]
                                                 .skill_value > 0])
            ...
            for i in range(len(start_times)):
                ...
                rcpsp_schedule[task_id] = {'start_time': start_times[i],
                                           'end_time': start_times[i]
                                           + self.rcpsp_model.mode_details[task_id][modes_dict[task_id]]['duration']}
...
class CP_MS_MRCPSP_MZN_PREEMPTIVE(CPSolver):
    ...
    def constraint_is_paused(self, task, is_paused):
        ...
        return "constraint is_paused[" + str(self.index_in_minizinc[task]) + "]==" + is_paused_str + ";\n"
    ...
    def constraint_used_employee(self, task, employee, part_id=1, indicator: bool=False):
        id_task = self.index_in_minizinc[task]
        id_employee = self.index_employees_in_minizinc[employee]
        tag = "true" if indicator else "false"
        if self.unit_usage_preemptive:
            return ["constraint unit_used_preemptive["+str(id_employee)+","+str(id_task)
                    + "," + str(part_id)+"]=="+str(tag)+";\n"]
        else:
            return ["constraint unit_used[" + str(id_employee) + "," + str(id_task) + "]==" + str(tag) + ";\n"]
    ...
    def constraint_end_time_string(self, task, end_time, sign: SignEnum = SignEnum.EQUAL) -> str:
        return "constraint s_preemptive["+str(self.index_in_minizinc[task])+", nb_preemptive]"\
               + str(sign.value)+str(end_time)+";\n"
    ...
    def init_model(self, **args):
        ...
        instance["possibly_preemptive"] = args.get("possibly_preemptive",
                                                   [True for task in self.rcpsp_model.tasks_list])
        ...
        self.second_objectives = {"weights": [],
                                  "name_penalty": []}
        if add_partial_solution_hard_constraint:
            strings = add_hard_special_constraints(p_s, self)
            for s in strings:
            ...
            for s in strings:
                self.instance.add_string(s)
            self.second_objectives = {"weights": [100]*len(name_penalty),
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP = ParametersCP.default()) -> ResultStorage:
        ...
        for i in range(len(starts)):
            ...
            for m in range(len(mruns[i])):
                if mruns[i][m]:
                    modes_dict[self.modeindex_map[m + 1]['task']] = self.modeindex_map[m + 1]['original_mode_index']
            if not self.unit_usage_preemptive:
                ...
                for task_id in usage:
                    usage[task_id] = [usage[task_id] for i in range(len(rcpsp_schedule[task_id]["starts"]))]
            else:
                ...
                for w in range(len(unit_used)):
                    for task in range(len(unit_used[w])):
                        ...
                        skills_needed = set([s
                                             ...
                                             and self.rcpsp_model.mode_details[task_id][mode][s] > 0])
                        if task_id not in usage:
                            usage[task_id] = [{} for j in range(len(rcpsp_schedule[task_id]["starts"]))]
                        for j in range(len(rcpsp_schedule[task_id]["starts"])):
                            if unit_used[w][task][j] == 1:
                                ...
                                intersection = skills_needed.intersection(skills_worker)
                                if len(intersection) > 0:
                                    usage[task_id][j][self.employees_position[w]] = intersection

            sol = MS_RCPSPSolution_Preemptive(problem=self.rcpsp_model,
                                              modes=modes_dict,
                                              schedule=rcpsp_schedule,
                                              employee_usage=usage)
            objective = self.aggreg_from_dict_values(self.rcpsp_model.evaluate(sol))
...
class CP_MS_MRCPSP_MZN_PARTIAL_PREEMPTIVE(CP_MS_MRCPSP_MZN_PREEMPTIVE):
    ...
    def init_model(self, **args):
        ...
        all_modes = [(act, mode, self.rcpsp_model.mode_details[act][mode],
                     ...
                     for act in sorted_tasks for mode in sorted(self.rcpsp_model.mode_details[act])]
        self.modeindex_map = {i + 1: {"task": all_modes[i][0],
                                      "original_mode_index": all_modes[i][1]}
                              for i in range(len(all_modes))}
        modes = [set() for t in sorted_tasks]
        for j in self.modeindex_map:
            modes[self.rcpsp_model.index_task[self.modeindex_map[j]["task"]]].add(j)
        ...
        skills_set = sorted(list(self.rcpsp_model.skills_set))
        nb_units = len(self.rcpsp_model.employees)
        skill_required = [[int(all_modes[i][2].get(s, 0))
                           for i in range(len(all_modes))] for s in skills_set]
        if "max_time" in args:
            instance["max_time"] = args["max_time"]
        else:
            instance["max_time"] = self.rcpsp_model.horizon
        ...
        is_releasable = [[1 if all_modes[i][3].get(res, True) else 0 for i in range(len(all_modes))]
                         for res in resources_list]
...
def stick_to_solution(solution: RCPSPSolution,
    ...
    for task in solution.rcpsp_schedule:
        ...
        list_strings += [cp_solver.constraint_start_time_string(task=task, start_time=start,
                                                                sign=SignEnum.EQUAL)]
        # list_strings += cp_solver.constraint_task_to_mode(task_id=task, mode=modes_dict[task])
    return list_strings


def stick_to_solution_preemptive(solution: RCPSPSolutionPreemptive,
                                 cp_solver: CP_MS_MRCPSP_MZN_PREEMPTIVE):
    list_strings = []
    modes_dict = solution.problem.build_mode_dict(solution.rcpsp_modes)
    for task in solution.rcpsp_schedule:
        ...
        for i in range(len(starts)):
            list_strings += [cp_solver.constraint_start_time_string_preemptive_i(task,
                                                                                 start_time=starts[i],
                                                                                 part_id=i+1,
                                                                                 sign=SignEnum.EQUAL)]
            list_strings += [cp_solver.constraint_duration_string_preemptive_i(task,
...
def soft_start_window(start_times_window: Dict[Hashable, Tuple[int, int]],
    ...
    max_t_start = max([max([x[1] for x in l_low], default=0), max([x[1] for x in l_up], default=0)])
    ...
    s = """
        ...
        int: max_time_start="""+str(max_t_start)+""";\n
...
def soft_end_window(end_times_window: Dict[Hashable, Tuple[int, int]],
    ...
    max_t_end = max([max([x[1] for x in l_low], default=0), max([x[1] for x in l_up], default=0)])
    s = """
        ...
        int: max_time_end="""+str(max_t_end)+""";\n
...
def add_hard_special_constraints(partial_solution: PartialSolution,
    ...
    if partial_solution.start_at_end_plus_offset is not None:
        constraint_strings += hard_start_at_end_plus_offset(list_start_at_end_plus_offset=
...
class SolutionPrecomputeEmployeesForTasks:
    ...
    def __str__(self):
        return "-".join([str(s)+" : "+str(getattr(self, s))
                         for s in self.__dict__.keys()])


class PrecomputeEmployeesForTasks:
    def __init__(self, ms_rcpsp_model: MS_RCPSPModel,
                 cp_solver_name: CPSolverName = CPSolverName.CHUFFED):
        self.ms_rcpsp_model = ms_rcpsp_model
        self.cp_solver_name = cp_solver_name
        self.instance: Instance = None
        self.skills_representation_str, self.skills_dict = \
    ...
    def init_model(self, **kwargs):
        ...
        instance = Instance(solver, model)
        instance["one_ressource_per_task"] = kwargs.get("one_ressource_per_task",
                                                        False)
        instance["one_worker_type_per_task"] = kwargs.get("one_worker_type_per_task",
                                                          False)
        instance["exact_skills_need"] = kwargs.get("exact_skills_need",
                                                   False)
        ...
        tasks_of_interest = kwargs.get("tasks_of_interest",
                                       [self.ms_rcpsp_model.tasks_list[0]])
        instance["n_tasks"] = len(tasks_of_interest)
        n_opt = sum([len(list(self.ms_rcpsp_model.mode_details[key].keys()))
                     for key in tasks_of_interest])
        # print('n_opt: ', n_opt)
        instance["n_opt"] = n_opt
        all_modes = [(act, mode, self.ms_rcpsp_model.mode_details[act][mode])
        ...
        rreq = [[all_modes[i][2].get(res, 0)
                 for i in range(len(all_modes))]
                for res in self.ms_rcpsp_model.resources_list]
        instance["rreq"] = rreq
        instance["nb_res"] = len(self.ms_rcpsp_model.resources_list)
        rcap = [int(max(self.ms_rcpsp_model.resources_availability[x]))
                for x in self.ms_rcpsp_model.resources_list]
        instance["rcap"] = rcap
        rtype = [2 if res in self.ms_rcpsp_model.non_renewable_resources else 1
                 for res in self.ms_rcpsp_model.resources_list]
        instance["rtype"] = rtype
        ...
        import math
        skillunits = [[int(math.floor(self.ms_rcpsp_model.employees[j]
                                      .dict_skill[s].skill_value))
                       if s in self.ms_rcpsp_model.employees[j].dict_skill else 0
                       for s in skills_set]
        ...
        instance["skills_worker_type"] = \
            [[self.skills_dict[wt][s].skill_value
              if s in self.skills_dict[wt] else 0
              for s in skills_set]
             for wt in worker_type_list]
        instance["capacity_worker_type"] = [len(self.skills_representation_str[wt])
                                            for wt in worker_type_list]
    ...
    def retrieve_solutions(self, result,
        ...
        for k in range(len(mruns)):
            ...
            for i in range(len(mruns[k])):
                if mruns[k][i]:
                    mode_dict[self.modeindex_map[i + 1]['task']] \
                        = self.modeindex_map[i + 1]['original_mode_index']
            modes_dict += [mode_dict]
        return [SolutionPrecomputeEmployeesForTasks(unit_used=u,
                                                    worker_type_used=w,
                                                    mode_dict=m,
                                                    overskill_unit=ou,
                                                    overskill_type=ow)
                for u, w, m, ou, ow in zip(units_used,
                                           workers_type_used,
                                           modes_dict,
                                           overskills_units,
                                           overskills_types)]
