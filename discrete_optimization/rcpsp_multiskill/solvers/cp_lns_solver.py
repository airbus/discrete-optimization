...
from discrete_optimization.rcpsp_multiskill.solvers.lns_post_process_rcpsp import PostProMSRCPSP
...
from discrete_optimization.rcpsp_multiskill.solvers.ms_rcpsp_cp_lns_solver import build_neighbor_operator, \
    OptionNeighbor
...
GENERIC_CLASS_MULTISKILL = Union[MS_RCPSPModel,
                                 MS_RCPSPModel_Variant]


def build_default_cp_model(rcpsp_problem: GENERIC_CLASS_MULTISKILL, partial_solution=None, **kwargs):
    if rcpsp_problem.preemptive:
        ...
        solver.init_model(output_type=True,
                          ...
                          possibly_preemptive=[rcpsp_problem.preemptive_indicator[t] for t in rcpsp_problem.tasks_list],
...
def build_constraint_handler(rcpsp_problem, graph, **kwargs):
    ...
    if constraint_handler_type == ConstraintHandlerType.MIX_SUBPROBLEMS:
        ...
        params_list = kwargs.get("params_list", [
            ParamsConstraintBuilder(minus_delta_primary=6000, plus_delta_primary=6000, minus_delta_secondary=400,
                                    plus_delta_secondary=400, constraint_max_time_to_current_solution=False),
            ParamsConstraintBuilder(minus_delta_primary=6000, plus_delta_primary=6000, minus_delta_secondary=0,
                                    plus_delta_secondary=0, constraint_max_time_to_current_solution=False)])
        print(params_list)
        constraint_handler = NeighborSubproblem(problem=rcpsp_problem,
...
class LargeNeighborhoodSearchMSRCPSP(SolverDO):
    def __init__(self,
        ...
        self.constraint_handler = build_constraint_handler(rcpsp_problem=self.rcpsp_problem,
                                                           graph=graph,
                                                           multiskill=isinstance(self.rcpsp_problem,
                                                                                 MS_RCPSPModel),
                                                           preemptive=self.rcpsp_problem.preemptive,
                                                           **kwargs)
        self.post_pro = None
        if not self.rcpsp_problem.preemptive:
            self.post_pro = PostProMSRCPSP(problem=self.rcpsp_problem,
        ...
        if self.initial_solution_provider is None:
            self.initial_solution_provider = InitialSolutionMS_RCPSP(problem=self.rcpsp_problem,
