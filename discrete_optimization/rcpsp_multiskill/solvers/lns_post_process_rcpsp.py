...
from discrete_optimization.rcpsp_multiskill.rcpsp_multiskill import MS_RCPSPModel, \
    MS_RCPSPModel_Variant, MS_RCPSPSolution, \
...
class PostProMSRCPSP(PostProcessSolution):
    def __init__(self, problem: MS_RCPSPModel, params_objective_function: ParamsObjectiveFunction=None):
...
class PostProMSRCPSPPreemptive(PostProcessSolution):
    ...
    def build_other_solution(self, result_storage: ResultStorage) -> ResultStorage:
        new_solution = \
            shift_left_method(solution=result_storage.get_last_best_solution()[0],
                              problem=self.problem,
                              predecessors_dict=self.immediate_predecessors)
        fit = self.aggreg_from_sol(new_solution)
        ...
        for s in random.sample(result_storage.list_solution_fits,
            ...
            if len(s[0].schedule) == self.problem.nb_tasks:
                new_solution = shift_left_method(solution=s[0],
                                                 problem=self.problem,
                                                 predecessors_dict=self.immediate_predecessors)
                fit = self.aggreg_from_sol(new_solution)
                result_storage.list_solution_fits += [(new_solution, fit)]
        if False:
            ...
            result_store = solver.solve(nb_iteration_max=300,
                                        init_solution=s)
...
def sgs_variant(solution: MS_RCPSPSolution,
    ...
    sorted_tasks = sorted(solution.schedule,
    ...
    for task in sorted_tasks:
        ...
        if len(times_predecessors) > 0:
            ...
            min_time = 0
        if problem.do_special_constraints and task in problem.special_constraints.start_times_window:
            if problem.special_constraints.start_times_window[task][0] is not None:
                min_time = max(min_time, problem.special_constraints.start_times_window[task][0])
                ...
                if all(all(worker_avail_in_time[emp][time]
                       ...
                       for emp in employee_used) and \
                        all(resource_avail_in_time[res][time] >= problem.mode_details[task][solution.modes[task]].get(res, 0)
                            ...
                            for time in range(t, t + solution.schedule[task]["end_time"]
...
def sgs_variant_preemptive(solution: MS_RCPSPSolution_Preemptive,
                           problem: MS_RCPSPModel,
                           predecessors_dict):
    new_proposed_schedule = {}
    task_that_be_shifted_left = []
    ...
    sorted_tasks = sorted(solution.schedule.keys(),
                          key=lambda x: (solution.get_start_time(x)))
    for task in list(sorted_tasks):
        if len(solution.schedule[task]["starts"]) > 1:
            new_proposed_schedule[task] = {"starts": solution.schedule[task]["starts"],
    ...
    while True:
        ...
        if len(solution.schedule[task]["starts"]) > 1:
            ...
            for s, e, emp in zip(new_proposed_schedule[task]["starts"],
    ...
    new_solution = MS_RCPSPSolution_Preemptive(problem=problem,
                                               schedule=new_proposed_schedule,
                                               modes=solution.modes,
                                               employee_usage=solution.employee_usage)
    print("New : ", problem.evaluate(new_solution), problem.satisfy(new_solution))
...
def compute_schedule_per_employees(solution: Union[MS_RCPSPSolution_Preemptive, MS_RCPSPSolution],
                                   problem: MS_RCPSPModel,
                                   predecessors_dict):
...
def shift_left_method(solution: Union[MS_RCPSPSolution_Preemptive, MS_RCPSPSolution],
                      problem: MS_RCPSPModel,
                      predecessors_dict):
    ...
    for t in problem.tasks_list:
        ...
        allparts_to_schedule += [(s, e, emp, t) for s, e, emp in zip(starts, ends, employees)]
    ...
    new_horizon = min(solution.get_end_time(problem.sink_task), problem.horizon)
    for res in problem.resources_set:
        resource_avail_in_time[res] = np.copy(problem.resources_availability[res][:new_horizon + 30])
    worker_avail_in_time = {}
    for i in problem.employees:
    ...
    def is_startable(tt):
        if "special_constraints" in problem.__dict__.keys():
            return all(x in dones
                       for x in problem.special_constraints.dict_start_after_nunit_reverse.get(tt, {})) and \
                   all(x in dones
                       for x in problem.special_constraints.dict_start_at_end_reverse.get(tt, {})) and \
                   all(x in dones
                       for x in problem.special_constraints.dict_start_at_end_offset_reverse.get(tt, {})) and \
                   all(t in dones for t in predecessors_dict[tt])
        return all(t in dones for t in predecessors_dict[tt])

    def get_ressource_available(res, time, task):
        return resource_avail_in_time[res][time] - sum([problem.mode_details[tasks][solution.modes[tasks]].get(res, 0)
                                                        for tasks in new_proposed_schedule
                                                        if tasks not in dones
                                                        and tasks != task
                                                        and
                                                        not problem.partial_preemption_data[tasks]
                                                        [solution.modes[tasks]][res]
    ...
    while True:
        ...
        if duration_done[task] >= problem.mode_details[task][solution.modes[task]]["duration"]-0.0001:
            dones.add(task)
        if task not in new_proposed_schedule:
            ...
            if task in problem.special_constraints.start_times_window:
                if problem.special_constraints.start_times_window[task][0] is not None:
                    times_predecessors += [problem.special_constraints.start_times_window[task][0]]
            if isinstance(solution, MS_RCPSPSolution_Preemptive_Variant):
                ...
                times_predecessors += [new_proposed_schedule[t]["ends"][-1]
                                       ...
                                       + problem.special_constraints.dict_start_at_end_offset_reverse.get(task)[t]
                                       for t in
                                       problem.special_constraints.dict_start_at_end_offset_reverse.get(task, {})]
        else:
        ...
        for t in range(min_time, problem.horizon):
            if all(get_ressource_available(res, time, task) >= problem.mode_details[task][modes_dict[task]].get(res, 0)
                   for res in
                   problem.resources_list
                   for time in range(t,
                                     t + duration_this_task)) \
                    and all(worker_avail_in_time[e][time] >= 1
                            for e in emp
                            for time in range(t,
                                              t + duration_this_task)):
                new_starting_time = t
        ...
        if task not in new_proposed_schedule:
            ...
            new_proposed_schedule[task]["starts"] += [new_starting_time]
            new_proposed_schedule[task]["ends"] += [new_starting_time+duration_this_task]
        employee_used = employees_used

        for s, e, emp in zip([new_proposed_schedule[task]["starts"][-1]],
                             [new_proposed_schedule[task]["ends"][-1]],
                             [employee_used]):
            for res in problem.resources_list:
                releasable = problem.partial_preemption_data[task][solution.modes[task]][res]
                if releasable:
                    resource_avail_in_time[res][s:e] \
                        -= problem.mode_details[task][modes_dict[task]].get(res, 0)
            for em in emp:
        ...
        if task in dones:
            for res in problem.resources_list:
                releasable = problem.partial_preemption_data[task][solution.modes[task]][res]
                if not releasable:
                    resource_avail_in_time[res][new_proposed_schedule[task]["starts"][0]:
                                                new_proposed_schedule[task]["ends"][-1]] -= \
                        problem.mode_details[task][modes_dict[task]].get(res, 0)
    ...
    if isinstance(solution, MS_RCPSPSolution_Preemptive):
        ...
        for t in solution.employee_usage:
            ...
            for i in range(len(solution.employee_usage[t])):
                employee_usage[t] += [{s: solution.employee_usage[t][i][s]
        ...
        schedule = {t: {"start_time": new_proposed_schedule[t]["starts"][0],
