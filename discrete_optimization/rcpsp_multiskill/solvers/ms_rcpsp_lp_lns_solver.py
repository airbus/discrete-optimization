...
from discrete_optimization.rcpsp.solver.rcpsp_lp_lns_solver import InitialMethodRCPSP, RCPSPSolution
...
class InitialSolutionMS_RCPSP(InitialSolution):
    ...
    def get_starting_solution(self) -> ResultStorage:
        ...
        for sol, fit in s.list_solution_fits:
            ...
            modes = {multi_skill_rcpsp.tasks_list_non_dummy[i]: mode[i] for i in range(len(mode))}
            ...
            #                                      employee_usage=None)
            ms_rcpsp_solution = self.problem.get_solution_type()(problem=self.problem,
