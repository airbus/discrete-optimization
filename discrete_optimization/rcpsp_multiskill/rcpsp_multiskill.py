...
from discrete_optimization.generic_tools.do_problem import Solution, Problem, EncodingRegister, TypeAttribute, \
    ObjectiveRegister, TypeObjective, ObjectiveHandling, TupleFitness, ModeOptim
from discrete_optimization.generic_tools.graph_api import Graph
from typing import List, Union, NamedTuple, Tuple, Dict, Set, Optional, Hashable, Iterable
...
class TaskDetails:
    ...
    def __str__(self):
        return "Start :"+str(self.start)+" End : "+str(self.end)+" Resource "+str(self.resource_units_used)
...
class TaskDetailsPreemptive:
    ...
    def __str__(self):
        return "Start :"+str(self.starts)+" End : "+str(self.ends)+" Resource "+str(self.resource_units_used)
...
class MS_RCPSPSolution(Solution):
    def __init__(self,
                 ...
                 schedule: Dict[Hashable, Dict[str, Union[int, List[int]]]],  # (task: {"start_time": start, "end_time": }}
                 employee_usage: Dict[Hashable, Dict[Hashable, Set[str]]]):  # {task: {employee: Set(skills})}):
    ...
    def employee_used(self, task):
        if task not in self.employee_usage:
            ...
            return [[e for e in self.employee_usage[task]
...
class MS_RCPSPSolution_Preemptive(MS_RCPSPSolution):
    def __init__(self, problem: Problem, modes: Dict[Hashable, int],
                 schedule: Dict[Hashable, Dict[str, List[int]]],
                 employee_usage: Dict[Hashable, List[Dict[Hashable, Set[str]]]]):  # {task: {employee: Set(skills})}):
        super().__init__(problem, modes, schedule, None)
        self.employee_usage = employee_usage

    def copy(self):
        return MS_RCPSPSolution_Preemptive(problem=self.problem,
                                           modes=deepcopy(self.modes),
                                           schedule=deepcopy(self.schedule),
                                           employee_usage=deepcopy(self.employee_usage))
    ...
    def get_min_duration_subtask(self):
        return min([e-s for t in self.schedule
                    for e, s in zip(self.schedule[t]["ends"],
                                    self.schedule[t]["starts"])
    ...
    def employee_used(self, task):
        if task not in self.employee_usage:
            ...
            return [[e for e in self.employee_usage[task][i]
...
def schedule_solution_to_variant(solution: MS_RCPSPSolution):
    ...
    solution = MS_RCPSPSolution_Variant(problem=s.problem,
                                        priority_list_task=[solution.problem.index_task_non_dummy[p]
                                                            for p in priority_list_task],
                                        priority_worker_per_task=workers,
                                        modes_vector=[s.modes[i]
                                                      for i
                                                      in s.problem.tasks_list_non_dummy])
    return solution
...
class MS_RCPSPSolution_Variant(MS_RCPSPSolution):
    def __init__(self, problem: Problem,
                 ...
                 fast: bool = True):  # {task: {employee: Set(skills})}):
        ...
        if priority_worker_per_task is None:
            ...
            self.priority_worker_per_task = \
                self.problem.convert_fixed_priority_worker_per_task_from_permutation(priority_worker_per_task)
        ...
        if self.modes_vector is None and self.modes_vector_from0 is None:
            if "fixed_modes" in self.problem.__dict__:
                ...
                self.modes_vector = [self.modes[x] for x in self.problem.tasks_list_non_dummy]
        ...
        if self.priority_list_task is None:
            sorted_task = [self.problem.index_task_non_dummy[i]
                           for i in sorted(self.schedule,
                                           key=lambda x: self.schedule[x]["start_time"])
    ...
    def do_recompute(self, fast=True):
        if not fast:
            ...
            rcpsp_schedule, skills_usage, unfeasible_non_renewable_resources = \
                self.problem.func_sgs(permutation_task=permutation_do_to_permutation_sgs_fast(rcpsp_problem=self.problem,
                                                                                              permutation_do=self.priority_list_task),     # permutation_task=array(task)->task index
                                      priority_worker_per_task=
                                      priority_worker_per_task_do_to_permutation_sgs_fast(rcpsp_problem=self.problem,
                                                                                          priority_worker_per_task=self.priority_worker_per_task), # array(task, worker)
                                      modes_array=np.array([0]+self.modes_vector_from0+[0]))
            self.update_infos_from_numba_output(rcpsp_schedule=rcpsp_schedule,
                                                skills_usage=skills_usage,
                                                unfeasible_non_renewable_resources=unfeasible_non_renewable_resources)

    def run_sgs_partial(self, current_t,
                        completed_tasks: Dict[Hashable, TaskDetails],
                        scheduled_tasks_start_times: Dict[Hashable, TaskDetails],
                        fast=True):
        if not fast:
            rcpsp_schedule, modes_extended, employee_usage, modes_dict = \
                sgs_multi_skill_partial_schedule(solution=self,
                                                 current_t=current_t,
            ...
            self.employee_usage = {}
            scheduled_task_indicator, scheduled_tasks_start_times_vector, scheduled_tasks_end_times_vector, worker_used = \
                build_partial_vectors(problem=self.problem,
                                      completed_tasks=completed_tasks,
                                      scheduled_tasks_start_times=scheduled_tasks_start_times)
            rcpsp_schedule, skills_usage, unfeasible_non_renewable_resources = \
                self.problem.func_sgs_partial(permutation_task=permutation_do_to_permutation_sgs_fast(rcpsp_problem=self.problem,
    ...
    def update_infos_from_numba_output(self, rcpsp_schedule, skills_usage, unfeasible_non_renewable_resources):
        if unfeasible_non_renewable_resources:
            for t in self.problem.tasks_list:
                self.schedule[t] = {"starts": [99999],
                                    "ends": [99999]}
            return
        self.schedule = {}
        for k in rcpsp_schedule:
            self.schedule[self.problem.tasks_list[k]] = {"start_time": rcpsp_schedule[k][0],
        ...
        if True:
            for act_id in skills_usage:
                ...
                for i, j in zip(non_z[0], non_z[1]):
                    ...
                    if self.problem.employees_list[i] not in self.employee_usage[self.problem.tasks_list[act_id]]:
                        self.employee_usage[self.problem.tasks_list[act_id]][self.problem.employees_list[i]] = set()
                    self.employee_usage[self.problem.tasks_list[act_id]][self.problem.employees_list[i]].add(
                        self.problem.skills_list[j])
        self.modes = {self.problem.tasks_list_non_dummy[i]: self.modes_vector[i]
...
class MS_RCPSPSolution_Preemptive_Variant(MS_RCPSPSolution_Preemptive):
    ...
    def do_recompute(self, fast=True):
        if not fast:
            rcpsp_schedule, modes_extended, employee_usage, modes_dict = sgs_multi_skill_preemptive(solution=self)
            self.schedule = rcpsp_schedule
            self.modes = modes_dict
            self.employee_usage = employee_usage
            self._schedule_to_recompute = False
            ...
            starts_dict, ends_dict, skills_usage, unfeasible_non_renewable_resources = \
                self.problem.func_sgs(permutation_task=permutation_do_to_permutation_sgs_fast(rcpsp_problem=self.problem,
                                      ...
                                      # array(task, worker)
                                      modes_array=np.array([0] + self.modes_vector_from0 + [0]))
            self.update_from_numba_output(starts_dict, ends_dict, skills_usage, unfeasible_non_renewable_resources)

    def update_from_numba_output(self, starts_dict, ends_dict, skills_usage, unfeasible_non_renewable_resources):
        if unfeasible_non_renewable_resources:
            self.schedule = {}
            for t in self.problem.tasks_list:
                self.schedule[t] = {"starts": [99999],
        ...
        for k in starts_dict:
            self.schedule[self.problem.tasks_list[k]] = {"starts": starts_dict[k],
                                                         "ends": ends_dict[k]}
        for act_id in skills_usage:
            self.employee_usage[self.problem.tasks_list[act_id]] = \
                [{}
                 for k in range(len(self.schedule[self.problem.tasks_list[act_id]]["starts"]))]
            non_z = np.nonzero(skills_usage[act_id])
            for i, j, k in zip(non_z[0], non_z[1], non_z[2]):
                if self.problem.employees_list[j] not in self.employee_usage[self.problem.tasks_list[act_id]][i]:
                    self.employee_usage[self.problem.tasks_list[act_id]][i][self.problem.employees_list[j]] = set()
                self.employee_usage[self.problem.tasks_list[act_id]][i][self.problem.employees_list[j]].add(
                    self.problem.skills_list[k])
        self.modes = {self.problem.tasks_list_non_dummy[i]: self.modes_vector[i]
                      for i in range(self.problem.n_jobs_non_dummy)}
        self.modes[self.problem.sink_task] = 1
        self.modes[self.problem.source_task] = 1
    ...
    def run_sgs_partial(self, current_t,
        ...
        if not fast:
            ...
            scheduled_task_indicator, scheduled_tasks_start_times_array, \
            scheduled_tasks_end_times_array, nb_subparts, worker_used = \
                build_partial_vectors_preemptive(problem=self.problem,
                                                 completed_tasks=completed_tasks,
                                                 scheduled_tasks_start_times=scheduled_tasks_start_times)
            starts_dict, ends_dict, skills_usage, unfeasible_non_renewable_resources = \
                self.problem.func_sgs_partial(permutation_task=
                                              permutation_do_to_permutation_sgs_fast(rcpsp_problem=self.problem,
                                                                                     permutation_do=self.priority_list_task),
                                              priority_worker_per_task=
                                              ...
                                              worker_used=worker_used,
                                              current_time=current_t)
            self.update_from_numba_output(starts_dict,
                                          ends_dict,
                                          skills_usage,
                                          unfeasible_non_renewable_resources)

    def copy(self):
        return MS_RCPSPSolution_Preemptive_Variant(problem=self.problem,
                                                   priority_list_task=deepcopy(self.priority_list_task),
                                                   modes_vector=deepcopy(self.modes_vector),
                                                   priority_worker_per_task=deepcopy(self.priority_worker_per_task),
                                                   modes=deepcopy(self.modes),
...
def schedule_solution_preemptive_to_variant(solution: MS_RCPSPSolution_Preemptive):
    ...
    for i in s.problem.tasks_list_non_dummy:
        ...
        w += [wi for wi in s.problem.employees if wi not in w]
        workers += [w]
    solution = MS_RCPSPSolution_Preemptive_Variant(problem=s.problem,
                                        priority_list_task=[s.problem.index_task_non_dummy[p]
                                                            for p in priority_list_task],
                                        priority_worker_per_task=workers,
                                        modes_vector=[s.modes[i]
                                                      for i in s.problem.tasks_list_non_dummy])
...
def sgs_multi_skill(solution: MS_RCPSPSolution_Variant):
    ...
    for res in problem.resources_set:
        resource_avail_in_time[res] = np.array(problem.resources_availability[res][:new_horizon + 1])
    worker_avail_in_time = {}
    for i in problem.employees:
        worker_avail_in_time[i] = np.array(problem.employees[i].calendar_employee[:new_horizon+1], dtype=np.bool)
    ...
    for act in problem.tasks_list:
        ...
        if problem.do_special_constraints:
            if act in problem.special_constraints.start_times_window:
                minimum_starting_time[act] = problem.special_constraints.start_times_window[act][0] \
                    if problem.special_constraints.start_times_window[act][0] is not None else 0
    perm_extended = [problem.tasks_list_non_dummy[x] for x in solution.priority_list_task]
    ...
    while len(perm_extended) > 0 \
                ...
                for res in resource_avail_in_time.keys():  # 10
                    if t < new_horizon:
                        if resource_avail_in_time[res][t] < \
                                problem.mode_details[act_id][modes_dict[act_id]].get(res, 0):  # 11
                            valid = False
                            break
                    else:
            ...
            if valid:
                ...
                if len(required_skills) > 0:
                    worker_ids = [worker for worker in worker_avail_in_time
                                  ...
                                  if any(s in problem.employees[worker].dict_skill and
                                         problem.employees[worker].dict_skill[s].skill_value>0
                                         for s in required_skills)]
                    if problem.one_unit_per_task_max:
                        good = False
                        ws = []
                        for worker in worker_ids:
                            if any(problem.employees[worker].dict_skill[s].skill_value < required_skills[s]
                                   for s in required_skills):
                                continue
                            else:
                                good = True
        ...
        if not unfeasible_non_renewable_resources and not unfeasible_in_horizon:
            end_t = current_min_time + problem.mode_details[act_id][modes_dict[act_id]]['duration'] - 1
            for res in resource_avail_in_time.keys():
                ...
                if res in problem.non_renewable_resources:
                    resource_avail_in_time[res][end_t + 1:] -= problem.mode_details[act_id][modes_dict[act_id]][res]
                    if resource_avail_in_time[res][-1] < 0:
                        unfeasible_non_renewable_resources = True
            if worker_ids is not None:
                priority_list_this_task = solution.priority_worker_per_task[problem.index_task_non_dummy[act_id]]
                worker_used = []
                current_skills = {s: 0. for s in required_skills}
                skills_fulfilled = False
                for w in priority_list_this_task:
                    if w in worker_ids and any(problem.employees[w].dict_skill.get(s, SkillDetail(0, 0, 0)).skill_value>0
                                               for s in required_skills):
                        worker_used += [w]
                        ...
                        worker_avail_in_time[w][current_min_time:
                                                current_min_time + problem.mode_details[act_id]
                                                [modes_dict[act_id]]['duration']] = False
                ...
                if not skills_fulfilled:
                    ...
                    print("Warning. You probably didnt give the right worker named in priority_worker_per_task")
                    break
            ...
            for s in problem.successors[act_id]:
                minimum_starting_time[s] = max(minimum_starting_time[s], activity_end_times[act_id])
        worker_ids = None
...
def sgs_multi_skill_preemptive(solution: MS_RCPSPSolution_Preemptive_Variant):
    ...
    scheduled = set()
    unfeasible_skills = False
    expected_durations_task = {k: problem.mode_details[k][modes_dict[k]]["duration"]
                               for k in modes_dict}

    def find_workers(required_skills, task, current_min_time):
        valid = True
        if len(required_skills) > 0:
            worker_ids = [worker for worker in solution.priority_worker_per_task[problem.index_task_non_dummy[task]]
            ...
            if problem.one_unit_per_task_max:
               ...
               if not all(sum([problem.employees[worker].dict_skill[s].skill_value
                               for worker in worker_ids]) >= required_skills[s]
                          for s in required_skills):
                   valid = False
            if valid:
                priority_list_this_task = solution.priority_worker_per_task[problem.index_task_non_dummy[task]]
                worker_used = []
                current_skills = {s: 0. for s in required_skills}
                skills_fulfilled = False
                for w in priority_list_this_task:
    ...
    while len(perm_extended) > 0 \
        ...
        while not valid:
            ...
            if expected_durations_task[act_id] == 0:
                ...
                for t in range(current_min_time,
                               current_min_time
                               + expected_durations_task[act_id]-cur_duration):
                    if t == current_min_time:
                        valid, skills_fulfilled, worker_used = find_workers(required_skills=required_skills,
                                                                            task=act_id,
                    ...
                    if t >= new_horizon:
                        ...
                        unfeasible_non_renewable_resources = True
                        break
                    if any(resource_avail_in_time[res][t] <
                           problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
            ...
            if not valid:
                current_min_time = next(
                    t for t in range(reached_t + 2 if reached_t is not None else current_min_time + 1,
                                     new_horizon)
                    if all(resource_avail_in_time[res][t] >=
                           problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                           for res in problem.resources_list))
            if current_min_time > new_horizon:
                unfeasible_in_horizon = True
        ...
        if not unfeasible_non_renewable_resources and not unfeasible_in_horizon:
            ...
            for s, e, ws, j in zip(starts, ends, workers, range(len(starts))):
                for res in resource_avail_in_time.keys():
                    resource_avail_in_time[res][s:e] -= \
                        problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                    if res in problem.non_renewable_resources and e == end_t:
                        resource_avail_in_time[res][end_t + 1:] -= problem.mode_details[act_id][modes_dict[act_id]][res]
                        if resource_avail_in_time[res][-1] < 0:
                            unfeasible_non_renewable_resources = True
                current_skills = {skill: 0. for skill in required_skills}
                for w in ws:
                    for skill in problem.employees[w].dict_skill:
                        if skill in current_skills:
                            current_skills[skill] += problem.employees[w].dict_skill[skill].skill_value
                            if act_id not in employee_usage:
                                employee_usage[act_id] = [{} for k in range(len(starts))]
                    ...
                    if all(current_skills[skill] >= required_skills[skill] for skill in required_skills):
                        skills_fulfilled = True
                        break
...
def sgs_multi_skill_preemptive_partial_schedule(solution: MS_RCPSPSolution_Preemptive_Variant,
                                                current_t, completed_tasks: Dict[Hashable, TaskDetailsPreemptive],
                                                scheduled_tasks_start_times: Dict[Hashable, TaskDetailsPreemptive]):
    problem: MS_RCPSPModel = solution.problem
    activity_end_times = {}
    unfeasible_non_renewable_resources = False
    ...
    worker_avail_in_time = {}
    for i in problem.employees:
        worker_avail_in_time[i] = np.array(problem.employees[i].calendar_employee[:new_horizon+1],
                                           dtype=np.bool)
    minimum_starting_time = {}
    for act in problem.tasks_list:
        ...
        if problem.do_special_constraints:
            if act in problem.special_constraints.start_times_window:
                minimum_starting_time[act] = max(problem.special_constraints.start_times_window[act][0], current_t) \
                    if problem.special_constraints.start_times_window[act][0] is not None else current_t
    perm_extended = [problem.tasks_list_non_dummy[x] for x in solution.priority_list_task]
    perm_extended.insert(0, problem.source_task)
    perm_extended.append(problem.sink_task)
    modes_dict = problem.build_mode_dict(solution.modes_vector)
        ...
        for s, e, ws, j in zip(start_time, end_time, workers, range(len(start_time))):
            for res in resource_avail_in_time.keys():
                ...
                if res in problem.non_renewable_resources and e == end_time[-1]:
                    resource_avail_in_time[res][end_time[-1]+1:] -= problem.mode_details[c][modes_dict[c]][res]
                    if resource_avail_in_time[res][-1] < 0:
                        unfeasible_non_renewable_resources = True
            for unit in ws:
            ...
            if len(ws) > 0:
                for unit in ws:
                    employee_usage[c][j][unit] = set(problem.employees[unit].dict_skill.keys())\
                        .intersection(set([s for s in problem.skills_set
...
def sgs_multi_skill_partial_schedule(solution: MS_RCPSPSolution_Variant,
                                     current_t,
                                     completed_tasks: Dict[Hashable, TaskDetails],
                                     scheduled_tasks_start_times: Dict[Hashable, TaskDetails]):
    problem: MS_RCPSPModel = solution.problem
    ...
    for c in completed_tasks:
        ...
        for res in resource_avail_in_time.keys():
            resource_avail_in_time[res][start_time:end_time] -= \
                problem.mode_details[c][modes_dict[c]].get(res, 0)
            if res in problem.non_renewable_resources:
                resource_avail_in_time[res][end_time:] -= problem.mode_details[c][modes_dict[c]][res]
                if resource_avail_in_time[res][-1] < 0:
                    unfeasible_non_renewable_resources = True
        ...
        if len(completed_tasks[c].resource_units_used) > 0:
            employee_usage[c] = {}
            for unit in completed_tasks[c].resource_units_used:
                employee_usage[c][unit] = set(problem.employees[unit].dict_skill.keys())\
                    .intersection(set([s for s in problem.skills_set if problem.mode_details[c][modes_dict[c]].get(s, 0) > 0]))
        scheduled.add(c)
        perm_extended.remove(c)
...
class SkillDetail:
    ...
    def __str__(self):
        return "skill-value:"+str(self.skill_value)+" efficiency:"\
               + str(self.efficiency_ratio)+" experience:"+str(self.experience)
...
class Employee:
    ...
    def to_json(self, with_calendar: bool=True):
        if with_calendar:
            return {"dict_skill": {s: {attr: getattr(self.dict_skill[s], attr)
...
class MS_RCPSPModel(Problem):
    ...
    def __init__(self,
        ...
        if source_task is None:
            self.source_task = min(self.tasks_list)  # tasks id should be comparable in this case
        self.sink_task = sink_task
        if sink_task is None:
            self.sink_task = max(self.tasks_list)  # tasks id should be comparable in this case
        self.tasks_list_non_dummy = [t for t in self.tasks_list if t not in {self.source_task, self.sink_task}]
        self.index_task_non_dummy = {self.tasks_list_non_dummy[i]: i for i in range(self.n_jobs_non_dummy)}
        ...
        self.is_multimode = max([len(self.mode_details[key1].keys()) for key1 in self.mode_details.keys()]) > 1
        self.is_calendar = False
        if any(isinstance(self.resources_availability[res], Iterable)
               for res in self.resources_availability):
            self.is_calendar = max([len(set(self.resources_availability[res])
                                        if isinstance(self.resources_availability[res], Iterable)
                                        else {self.resources_availability[res]})
                                    for res in self.resources_availability]) > 1
        ...
        self.max_number_of_mode = max([len(self.mode_details[x]) for x in self.mode_details])
        ...
        self.index_employee = {self.employees_list[i]: i for i in range(len(self.employees_list))}
        ...
        self.graph = self.compute_graph()
        self.predecessors = self.graph.predecessors_dict
        self.total_number_step_available = {employee:
                                            sum(self.employees[employee].calendar_employee[:self.horizon])
        ...
        if all(f is None for f in [self.partial_preemption_data,
            ...
            self.partial_preemption_data = {t: {m: {r: True for r in self.resources_set}
                                                for m in self.mode_details[t]} for t in self.mode_details}
        if self.always_releasable_resources is not None and self.never_releasable_resources is not None and \
                len(self.always_releasable_resources)+len(self.never_releasable_resources) == len(self.resources_set):
            if self.partial_preemption_data is None:
                self.partial_preemption_data = {t: {m: {r: r in self.always_releasable_resources
                                                        for r in self.resources_set}
                                                    for m in self.mode_details[t]} for t in self.mode_details}
        if self.partial_preemption_data is not None and self.always_releasable_resources is None\
                and self.never_releasable_resources is None:
            self.always_releasable_resources = set()
            self.never_releasable_resources = set()
            for r in self.resources_set:
                if all(self.partial_preemption_data[t][m].get(r, True)
                       for t in self.partial_preemption_data for m in self.partial_preemption_data[t]):
                    self.always_releasable_resources.add(r)
        self.strictly_disjunctive_subtasks = strictly_disjunctive_subtasks
        self.func_sgs, self.func_sgs_partial = create_np_data_and_jit_functions(rcpsp_problem=self)
    ...
    def build_multimode_rcpsp_calendar_representative(self):
        ...
        rcpsp_model = RCPSPModel(resources=res_availability,
    ...
    def build_mode_dict(self, rcpsp_modes_from_solution):
        modes_dict = {self.tasks_list_non_dummy[i]: rcpsp_modes_from_solution[i]
    ...
    def evaluate(self, rcpsp_sol: MS_RCPSPSolution) -> Dict[str, float]:
        ...
        if self.includes_special_constraint():
            penalty = evaluate_constraints(solution=rcpsp_sol, constraints=self.special_constraints)
    ...
    def satisfy_classic(self, rcpsp_sol: MS_RCPSPSolution) -> bool:
        ...
        overlaps = [(t1, t2) for t1 in self.tasks for t2 in self.tasks
                    ...
                    and intersect((rcpsp_sol.schedule[t1]["start_time"],
                                   rcpsp_sol.schedule[t1]["end_time"]),
                                  (rcpsp_sol.schedule[t2]["start_time"],
                                   rcpsp_sol.schedule[t2]["end_time"]))
                    is not None
                    and rcpsp_sol.schedule[t1]["end_time"] > rcpsp_sol.schedule[t1]["start_time"]
                    and rcpsp_sol.schedule[t2]["end_time"] > rcpsp_sol.schedule[t2]["start_time"]]
            ...
            for act_id in self.tasks:
                ...
                if start <= t and t < end:
                    ...
                    for res in self.resources_set:  # self.mode_details[act_id][mode]:
                        ...
                        resource_usage[res] += self.mode_details[act_id][mode].get(res, 0)
            for res in self.resources_set:
    ...
    def satisfy_preemptive(self, rcpsp_sol: MS_RCPSPSolution_Preemptive) -> bool:
        ...
        for task in self.tasks:
            ...
            required_skills = {s: self.mode_details[task][mode][s]
                               ...
                               if s in self.skills_set and self.mode_details[task][mode][s] > 0}
            # Skills for the given task are used
            if len(required_skills) > 0 and self.mode_details[task][mode]["duration"]>0:
                for skill in required_skills:
            ...
            if task in rcpsp_sol.employee_usage:
                for i in range(rcpsp_sol.get_number_of_part(task)):
                    employee_used = [emp for emp in rcpsp_sol.employee_usage[task][i]
                                     if len(rcpsp_sol.employee_usage[task][i][emp]) > 0]
                    if len(employee_used) > 0:
                        for e in employee_used:
                            if not all(self.employees[e].calendar_employee[t]
                                       for t in range(rcpsp_sol.schedule[task]["starts"][i],
                                                      rcpsp_sol.schedule[task]["ends"][i])):
                                print("Task : ", task)
                                print("Employee : ", e)
                                print(rcpsp_sol.schedule[task]["starts"][i],
                                      rcpsp_sol.schedule[task]["ends"][i],
                                      [self.employees[e].calendar_employee[t] for t in range(rcpsp_sol.schedule[task]["starts"][i],
                                                                                             rcpsp_sol.schedule[task]["ends"][i])])
                                print('Problem with employee availability')
                                return False
        for employee in self.employees:
            usage = np.zeros((rcpsp_sol.get_end_time(self.sink_task)+1))
            task_employees = set([(t, j) for t in rcpsp_sol.employee_usage
    ...
    def get_objective_register(self) -> ObjectiveRegister:
        ...
        if self.includes_special_constraint():
            dict_objective["constraint_penalty"] = {"type": TypeObjective.PENALTY, "default_weight": -100}
    ...
    def to_variant_model(self):
        return MS_RCPSPModel_Variant(skills_set=self.skills_set,
                                     ...
                                     special_constraints=None if not self.do_special_constraints
...
class MS_RCPSPModel_Variant(MS_RCPSPModel):
    ...
    def get_dummy_solution(self, preemptive: Optional[bool] = None):
        ...
        if not preemptive:
            ...
            return MS_RCPSPSolution_Preemptive_Variant(problem=self,
                                                       priority_list_task=[i for i in range(self.n_jobs_non_dummy)],
                                                       modes_vector=[1 for i in range(self.n_jobs_non_dummy)],
                                                       priority_worker_per_task=[[w for w in self.employees]
                                                                                 for i in range(self.n_jobs_non_dummy)],
...
def permutation_do_to_permutation_sgs_fast(rcpsp_problem: MS_RCPSPModel, permutation_do):
    perm_extended = [rcpsp_problem.index_task[rcpsp_problem.tasks_list_non_dummy[x]]
...
def priority_worker_per_task_do_to_permutation_sgs_fast(rcpsp_problem: MS_RCPSPModel, priority_worker_per_task):
    ...
    for i in range(len(priority_worker_per_task)):
        p[i+1, :] = np.array([rcpsp_problem.index_employee[e] for e in priority_worker_per_task[i]])
    return p


def build_partial_vectors(problem: MS_RCPSPModel,
                          completed_tasks: Dict[Hashable, TaskDetails],
                          scheduled_tasks_start_times: Dict[Hashable, TaskDetails]):
    ...
    for dict_data in [completed_tasks, scheduled_tasks_start_times]:
        for t in dict_data:
            ...
            scheduled_tasks_start_times_vector[problem.index_task[t]] = dict_data[t].start
    ...
    return scheduled_task_indicator, scheduled_tasks_start_times_vector, scheduled_tasks_end_times_vector, worker_used


def build_partial_vectors_preemptive(problem: MS_RCPSPModel,
                                     completed_tasks: Dict[Hashable, TaskDetailsPreemptive],
                                     scheduled_tasks_start_times: Dict[Hashable, TaskDetailsPreemptive]):
    scheduled_task_indicator = np.zeros((problem.n_jobs))
    ...
    for dict_data in [completed_tasks, scheduled_tasks_start_times]:
        for t in dict_data:
            ...
            scheduled_tasks_start_times_array[problem.index_task[t], :nb_subparts[problem.index_task[t]]] =\
                np.array(dict_data[t].starts)
            scheduled_tasks_end_times_array[problem.index_task[t], :nb_subparts[problem.index_task[t]]] =\
    ...
    return scheduled_task_indicator, scheduled_tasks_start_times_array,\
           scheduled_tasks_end_times_array, nb_subparts, worker_used


def create_fake_tasks_multiskills(rcpsp_problem: Union[MS_RCPSPModel, MS_RCPSPSolution_Variant]):
    ressources_arrays = {r: np.array(rcpsp_problem.resources_availability[r])
                         for r in rcpsp_problem.resources_list}
    max_capacity = {r: np.max(ressources_arrays[r])
    ...
    for r in ressources_arrays:
        ...
        if ressources_arrays[r][0] < max_capacity[r]:
            consume = {r: int(max_capacity[r] - ressources_arrays[r][0]),
        ...
        for j in range(len(index_non_zero)-1):
            ...
            if value != max_capacity[r]:
                consume = {r: int(max_capacity[r]-value),
                           "duration": int(index_non_zero[j+1]-ind),
                           "start": int(ind+1)}
                fake_tasks += [consume]
    unit_arrays = {j: np.array(rcpsp_problem.employees[j].calendar_employee, dtype=np.int)
    ...
    for r in unit_arrays:
        ...
        if unit_arrays[r][0] < max_capacity[r]:
            consume = {r: int(max_capacity[r] - unit_arrays[r][0]),
                       "duration": int(index_non_zero[0] + 1),
                       "start": 0}
...
def cluster_employees_to_resource_types(ms_rcpsp_problem: MS_RCPSPModel):
    ...
    for employee in ms_rcpsp_problem.employees:
        skills = [s for s in sorted(ms_rcpsp_problem.employees[employee].dict_skill.keys())
                  if ms_rcpsp_problem.employees[employee].dict_skill[s].skill_value > 0]
        str_representation = "-".join([str(s)+"-"+str(ms_rcpsp_problem.employees[employee].dict_skill[s])
                                       for s in skills])
        if str_representation not in skills_representation_str:
            skills_representation_str[str_representation] = set()
            skills_dict[str_representation] = {s:
                                               ms_rcpsp_problem.employees[employee].dict_skill[s]
                                               for s in skills}
        skills_representation_str[str_representation].add(employee)
    return skills_representation_str,  skills_dict


def create_np_data_and_jit_functions(rcpsp_problem: Union[MS_RCPSPModel, MS_RCPSPModel_Variant]):
    consumption_array = np.zeros((rcpsp_problem.n_jobs, rcpsp_problem.max_number_of_mode,
                                  len(rcpsp_problem.resources_list)), dtype=np.int32)
    is_releasable_array = np.zeros((rcpsp_problem.n_jobs, rcpsp_problem.max_number_of_mode,
                                    len(rcpsp_problem.resources_list)), dtype=np.int32)
    skills_need = np.zeros((rcpsp_problem.n_jobs,
                            rcpsp_problem.max_number_of_mode,
                            len(rcpsp_problem.skills_list)),
                           dtype=np.int32)
    duration_array = np.zeros((rcpsp_problem.n_jobs,
                               rcpsp_problem.max_number_of_mode), dtype=np.int)

    predecessors = np.zeros((rcpsp_problem.n_jobs,
                             rcpsp_problem.n_jobs), dtype=np.int)
    successors = np.zeros((rcpsp_problem.n_jobs,
                           rcpsp_problem.n_jobs), dtype=np.int)
    horizon = rcpsp_problem.horizon
    ressource_available = np.zeros((len(rcpsp_problem.resources_list), horizon), dtype=np.int32)
    worker_available = np.zeros((len(rcpsp_problem.employees_list), horizon), dtype=np.int32)
    ressource_renewable = np.ones((len(rcpsp_problem.resources_list)), dtype=bool)
    worker_skills = np.zeros((len(rcpsp_problem.employees_list), len(rcpsp_problem.skills_list)),
                             dtype=np.int)
    ...
    for i in range(len(rcpsp_problem.tasks_list)):
        ...
        index_mode = 0
        for mode in sorted(rcpsp_problem.mode_details[rcpsp_problem.tasks_list[i]].keys()):
            for k in range(len(rcpsp_problem.resources_list)):
                consumption_array[i, index_mode, k] = \
                    rcpsp_problem.mode_details[task][mode].get(rcpsp_problem.resources_list[k], 0)
                is_releasable_array[i, index_mode, k] = 1 if \
                rcpsp_problem.partial_preemption_data[task][mode].get(rcpsp_problem.resources_list[k], True) else 0
            ...
            for s in range(len(rcpsp_problem.skills_list)):
                skills_need[i, index_mode, s] = \
                    rcpsp_problem.mode_details[task][mode].get(rcpsp_problem.skills_list[s], 0)
            duration_array[i, index_mode] = rcpsp_problem.mode_details[task][mode]["duration"]
            index_mode += 1
    ...
    for k in range(len(rcpsp_problem.resources_list)):
        ressource_available[k, :] = \
            rcpsp_problem.resources_availability[rcpsp_problem.resources_list[k]][:horizon]
    ...
    for emp in range(len(rcpsp_problem.employees_list)):
        worker_available[emp, :] = np.array(rcpsp_problem.employees[rcpsp_problem.employees_list[emp]]
                                            .calendar_employee, dtype=np.int)[:horizon]
        for s in range(len(rcpsp_problem.skills_list)):
            worker_skills[emp, s] = rcpsp_problem.employees[rcpsp_problem.employees_list[emp]]\
                .dict_skill.get(rcpsp_problem.skills_list[s],
                                SkillDetail(skill_value=0, efficiency_ratio=0, experience=0)).skill_value
    for i in range(len(rcpsp_problem.tasks_list)):
        task = rcpsp_problem.tasks_list[i]
    ...
    if rcpsp_problem.includes_special_constraint():
        start_at_end_plus_offset = np.zeros((len(rcpsp_problem.special_constraints.start_at_end_plus_offset), 3),
                                            dtype=int)
        start_after_nunit = np.zeros((len(rcpsp_problem.special_constraints.start_after_nunit), 3),
                                     dtype=int)
        j = 0
    ...
    if rcpsp_problem.preemptive:
        ...
        for t in rcpsp_problem.preemptive_indicator:
            preemptive_tag[rcpsp_problem.index_task[t]] = 1 if rcpsp_problem.preemptive_indicator[t] else 0
    ...
    if rcpsp_problem.preemptive:
        if rcpsp_problem.includes_special_constraint() and (start_after_nunit.shape[0] > 0
                                                            or start_at_end_plus_offset.shape[0] > 0):
            func_sgs = partial(sgs_fast_ms_preemptive_some_special_constraints,
            ...
            func_sgs = partial(sgs_fast_ms_preemptive,
                               consumption_array=consumption_array,
                               skills_needs=skills_need,
                               worker_skills=worker_skills,
                               worker_available=worker_available,
                               ...
                               consider_partial_preemptive=consider_partial_preemptive,
                               strictly_disjunctive_subtasks=rcpsp_problem.strictly_disjunctive_subtasks)
        func_sgs_partial = partial(sgs_fast_ms_preemptive_partial_schedule,
                                   consumption_array=consumption_array,
                                   skills_needs=skills_need,
                                   worker_skills=worker_skills,
                                   worker_available=worker_available,
        ...
        func_sgs = partial(sgs_fast_ms,
                           ...
                           ressource_renewable=ressource_renewable,
                           one_unit_per_task=rcpsp_problem.one_unit_per_task_max)
        func_sgs_partial = partial(sgs_fast_ms_partial_schedule,
                                   consumption_array=consumption_array,
                                   skills_needs=skills_need,
                                   worker_skills=worker_skills,
                                   worker_available=worker_available,
...
def employee_usage(solution: Union[MS_RCPSPSolution, MS_RCPSPSolution_Preemptive],
    ...
    for task in problem.tasks_list:
        ...
        for i in range(len(employees)):
            for e in employees[i]:
                ...
                employee_usage_matrix[problem.index_employee[e], starts[i]:ends[i]] = 1
    sum_usage = np.sum(employee_usage_matrix, axis=1)
    return employee_usage_matrix, sum_usage, employees_usage_dict


def evaluate_constraints(solution: Union[MS_RCPSPSolution, MS_RCPSPSolution_Preemptive],
...
def compute_constraints_details(solution: Union[MS_RCPSPSolution, MS_RCPSPSolution_Preemptive],
                                constraints: SpecialConstraintsDescription):
    ...
    for (t1, t2) in start_together:
        ...
        b = (time1 == time2)
        if not b:
            list_constraints_not_respected += [("start_together", t1, t2, time1, time2, abs(time2-time1))]
    for (t1, t2) in start_at_end:
        time1 = solution.get_end_time(t1)
        time2 = solution.get_start_time(t2)
        b = (time1 == time2)
        if not b:
            list_constraints_not_respected += [("start_at_end", t1, t2, time1, time2, abs(time2-time1))]
    for (t1, t2, off) in start_at_end_plus_offset:
        time1 = solution.get_end_time(t1) + off
        time2 = solution.get_start_time(t2)
        b = (time2 >= time1)
        if not b:
            list_constraints_not_respected += [("start_at_end_plus_offset", t1, t2, time1, time2, abs(time2-time1))]
    for (t1, t2, off) in start_after_nunit:
        time1 = solution.get_start_time(t1)+off
        time2 = solution.get_start_time(t2)
        b = (time2 >= time1)
        if not b:
            list_constraints_not_respected += [("start_after_nunit", t1, t2, time1, time2, abs(time2-time1))]
    for t1, t2 in disjunctive:
        b = intersect([solution.get_start_time(t1),
                       solution.get_end_time(t1)],
                      [solution.get_start_time(t2),
                       solution.get_end_time(t2)])
        if b is not None:
            list_constraints_not_respected += [("disjunctive", t1, t2, None, None, b[1]-b[0])]
    for t in constraints.start_times_window:
        if constraints.start_times_window[t][0] is not None:
            if solution.get_start_time(t) < constraints.start_times_window[t][0]:
                list_constraints_not_respected += [("start_window_0", t, t, None, None,
                                                    constraints.start_times_window[t][0] - solution.get_start_time(t))]
        ...
        if constraints.start_times_window[t][1] is not None:
            if solution.get_start_time(t) > constraints.start_times_window[t][1]:
                list_constraints_not_respected += [("start_window_1", t, t, None, None,
                                                    -constraints.start_times_window[t][1] + solution.get_start_time(t))]
                #print(list_constraints_not_respected[-1])
    ...
    for t in constraints.end_times_window:
        if constraints.end_times_window[t][0] is not None:
            if solution.get_end_time(t) < constraints.end_times_window[t][0]:
                list_constraints_not_respected += [("end_window_0", t, t, None, None,
        ...
        if constraints.end_times_window[t][1] is not None:
            if solution.get_end_time(t) > constraints.end_times_window[t][1]:
                list_constraints_not_respected += [("end_window_1", t, t, None, None,
...
def start_together_problem_description(solution: Union[MS_RCPSPSolution, MS_RCPSPSolution_Preemptive],
                                       constraints: SpecialConstraintsDescription):
    start_together = constraints.start_together
    list_constraints_not_respected = []
    for (t1, t2) in start_together:
        ...
        if not b:
            ...
            intersection_employees = employees_used_t1_first_part.intersection(employees_used_t2_first_part)
            ...
            list_constraints_not_respected += [("start_together", t1, t2, time1, time2,
                                                abs(time2 - time1), intersection_employees)]
    return list_constraints_not_respected


def check_solution(problem: Union[MS_RCPSPModel],
    ...
    for t in problem.special_constraints.start_times_window:
        if problem.special_constraints.start_times_window[t][0] is not None:
            if solution.get_start_time(t) < problem.special_constraints.start_times_window[t][0]:
                return False
        if problem.special_constraints.start_times_window[t][1] is not None:
            if solution.get_start_time(t) > problem.special_constraints.start_times_window[t][1]:
                return False
    for t in problem.special_constraints.end_times_window:
        if problem.special_constraints.end_times_window[t][0] is not None:
            if solution.get_end_time(t) < problem.special_constraints.end_times_window[t][0]:
                return False
        if problem.special_constraints.end_times_window[t][1] is not None:
            if solution.get_end_time(t) > problem.special_constraints.end_times_window[t][1]:
                return False
    return True


def compute_skills_missing_problem(problem: MS_RCPSPModel,
                                   solution: Union[MS_RCPSPSolution, MS_RCPSPSolution_Preemptive]):
    problems = []
    for task in problem.tasks:
        mode = solution.modes[task]
        required_skills = {s: problem.mode_details[task][mode][s]
                           for s in problem.mode_details[task][mode]
                           if s in problem.skills_set
                           and problem.mode_details[task][mode][s] > 0}
        # Skills for the given task are used
        if len(required_skills) > 0:
            for skill in required_skills:
                employees_used = [problem.employees[emp].dict_skill[skill].skill_value
            ...
            if task in solution.employee_usage:
                for emp in solution.employee_usage[task]:
                    set_sk = set([s for s in problem.employees[emp].dict_skill
...
def compute_ressource_array_preemptive(problem: MS_RCPSPModel,
                                       solution: Union[MS_RCPSPSolution, MS_RCPSPSolution_Preemptive]):
    resource_avail_in_time = {}
    makespan = solution.get_end_time(problem.sink_task)
    for res in problem.resources_list:
        resource_avail_in_time[res] = np.copy(problem.get_resource_availability_array(res))
...
def compute_overskill(problem: MS_RCPSPModel, solution: MS_RCPSPSolution_Preemptive):
    ...
    for task in problem.tasks_list:
        ...
        # Skills for the given task are used
        if len(required_skills) > 0 and problem.mode_details[task][mode]["duration"] > 0:
            for skill in required_skills:
                for i in range(solution.get_number_of_part(task)):
                    skills_provided = [problem.employees[emp].dict_skill[skill].skill_value
                    ...
                    if sum(skills_provided)>required_skills[skill]:
                        ...
                        overskill[task][skill][i] = (sum(skills_provided),
                                                     required_skills[skill])
    return overskill