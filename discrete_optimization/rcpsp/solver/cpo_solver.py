...
class CPOptSolver(CPSolver):
    ...
    def init_model(self, **args):
        ...
        DURATIONS = [
            self.rcpsp_model.mode_details[t][1]["duration"] for t in tasks_list
        ]

        # Extract demand of each task
        DEMANDS = [
            [
                self.rcpsp_model.mode_details[t][1][r]
        ...
        SUCCESSORS = [
            [self.index_task[tt] for tt in self.rcpsp_model.successors[t]]
            for t in tasks_list
        ]
        ...
        tasks = [
            interval_var(name="T{}".format(i + 1), size=DURATIONS[i])
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP) -> ResultStorage:
        ...
        for i in range(len(self.variables["tasks"])):
            ...
            schedule[self.rcpsp_model.tasks_list[i]] = {
                "start_time": itv.get_start(),
    ...
    def solve(self, parameters_cp: ParametersCP, **args) -> ResultStorage:
        result: CpoSolveResult = self.model.solve(
            TimeLimit=parameters_cp.TimeLimit,
    ...
    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        expression = minimize(
            max(
                end_of(self.variables["tasks"][self.index_task[t]]) for t in set_of_jobs
            )
        )
    ...
    def constraint_start_time_string(
        ...
        if sign == SignEnum.EQUAL:
            expression = (
                start_of(self.variables["tasks"][self.index_task[task]]) == start_time
            )
...
def vizu(result_storage: ResultStorage):
    if visu.is_visu_enabled():
        ...
        for i in range(problem.n_jobs):
            visu.interval(
                solution.rcpsp_schedule[problem.tasks_list[i]]["start_time"],
                solution.rcpsp_schedule[problem.tasks_list[i]]["end_time"],
                i,
                str(problem.tasks_list[i]),
            )
        ...
        for r in load:
            visu.panel("R-" + str(r))
            visu.function(
                segments=[(INTERVAL_MIN, INTERVAL_MAX, problem.resources[r])],
                style="area",
                color="lightgrey",
            )
