...
class PileSolverRCPSP(SolverDO):
    ...
    def solve(self, **kwargs)->ResultStorage:
        ...
        sol = RCPSPSolution(problem=self.rcpsp_model,
                            ...
                            rcpsp_modes=[self.modes_dict[t] for t in self.rcpsp_model.tasks_list_non_dummy],
...
class PileSolverRCPSP_Calendar(SolverDO):
    ...
    def solve(self, **kwargs)->ResultStorage:
        ...
        for r in self.resources:
            # current_ressource_available[r] = list(self.rcpsp_model.get_resource_availability_array(r))
            if isinstance(self.resources[r], Iterable):
                current_ressource_available[r] = list(self.resources[r])+[self.resources[r][-1]]*100
            else:
                current_ressource_available[r] = [self.resources[r] for i in range(self.rcpsp_model.horizon)]

        current_ressource_non_renewable = {nr: current_ressource_available[nr]
