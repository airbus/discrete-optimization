...
from discrete_optimization.rcpsp.rcpsp_model import RCPSPSolution, SingleModeRCPSPModel,\
    MultiModeRCPSPModel, RCPSPModelCalendar, PartialSolution, RCPSPModel
from discrete_optimization.rcpsp.solver.rcpsp_pile import PileSolverRCPSP, GreedyChoice, PileSolverRCPSP_Calendar
...
class LP_RCPSP(MilpSolver):
    ...
    def init_model(self,  **args):
        ...
        u = [[self.rcpsp_model.mode_details[t][1].get(r, 0) for r in resources]
        ...
        self.x: List[List[Var]] = [[self.model.add_var(name="x({},{})".format(task, t),
                                                       var_type=BINARY) for t in self.index_time]
                                   for task in sorted_tasks]
        self.index_in_var = {t: self.rcpsp_model.return_index_task(task=t, offset=0)
                             for t in sorted_tasks}
        self.model.objective = xsum(self.x[self.index_in_var[self.rcpsp_model.sink_task]][t] * t
        ...
        for (r, t) in product(self.index_resource, self.index_time):
            self.model += (xsum(u[j][r] * self.x[j][t2] for j in self.index_task
                                for t2 in range(max(0, t - p[j] + 1), t + 1))
                           <= c[r])

        for (j, s) in S:
            self.model += xsum(t * self.x[self.index_in_var[s]][t] - t * self.x[self.index_in_var[j]][t]
                               for t in self.index_time) >= p[self.index_in_var[j]]
        start = []
        for j in self.index_task:
            for t in self.index_time:
                if self.start_solution.rcpsp_schedule[self.rcpsp_model.tasks_list[j]]["start_time"] == t:
                    start += [(self.x[j][t], 1)]
                else:
                    start += [(self.x[j][t], 0)]
        ...
        if p_s is not None:
            ...
            if p_s.start_times is not None:
                for task in p_s.start_times:
                    constraints += [self.model.add_constr(xsum([j*self.x[self.index_in_var[task]][j]
                                                                for j in self.index_time]) == p_s.start_times[task])]
                    constraints += [self.model.add_constr(self.x[self.index_in_var[task]][p_s.start_times[task]] == 1)]

            if p_s.partial_permutation is not None:
                for t1, t2 in zip(p_s.partial_permutation[:-1], p_s.partial_permutation[1:]):
                    constraints += [self.model.add_constr(xsum([t * self.x[self.index_in_var[t1]][t]
                                                                - t * self.x[self.index_in_var[t2]][t]
                                                                for t in self.index_time]) <= 0)]
            ...
            for j in self.index_task:
                self.starts[j] = self.model.add_var(name="start_" + str(j),
                                                    lb=0, ub=makespan)
                self.model.add_constr(xsum(t * self.x[j][t] for t in self.index_time) == self.starts[j])
            if p_s.start_at_end is not None:
                for i, j in p_s.start_at_end:
                    constraints += [self.model.add_constr(self.starts[self.index_in_var[j]]
            ...
            if p_s.start_after_nunit is not None:
                for t1, t2, delta in p_s.start_after_nunit:
                    constraints += [self.model.add_constr(self.starts[self.index_in_var[t2]] >=
        ...
        for s in range(nb_solution):
            ...
            for (task_index, time) in product(self.index_task, self.index_time):
                ...
                if value >= 0.5:
                    ...
                    rcpsp_schedule[task] = \
                        {'start_time': time,
                         'end_time': time + self.rcpsp_model.mode_details[task][1]['duration']}
    ...
    def solve(self, parameters_milp: ParametersMilp = ParametersMilp.default(), **kwargs)\
            -> ResultStorage:
...
class LP_MRCPSP(MilpSolver):
    ...
    def init_model(self,  **args):
        ...
        if self.rcpsp_model.is_varying_resource():
            ...
            renewable_quantity = {r: [renewable[r]]*len(self.index_time) for r in renewable}

        if self.rcpsp_model.is_varying_resource():
            non_renewable_quantity = {r: non_renewable[r] for r in non_renewable}
        else:
            non_renewable_quantity = {r: [non_renewable[r]]*len(self.index_time) for r in non_renewable}
...
class LP_MRCPSP_GUROBI(MilpSolver):
    ...
    def init_model(self,  **args):
        ...
        for j in self.start_solution.rcpsp_schedule:
            start_time_j = self.start_solution.rcpsp_schedule[j]["start_time"]
            start += [(self.durations[j], self.rcpsp_model.mode_details[j][modes_dict[j]]["duration"])]
            for k in self.variable_per_task[j]:
                task, mode, time = k
        ...
        if p_s is not None:
            constraints = []
            if p_s.start_times is not None:
                constraints = self.model.addConstrs(gurobi.quicksum([self.x[k]
