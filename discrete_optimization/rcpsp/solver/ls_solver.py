...
class LS_RCPSP_Solver(SolverDO):
    def __init__(self, model: ANY_RCPSP,
    ...
    def solve(self, **kwargs):
        ...
        res = RestartHandlerLimit(nb_iteration_no_improvement=kwargs.get("nb_iteration_no_improvement", 300),
                                  cur_solution=dummy,
        ...
        if self.ls_solver == LS_SOLVER.SA:
            ls = SimulatedAnnealing(evaluator=model,
                                    ...
                                    restart_handler=res,
                                    temperature_handler=
                                    TemperatureSchedulingFactor(temperature=kwargs.get("temperature",
                                                                                       3),
                                                                restart_handler=res,
                                                                coefficient=kwargs.get("decay_temperature",
                                                                                       0.9999)),
