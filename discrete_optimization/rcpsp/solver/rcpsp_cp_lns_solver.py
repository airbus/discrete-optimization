...
from discrete_optimization.rcpsp.solver.rcpsp_lp_lns_solver import InitialSolutionRCPSP, InitialMethodRCPSP
...
from discrete_optimization.rcpsp.solver.cp_solvers import CP_RCPSP_MZN, CP_MRCPSP_MZN, CPSolver, CP_MRCPSP_MZN_WITH_FAKE_TASK
from discrete_optimization.generic_tools.lns_cp import ConstraintHandler, SolverDO, LNS_CP
from discrete_optimization.rcpsp.rcpsp_model import RCPSPModel, PartialSolution, RCPSPModelCalendar, RCPSPSolution
import numpy as np
import random
...
class ConstraintHandlerFixStartingTime(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN,
                                                                    CP_MRCPSP_MZN],
                                             child_instance,
        ...
        for job in jobs_to_fix:
            ...
            if isinstance(cp_solver, CP_RCPSP_MZN):
                string1 = "constraint s[" + str(cp_solver.index_in_minizinc[job]) + "]==" + str(start_time_j) + ";\n"
            elif isinstance(cp_solver, CP_MRCPSP_MZN):
                string1 = "constraint start[" + str(cp_solver.index_in_minizinc[job]) + "]==" + str(start_time_j) + ";\n"
...
class ConstraintHandlerStartTimeInterval_CP(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN,
        ...
        for t in modes_dict:
            if isinstance(cp_solver, CP_MRCPSP_MZN):
                list_strings += cp_solver.constraint_task_to_mode(task_id=t,
                                                                  mode=modes_dict[t])
        for job in jobs_to_fix:
            start_time_j = current_solution.rcpsp_schedule[job]["start_time"]
            ...
            if isinstance(cp_solver, CP_RCPSP_MZN):
                string1 = "constraint s[" + str(cp_solver.index_in_minizinc[job]) + "] <= " + str(max_st) + ";\n"
                string2 = "constraint s[" + str(cp_solver.index_in_minizinc[job]) + "] >= " + str(min_st) + ";\n"
            elif isinstance(cp_solver, CP_MRCPSP_MZN):
                string1 = "constraint start[" + str(cp_solver.index_in_minizinc[job]) + "] <= " + str(max_st) + ";\n"
                string2 = "constraint start[" + str(cp_solver.index_in_minizinc[job]) + "] >= " + str(min_st) + ";\n"
                list_strings += cp_solver.constraint_task_to_mode(task_id=job,
                                                                  mode=modes_dict[job])
            list_strings += [string1]
...
class ConstraintHandlerMix(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: CP_MRCPSP_MZN,
        ...
        if random.random() <= 0.95:
            ...
            max_improvement = max([self.status[x]["nb_improvement"]/max(self.status[x]["nb_usage"], 1)
...
class PostProcessLeftShift(PostProcessSolution):
    def __init__(self, rcpsp_problem: RCPSPModel,
    ...
    def build_other_solution(self, result_storage: ResultStorage) -> ResultStorage:
        for sol in list(result_storage.list_solution_fits):  # [-min(2, len(result_storage.list_solution_fits)):]:
            if "satisfy" not in sol[0].__dict__.keys():
                s: RCPSPSolution = sol[0]
                sol[0].satisfy = self.check_sol(problem=self.rcpsp_problem, solution=s) \
                                 and self.rcpsp_problem.satisfy(s)
            if sol[0].satisfy is False or True:
                if self.partial_solution is None:
                    s: RCPSPSolution = sol[0]
                    solution = RCPSPSolution(problem=self.rcpsp_problem,
                                             rcpsp_permutation=s.rcpsp_permutation,
                                             rcpsp_modes=s.rcpsp_modes)
                    solution.satisfy = self.check_sol(problem=self.rcpsp_problem, solution=solution) \
                                                      and self.rcpsp_problem.satisfy(solution)
                    result_storage.list_solution_fits += [(solution,
                                                           -self.rcpsp_problem.evaluate(solution)["makespan"])]
                    print(result_storage.list_solution_fits[-1][1])
        if self.partial_solution is None:
            solver = LS_RCPSP_Solver(model=self.rcpsp_problem,
            ...
            result_store = solver.solve(nb_iteration_max=50,
                                        init_solution=s)
...
class LNS_CP_RCPSP_SOLVER(SolverDO):
    def __init__(self, rcpsp_model: RCPSPModel,
        ...
        if self.with_varying_resource:
            self.solver = CP_MRCPSP_MZN_WITH_FAKE_TASK(rcpsp_model=self.rcpsp_model,
        ...
        self.lns_solver = LNS_CP(problem=self.rcpsp_model,
                                 ...
                                 PostProcessLeftShift(self.rcpsp_model,
                                                      partial_solution=kwargs.get("partial_solution", None)),
