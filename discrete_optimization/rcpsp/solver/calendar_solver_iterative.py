...
from discrete_optimization.rcpsp.solver.rcpsp_cp_lns_solver import ConstraintHandlerStartTimeInterval_CP,  \
...
from discrete_optimization.generic_tools.do_problem import get_default_objective_setup, ModeOptim
...
from discrete_optimization.rcpsp.rcpsp_model import RCPSPModel, RCPSPSolution, RCPSPModelCalendar, \
    PartialSolution
import numpy as np


def get_ressource_breaks(problem_calendar: RCPSPModel,
    ...
    for r in ressources:
        ...
        ressource_arrays_usage[r] = np.zeros((solution.get_max_end_time()+1,
        ...
        for t in task_concerned[r]:
            ...
            first_possible_start_before = next((st for st in range(current_start, -1, -1)
                                               if problem_calendar.resources[r][st]
                                                >= problem_calendar.mode_details[t][1][r]),
                                               None)
            first_possible_start_before = next((st - problem_calendar.mode_details[t][1]["duration"] + 1
                                                for st in range(solution.rcpsp_schedule[t]["end_time"], -1, -1)
                                                if problem_calendar.resources[r][st - 1]
                                                >= problem_calendar.mode_details[t][1][r]
                                                and problem_calendar.resources[r][max(0,
                                                                                      st -
...
class ConstraintHandlerAddCalendarConstraint(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN,
                ...
                for t in constraints[r]:
                    ...
                    if isinstance(cp_solver, CP_MRCPSP_MZN):
                        if constraints[r][t][0] is not None and constraints[r][t][1] is not None:
                            s = """constraint start["""+str(cp_solver.index_in_minizinc[t])+"""]<="""\
                                + str(constraints[r][t][0]) + " \/ " \
                                "start["""+str(cp_solver.index_in_minizinc[t])+"""]>="""+str(constraints[r][t][1])+""";\n"""
                        ...
                        if constraints[r][t][0] is not None and constraints[r][t][1] is not None:
                            s = """constraint s["""+str(cp_solver.index_in_minizinc[t])+"""]<="""\
                                + str(constraints[r][t][0]) + " \/ " \
                                "s["""+str(cp_solver.index_in_minizinc[t])+"""]>="""+str(constraints[r][t][1])+""";\n"""
                        elif constraints[r][t][0] is None and constraints[r][t][1] is not None:
                            s = """constraint s[""" + str(cp_solver.index_in_minizinc[t]) + """]>=""" \
                                + str(constraints[r][t][1]) + """;\n"""
                        elif constraints[r][t][0] is not None and constraints[r][t][1] is None:
        ...
        for r in ressource_breaks:
            ...
            for index in [ressource_breaks[r][i] for i in range(0, len(ressource_breaks[r]), 20)] \
                          + [ressource_breaks[r][-1]]:
                ind = index[0]
...
class SolverWithCalendarIterative(SolverDO):
    def __init__(self, problem_calendar: RCPSPModelCalendar, partial_solution: PartialSolution=None,
                 option_neighbor: OptionNeighbor=OptionNeighbor.MIX_ALL, **kwargs):
        self.problem_calendar = problem_calendar
        self.problem_no_calendar = RCPSPModel(resources={r: int(self.problem_calendar.get_max_resource_capacity(r))
        ...
        self.post_process_solution = PostProcessSolutionNonFeasible(self.problem_calendar,
                                                                    self.problem_no_calendar,
                                                                    partial_solution=partial_solution)
        self.lns_solver = LNS_CP(problem=self.problem_calendar,
