...
import logging
from typing import Any, Dict, Hashable, List, Optional, Tuple, Set
...
class CPSatRCPSPSolver(SolverDO):
    ...
    def init_temporal_variable(self, model: CpModel) -> Tuple[Dict[Hashable, IntVar],
    ...
    def add_classical_precedence_constraints(self,
                                             model: CpModel,
    ...
    def add_one_mode_selected_per_task(self, model: CpModel,
    ...
    def create_cumulative_constraint(self,
                                     model: CpModel,
                                     resource: str,
    ...
    def create_mode_pair_constraint(self,
        ...
        if pair_mode_constraint.allowed_mode_assignment is not None:
            for task1, task2 in pair_mode_constraint.allowed_mode_assignment:
                pairs_allowed = pair_mode_constraint.allowed_mode_assignment[(task1, task2)]
    ...
    def init_model(self, **kwargs):
        include_special_constraints = kwargs.get("include_special_constraints", self.problem.includes_special_constraint())
        model = CpModel()
        starts_var, ends_var, is_present_var, interval_var, interval_per_tasks = (
            self.init_temporal_variable(model=model))
        self.add_one_mode_selected_per_task(model=model, is_present_var=is_present_var,
                                            interval_per_tasks=interval_per_tasks)
        self.add_classical_precedence_constraints(model=model,
                                                  starts_var=starts_var,
                                                  ends_var=ends_var)
        fake_task = self.create_fake_tasks()
        resources = self.problem.resources_list
        for resource in resources:
            self.create_cumulative_constraint(model=model,
                                              resource=resource,
        ...
        if include_special_constraints:
            if self.problem.special_constraints.pair_mode_constraint is not None:
                self.create_mode_pair_constraint(model=model,
                                                 interval_per_tasks=interval_per_tasks,
                                                 is_present_var=is_present_var,
                                                 pair_mode_constraint=
                                                 self.problem.special_constraints.pair_mode_constraint)
        self.cp_model = model
        self.variables = {
            "start": starts_var,
            "end": ends_var,
...
class CPSatRCPSPSolverResource(CPSatRCPSPSolver):
    ...
    """
    def create_cumulative_constraint_and_used_resource(self,
                                                       model: CpModel,
                                                       resource: str,
    ...
    def init_model(self, **kwargs):
        include_special_constraints = kwargs.get("include_special_constraints",
                                                 self.problem.includes_special_constraint())
        ...
        resources = self.problem.resources_list
        is_used_resource = {
            res: model.NewBoolVar(f"used_{res}") for res in resources
        }
        for resource in resources:
            self.create_cumulative_constraint_and_used_resource(model=model,
                                                                resource=resource,
...
class CPSatRCPSPSolverCumulativeResource(CPSatRCPSPSolver):
    ...
    def create_cumulative_constraint_and_resource_capa(self,
                                                       model: CpModel,
                                                       resource: str,
    ...
    def init_model(self, **kwargs):
        ...
        self.add_classical_precedence_constraints(model=model,
                                                  starts_var=starts_var,
                                                  ends_var=ends_var)
        self.add_one_mode_selected_per_task(model=model,
                                            is_present_var=is_present_var,
                                            interval_per_tasks=interval_per_tasks)
        fake_task = self.create_fake_tasks()
        resources = self.problem.resources_list
        for resource in resources:
            self.create_cumulative_constraint_and_resource_capa(model=model,
                                                                resource=resource,
                                                                resource_capacity_var=resource_capacity_var,
                                                                interval_var=interval_var,
                                                                is_present_var=is_present_var,
                                                                fake_task=fake_task,
                                                                use_overlap_for_disjunctive_resource=
                                                                use_overlap_for_disjunctive_resource)
        if include_special_constraints:
            if self.problem.special_constraints.pair_mode_constraint is not None:
                self.create_mode_pair_constraint(model=model,
