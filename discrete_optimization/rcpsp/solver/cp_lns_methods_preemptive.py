from typing import Iterable, Any, Union, List, Set, Hashable
import math
...
from discrete_optimization.rcpsp.solver.cp_solvers import CP_RCPSP_MZN, CP_MRCPSP_MZN,\
    CPSolver, CP_RCPSP_MZN_PREEMMPTIVE, CP_MRCPSP_MZN_PREEMMPTIVE
...
from discrete_optimization.rcpsp.specialized_rcpsp.rcpsp_specialized_constraints \
    import RCPSPModelSpecialConstraintsPreemptive, RCPSPModelSpecialConstraints, compute_constraints_details, RCPSPSolutionSpecialPreemptive
from discrete_optimization.generic_rcpsp_tools.graph_tools_rcpsp import GraphRCPSP, GraphRCPSPSpecialConstraints
...
class PostProLeftShift(PostProcessSolution):
    def __init__(self, problem: RCPSPModelPreemptive,
        ...
        if isinstance(problem, (RCPSPModelSpecialConstraintsPreemptive, RCPSPModelSpecialConstraints)):
    ...
    def build_other_solution(self, result_storage: ResultStorage) -> ResultStorage:
        new_solution = sgs_variant(solution=last_opti_solution(result_storage),
        ...
        #         result_storage.add_solution(new_solution, fit)
        if self.do_ls:
            solver = LS_RCPSP_Solver(model=self.problem,
                                     ls_solver=LS_SOLVER.SA)
            s = result_storage.get_best_solution().copy()
            if self.problem != s.problem:
                s.change_problem(self.problem)
            result_store = solver.solve(nb_iteration_max=self.dict_params.get("nb_iteration_max", 200),
                                        init_solution=s)
...
def sgs_variant(solution: RCPSPSolutionPreemptive,
    ...
    sorted_tasks = sorted(solution.rcpsp_schedule.keys(),
                          key=lambda x: (solution.get_start_time(x), x))
    for task in list(sorted_tasks):
        if len(solution.rcpsp_schedule[task]["starts"]) > 1:
            new_proposed_schedule[task] = {"starts": solution.rcpsp_schedule[task]["starts"],
                                           "ends": solution.rcpsp_schedule[task]["ends"]}
            for s, e in zip(new_proposed_schedule[task]["starts"], new_proposed_schedule[task]["ends"]):
    ...
    def is_startable(tt):
        if isinstance(problem, RCPSPModelSpecialConstraintsPreemptive):
            return all(x in new_proposed_schedule
                       for x in problem.special_constraints.dict_start_after_nunit_reverse.get(tt, {})) and \
                   all(x in new_proposed_schedule
                       for x in problem.special_constraints.dict_start_at_end_reverse.get(tt, {})) and \
                   all(x in new_proposed_schedule
                       for x in problem.special_constraints.dict_start_at_end_offset_reverse.get(tt, {})) and \
                   all(t in new_proposed_schedule for t in predecessors_dict[tt])
    ...
    while True:
        ...
        if len(solution.rcpsp_schedule[task]["starts"]) > 1:
            continue
        times_predecessors = [new_proposed_schedule[t]["ends"][-1]
                              if t in new_proposed_schedule
                              else solution.rcpsp_schedule[t]["ends"][-1]
                              for t in predecessors_dict[task]]
        if isinstance(problem, RCPSPModelSpecialConstraintsPreemptive):
            times_predecessors += [new_proposed_schedule[t]["starts"][0]
                                   if t in new_proposed_schedule
                                   else solution.rcpsp_schedule[t]["starts"][0]
                                   for t in problem.special_constraints.dict_start_together.get(task, set())]
            times_predecessors += [new_proposed_schedule[t]["starts"][0]
                                   + problem.special_constraints.dict_start_after_nunit_reverse.get(task)[t]
                                   if t in new_proposed_schedule
                                   else solution.rcpsp_schedule[t]["starts"][0]
                                        + problem.special_constraints.dict_start_after_nunit_reverse.get(task)[t]
                                   for t in problem.special_constraints.dict_start_after_nunit_reverse.get(task, {})]
            times_predecessors += [new_proposed_schedule[t]["ends"][-1]
                                   if t in new_proposed_schedule
                                   else solution.rcpsp_schedule[t]["ends"][-1]
                                   for t in problem.special_constraints.dict_start_at_end_reverse.get(task, {})]
            times_predecessors += [new_proposed_schedule[t]["ends"][-1]
                                   + problem.special_constraints.dict_start_at_end_offset_reverse.get(task)[t]
                                   if t in new_proposed_schedule
                                   else solution.rcpsp_schedule[t]["ends"][-1]
                                        + problem.special_constraints.dict_start_at_end_offset_reverse.get(task)[t]
                                   for t in problem.special_constraints.dict_start_at_end_offset_reverse.get(task, {})]
        if len(times_predecessors) > 0:
            min_time = max(times_predecessors)
        else:
            min_time = 0
        if solution.get_start_time(task) == solution.get_end_time(task):
            new_proposed_schedule[task] = {"starts": [min_time],
                                           "ends": [min_time]}
        if len(solution.rcpsp_schedule[task]["starts"]) > 1:
            new_proposed_schedule[task] = {"starts": solution.rcpsp_schedule[task]["starts"],
                                           "ends": solution.rcpsp_schedule[task]["ends"]}
            ...
            for t in range(min_time, problem.horizon):
                if all(resource_avail_in_time[res][time] >= problem.mode_details[task][modes_dict[task]].get(res, 0)
                       for res in
                       problem.resources_list
                       for time in range(t,
                                         t + problem.mode_details[task][modes_dict[task]]["duration"])):
                    new_starting_time = t
                    break
            new_proposed_schedule[task] = {"starts": [new_starting_time],
                                           "ends": [new_starting_time
        ...
        if len(sorted_tasks) == 0:
            break
    new_solution = RCPSPSolutionSpecialPreemptive(problem=problem,
...
def constraints_strings(current_solution: RCPSPSolutionPreemptive,
    ...
    for job in subtasks:
        for j in range(len(current_solution.rcpsp_schedule[job]["starts"])):
            ...
            string1Start = cp_solver.constraint_start_time_string_preemptive_i(task=job,
                                                                               start_time=max(0,
                                                                                              start_time_j - minus_delta),
                                                                               sign=SignEnum.UEQ,
                                                                               part_id=j + 1)
            string2Start = cp_solver.constraint_start_time_string_preemptive_i(task=job,
                                                                               start_time=min(max_time,
                                                                                              start_time_j + plus_delta)
                                                                               if constraint_max_time
                                                                               else start_time_j + plus_delta,
                                                                               sign=SignEnum.LEQ,
                                                                               part_id=j + 1)
            string1Dur = cp_solver.constraint_duration_string_preemptive_i(task=job,
                                                                           duration=max(duration_j - 5, 0),
                                                                           sign=SignEnum.UEQ,
                                                                           part_id=j + 1)
            string2Dur = cp_solver.constraint_duration_string_preemptive_i(task=job,
                                                                           duration=duration_j + 5,
                                                                           sign=SignEnum.LEQ,
                                                                           part_id=j + 1)
            list_strings += [string1Dur, string1Start,
                             string2Dur, string2Start]
        for k in range(len(current_solution.rcpsp_schedule[job]["starts"]),
    ...
    for job in jobs_to_fix:
        ...
        for k in range(len(current_solution.rcpsp_schedule[job]["starts"]),
            ...
            if minus_delta_2 == 0 and plus_delta_2 == 0:
                ...
                string1Start = cp_solver.constraint_start_time_string_preemptive_i(task=job,
                                                                                   start_time=current_solution.rcpsp_schedule
                                                                                   [job]["ends"][-1]-minus_delta_2,
                                                                                   sign=SignEnum.UEQ,
                                                                                   part_id=k + 1)
                string2Start = cp_solver.constraint_start_time_string_preemptive_i(task=job,
                                                                                   start_time=min(max_time,
                                                                                                  current_solution.rcpsp_schedule
                                                                                                  [job]["ends"][-1] + plus_delta_2)
                                                                                   if constraint_max_time
                                                                                   else current_solution.rcpsp_schedule
                                                                                   [job]["ends"][-1] + plus_delta_2,
                                                                                   sign=SignEnum.LEQ,
                                                                                   part_id=k + 1)
...
class NeighborFixStart(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
        ...
        max_time = get_max_time_solution(solution=current_solution)
        last_jobs = get_tasks_ending_between_two_times(solution=current_solution,
        ...
        subtasks = set([s for s in current_solution.rcpsp_schedule if s not in jobs_to_fix])
...
class NeighborFixStartSubproblem(ConstraintHandler):
    def __init__(self, problem: RCPSPModelPreemptive,
        ...
        self.nb_jobs_subproblem = int(self.fraction_size_subproblem * self.problem.n_jobs)
        self.method = method
        self.graph = self.problem.compute_graph()
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
        ...
        if last_result_store is not None:
            current_solution, fit = next((last_result_store.list_solution_fits[j]
                                          for j in range(len(last_result_store.list_solution_fits))
                                          if "opti_from_cp" in last_result_store.list_solution_fits[j][0].__dict__.keys()), (None, None))
        ...
        # method = 0
        if method == 0:
            subtasks = create_subproblem_cut_time(current_solution=current_solution,
                                                  neighbor_fix_problem=self)
        elif method == 1:
            subtasks = create_subproblems_random_and_predecessors(current_solution=current_solution,
                                                                  neighbor_fix_problem=self)
        elif method == 2:
            subtasks = create_subproblems_problems(current_solution=current_solution,
                                                   neighbor_fix_problem=self)
        ...
        for s in list_strings:
            child_instance.add_string(s)
        child_instance.add_string("constraint sec_objective<="+str(100*eval["constraint_penalty"])+";\n")
        if eval["constraint_penalty"] > 0:
...
def create_subproblem_cut_time(current_solution,
                               neighbor_fix_problem: NeighborFixStartSubproblem, subtasks=None):
    nb_job_sub = math.ceil(neighbor_fix_problem.problem.n_jobs/neighbor_fix_problem.nb_cut_part)
    task_of_interest = sorted(current_solution.rcpsp_schedule,
                              key=lambda x: current_solution.rcpsp_schedule[x]["ends"][-1])
    task_of_interest = task_of_interest[neighbor_fix_problem.current_sub_part*nb_job_sub:
                                        (neighbor_fix_problem.current_sub_part+1)*nb_job_sub]
    if subtasks is None:
...
def create_subproblems_random_and_predecessors(current_solution,
                                               neighbor_fix_problem: NeighborFixStartSubproblem, subtasks=None):
    ...
    while len_subtask < neighbor_fix_problem.nb_jobs_subproblem:
        ...
        interval = (current_solution.rcpsp_schedule[random_pick]["starts"][0],
                    current_solution.rcpsp_schedule[random_pick]["ends"][0])
        task_intersect = [t for t in current_solution.rcpsp_schedule if
                          intersect(interval,
                                    (current_solution.rcpsp_schedule[t]["starts"][0],
                                     current_solution.rcpsp_schedule[t]["ends"][0])) is not None]
        for k in set(task_intersect):
            ...
            if isinstance(neighbor_fix_problem.problem, RCPSPModelSpecialConstraintsPreemptive):
                 task_intersect += list(neighbor_fix_problem.problem.special_constraints
                                        .dict_start_at_end.get(k, {}))
                 task_intersect += list(neighbor_fix_problem.problem.special_constraints
                                        .dict_start_at_end_reverse.get(k, {}))
...
def create_subproblems_problems(current_solution,
                                neighbor_fix_problem: NeighborFixStartSubproblem):
    if neighbor_fix_problem.special_constraints:
        ...
        if len(sorted_constraints) == 0:
            ...
            print("Hell")
        while j <= len(sorted_constraints)-1 and len_subtasks < 4*neighbor_fix_problem.nb_jobs_subproblem:
            t1, t2 = sorted_constraints[j][1], sorted_constraints[j][2]
            subtasks.add(t1)
            subtasks.add(t2)
            subtasks.update(neighbor_fix_problem.graph_rcpsp
                            .components_graph_constraints[neighbor_fix_problem.graph_rcpsp.index_components[t1]])
            for c in set(neighbor_fix_problem.graph_rcpsp
                         .components_graph_constraints[neighbor_fix_problem.graph_rcpsp.index_components[t1]]):
                subtasks.update(neighbor_fix_problem.graph_rcpsp.get_next_activities(c))
                subtasks.update(neighbor_fix_problem.graph_rcpsp.get_descendants_activities(c))
                subtasks.update(neighbor_fix_problem.graph_rcpsp.get_ancestors_activities(c))
        ...
        if len_subtasks < neighbor_fix_problem.nb_jobs_subproblem:
            subtasks = create_subproblem_cut_time(current_solution, neighbor_fix_problem, subtasks)
...
class NeighborFlexibleStart(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
        ...
        list_strings = constraints_strings(current_solution,
...
def get_ressource_breaks(problem_calendar: RCPSPModelPreemptive,
    ...
    for r in ressources:
        ...
        ressource_arrays_usage[r] = np.zeros((makespan,
    ...
    for ji in range(len(sorted_keys_schedule)):
        ...
        for r in problem_calendar.resources_list:
            if problem_calendar.mode_details[j][modes[j]].get(r, 0) == 0:
                continue
            if solution.rcpsp_schedule[j]["starts"][0] == solution.rcpsp_schedule[j]["ends"][-1]:
                continue
            for s, e, index in zip(solution.rcpsp_schedule[j]["starts"],
                                   solution.rcpsp_schedule[j]["ends"],
                                   range(len(solution.rcpsp_schedule[j]["starts"]))):
                ressource_arrays_usage[r][s:e, ji] = index+1
                ressource_arrays[r][s:e] += \
                    problem_calendar.mode_details[j][modes[j]][r]
    index_ressource = {}
    task_concerned = {}
    constraints = {}
    for r in ressource_arrays:
        if problem_calendar.is_varying_resource():
            index = np.argwhere(ressource_arrays[r] > problem_calendar.resources[r][:makespan])
        else:
        ...
        for t in task_concerned[r]:
            ...
            first_possible_start_before = next((st for st in range(current_start, -1, -1)
                                               if problem_calendar.get_resource_available(r, st)
                                                >= problem_calendar.mode_details[t][modes[t]][r]),
                                               None)
            first_possible_start_before = next((st - problem_calendar.mode_details[t][modes[t]]["duration"] + 1
                                                for st in range(solution.rcpsp_schedule[t]["ends"][-1], -1, -1)
                                                if problem_calendar.get_resource_available(r, st-1)
                                                >= problem_calendar.mode_details[t][modes[t]][r]
                                                and problem_calendar.get_resource_available(r,
                                                                                            max(0,
                                                                                                st -
                                                                                                problem_calendar.mode_details[
                                                                                                    t][modes[t]][
                                                                                                    "duration"] + 1))
...
class PostProcessSolutionNonFeasible(PostProcessSolution):
    def __init__(self, problem_calendar: RCPSPModelPreemptive,
        ...
        self.aggreg_from_sol, self.aggreg_from_dict, self.params_objective_function = \
            build_aggreg_function_and_params_objective(problem=self.problem_calendar,
                                                       params_objective_function=
                                                       params_objective_function)
        if self.partial_solution is None:
            ...
            def check_solution(problem, solution):
                ...
                for (t1, t2) in start_together:
                    b = solution.rcpsp_schedule[t1]["starts"][0] == solution.rcpsp_schedule[t2]["starts"][0]
                    if not b:
                        return False
                for (t1, t2) in start_at_end:
                    b = solution.rcpsp_schedule[t2]["starts"][0] == solution.rcpsp_schedule[t1]["ends"][-1]
                    if not b:
                        return False
                for (t1, t2, off) in start_at_end_plus_offset:
                    b = solution.rcpsp_schedule[t2]["starts"][0] >= solution.rcpsp_schedule[t1]["ends"][-1] + off
                    if not b:
                        return False
                for (t1, t2, off) in start_after_nunit:
                    b = solution.rcpsp_schedule[t2]["starts"][0] >= solution.rcpsp_schedule[t1]["starts"][0] + off
                    if not b:
                        return False
                return True
    ...
    def build_other_solution(self, result_storage: ResultStorage) -> ResultStorage:
        for sol in list(result_storage.list_solution_fits):
            ...
            if solution.satisfy is False or True:
                if self.partial_solution is None:
                    solution_p = RCPSPSolutionPreemptive(problem=self.problem_calendar,
                    ...
                    result_storage.list_solution_fits += [(solution_p,
                                                           -self.aggreg_from_sol(solution_p))]
        if self.do_ls:
            solver = LS_RCPSP_Solver(model=self.problem_calendar,
                                     ls_solver=LS_SOLVER.SA)
            satisfiable = [(s, f) for s, f in result_storage.list_solution_fits if s.satisfy]
            if len(satisfiable) > 0:
                s: RCPSPSolutionPreemptive = max(satisfiable, key=lambda x: x[1])[0].copy()
            else:
                s = result_storage.get_best_solution().copy()
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
        ...
        for r in ressource_breaks:
            ...
            for index_range in range(len(ressource_breaks[r])):
                ...
                if (prev_time is None or ind >= prev_time+20) or (index_range >= 1
                                                                  and ind > ressource_breaks[r][index_range-1][0]+1):
                    prev_time = ind
                else:
                    continue
                rq = self.problem_calendar.get_resource_available(r, ind)
                s = cp_solver.constraint_ressource_requirement_at_time_t(time=ind,
                                                                         ressource=r,
                                                                         ressource_number=rq,
                                                                         sign=SignEnum.UEQ)
                child_instance.add_string(s)
                ...
                # print("Res", r)
                # print("Time", index)
        satisfiable = [(s, f) for s, f in last_result_store.list_solution_fits
                       if "satisfy" in s.__dict__.keys()
                       and s.satisfy]
        if len(satisfiable) > 0:
            res = ResultStorage(list_solution_fits=satisfiable,
                                mode_optim=result_storage.mode_optim)
            self.other_constraint.adding_constraint_from_results_store(cp_solver,
                                                                       child_instance,
                                                                       res,
                                                                       last_result_store)
...
class ConstraintHandlerMix(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_MRCPSP_MZN_PREEMMPTIVE,
...
def build_neighbor_operator(option_neighbor: OptionNeighbor, rcpsp_model):
    params_om = [Params(fraction_to_fix=0.98,
                        minus_delta=0,
                        plus_delta=0)]
