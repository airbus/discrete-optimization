...
from typing import Union, List, Dict, Hashable, Tuple, Set
...
files_mzn = {"single": os.path.join(this_path, "../minizinc/rcpsp_single_mode_mzn.mzn"),
             "single-resource": os.path.join(this_path, "../minizinc/rcpsp_minimize_resource.mzn"),
             "single-no-search": os.path.join(this_path, "../minizinc/rcpsp_single_mode_mzn_no_search.mzn"),
...
class CP_RCPSP_MZN(CPSolver):
    ...
    def init_model(self, **args):
        ...
        if model_type == "single-resource":
            instance["add_objective_resource"] = args.get("add_objective_resource", True)
        ...
        dict_to_add = add_fake_task_cp_data(rcpsp_model=self.rcpsp_model,
        ...
        rcap = [int(self.rcpsp_model.get_max_resource_capacity(x)) for x in sorted_resources]
        instance["rc"] = rcap
        ...
        all_modes = [(act, 1, self.rcpsp_model.mode_details[act][1])
                     for act in sorted_tasks]
        rr = [[all_modes[i][2].get(res, 0) for i in range(len(all_modes))] for res in sorted_resources]
        instance["rr"] = rr
        self.keys_in_instance += ["rr"]
        suc = [set([self.rcpsp_model.return_index_task(x, offset=1)
                    for x in self.rcpsp_model.successors[task]])
               for task in sorted_tasks]
        instance["suc"] = suc
        self.keys_in_instance += ["suc"]
        self.instance = instance
        self.index_in_minizinc = {task: self.rcpsp_model.return_index_task(task, offset=1)
        ...
        if add_partial_solution_hard_constraint:
            ...
            for s in strings:
                self.instance.add_string(s)
                self.strings_to_add += [s]
            strings = define_second_part_objective([100] * len(name_penalty),
                                                   name_penalty)
            for s in strings:
                self.instance.add_string(s)
                self.strings_to_add += [s]
    ...
    def constraint_sum_of_ending_time(self, set_subtasks: Set[Hashable]):
        ...
        s = """int: nb_indexes="""+str(len(indexes))+""";\n
               array[1..nb_indexes] of Tasks: index_tasks="""+str(indexes)+""";\n
    ...
    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        ...
        for j in set_of_jobs:
            s += ["constraint s["+str(self.index_in_minizinc[j])+"]+d["+str(self.index_in_minizinc[j])+"]<=objective;\n"]
        return s

    def constraint_start_time_string(self, task, start_time, sign: SignEnum = SignEnum.EQUAL) -> str:
        return "constraint s[" + str(self.index_in_minizinc[task]) + "]"+str(sign.value) + str(start_time) + ";\n"

    def constraint_end_time_string(self, task, end_time, sign: SignEnum = SignEnum.EQUAL) -> str:
        return "constraint s[" + str(self.index_in_minizinc[task]) \
               + "]+d["+str(self.index_in_minizinc[task])+"]"+str(sign.value) + str(end_time) + ";\n"
...
class CP_MRCPSP_MZN(CPSolver):
    ...
    def init_model(self, **args):
        ...
        keys += ["n_opt"]
        all_modes = [(act, mode, self.rcpsp_model.mode_details[act][mode])
                     for act in sorted_tasks for mode in sorted(self.rcpsp_model.mode_details[act])]
        self.modeindex_map = {i+1: {"task": all_modes[i][0], "original_mode_index": all_modes[i][1]}
        ...
        instance['dur'] = dur
        keys += ["dur"]
        rreq = [[all_modes[i][2].get(res, 0) for i in range(len(all_modes))] for res in resources_list]
        instance["rreq"] = rreq
        keys += ["rreq"]
        ...
        keys += ["rtype"]
        succ = [set([self.rcpsp_model.return_index_task(x, offset=1)
                     for x in self.rcpsp_model.successors.get(task, [])]) for task in sorted_tasks]
        instance["succ"] = succ
        keys += ["succ"]
        if self.calendar:
    ...
    def constraint_task_to_mode(self, task_id, mode):
        ...
        for m in modes:
            if m == mode:
                ...
                bool_str = "false"
            s = "constraint mrun["+str(self.mode_dict_task_mode_to_index_minizinc[(task_id, m)])+"]="+bool_str+";\n"
            list_strings += [s]
        return list_strings
    ...
    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        s = []
        for j in set_of_jobs:
            s += ["constraint start["+str(self.index_in_minizinc[j])+"]+adur["+str(self.index_in_minizinc[j])
                  + "]<=objective;\n" ]
        return s
    ...
    def constraint_start_time_string(self, task, start_time, sign: SignEnum=SignEnum.EQUAL)->str:
        return "constraint start[" + str(self.index_in_minizinc[task]) + "]"+str(sign.value) + str(start_time) + ";\n"

    def constraint_end_time_string(self, task, end_time, sign: SignEnum=SignEnum.EQUAL)->str:
        return "constraint start[" + str(self.index_in_minizinc[task]) + "]+adur["+str(self.index_in_minizinc[task])+"]"+str(sign.value) + str(end_time) + ";\n"
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP = ParametersCP.default()):
        ...
        for start_times, mrun in zip(starts, mruns):
            ...
            for i in range(len(mrun)):
                if mrun[i]:
                    modes_dict[self.modeindex_map[i + 1]['task']] = self.modeindex_map[i + 1]['original_mode_index']
            rcpsp_schedule = {}
                ...
                rcpsp_schedule[self.rcpsp_model.tasks_list[i]] = \
                    {'start_time': start_times[i],
                     ...
                     + self.rcpsp_model.mode_details[self.rcpsp_model.tasks_list[i]]
                                 [modes_dict[self.rcpsp_model.tasks_list[i]]]['duration']}
            sol = RCPSPSolution(problem=self.rcpsp_model,
                                rcpsp_schedule=rcpsp_schedule,
                                rcpsp_modes=[modes_dict[t]
                                             for t in self.rcpsp_model.tasks_list_non_dummy],
                                rcpsp_schedule_feasible=True)
            objective = self.aggreg_from_dict_values(self.rcpsp_model.evaluate(sol))
            if objective > best_makespan:
                best_makespan = objective
...
class CP_MRCPSP_MZN_WITH_FAKE_TASK(CP_MRCPSP_MZN):
    def __init__(self, rcpsp_model: RCPSPModel, cp_solver_name: CPSolverName = CPSolverName.CHUFFED,
                 params_objective_function: ParamsObjectiveFunction = None, **kwargs):
        super().__init__(rcpsp_model, cp_solver_name, params_objective_function, **kwargs)
    ...
    def init_model(self, **args):
        ...
        instance["add_objective_makespan"] = add_objective_makespan
        instance["ignore_sec_objective"] = ignore_sec_objective
        keys += ["add_objective_makespan",
                 "ignore_sec_objective"]
        n_res = len(resources_list)
        instance["n_res"] = n_res
        keys += ["n_res"]
        n_tasks = self.rcpsp_model.n_jobs
    ...
    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        s = []
        for j in set_of_jobs:
            s += ["constraint start["+str(self.index_in_minizinc[j])+"]<=objective;\n"]
        return s
...
class CP_RCPSP_MZN_PREEMMPTIVE(CPSolver):
    def __init__(self, rcpsp_model: RCPSPModelPreemptive,
                 cp_solver_name: CPSolverName = CPSolverName.CHUFFED,
                 params_objective_function: ParamsObjectiveFunction=None, **kwargs):
        self.rcpsp_model = rcpsp_model
        self.instance: Instance = None
    ...
    def init_model(self, **args):
        ...
        add_partial_solution_hard_constraint = args.get("add_partial_solution_hard_constraint", True)
        if model_type is None:
            model_type = "single-preemptive" if not self.calendar else "single-preemptive-calendar"
        keys = []
        model = Model(files_mzn[model_type])
        custom_output_type = args.get("output_type",  False)
        if custom_output_type:
        ...
        keys += ["nb_preemptive"]
        instance["possibly_preemptive"] = args.get("possibly_preemptive",
                                                   [self.rcpsp_model.can_be_preempted(task)
                                                    for task in self.rcpsp_model.tasks_list])
        keys += ["possibly_preemptive"]
        instance["max_preempted"] = args.get("max_preempted", min(self.rcpsp_model.n_jobs_non_dummy, 5))
        keys += ["max_preempted"]
        ...
        sorted_tasks = self.rcpsp_model.tasks_list
        d = [int(self.rcpsp_model.mode_details[key][1]['duration'])
        ...
        for res in sorted_resources:
            rr.append([])
            for task in sorted_tasks:
                rr[index].append(int(self.rcpsp_model.mode_details[task][1].get(res, 0)))
            index += 1
        instance["rr"] = rr
        keys += ["rr"]
    ...
    def constraint_duration_to_min_duration_preemptive(self, task, min_duration):
        ...
        for i in range(2, self.nb_preemptive+1):
            s = "constraint d_preemptive["+str(self.index_in_minizinc[task])+","+str(i)+"]>"+str(min_duration)+"\/"+\
                "d_preemptive["+str(self.index_in_minizinc[task])+","+str(i)+"]=0;\n"
            list_strings += [s]
        return list_strings
    ...
    def constraint_minduration_all_tasks(self):
        ...
        for task in self.rcpsp_model.tasks_list:
            if self.rcpsp_model.duration_subtask[task][0]:
                list_strings += \
                    self.constraint_duration_to_min_duration_preemptive(task=task,
                                                                        min_duration=
                                                                        self.rcpsp_model.duration_subtask[task][1])
        return list_strings
    ...
    def constraint_end_time_string(self, task, end_time, sign: SignEnum=SignEnum.EQUAL)->str:
        return "constraint s_preemptive[" + str(self.index_in_minizinc[task]) + ", nb_preemptive]"+str(sign.value) + str(end_time) + ";\n"

    def constraint_start_time_string_preemptive_i(self, task,
                                                  start_time,
                                                  part_id=1,
                                                  sign: SignEnum = SignEnum.EQUAL) -> str:
        return "constraint s_preemptive[" + str(self.index_in_minizinc[task])+","+str(part_id)+"]" \
               + str(sign.value) + str(start_time) + ";\n"

    def constraint_duration_string_preemptive_i(self,
                                                task,
                                                duration,
                                                part_id=1,
                                                sign: SignEnum = SignEnum.EQUAL) -> str:
        return "constraint d_preemptive[" + str(self.index_in_minizinc[task])+","+str(part_id)+"]" \
               + str(sign.value) + str(duration) + ";\n"

    def constraint_ressource_requirement_at_time_t(self, time,
                                                   ressource, ressource_number,
                                                   sign: SignEnum=SignEnum.LEQ):
        index_ressource = self.resources_index.index(ressource)+1
        s = """constraint """ + str(ressource_number) + str(sign.value)+"""sum( i in Tasks, j in PREEMPTIVE) (
                                            bool2int(s_preemptive[i, j] <=""" + str(time) + """ /\ """ + str(time) \
            + """< s_preemptive[i, j] + d_preemptive[i, j]) * rr[""" + str(index_ressource) + """,i]);\n"""
        return s
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP=ParametersCP.default())->ResultStorage:
        ...
        for i in range(len(starts)):
            ...
            for k in range(len(starts_preemptive[i])):
                ...
                rcpsp_schedule[self.rcpsp_model.tasks_list[k]] = \
                    {'starts': starts_k,
                     'ends': ends_k}
            sol = RCPSPSolutionPreemptive(problem=self.rcpsp_model,
                                          rcpsp_schedule=rcpsp_schedule,
                                          rcpsp_schedule_feasible=True)
            objective = self.aggreg_from_dict_values(self.rcpsp_model.evaluate(sol))
            if objective > best_makespan:
        ...
        if len(list_solutions_fit) > 0:
            list_solutions_fit[index_best][0].opti_from_cp = True  # Flag the solution obtained for the optimum of cp
        result_storage = ResultStorage(list_solution_fits=list_solutions_fit,
                                       best_solution=best_solution,
                                       mode_optim=self.params_objective_function.sense_function,
                                       limit_store=False)
        ...
        for t in intervals:
            ...
            for interv in intervals[t]:
                if interv[0] is not None and interv[1] is not None:
                    s_list += ["((" + str(interv[0]) + "<=s[" + str(self.index_in_minizinc[t]) + "])/\("
                               + "s[" + str(self.index_in_minizinc[t]) + "]<=" + str(interv[1] - 1) + "))"]
    ...
    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        s = []
        for j in set_of_jobs:
            s += ["constraint s_preemptive["+str(self.index_in_minizinc[j])+", nb_preemptive]<=objective;\n"]
        return s

    def constraint_objective_equal_makespan(self, task_sink):
        ind = self.index_in_minizinc[task_sink]
        s = "constraint (s_preemptive["+str(ind) + \
            ", nb_preemptive]+d_preemptive["+str(ind)+",nb_preemptive]==objective);\n"
        return [s]
    ...
    def constraint_sum_of_ending_time(self, set_subtasks: Set[Hashable]):
        ...
        s = """int: nb_indexes="""+str(len(indexes))+""";\n
               array[1..nb_indexes] of int: weights="""+str(weights)+""";\n 
               array[1..nb_indexes] of Tasks: index_tasks="""+str(indexes)+""";\n
...
class CP_MRCPSP_MZN_PREEMMPTIVE(CP_RCPSP_MZN_PREEMMPTIVE):
    def __init__(self, rcpsp_model: RCPSPModelPreemptive, cp_solver_name: CPSolverName = CPSolverName.CHUFFED,
                 params_objective_function: ParamsObjectiveFunction = None, **kwargs):
        super().__init__(rcpsp_model, cp_solver_name, params_objective_function, **kwargs)
        # self.rcpsp_model = rcpsp_model
        # self.instance: Instance = None
    ...
    def init_model(self, **args):
        ...
        add_partial_solution_hard_constraint = args.get("add_partial_solution_hard_constraint", True)
        if model_type is None:
            model_type = "multi-preemptive" if not self.calendar else "multi-preemptive-calendar"
        keys = []
        model = Model(files_mzn[model_type])
        ...
        sorted_resources = self.rcpsp_model.resources_list
        self.resources_index = sorted_resources
        rreq = [[all_modes[i][2].get(res, 0) for i in range(len(all_modes))] for res in sorted_resources]
        instance["rreq"] = rreq
        keys += ["rreq"]
        rtype = [2 if res in self.rcpsp_model.non_renewable_resources else 1
                 for res in sorted_resources]
    ...
    def constraint_ressource_requirement_at_time_t(self, time,
        ...
        s = """constraint """ + str(ressource_number) + str(sign.value)+"""sum( i in Tasks, j in PREEMPTIVE) (
                                            bool2int(s_preemptive[i, j] <=""" + str(time) + """ /\ """ + str(time) \
            + """< s_preemptive[i, j] + d_preemptive[i, j]) * arreq[""" + str(index_ressource) + """,i]);\n"""
        return s
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP=ParametersCP.default())->ResultStorage:
        ...
        for i in range(len(starts)):
            ...
            for k in range(len(modes[i])):
                if modes[i][k]:
                    modes_dict[self.modeindex_map[k + 1]['task']] = self.modeindex_map[k + 1]['original_mode_index']
            for k in range(len(starts_preemptive[i])):
                starts_k = []
                ends_k = []
                for j in range(len(starts_preemptive[i][k])):
            ...
            sol = RCPSPSolutionPreemptive(problem=self.rcpsp_model,
                                          rcpsp_schedule=rcpsp_schedule,
                                          rcpsp_modes=[modes_dict[t] for t in self.rcpsp_model.tasks_list_non_dummy],
                                          rcpsp_schedule_feasible=True)
            objective = self.aggreg_from_dict_values(self.rcpsp_model.evaluate(sol))
            if objective > best_makespan:
                best_makespan = objective
...
class CP_MRCPSP_MZN_NOBOOL(CPSolver):
        ...
        if p_s is not None and add_partial_solution_hard_constraint:
            ...
            if p_s.start_times is not None:
                for task in p_s.start_times:
                    string = "constraint start[" + str(self.index_in_minizinc[task]) + "] == " + str(p_s.start_times[task]) + ";\n"
            ...
            if p_s.partial_permutation is not None:
                for t1, t2 in zip(p_s.partial_permutation[:-1], p_s.partial_permutation[1:]):
                    string = "constraint start[" + str(self.index_in_minizinc[t1]) + \
                             "] <= start[" + str(self.index_in_minizinc[t2]) + "];\n"
                    self.instance.add_string(string)
                    constraint_strings += [string]
    ...
    def constraint_objective_max_time_set_of_jobs(self, set_of_jobs):
        s = []
        for j in set_of_jobs:
            s += ["constraint start["+str(self.index_in_minizinc[j])
                  + "]+adur["+str(self.index_in_minizinc[j])+"]<=objective;\n"]
        return s
    ...
    def constraint_end_time_string(self, task, end_time, sign: SignEnum = SignEnum.EQUAL) -> str:
        return "constraint start[" + str(self.index_in_minizinc[task]) \
               + "]+adur["+str(self.index_in_minizinc[task])+"]" + str(sign.value) + str(end_time) + ";\n"
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP=ParametersCP.default()):
        ...
        for res in object_result:
            ...
            for i in range(len(start_times)):
                ...
                rcpsp_schedule[t] = {'start_time': start_times[i],
                                     'end_time': start_times[i]
...
class CP_MRCPSP_MZN_MODES:
    ...
    def retrieve_solutions(self, result, parameters_cp: ParametersCP = None):
...
def precompute_posssible_starting_time_interval(problem: RCPSPModelPreemptive):
    ...
    for task in interval_possible:
        ...
        while time is not None:
            time_1 = next((t for t in range(time, problem.horizon)
                          if all(problem.get_resource_available(r, t) >= problem.mode_details[task][1].get(r, 0)
                                 for r in problem.resources)), None)
            time = time_1
            if time_1 is not None:
                time_2 = next((t for t in range(time_1, problem.horizon)
                              if any(problem.get_resource_available(r, t) < problem.mode_details[task][1].get(r, 0)
                                     for r in problem.resources)), None)
                time = time_2
            else:
                time_2 = None
...
def hard_start_times(dict_start_times: Dict[Hashable, int],
                     cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                      CP_MRCPSP_MZN_PREEMMPTIVE,
                                      CP_RCPSP_MZN]):
    constraint_strings = []
    for task in dict_start_times:
        string = "constraint s[" + str(cp_solver.index_in_minizinc[task]) + \
                 "] == " + str(dict_start_times[task]) + ";\n"
        constraint_strings += [string]
    return constraint_strings


def soft_start_times(dict_start_times: Dict[Hashable, int],
                     cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                      CP_MRCPSP_MZN_PREEMMPTIVE,
                                      CP_RCPSP_MZN]):
    ...
    s = """
        ...
        int: nb_start_times=""" + str(len(list_task)) + """;\n""" + \
        """
        array[1..nb_start_times] of Tasks: st1_0=""" + str([cp_solver.index_in_minizinc[t1] for t1 in list_task]) + """;\n"""+  \
        """
        array[1..nb_start_times] of 0..max_time: array_start_0="""+str([dict_start_times[t1] for t1 in list_task])+""";\n"""+ \
        """
        constraint sum(i in 1..nb_start_times)(abs(s[st1_0[i]]-array_start_0[i]))==penalty_start_times;\n"""
    return [s], ["penalty_start_times"]


def hard_start_times_mrcpsp(dict_start_times: Dict[Hashable, int],
                            cp_solver: Union[CP_MRCPSP_MZN,
                                             CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                             CP_MRCPSP_MZN_NOBOOL]):
    constraint_strings = []
    for task in dict_start_times:
        string = "constraint start[" + str(cp_solver.index_in_minizinc[task]) + \
                 "] == " + str(dict_start_times[task]) + ";\n"
        constraint_strings += [string]
    return constraint_strings


def soft_start_times_mrcpsp(dict_start_times: Dict[Hashable, int],
                            cp_solver: Union[CP_MRCPSP_MZN,
                                             CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                             CP_MRCPSP_MZN_NOBOOL]):
    list_task = list(dict_start_times.keys())
    s = """
        ...
        int: nb_start_times=""" + str(len(list_task)) + """;\n""" + \
        """
        array[1..nb_start_times] of Act: st1_1=""" + str([cp_solver.index_in_minizinc[t1] for t1 in list_task]) + """;\n"""+  \
        """
        array[1..nb_start_times] of 0..max_time: array_start_1="""+str([dict_start_times[t1] for t1 in list_task])+""";\n"""+ \
        """
        constraint sum(i in 1..nb_start_times)(abs(start[st1_1[i]]-array_start_1[i]))==penalty_start_times;\n"""
    return [s], ["penalty_start_times"]


def soft_start_together_mrcpsp(list_start_together: List[Tuple[Hashable, Hashable]],
                               cp_solver: Union[CP_MRCPSP_MZN,
                                                CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                                CP_MRCPSP_MZN_NOBOOL]):
    s = """
        var 0..max_time*nb_start_together: penalty_start_together;\n
        int: nb_start_together=""" + str(len(list_start_together)) + """;\n""" + \
        """
        array[1..nb_start_together] of Act: st1_2=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2 in list_start_together]) + """;\n""" + \
        """
        array[1..nb_start_together] of Act: st2_2=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2 in list_start_together]) + """;\n""" + \
        """
...
def hard_start_together_mrcpsp(list_start_together: List[Tuple[Hashable, Hashable]],
                               cp_solver: Union[CP_MRCPSP_MZN,
                                                CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                                CP_MRCPSP_MZN_NOBOOL]):
    constraint_strings = []
    for t1, t2 in list_start_together:
        string = "constraint start[" + str(cp_solver.index_in_minizinc[t1]) + \
                 "] == start[" + str(cp_solver.index_in_minizinc[t2]) + "];\n"
        # cp_solver.instance.add_string(string)
        constraint_strings += [string]
    return constraint_strings


def soft_start_together(list_start_together: List[Tuple[Hashable, Hashable]],
                        cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                         CP_MRCPSP_MZN_PREEMMPTIVE,
                                         CP_RCPSP_MZN]):
    s = """
        ...
        int: nb_start_together=""" + str(len(list_start_together)) + """;\n""" + \
        """
        array[1..nb_start_together] of Tasks: st1_3=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2 in list_start_together]) + """;\n""" + \
        """
        array[1..nb_start_together] of Tasks: st2_3=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2 in list_start_together]) + """;\n""" + \
        """
...
def hard_start_together(list_start_together: List[Tuple[Hashable, Hashable]],
                        cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                         CP_MRCPSP_MZN_PREEMMPTIVE,
                                         CP_RCPSP_MZN]):
    constraint_strings = []
    for t1, t2 in list_start_together:
        string = "constraint s[" + str(cp_solver.index_in_minizinc[t1]) + \
...
def hard_start_after_nunit(list_start_after_nunit: List[Tuple[Hashable, Hashable, int]],
                           cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                            CP_MRCPSP_MZN_PREEMMPTIVE,
                                            CP_RCPSP_MZN]):
    constraint_strings = []
    for t1, t2, delta in list_start_after_nunit:
        string = "constraint s[" + str(cp_solver.index_in_minizinc[t2]) + \
                 "] >= s[" + str(cp_solver.index_in_minizinc[t1]) + \
                 "]+" + str(delta) + ";\n"
        # cp_solver.instance.add_string(string)
        constraint_strings += [string]
    return constraint_strings
...
def soft_start_after_nunit(list_start_after_nunit: List[Tuple[Hashable, Hashable, int]],
                           cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                            CP_MRCPSP_MZN_PREEMMPTIVE,
                                            CP_RCPSP_MZN]):
    s = """
        var 0..max_time*nb_start_after_nunit: penalty_start_after_nunit;\n
        int: nb_start_after_nunit=""" + str(len(list_start_after_nunit)) + """;\n""" + \
        """
        array[1..nb_start_after_nunit] of Tasks: st1_4=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2, delta in list_start_after_nunit]) + """;\n""" + \
        """
        array[1..nb_start_after_nunit] of Tasks: st2_4=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2, delta in list_start_after_nunit]) + """;\n""" + \
        """
        array[1..nb_start_after_nunit] of int: nunits_4="""+str([delta for t1, t2, delta in list_start_after_nunit])+""";\n"""+\
        """
...
def hard_start_after_nunit_mrcpsp(list_start_after_nunit: List[Tuple[Hashable, Hashable, int]],
                                  cp_solver: Union[CP_MRCPSP_MZN,
                                                   CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                                   CP_MRCPSP_MZN_NOBOOL]):
    constraint_strings = []
    for t1, t2, delta in list_start_after_nunit:
        string = "constraint start[" + str(cp_solver.index_in_minizinc[t2]) + \
                 "] >= start[" + str(cp_solver.index_in_minizinc[t1]) + \
                 "]+" + str(delta) + ";\n"
        # cp_solver.instance.add_string(string)
        constraint_strings += [string]
    return constraint_strings
...
def soft_start_after_nunit_mrcpsp(list_start_after_nunit: List[Tuple[Hashable, Hashable, int]],
                                  cp_solver: Union[CP_MRCPSP_MZN,
                                                   CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                                   CP_MRCPSP_MZN_NOBOOL]):
    s = """
        ...
        int: nb_start_after_nunit=""" + str(len(list_start_after_nunit)) + """;\n""" + \
        """
        array[1..nb_start_after_nunit] of Act: st1_5=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2, delta in list_start_after_nunit]) + """;\n""" + \
        """
        array[1..nb_start_after_nunit] of Act: st2_5=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2, delta in list_start_after_nunit]) + """;\n""" + \
        """
        array[1..nb_start_after_nunit] of int: nunit_5="""+str([delta for t1, t2, delta in list_start_after_nunit])+""";\n"""+\
        """
...
def hard_start_at_end_plus_offset_mrcpsp(list_start_at_end_plus_offset,
                                         cp_solver: Union[CP_MRCPSP_MZN,
                                                          CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                                          CP_MRCPSP_MZN_NOBOOL]):
    constraint_strings = []
    for t1, t2, delta in list_start_at_end_plus_offset:
        string = "constraint start[" + str(cp_solver.index_in_minizinc[t2]) + \
                 "] >= start[" + str(cp_solver.index_in_minizinc[t1]) + \
                 "]+adur[" + str(cp_solver.index_in_minizinc[t1]) + "]+" + str(delta) + ";\n"
        # cp_solver.instance.add_string(string)
        constraint_strings += [string]
    return constraint_strings


def hard_start_at_end_plus_offset(list_start_at_end_plus_offset,
                                  cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                                   CP_MRCPSP_MZN_PREEMMPTIVE,
                                                   CP_RCPSP_MZN]):
    constraint_strings = []
    for t1, t2, delta in list_start_at_end_plus_offset:
        if isinstance(cp_solver, CP_RCPSP_MZN):
            string = "constraint s[" + str(cp_solver.index_in_minizinc[t2]) + \
                     "] >= s[" + str(cp_solver.index_in_minizinc[t1]) + \
                     "]+d[" + str(cp_solver.index_in_minizinc[t1]) + "]+" + str(delta) + ";\n"
        else:
            string = "constraint s[" + str(cp_solver.index_in_minizinc[t2]) + \
                     "] >= s_preemptive[" + str(cp_solver.index_in_minizinc[t1]) + ", nb_preemptive]+" \
...
def soft_start_at_end_plus_offset_mrcpsp(list_start_at_end_plus_offset: List[Tuple[Hashable, Hashable, int]],
                                         cp_solver: Union[CP_MRCPSP_MZN,
                                                          CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                                          CP_MRCPSP_MZN_NOBOOL]):
    s = """
        var 0..max_time*nb_start_at_end_plus_offset: penalty_start_at_end_plus_offset;\n
        int: nb_start_at_end_plus_offset=""" + str(len(list_start_at_end_plus_offset)) + """;\n""" + \
        """
        array[1..nb_start_at_end_plus_offset] of Act: st1_6=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2, delta in list_start_at_end_plus_offset]) + """;\n""" + \
        """
        array[1..nb_start_at_end_plus_offset] of Act: st2_6=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2, delta in list_start_at_end_plus_offset]) + """;\n""" + \
        """
        array[1..nb_start_at_end_plus_offset] of int: nunits_6="""+str([delta for t1, t2, delta in list_start_at_end_plus_offset])+""";\n"""+\
        """
...
def soft_start_at_end_plus_offset(list_start_at_end_plus_offset: List[Tuple[Hashable, Hashable, int]],
                                  cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
                                                   CP_MRCPSP_MZN_PREEMMPTIVE,
                                                   CP_RCPSP_MZN]):
    s = """
        ...
        int: nb_start_at_end_plus_offset=""" + str(len(list_start_at_end_plus_offset)) + """;\n""" + \
        """
        array[1..nb_start_at_end_plus_offset] of Tasks: st1_7=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2, delta in list_start_at_end_plus_offset]) + """;\n""" + \
        """
        array[1..nb_start_at_end_plus_offset] of Tasks: st2_7=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2, delta in list_start_at_end_plus_offset]) + """;\n""" + \
        """
        array[1..nb_start_at_end_plus_offset] of int: nunits="""+str([delta for t1, t2, delta in list_start_at_end_plus_offset])+""";\n"""
    if isinstance(cp_solver, CP_RCPSP_MZN):
        s += """
...
def hard_start_at_end_mrcpsp(list_start_at_end: List[Tuple[Hashable, Hashable]],
                             cp_solver: Union[CP_MRCPSP_MZN,
                                              CP_MRCPSP_MZN_WITH_FAKE_TASK,
                                              CP_MRCPSP_MZN_NOBOOL]):
    constraint_strings = []
    for t1, t2 in list_start_at_end:
        string = "constraint start[" + str(cp_solver.index_in_minizinc[t2]) + \
                 "] == start[" + str(cp_solver.index_in_minizinc[t1]) + \
                 "]+adur[" + str(cp_solver.index_in_minizinc[t1]) + "];\n"
        # cp_solver.instance.add_string(string)
        constraint_strings += [string]
    return constraint_strings


def soft_start_at_end_mrcpsp(list_start_at_end: List[Tuple[Hashable, Hashable]],
    ...
    s = """
        ...
        int: nb_start_at_end=""" + str(len(list_start_at_end)) + """;\n""" + \
        """
        array[1..nb_start_at_end] of Act: st1_8=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2 in list_start_at_end]) + """;\n""" + \
        """
        array[1..nb_start_at_end] of Act: st2_8=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2 in list_start_at_end]) + """;\n""" + \
        """
...
def hard_start_at_end(list_start_at_end: List[Tuple[Hashable, Hashable]],
    ...
    for t1, t2 in list_start_at_end:
        if isinstance(cp_solver, CP_RCPSP_MZN):
            string = "constraint s[" + str(cp_solver.index_in_minizinc[t2]) + \
                     "] == s[" + str(cp_solver.index_in_minizinc[t1]) + \
                     "]+d[" + str(cp_solver.index_in_minizinc[t1]) + "];\n"
        else:
            string = "constraint s_preemptive[" + str(cp_solver.index_in_minizinc[t2]) + \
                     ", 1] == s_preemptive[" + str(cp_solver.index_in_minizinc[t1]) + \
                     ", nb_preemptive];\n"
        constraint_strings += [string]
    return constraint_strings


def soft_start_at_end(list_start_at_end: List[Tuple[Hashable, Hashable]],
                      cp_solver: Union[CP_RCPSP_MZN, CP_MRCPSP_MZN_PREEMMPTIVE, CP_RCPSP_MZN_PREEMMPTIVE]):
    s = """
        var 0..max_time*nb_start_at_end: penalty_start_at_end;\n
        int: nb_start_at_end=""" + str(len(list_start_at_end)) + """;\n""" + \
        """
        array[1..nb_start_at_end] of Tasks: st1_9=""" + str(
        [cp_solver.index_in_minizinc[t1] for t1, t2 in list_start_at_end]) + """;\n""" + \
        """
        array[1..nb_start_at_end] of Tasks: st2_9=""" + str(
        [cp_solver.index_in_minizinc[t2] for t1, t2 in list_start_at_end]) + """;\n"""
...
def soft_start_window(start_times_window: Dict[Hashable, Tuple[int, int]],
                      cp_solver: Union[CP_RCPSP_MZN, CP_MRCPSP_MZN_PREEMMPTIVE, CP_RCPSP_MZN_PREEMMPTIVE]):
    ...
    max_time_start = max([max([x[1] for x in l_low], default=0), max([x[1] for x in l_up], default=0)])
    s = """
        var 0..max_time*nb_start_window_low: penalty_start_low;\n
        int: nb_start_window_low=""" + str(len(l_low)) + """;\n
        var 0..max_time*nb_start_window_up: penalty_start_up;\n
        int: nb_start_window_up=""" + str(len(l_up)) + """;\n
        int: max_time_start="""+str(max_time_start)+""";\n
        array[1..nb_start_window_low] of Tasks: task_id_low_start="""+str([cp_solver.index_in_minizinc[x[0]] for x in l_low])+""";\n
        array[1..nb_start_window_up] of Tasks:  task_id_up_start="""+str([cp_solver.index_in_minizinc[x[0]] for x in l_up])+""";\n
        array[1..nb_start_window_low] of 0..max_time_start: times_low_start="""+str([int(x[1]) for x in l_low])+""";\n
        array[1..nb_start_window_up] of 0..max_time_start: times_up_start="""+str([int(x[1]) for x in l_up])+""";\n"""
    s += \
        """
...
def soft_end_window(end_times_window: Dict[Hashable, Tuple[int, int]],
                    cp_solver: Union[CP_RCPSP_MZN, CP_MRCPSP_MZN_PREEMMPTIVE, CP_RCPSP_MZN_PREEMMPTIVE]):
    ...
    max_time_end = max([max([x[1] for x in l_low], default=0), max([x[1] for x in l_up], default=0)])
    s = """
        var 0..max_time*nb_end_window_low: penalty_end_low;\n
        int: nb_end_window_low=""" + str(len(l_low)) + """;\n
        var 0..max_time*nb_end_window_up: penalty_end_up;\n
        int: nb_end_window_up=""" + str(len(l_up)) + """;\n
        int: max_time_end="""+str(max_time_end)+""";\n
        array[1..nb_end_window_low] of Tasks: task_id_low_end="""+str([cp_solver.index_in_minizinc[x[0]] for x in l_low])+""";\n
        array[1..nb_end_window_up] of Tasks:  task_id_up_end="""+str([cp_solver.index_in_minizinc[x[0]] for x in l_up])+""";\n
        array[1..nb_end_window_low] of 0..max_time_end: times_low_end="""+str([int(x[1]) for x in l_low])+""";\n
        array[1..nb_end_window_up] of 0..max_time_end: times_up_end="""+str([int(x[1]) for x in l_up])+""";\n"""
    if isinstance(cp_solver, (CP_RCPSP_MZN_PREEMMPTIVE, CP_MRCPSP_MZN_PREEMMPTIVE)):
        s += \
            """
...
def soft_start_window_mrcpsp(start_times_window: Dict[Hashable, Tuple[int, int]],
                      cp_solver: Union[CP_MRCPSP_MZN, CP_MRCPSP_MZN_WITH_FAKE_TASK, CP_MRCPSP_MZN_NOBOOL]):
    l_low = [(t, start_times_window[t][0])
            for t in start_times_window if start_times_window[t][0] is not None]
    l_up = [(t, start_times_window[t][1])
...
def soft_end_window_mrcpsp(end_times_window: Dict[Hashable, Tuple[int, int]],
                           cp_solver: Union[CP_MRCPSP_MZN, CP_MRCPSP_MZN_WITH_FAKE_TASK, CP_MRCPSP_MZN_NOBOOL]):
    l_low = [(t, end_times_window[t][0])
             for t in end_times_window if end_times_window[t][0] is not None]
    l_up = [(t, end_times_window[t][1])
...
def hard_start_window(start_times_window: Dict[Hashable, Tuple[int, int]],
                      cp_solver: Union[CP_RCPSP_MZN, CP_MRCPSP_MZN_PREEMMPTIVE, CP_RCPSP_MZN_PREEMMPTIVE,
                                       CP_MRCPSP_MZN, CP_MRCPSP_MZN_WITH_FAKE_TASK, CP_MRCPSP_MZN_NOBOOL]):
    l = []
    for task in start_times_window:
        if start_times_window[task][0] is not None:
            l += [cp_solver.constraint_start_time_string(task=task,
                                                         start_time=start_times_window[task][0],
                                                         sign=SignEnum.UEQ)]
        if start_times_window[task][1] is not None:
            l += [cp_solver.constraint_start_time_string(task=task,
                                                         start_time=start_times_window[task][1],
                                                         sign=SignEnum.LEQ)]
    return l


def hard_end_window(end_times_window: Dict[Hashable, Tuple[int, int]],
                    cp_solver: Union[CP_RCPSP_MZN, CP_MRCPSP_MZN_PREEMMPTIVE, CP_RCPSP_MZN_PREEMMPTIVE,
                                     CP_MRCPSP_MZN, CP_MRCPSP_MZN_WITH_FAKE_TASK, CP_MRCPSP_MZN_NOBOOL]):
    l = []
    for task in end_times_window:
        if end_times_window[task][0] is not None:
            l += [cp_solver.constraint_end_time_string(task=task,
                                                       end_time=end_times_window[task][0],
                                                       sign=SignEnum.UEQ)]
        if end_times_window[task][1] is not None:
            l += [cp_solver.constraint_end_time_string(task=task,
                                                       end_time=end_times_window[task][1],
                                                       sign=SignEnum.LEQ)]
    return l


def add_hard_special_constraints_mrcpsp(partial_solution: PartialSolution,
                                        cp_solver: Union[CP_MRCPSP_MZN, CP_MRCPSP_MZN_WITH_FAKE_TASK, CP_MRCPSP_MZN_NOBOOL]):
    if partial_solution is not None:
        constraint_strings = []
        if partial_solution.start_times is not None:
            constraint_strings += hard_start_times_mrcpsp(dict_start_times=partial_solution.start_times,
                                                          cp_solver=cp_solver)
        if partial_solution.partial_permutation is not None:
            for t1, t2 in zip(partial_solution.partial_permutation[:-1],
        ...
        if partial_solution.task_mode is not None:
            for task in partial_solution.task_mode:
                indexes = [i for i in cp_solver.modeindex_map if cp_solver.modeindex_map[i]["task"] == task
                           and cp_solver.modeindex_map[i]["original_mode_index"] == partial_solution.task_mode[task]]
                if len(indexes) >= 0:
                    string = "constraint mrun[" + str(indexes[0]) + "] == 1;"
                    # cp_solver.instance.add_string(string)
                    constraint_strings += [string]
        if partial_solution.start_together is not None:
            constraint_strings += hard_start_together_mrcpsp(list_start_together=partial_solution.start_together,
                                                             cp_solver=cp_solver)
        if partial_solution.start_after_nunit is not None:
            constraint_strings += hard_start_after_nunit_mrcpsp(list_start_after_nunit=partial_solution.start_after_nunit,
                                                                cp_solver=cp_solver)
        if partial_solution.start_at_end_plus_offset is not None:
            constraint_strings += hard_start_at_end_plus_offset_mrcpsp(list_start_at_end_plus_offset=
                                                                       partial_solution.start_at_end_plus_offset,
                                                                       cp_solver=cp_solver)
        if partial_solution.start_at_end is not None:
            constraint_strings += hard_start_at_end_mrcpsp(list_start_at_end=partial_solution.start_at_end,
                                                           cp_solver=cp_solver)
        if partial_solution.start_times_window is not None:
            constraint_strings += hard_start_window(start_times_window=partial_solution.start_times_window,
                                                    cp_solver=cp_solver)
        if partial_solution.end_times_window is not None:
            constraint_strings += hard_end_window(end_times_window=partial_solution.end_times_window,
                                                  cp_solver=cp_solver)
        return constraint_strings
...
def add_soft_special_constraints_mrcpsp(partial_solution: PartialSolution,
                                        cp_solver: Union[CP_MRCPSP_MZN]):
    if partial_solution is not None:
        constraint_strings = []
        name_penalty = []
        if partial_solution.start_times is not None:
            c, n = soft_start_times_mrcpsp(dict_start_times=partial_solution.start_times,
                                           cp_solver=cp_solver)
        ...
        if partial_solution.task_mode is not None:
            for task in partial_solution.task_mode:
                ...
                if len(indexes) >= 0:
                    ...
                    constraint_strings += [string]
        if partial_solution.start_together is not None:
            c, n = soft_start_together_mrcpsp(list_start_together=partial_solution.start_together,
                                              cp_solver=cp_solver)
            constraint_strings += c
            name_penalty += n
        if partial_solution.start_after_nunit is not None:
            c, n = soft_start_after_nunit_mrcpsp(list_start_after_nunit=partial_solution.start_after_nunit,
                                                 cp_solver=cp_solver)
            constraint_strings += c
            ...
            name_penalty += n
        if partial_solution.start_at_end_plus_offset is not None:
            c, n = soft_start_at_end_plus_offset_mrcpsp(list_start_at_end_plus_offset=
                                                        partial_solution.start_at_end_plus_offset,
                                                        cp_solver=cp_solver)
            constraint_strings += c
            ...
            name_penalty += n
        if partial_solution.start_at_end is not None:
            c, n = soft_start_at_end_mrcpsp(list_start_at_end=partial_solution.start_at_end,
                                            cp_solver=cp_solver)
            constraint_strings += c
            name_penalty += n
        if partial_solution.start_times_window is not None:
            c, n = soft_start_window_mrcpsp(partial_solution.start_times_window, cp_solver=cp_solver)
            constraint_strings += c
            name_penalty += n
        if partial_solution.end_times_window is not None:
            c, n = soft_end_window_mrcpsp(partial_solution.end_times_window, cp_solver=cp_solver)
            constraint_strings += c
            name_penalty += n
...
def add_hard_special_constraints(partial_solution: PartialSolution,
                                 cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE, CP_MRCPSP_MZN_PREEMMPTIVE, CP_RCPSP_MZN]):
    if partial_solution is None:
        return []
    constraint_strings = []
    if partial_solution.start_times is not None:
        constraint_strings += hard_start_times(dict_start_times=partial_solution.start_times,
                                               cp_solver=cp_solver)
    if partial_solution.partial_permutation is not None:
        for t1, t2 in zip(partial_solution.partial_permutation[:-1], partial_solution.partial_permutation[1:]):
            string = "constraint s[" + str(cp_solver.index_in_minizinc[t1]) + "] " \
                                                                                               "<= s[" + str(
    ...
    if partial_solution.list_partial_order is not None:
        for l in partial_solution.list_partial_order:
            for t1, t2 in zip(l[:-1], l[1:]):
                ...
                constraint_strings += [string]
    if partial_solution.start_together is not None:
        constraint_strings += hard_start_together(list_start_together=partial_solution.start_together,
                                                  cp_solver=cp_solver)
    if partial_solution.start_after_nunit is not None:
        constraint_strings += hard_start_after_nunit(list_start_after_nunit=partial_solution.start_after_nunit,
                                                     cp_solver=cp_solver)
    if partial_solution.start_at_end_plus_offset is not None:
        constraint_strings += hard_start_at_end_plus_offset(list_start_at_end_plus_offset=partial_solution.start_at_end_plus_offset,
                                                            cp_solver=cp_solver)
    if partial_solution.start_at_end is not None:
        constraint_strings += hard_start_at_end(list_start_at_end=partial_solution.start_at_end,
                                                cp_solver=cp_solver)
    if partial_solution.start_times_window is not None:
        constraint_strings += hard_start_window(start_times_window=partial_solution.start_times_window,
                                                cp_solver=cp_solver)
...
def add_soft_special_constraints(partial_solution: PartialSolution,
                                 cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,CP_MRCPSP_MZN_PREEMMPTIVE,CP_RCPSP_MZN]):
    if partial_solution is None:
        return [], []
    constraint_strings = []
    name_penalty = []
    if partial_solution.start_times is not None:
        c, n = soft_start_times(dict_start_times=partial_solution.start_times,
                                cp_solver=cp_solver)
        constraint_strings += c
    ...
    if partial_solution.list_partial_order is not None:
        for l in partial_solution.list_partial_order:
            for t1, t2 in zip(l[:-1], l[1:]):
                ...
                constraint_strings += [string]
    if partial_solution.start_together is not None:
        c, n = soft_start_together(list_start_together=partial_solution.start_together,
                                   cp_solver=cp_solver)
        constraint_strings += c
    ...
    if partial_solution.start_after_nunit is not None:
        c, n = soft_start_after_nunit(list_start_after_nunit=partial_solution.start_after_nunit,
                                      cp_solver=cp_solver)
        constraint_strings += c
        ...
        name_penalty += n
    if partial_solution.start_at_end_plus_offset is not None:
        c, n = soft_start_at_end_plus_offset(list_start_at_end_plus_offset=partial_solution.start_at_end_plus_offset,
                                             cp_solver=cp_solver)
        constraint_strings += c
        ...
        name_penalty += n
    if partial_solution.start_at_end is not None:
        c, n = soft_start_at_end(list_start_at_end=partial_solution.start_at_end,
                                 cp_solver=cp_solver)
        constraint_strings += c
        name_penalty += n
    if partial_solution.start_times_window is not None:
        c, n = soft_start_window(partial_solution.start_times_window, cp_solver=cp_solver)
        constraint_strings += c
...
def define_second_part_objective(weights,
                                 name_penalty, equal=False):
