...
from discrete_optimization.rcpsp.mutations.mutation_rcpsp import PermutationMutationRCPSP
from discrete_optimization.rcpsp.solver.rcpsp_lp_solver import LP_RCPSP_Solver, LP_RCPSP, LP_MRCPSP, LP_MRCPSP_GUROBI
from discrete_optimization.generic_tools.lp_tools import MilpSolver, ParametersMilp
from discrete_optimization.generic_tools.lns_mip import LNS_MILP, ConstraintHandler, InitialSolution, SolverDO
from discrete_optimization.rcpsp.rcpsp_model import RCPSPModel, RCPSPModelCalendar, RCPSPSolution
...
class InitialSolutionRCPSP(InitialSolution):
    def __init__(self, problem: Union[RCPSPModel, RCPSPModelCalendar],
...
class ConstraintHandlerFixStartTime(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, milp_solver: LP_RCPSP,
            ...
            for t in milp_solver.index_time:
                if t < min_st or t > max_st:
                    constraints_dict["range_start_time"].append(
                        milp_solver.model.add_constr(milp_solver.x[milp_solver.index_in_var[job]][t] == 0))
        if milp_solver.lp_solver == LP_RCPSP_Solver.GRB:
            milp_solver.model.solver.update()
        return constraints_dict
