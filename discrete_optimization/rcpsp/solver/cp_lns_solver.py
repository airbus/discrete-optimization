...
from discrete_optimization.generic_rcpsp_tools.neighbor_tools_rcpsp import BasicConstraintBuilder, \
    ParamsConstraintBuilder, NeighborBuilderMix, NeighborBuilderSubPart, \
    NeighborRandomAndNeighborGraph, NeighborConstraintBreaks
from discrete_optimization.rcpsp.solver.cp_lns_methods_clean import NeighborSubproblem, RCPSPModel, \
    RCPSPModelSpecialConstraints, RCPSPModelPreemptive, RCPSPModelSpecialConstraintsPreemptive
...
from discrete_optimization.rcpsp.solver.cp_solvers import CP_RCPSP_MZN_PREEMMPTIVE, CP_MRCPSP_MZN_PREEMMPTIVE, \
    CP_MRCPSP_MZN, CP_RCPSP_MZN
from discrete_optimization.generic_tools.do_problem import get_default_objective_setup
from discrete_optimization.generic_tools.result_storage.result_storage import ResultStorage
from discrete_optimization.generic_tools.lns_cp import SolverDO, LNS_CP
from discrete_optimization.rcpsp.rcpsp_model_preemptive import  \
    RCPSPModelPreemptive, PartialSolutionPreemptive
from discrete_optimization.rcpsp.solver.cp_lns_methods_preemptive import build_neighbor_operator, \
    NeighborFixStartSubproblem, MethodSubproblem
...
def build_default_cp_model(rcpsp_problem: GENERIC_CLASS,
                           partial_solution=None, **kwargs):
    if isinstance(rcpsp_problem, (RCPSPModelPreemptive, RCPSPModelSpecialConstraintsPreemptive)):
...
class LargeNeighborhoodSearchRCPSP(SolverDO):
    def __init__(self,
                 ...
                 partial_solution: PartialSolutionPreemptive = None, **kwargs):
        ...
        if option == 2:
            from discrete_optimization.generic_rcpsp_tools.neighbor_builder import build_neighbor_mixing_methods
            constraint_handler = build_neighbor_mixing_methods(rcpsp_model=self.rcpsp_problem,
                                                               graph=graph,
                                                               fraction_subproblem=kwargs.get("fraction_subproblem",
                                                                                              0.05),
                                                               cut_part=kwargs.get("cut_part", 10),
        ...
        if initial_solution_provider is None:
            initial_solution_provider = InitialSolutionRCPSP(problem=self.rcpsp_problem,
        ...
        if isinstance(self.rcpsp_problem, (RCPSPModelPreemptive, RCPSPModelSpecialConstraintsPreemptive)):
            from discrete_optimization.rcpsp.solver.cp_lns_methods_preemptive import PostProLeftShift
            self.post_process_solution = PostProLeftShift(problem=self.rcpsp_problem,
                                                          params_objective_function=params_objective_function,
                                                          do_ls=kwargs.get("do_ls", False))
        else:
            import discrete_optimization.rcpsp.solver.rcpsp_cp_lns_solver as rcpsp_lns
            self.post_process_solution = rcpsp_lns.PostProcessLeftShift(rcpsp_problem=self.rcpsp_problem,
