...
from discrete_optimization.rcpsp.solver import CP_RCPSP_MZN, CP_MRCPSP_MZN
...
class NeighborSubproblem(ConstraintHandler):
    def __init__(self, problem: Union[RCPSPModel,
        ...
        if params_list is None:
            self.params_list = [
                ParamsConstraintBuilder(minus_delta_primary=6000, plus_delta_primary=6000, minus_delta_secondary=1,
                                        plus_delta_secondary=1),
                ParamsConstraintBuilder(minus_delta_primary=6000, plus_delta_primary=6000, minus_delta_secondary=300,
    ...
    def adding_constraint_from_results_store(self, cp_solver: Union[CP_RCPSP_MZN_PREEMMPTIVE,
        ...
        child_instance.add_string("constraint sec_objective<="+str(int(1.01*100*eval.get("constraint_penalty", 0))+1000)
                                  + ";\n")
        if eval.get("constraint_penalty", 0) > 0:
            strings = []
        else:
