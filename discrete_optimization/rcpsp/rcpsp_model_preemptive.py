...
from typing import List, Union, Tuple, Dict, Iterable, Any, Hashable
import numpy as np
from enum import Enum
from copy import deepcopy
import matplotlib.pyplot as plt
from collections import defaultdict
from discrete_optimization.rcpsp.fast_function_rcpsp import compute_mean_ressource, \
    sgs_fast_preemptive,\
...
class RCPSPSolutionPreemptive(Solution):
    ...
    def get_nb_task_preemption(self):
        return len([t for t in self.rcpsp_schedule if len(self.rcpsp_schedule[t]["starts"])>1])
    ...
    def get_min_duration_subtask(self):
        return min([e-s for t in self.rcpsp_schedule
                    for e, s in zip(self.rcpsp_schedule[t]["ends"],
                                    self.rcpsp_schedule[t]["starts"])
    ...
    def get_task_preempted(self):
        return [t for t in self.rcpsp_schedule if len(self.rcpsp_schedule[t]["starts"]) > 1]
    ...
    def get_active_time(self, task):
        l = []
        for s, e in zip(self.rcpsp_schedule[task]["starts"], self.rcpsp_schedule[task]["ends"]):
            l += list(range(s, e))
        return l
    ...
    def copy(self):
        return RCPSPSolutionPreemptive(problem=self.problem,
    ...
    def generate_permutation_from_schedule(self):
        sorted_task = [self.problem.index_task[i]-1
                       for i in sorted(self.rcpsp_schedule,
                                       key=lambda x: self.rcpsp_schedule[x]["starts"][0])]
    ...
    def compute_mean_resource_reserve(self, fast=True):
        if not fast:
            return compute_mean_resource_reserve(solution=self,
            ...
            return self.problem.compute_mean_resource(horizon=makespan,
                                                      modes_array=
                                                      np.array(self.problem.build_mode_array(self.rcpsp_modes))-1,
                                                      start_array=np.array([self.rcpsp_schedule[t]["starts"][0]
                                                                            for t in self.problem.tasks_list]),
                                                      end_array=np.array([self.rcpsp_schedule[t]["ends"][0]
                                                                          for t in self.problem.tasks_list]))

    def generate_schedule_from_permutation_serial_sgs(self, do_fast=True):
        if do_fast:
            starts_dict, ends_dict, unfeasible = \
                self.problem.func_sgs(permutation_task=
                                      permutation_do_to_permutation_sgs_fast(self.problem, self.rcpsp_permutation),
                                      modes_array=np.array(self.problem.build_mode_array(self.rcpsp_modes))-1)
            self.rcpsp_schedule_feasible = not unfeasible
            self.rcpsp_schedule = {}
            self.rcpsp_schedule[self.problem.sink_task] = {"starts": [99999999],
                                                           "ends": [99999999]}
            for k in starts_dict:
                self.rcpsp_schedule[self.problem.tasks_list[k]] = {"starts": list(starts_dict[k]),
    ...
    def generate_schedule_from_permutation_serial_sgs_2(self,
        ...
        if do_fast:
            starts_dict, ends_dict, unfeasible = \
                self.problem.func_sgs_2(current_time=current_t,
                                        completed_task_indicator=np.array([1 if self.problem.tasks_list[i]
                                                                           in completed_tasks
                                                                           else 0
                                                                           for i in range(self.problem.n_jobs)]),
                                        partial_schedule_starts=np.array([[partial_schedule.get(self.problem.tasks_list[i], {}).get("starts", [])[k]
                                                                           if k < len(partial_schedule.get(self.problem.tasks_list[i], {}).get("starts", []))
                                                                           else -1
                                                                           for k in range(10)]
                                                                          for i in range(self.problem.n_jobs)]),
                                        partial_schedule_ends=np.array([[partial_schedule.get(self.problem.tasks_list[i], {}).get("ends", [])[k]
                                                                         if k < len(partial_schedule.get(self.problem.tasks_list[i], {}).get("ends", []))
                                                                         else -1
                                                                         for k in range(10)]
                                                                        for i in range(self.problem.n_jobs)]),
                                        permutation_task=
                                        permutation_do_to_permutation_sgs_fast(self.problem,
                                                                               self.rcpsp_permutation),
                                        modes_array=np.array(self.problem.build_mode_array(self.rcpsp_modes))-1)
            self.rcpsp_schedule_feasible = not unfeasible
            self.rcpsp_schedule = {}
            for k in starts_dict:
...
class RCPSPModelPreemptive(Problem):
    ...
    resources: Union[Dict[str, int], Dict[str, List[int]]]  # {resource_name: number_of_resource}
    ...
    def __init__(self,
        ...
        self.any_duration_subtask_limited = any(self.duration_subtask[x][0] for x in self.duration_subtask)
        ...
        self.max_number_of_mode = max([len(self.mode_details[key1].keys()) for key1 in self.mode_details.keys()])
        ...
        if any(isinstance(self.resources[res], Iterable) for res in self.resources):
            self.is_calendar = max([len(set(self.resources[res])
                                        if isinstance(self.resources[res], Iterable)
                                        else {self.resources[res]})
                                    for res in self.resources]) > 1
    ...
    def evaluate_from_encoding(self, int_vector, encoding_name):
        if encoding_name == 'rcpsp_permutation':
            ...
            rcpsp_sol = RCPSPSolutionPreemptive(problem=self,
    ...
    def satisfy(self, rcpsp_sol: RCPSPSolutionPreemptive)->bool:
        if rcpsp_sol.rcpsp_schedule_feasible is False:
            ...
            modes_dict = self.build_mode_dict(rcpsp_modes_from_solution=rcpsp_sol.rcpsp_modes)
            if not self.is_varying_resource():
                start_times = [rcpsp_sol.rcpsp_schedule[t]["starts"][k]
                               for t in rcpsp_sol.rcpsp_schedule
                               for k in range(len(rcpsp_sol.rcpsp_schedule[t]["starts"]))]
            else:
                start_times = range(rcpsp_sol.rcpsp_schedule[self.sink_task]["ends"][-1])
            ...
            if False:
                ...
                resource_avail_in_time = compute_resource(solution=rcpsp_sol, rcpsp_problem=self)
            ...
            for res in self.non_renewable_resources:
                ...
                for act_id in rcpsp_sol.rcpsp_schedule:
                    ...
                    if usage > self.resources[res]:
                        print('Non-renewable resource violation: act_id: ', act_id,
                              'res', res, 'res_usage: ', usage, 'res_avail: ', self.resources[res])
                        return False
            ...
            for act_id in self.successors:
                for succ_id in self.successors[act_id]:
                    ...
                    if start_succ < end_pred:
                        print('Precedence relationship broken: ', act_id,
                              'end at ', end_pred,
                              'while ', succ_id,
                              'start at', start_succ)
                        return False
    ...
    def get_attribute_register(self) -> EncodingRegister:
        ...
        mode_arrity = [len(self.mode_details[task])
        ...
        for act_id in rcpsp_sol.rcpsp_schedule:
            for ir in range(len(self.resources)):
                consumptions[ir,
                    ...
                    += self.mode_details[act_id][modes_dict[act_id]][self.resources_list[ir]]
        return consumptions
    ...
    def copy(self):
        return RCPSPModelPreemptive(resources=self.resources,
    ...
    def copy_with_multiplier(self, multiplier=0.5):
        ...
        for t in mode_details:
            for m in mode_details[t]:
                mode_details[t][m]["duration"] = int(math.ceil(multiplier*mode_details[t][m]["duration"]))
        return RCPSPModelPreemptive(resources={r: self.resources[r][::n]*n for r in self.resources},
                                    non_renewable_resources=self.non_renewable_resources,
    ...
    def get_dummy_solution(self):
        sol = RCPSPSolutionPreemptive(problem=self,
    ...
    def __str__(self):
        val = "I'm a RCPSP model with "+str(self.n_jobs)+" tasks.."+" and ressources ="+str(self.resources_list)
        return val


def generate_schedule_from_permutation_serial_sgs(solution: RCPSPSolutionPreemptive,
    ...
    while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
        ...
        while not valid:  # 7ok
            ...
            if expected_durations_task[act_id] == 0:
                ...
                for t in range(current_min_time,
                    ...
                    if any(resource_avail_in_time[res][t] <
                           rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                           for res in rcpsp_problem.resources_list):
                        reached_end = False
                        break
                    else:
            ...
            if not valid:
                current_min_time = next((t for t in range(reached_t+2 if reached_t is not None
                                                          else current_min_time+1,
                                                          new_horizon)
                                        if all(resource_avail_in_time[res][t] >=
                                               rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                                               for res in rcpsp_problem.resources_list)), None)
            ...
            for s, e in zip(starts, ends):
                for t in range(s, e):
                    for res in resource_avail_in_time:
                        if rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0) == 0:
                            continue
                        resource_avail_in_time[res][t] -= rcpsp_problem.mode_details[act_id][modes_dict[act_id]][res]
                        if res in rcpsp_problem.non_renewable_resources and t == end_t - 1:
                            for tt in range(end_t, new_horizon):
                                resource_avail_in_time[res][tt] -= \
                                    rcpsp_problem.mode_details[act_id][modes_dict[act_id]][res]
...
def generate_schedule_from_permutation_serial_sgs_partial_schedule(solution: RCPSPSolutionPreemptive,
                                                                   rcpsp_problem: RCPSPModelPreemptive,
                                                                   partial_schedule: Dict[Hashable,
                                                                                          Dict[str, List[int]]],
                                                                   current_t,
                                                                   completed_tasks):
    activity_end_times = {}
    ...
    while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
        ...
        while not valid:
            ...
            if expected_durations_task[act_id] == 0:
                ...
                for t in range(current_min_time,
                               current_min_time
                               + expected_durations_task[act_id] - done_duration_task[act_id]):
                    if t >= new_horizon:
                        reached_end = False
                        unfeasible_non_renewable_resources = True
                    if any(resource_avail_in_time[res][t] <
            ...
            if not valid:
                current_min_time = next(t for t in range(reached_t+2 if reached_t is not None else current_min_time+1,
                                                         new_horizon)
                                        if all(resource_avail_in_time[res][t] >=
                                               rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                                               for res in rcpsp_problem.resources_list))
                    ...
                    for res in resource_avail_in_time:
                        ...
                        if res in rcpsp_problem.non_renewable_resources and t == end_t - 1:
                            for tt in range(end_t, new_horizon):
                                ...
                                if resource_avail_in_time[res][tt] < 0:
                                    unfeasible_non_renewable_resources = True
            schedules[act_id] = {"starts": schedules.get(act_id, {}).get("starts", [])+starts,
...
def compute_mean_resource_reserve(solution: RCPSPSolutionPreemptive,
                                  rcpsp_problem: RCPSPModelPreemptive):
    ...
    for res in rcpsp_problem.resources_list:
        if rcpsp_problem.is_varying_resource():
            ...
            resource_avail_in_time[res] = np.full(makespan,
            ...
            for t in range(s, e):
                for res in resource_avail_in_time:
                    if rcpsp_problem.mode_details[act_id][mode].get(res, 0) == 0:
                        continue
                    resource_avail_in_time[res][t] -= rcpsp_problem.mode_details[act_id][mode][res]
                    if res in rcpsp_problem.non_renewable_resources and t == e-1:
                        for tt in range(e, makespan):
                            resource_avail_in_time[res][tt] -= rcpsp_problem.mode_details[act_id][mode][res]
    mean_avail = {}
    for res in resource_avail_in_time:
        mean_avail[res] = np.mean(resource_avail_in_time[res])
    mean_resource_reserve = np.mean([mean_avail[res] /
                                     max(rcpsp_problem.resources[res]) if rcpsp_problem.is_varying_resource()
                                     else mean_avail[res] / rcpsp_problem.resources[res]
                                     for res in rcpsp_problem.resources_list])
    return mean_resource_reserve


def compute_resource(solution: RCPSPSolutionPreemptive,
                     rcpsp_problem: RCPSPModelPreemptive):
    new_horizon = rcpsp_problem.horizon
    resource_avail_in_time = {}
    modes_dict = rcpsp_problem.build_mode_dict(solution.rcpsp_modes)
    for r in rcpsp_problem.resources_list:
        if rcpsp_problem.is_varying_resource():
            resource_avail_in_time[r] = np.copy(rcpsp_problem.resources[r][:new_horizon])
        else:
            resource_avail_in_time[r] = rcpsp_problem.resources[r]*np.ones((new_horizon))
    for t in solution.rcpsp_schedule:
        ...
        for s, e in zip(start_times, end_times):
            for r in rcpsp_problem.resources_list:
                resource_avail_in_time[r][s:e] -= rcpsp_problem.mode_details[t][modes_dict[t]].get(r, 0)
                if np.any(resource_avail_in_time[r][s:e] < 0):
                    print("Missing ressource, "+str(__file__))
    return resource_avail_in_time


def permutation_do_to_permutation_sgs_fast(rcpsp_problem: RCPSPModelPreemptive, permutation_do):
...
def get_rcpsp_modelp_preemptive(rcpsp_model):
    return RCPSPModelPreemptive(resources=rcpsp_model.resources,
                                ...
                                preemptive_indicator={rcpsp_model.tasks_list[k]: True
                                                      for k in range(rcpsp_model.n_jobs)},
