...
def compute_resource_consumption(
    ...
    modes_dict = rcpsp_problem.get_modes_dict(rcpsp_sol)
    modes_dict[rcpsp_problem.source_task] = 1
    modes_dict[rcpsp_problem.sink_task] = 1
...
def plot_ressource_view(
    ...
    for j in sorted_activities:
        ...
        for time_start, time_end in zip(time_starts, time_ends):
            for i in range(len(list_resource)):
                cons = rcpsp_problem.mode_details[j][modes_dict[j]].get(
                    list_resource[i], 0
                )
                if cons == 0:
                    continue
                bound = (
                    rcpsp_problem.get_resource_available(list_resource[i], 0)
                    if not with_calendar
                    else max(
                        rcpsp_problem.get_resource_availability_array(list_resource[i])
                    )
                )
    ...
    for i in range(len(list_resource)):
        ...
        if not with_calendar:
            ax[i].axhline(
                y=rcpsp_problem.get_resource_available(list_resource[i], 0),
                linestyle="--",
            ...
            ax[i].plot(
                merged_times[i],
                [
                    rcpsp_problem.get_resource_available(list_resource[i], m)
...
def compute_schedule_per_resource_individual(
    ...
    modes_dict = rcpsp_problem.build_mode_dict(
    ...
    array_ressource_usage = {
        resources[i]: {
            "activity": np.zeros(
                (
                    max_time - min_time + 1,
                    max(rcpsp_problem.resources[resources[i]])
                    if with_calendar
                    else rcpsp_problem.resources[resources[i]],
                )
            ),
    ...
    for activity in sorted_task_by_start:
        ...
        for start_time, end_time in zip(start_times, end_times):
            if end_time == start_time:
                continue
            resources_needed = {
                r: rcpsp_problem.mode_details[activity][mode].get(r, 0)
                for r in resources
            }
            for r in resources_needed:
                ...
                if not with_calendar:
                    ...
                    range_interest = range(
                        rcpsp_problem.resources[r][time_to_index[start_time]]
                    )
                while rneeded > 0:
                    availables_people_r = [
                        i
...
def plot_resource_individual_gantt(
    ...
    for i in range(len(resources_list)):
        ...
        try:
            ...
            ax[i].set_yticks(range(rcpsp_problem.resources[resources_list[i]]))
            ax[i].set_yticklabels(
                tuple([j for j in range(rcpsp_problem.resources[resources_list[i]])]),
                fontdict={"size": 7},
            )
        except:
            m = max(rcpsp_problem.resources[resources_list[i]])
            ax[i].set_ylim((-0.5, m))
