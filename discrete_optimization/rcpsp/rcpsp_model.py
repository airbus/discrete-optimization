...
from typing import List, Union, Tuple, Dict, Iterable, Hashable
import numpy as np
...
from discrete_optimization.rcpsp.fast_function_rcpsp import sgs_fast, sgs_fast_partial_schedule, compute_mean_ressource, \
...
class RCPSPSolution(Solution):
    ...
    def __init__(self, problem,
                 ...
                 standardised_permutation=None, fast=True):
    ...
    def compute_mean_resource_reserve(self, fast=True):
        if not fast:
            return compute_mean_resource_reserve(solution=self, rcpsp_problem=self.problem)
        else:
            ...
            return self.problem.compute_mean_resource(horizon=makespan,
                                                      modes_array=
                                                      np.array(self.problem.build_mode_array(self.rcpsp_modes))-1,  # permutzation_task=array(task)->task index
                                                      start_array=np.array([self.rcpsp_schedule[t]["start_time"]
                                                                            for t in self.problem.tasks_list]),
                                                      end_array=np.array([self.rcpsp_schedule[t]["end_time"]
                                                                          for t in self.problem.tasks_list]))
    ...
    def generate_schedule_from_permutation_serial_sgs(self, do_fast=True):
        if do_fast:
            ...
            self.rcpsp_schedule = {}
            for k in schedule:
                self.rcpsp_schedule[self.problem.tasks_list[k]] = {"start_time": schedule[k][0],
                                                                   "end_time": schedule[k][1]}
            if self.problem.sink_task not in self.rcpsp_schedule:
                self.rcpsp_schedule[self.problem.sink_task] = {"start_time": 99999999,
                                                               "end_time": 99999999}
            self._schedule_to_recompute = False
        else:
        ...
        if do_fast:
            ...
            schedule, feasible = \
                self.problem.func_sgs_2(current_time=current_t,
                                        ...
                                        completed_task_times=np.array([completed_tasks[self.problem.tasks_list[i]].end
                                                                       if self.problem.tasks_list[i]
                                                                       in completed_tasks
                                                                       else 0 for i in range(self.problem.n_jobs)]),
                                        scheduled_task=np.array([scheduled_tasks_start_times[self.problem.tasks_list[i]]
                                                                 if self.problem.tasks_list[i]
                                                                 in scheduled_tasks_start_times
                                                                 else -1 for i in range(self.problem.n_jobs)]),
                                        permutation_task=
            ...
            for k in schedule:
                self.rcpsp_schedule[self.problem.tasks_list[k]] = {"start_time": schedule[k][0],
                                                                   "end_time": schedule[k][1]}
            if self.problem.sink_task not in self.rcpsp_schedule:
                self.rcpsp_schedule[self.problem.sink_task] = {"start_time": 999999999,
                                                               "end_time": 999999999}
    ...
    def get_active_time(self, task):
        return list(range(self.get_start_time(task),
...
class RCPSPModel(Problem):
    ...
    def __init__(self,
        ...
        self.costs = {"makespan": True, "mean_resource_reserve": args.get("mean_resource_reserve", False)}
        self.graph = self.compute_graph()

    def update_functions(self):
        self.func_sgs, self.func_sgs_2, self.compute_mean_resource = create_np_data_and_jit_functions(rcpsp_problem=self)
    ...
    def satisfy(self, rcpsp_sol: RCPSPSolution)->bool:
        if rcpsp_sol.rcpsp_schedule_feasible is False:
            ...
            start_times = [rcpsp_sol.rcpsp_schedule[t]["start_time"] for t in rcpsp_sol.rcpsp_schedule]
            for t in start_times:
                ...
                for res in self.resources.keys():
                    if resource_usage[res] > self.get_resource_available(res, t):
                        print([act for act in rcpsp_sol.rcpsp_schedule
                               if rcpsp_sol.rcpsp_schedule[act]['start_time']
                               <= t < rcpsp_sol.rcpsp_schedule[act]['end_time']])
    ...
    def compute_resource_consumption(self, rcpsp_sol: RCPSPSolution):
        ...
        for act_id in rcpsp_sol.rcpsp_schedule:
            for ir in range(len(self.resources)):
                consumptions[ir,
                             rcpsp_sol.rcpsp_schedule[act_id]["start_time"]+1:
...
def create_np_data_and_jit_functions(rcpsp_problem: Union[RCPSPModel, RCPSPModelCalendar]):
    ...
    func_sgs_2 = partial(sgs_fast_partial_schedule_incomplete_permutation_tasks,
                         consumption_array=consumption_array,
                         duration_array=duration_array,
                         predecessors=predecessors,
                         successors=successors,
...
def generate_schedule_from_permutation_serial_sgs(solution: RCPSPSolution,
        ...
        perm_extended = [rcpsp_problem.tasks_list_non_dummy[x]
                         for x in solution.rcpsp_permutation]
                ...
                for t in range(current_min_time,
                    ...
                    for res in rcpsp_problem.resources_list:
                        if rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0) == 0:
                            continue
                        if t < new_horizon:
                            if resource_avail_in_time[res][t] < rcpsp_problem.mode_details[act_id][modes_dict[act_id]][res]:  # 11
                                valid = False  # 12
                        else:
                    ...
                    for res in resource_avail_in_time:
                        ...
                        if res in rcpsp_problem.non_renewable_resources and t == end_t-1:
                            for tt in range(end_t, new_horizon):
                                resource_avail_in_time[res][tt] -= rcpsp_problem.mode_details[act_id][modes_dict[act_id]][res]
        ...
        for act_id in activity_end_times:
            ...
            rcpsp_schedule[act_id]['start_time'] = activity_end_times[act_id] - rcpsp_problem.mode_details[act_id][modes_dict[act_id]]['duration']
...
def generate_schedule_from_permutation_serial_sgs_partial_schedule(solution: RCPSPSolution,
        ...
        for act_id in scheduled_tasks_start_times:
            ...
            for t in range(current_min_time,
                ...
                for res in resource_avail_in_time:
                    ...
                    if res in rcpsp_problem.non_renewable_resources and t == end_t-1:
                        for tt in range(end_t, new_horizon):
                            resource_avail_in_time[res][tt] -= rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                            if resource_avail_in_time[res][tt] < 0:
                                unfeasible_non_renewable_resources = True
            activity_end_times[act_id] = end_t
            perm_extended.remove(act_id)
        ...
        while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
            ...
            while not valid:  # 7
                valid = True  # 8
                for t in range(current_min_time,
                               current_min_time+rcpsp_problem.mode_details[act_id][modes_dict[act_id]]['duration']):  # 9
                    for res in resource_avail_in_time:
                        if t < new_horizon:
...
def compute_mean_resource_reserve(solution: RCPSPSolution,
                                  rcpsp_problem: RCPSPModel):
...
class SGSWithoutArray:
        ...
        for res in self.rcpsp_model.resources_list:
            if self.rcpsp_model.is_varying_resource():
                self.resource_avail_in_time[res] = \
                    np.array(self.rcpsp_model.resources[res][:self.rcpsp_model.horizon + 1])
            else:
                self.resource_avail_in_time[res] = np.full(self.rcpsp_model.horizon,
                                                           self.rcpsp_model.resources[res],
                                                           dtype=int)
        self.dict_step_ressource = {}
        for res in self.resource_avail_in_time:
            self.dict_step_ressource[res] = SGSWithoutArray.create_absolute_dict(self.resource_avail_in_time[res])
    ...
    @staticmethod
    def add_event_delta(sdict: SortedDict,
                        time_start,
                        delta,
                        time_end):
    ...
    @staticmethod
    def add_event_delta_in_absolute(sdict: SortedDict,
                                    time_start,
                                    delta, # Negative usually
                                    time_end, liberate=True):
    ...
    def generate_schedule_from_permutation_serial_sgs(self,
        ...
        while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
            ...
            while not valid:
                ...
                end_time = current_min_time+rcpsp_problem.mode_details[act_id][modes_dict[act_id]]["duration"]
                for res in rcpsp_problem.resources_list:
                    need = rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                    if need == 0:
                        continue
                    else:
                        if self.get_available_from_absolute(sdict=resource_avail_in_time[res],
                                                            time=current_min_time) < need:
                            current_min_time = next((k for k in resource_avail_in_time[res]
                                                     if k > start_time and resource_avail_in_time[res][k] >= need),
                                                    None)
                            valid = False
                            break
                        keys = [k for k in resource_avail_in_time[res] if start_time <= k < end_time]
                        for k in keys:
                            if resource_avail_in_time[res][k] < need:
                                current_min_time = next((ki for ki in resource_avail_in_time[res]
                                                         if ki > k and resource_avail_in_time[res][ki] >= need),
                                                        None)
                                valid = False
                                break
            if not unfeasible_non_renewable_resources:
                ...
                for res in resource_avail_in_time:
                    ...
                    if res not in rcpsp_problem.non_renewable_resources:
                        self.add_event_delta_in_absolute(resource_avail_in_time[res],
