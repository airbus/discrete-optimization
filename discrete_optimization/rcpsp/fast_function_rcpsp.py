...
@njit
def sgs_fast(permutation_task,
             modes_array,          # permutzation_task=array(task)->task index
        ...
        if not unfeasible_non_renewable_resources:
            ...
            for j in range(nb_task):
                if successors[act_id, permutation_task[j]] == 1:
                    minimum_starting_time[permutation_task[j]] = max(int(minimum_starting_time[permutation_task[j]]),
...
@njit
def sgs_fast_preemptive(permutation_task,
                        modes_array,          # permutzation_task=array(task)->task index
                        consumption_array,    # modes=array(task)->0, 1... # consumption_array=array3D(task, mode, res),
                        duration_array,
    ...
    while done < nb_task and not unfeasible_non_renewable_resources:
        ...
        for i in range(nb_task):
            if pred_links[permutation_task[i]] == 0 and done_np[permutation_task[i]] == 0:
                act_id = permutation_task[i]
                break
        current_min_time = int(minimum_starting_time[act_id])  # 5
        valid = False  # 6
        starts = []
        ends = []
        while not valid:  # 7
            ...
            if duration_array[act_id, modes_array[act_id]] == 0:
                ...
                if reached_t is not None and preemptive_tag[act_id] \
                        and (reached_t + 1-current_min_time >= duration_array[act_id, modes_array[act_id]]/8 or reached_end):
                    starts.append(current_min_time)
...
@njit
def sgs_fast_preemptive_some_special_constraints(permutation_task,
            ...
            if duration_array[act_id, modes_array[act_id]] == 0:
                ...
                for t in range(current_min_time,
                    ...
                    if reached_end:
                        ...
                        break
                if reached_t is not None and preemptive_tag[act_id] and \
                        (reached_t + 1-current_min_time >= duration_array[act_id, modes_array[act_id]]/8 or reached_end):
                    starts.append(current_min_time)
                    ends.append(reached_t + 1)
                    done_duration[act_id] += ends[-1] - starts[-1]
                if reached_end and not preemptive_tag[act_id]:
...
#@njit
def sgs_fast_preemptive_minduration(permutation_task,
                                    modes_array,          # permutation_task=array(task)->task index
                                    consumption_array,    # modes=array(task)->0, 1... # consumption_array=array3D(task, mode, res),
                                    duration_array,
                                    preemptive_tag,       # array(task)->bool
                                    ...
                                    min_duration_preemptive): # array(res)->bool
    activity_end_times = {}
    unfeasible_non_renewable_resources = False
    new_horizon = horizon
    resource_avail_in_time = {}
            ...
            if duration_array[act_id, modes_array[act_id]] == 0:
                ...
                for t in range(current_min_time,
                    ...
                    if reached_end:
                        ...
                        break
                if reached_t is not None and preemptive_tag[act_id]:
                    if not reached_end and min_duration_preemptive_bool[act_id] \
                            and (min_duration_preemptive[act_id] > (reached_t + 1 - current_min_time)
                                 or
                                 (duration_array[act_id, modes_array[act_id]]
                                  - done_duration[act_id] < min_duration_preemptive[act_id])):
                        pass
                    else:
                        starts.append(current_min_time)
                        ends.append(reached_t + 1)
...
@njit
def sgs_fast_partial_schedule(current_time,
                              permutation_task,
                              modes_array,
                              ...
                              horizon,
                              ressource_available,
                              ressource_renewable, minimum_starting_time_array):
    activity_end_times = {}
    unfeasible_non_renewable_resources = False
    new_horizon = horizon
    resource_avail_in_time = {}
    ...
    for t in range(nb_task):
        if scheduled_task[t] != -1:
            activity_end_times[t] = scheduled_task[t]+duration_array[t, modes_array[t]]
            for res in range(ressource_available.shape[0]):
                if ressource_renewable[res]:
                    resource_avail_in_time[res][scheduled_task[t]:activity_end_times[t]] \
                        -= consumption_array[t, modes_array[t], res]
                else:
                    resource_avail_in_time[res][scheduled_task[t]:] -= consumption_array[t, modes_array[t], res]
                    if resource_avail_in_time[res][horizon] < 0:
                        unfeasible_non_renewable_resources = True
                        break
            if unfeasible_non_renewable_resources:
...
@njit
def sgs_fast_partial_schedule_incomplete_permutation_tasks(current_time,
                                                           permutation_task,
                                                           modes_array,
                                                           completed_task_indicator,
                                                           completed_task_times,
    ...
    minimum_starting_time = {}
    for act in range(permutation_task.shape[0]):
        minimum_starting_time[permutation_task[act]] = max(current_time, minimum_starting_time_array[act])
    done = 0
    nb_task = permutation_task.shape[0]
    pred_links = np.sum(predecessors[permutation_task, :], axis=1)
...
@njit
def sgs_fast_partial_schedule_preemptive(current_time,
                                         permutation_task,
                                         modes_array,
                                         completed_task_indicator,
    ...
    for t in range(nb_task):
        ...
        for i in range(partial_schedule_starts.shape[1]):
            if partial_schedule_starts[t, i] != -1:
                ...
                done_duration[t] += partial_schedule_ends[t, i]-partial_schedule_starts[t, i]
                for res in range(ressource_available.shape[0]):
                    if ressource_renewable[res]:
                        resource_avail_in_time[res][partial_schedule_starts[t, i]:partial_schedule_ends[t, i]] \
                            -= consumption_array[t, modes_array[t], res]
                    else:
                        if done_duration[t] == duration_array[t, modes_array[t]]:
                            resource_avail_in_time[res][partial_schedule_starts[t, i]:] -= \
                                consumption_array[t, modes_array[t], res]
                            if resource_avail_in_time[res][horizon] < 0:
                                unfeasible_non_renewable_resources = True
                                break
                if unfeasible_non_renewable_resources:
                    break
        if done_duration[t] == duration_array[t, modes_array[t]] and duration_array[t, modes_array[t]] >= 1:
        ...
        if completed_task_indicator[t] == 1:
            ...
            starts_dict[t] = np.array([k for k in partial_schedule_starts[t, :]
                                      if k != -1])
            ends_dict[t] = np.array([k for k in partial_schedule_ends[t, :]
                                     if k != -1])
            for s in range(successors.shape[1]):
                if successors[t, s] == 1:
                    minimum_starting_time[s] = max(int(minimum_starting_time[s]),
    ...
    while done < nb_task and not unfeasible_non_renewable_resources:
        ...
        if not unfeasible_non_renewable_resources:
            ...
            if unfeasible_non_renewable_resources:
                break
            activity_end_times[act_id] = end_t
            starts_dict[act_id] = np.array([k for k in partial_schedule_starts[act_id, :]
                                            if k != -1]+starts)
            ends_dict[act_id] = np.array([k for k in partial_schedule_ends[act_id, :]
                                          if k != -1]+ends)
            done_np[act_id] = 1
            done += 1
            for s in range(successors.shape[1]):
...
@njit
def sgs_fast_partial_schedule_preemptive_minduration(current_time,
                                                     permutation_task,
                                                     modes_array,
                                                     completed_task_indicator,
                                                     partial_schedule_starts, # array(task, 5)
...
@njit
def compute_mean_ressource(modes_array,
                           consumption_array,
                           start_array,
                           end_array,
                           horizon,
    ...
    for t in range(nb_task):
        ...
        for res_index in resource_avail_in_time:
            if ressource_renewable[res_index]:
                resource_avail_in_time[res_index][start_time:end_time] -= consumption_array[t, modes_array[t],
                                                                                            res_index]
            else:
                resource_avail_in_time[res_index][start_time:] -= consumption_array[t, modes_array[t], res_index]
    mean_avail = {}
    for res in resource_avail_in_time:
        mean_avail[res] = np.mean(resource_avail_in_time[res])
    mean_resource_reserve = np.mean(np.array([mean_avail[res_index] /
                                             np.max(ressource_available[res_index, :])
