...
from discrete_optimization.generic_tools.do_problem import Problem, ObjectiveRegister, TypeObjective, ModeOptim, \
    ObjectiveHandling

from discrete_optimization.rcpsp.rcpsp_model import RCPSPModel, PartialSolution, RCPSPSolution
...
from typing import Dict, Tuple, List, Hashable, Union
import networkx as nx
import numpy as np
import random
from functools import partial
from discrete_optimization.rcpsp.fast_function_rcpsp import sgs_fast, \
    sgs_fast_preemptive_some_special_constraints,\
    sgs_fast_partial_schedule_preemptive, sgs_fast_partial_schedule,\
    sgs_fast_preemptive_minduration, sgs_fast_partial_schedule_preemptive_minduration,\
    compute_mean_ressource, sgs_fast_preemptive
...
class SpecialConstraintsDescription(PartialSolution):
    def __init__(self,
                 ...
                 start_times_window: Dict[Hashable, Tuple[Union[int, None],
                                                          Union[int, None]]] = None,
                 end_times_window: Dict[Hashable, Tuple[Union[int, None],
                                                        Union[int, None]]] = None,
        ...
        super().__init__(task_mode=task_mode,
        ...
        self.components = [c for c in nx.connected_components(self.graph_start_together)]
...
class RCPSPSolutionSpecial(RCPSPSolution):
    ...
    def copy(self):
        return RCPSPSolutionSpecial(problem=self.problem,
    ...
    def generate_permutation_from_schedule(self):
        sorted_task = [self.problem.index_task[i]-1
                       for i in sorted(self.rcpsp_schedule,
    ...
    def generate_schedule_from_permutation_serial_sgs_2(self,
                                                        current_t=0,
        ...
        if scheduled_tasks_start_times is None:
            scheduled_tasks_start_times = None
        schedule, feasible = \
                generate_schedule_from_permutation_serial_sgs_partial_schedule(solution=self,
                                                                               current_t=current_t,
...
class RCPSPModelSpecialConstraints(RCPSPModel):
    def __init__(self,
        ...
        self.sgs_func_partial = generate_schedule_from_permutation_serial_sgs_partial_schedule
    ...
    def evaluate_from_encoding(self, int_vector, encoding_name):
        if encoding_name == 'rcpsp_permutation':
            ...
            rcpsp_sol = RCPSPSolutionSpecial(problem=self,
    ...
    def get_objective_register(self) -> ObjectiveRegister:
        dict_objective = {"makespan": {"type": TypeObjective.OBJECTIVE, "default_weight": -1},
                          "constraint_penalty": {"type": TypeObjective.PENALTY, "default_weight": -100}}
    ...
    def satisfy(self, rcpsp_sol: RCPSPSolution) -> bool:
        s = check_solution(problem=self, solution=rcpsp_sol,
    ...
    def get_dummy_solution(self):
        sol = RCPSPSolutionSpecial(problem=self,
...
class RCPSPSolutionSpecialPreemptive(RCPSPSolutionPreemptive):
    ...
    def copy(self):
        return RCPSPSolutionSpecialPreemptive(problem=self.problem,
                                              rcpsp_permutation=deepcopy(self.rcpsp_permutation),
                                              rcpsp_modes=deepcopy(self.rcpsp_modes),
                                              rcpsp_schedule=deepcopy(self.rcpsp_schedule),
                                              rcpsp_schedule_feasible=self.rcpsp_schedule_feasible,
    ...
    def generate_schedule_from_permutation_serial_sgs(self, do_fast=True):
        if do_fast:
            ...
            schedule, feasible = self.problem.sgs_func(solution=self,
                                                       rcpsp_problem=self.problem)
            self.rcpsp_schedule = schedule
            self.rcpsp_schedule_feasible = feasible
            self._schedule_to_recompute = False
    ...
    def generate_schedule_from_permutation_serial_sgs_2(self,
                                                        ...
                                                        do_fast=True):
        if do_fast:
            super().generate_schedule_from_permutation_serial_sgs_2(current_t=current_t,
                                                                    completed_tasks=completed_tasks,
            ...
            if partial_schedule is None:
                partial_schedule = partial_schedule
            schedule, feasible = \
                self.problem.sgs_func_partial(solution=self,
                                              current_t=current_t,
                                              partial_schedule=partial_schedule,
                                              completed_tasks=completed_tasks,
                                              rcpsp_problem=self.problem)
...
class RCPSPModelSpecialConstraintsPreemptive(RCPSPModelPreemptive):
    def __init__(self,
                 ...
                 source_task=None,
                 sink_task=None,
                 name_task: Dict[int, str] = None, **kwargs):
        ...
        self.sgs_func_partial = generate_schedule_from_permutation_serial_sgs_partial_schedule_preempptive
        self.relax_the_start_at_end = relax_the_start_at_end
        self.func_sgs, self.func_sgs_2, self.compute_mean_resource = create_np_data_and_jit_functions(self)
    ...
    def copy(self):
        return RCPSPModelSpecialConstraintsPreemptive(resources=deepcopy(self.resources),
    ...
    def lazy_copy(self):
        return RCPSPModelSpecialConstraintsPreemptive(resources=self.resources,
    ...
    def evaluate_from_encoding(self, int_vector, encoding_name):
        if encoding_name == 'rcpsp_permutation':
            single_mode_list = [1 for i in range(self.n_jobs_non_dummy)]
            rcpsp_sol = RCPSPSolutionSpecialPreemptive(problem=self,
                                                       rcpsp_permutation=int_vector,
                                                       rcpsp_modes=single_mode_list)
            objectives = self.evaluate(rcpsp_sol)
            return objectives
    ...
    def get_dummy_solution(self, random_perm: bool = False):
        ...
        sol = RCPSPSolutionSpecialPreemptive(problem=self,
...
def from_rcpsp_model(rcpsp_model: RCPSPModel, constraints: SpecialConstraintsDescription, preemptive=False):
    if preemptive:
        return RCPSPModelSpecialConstraintsPreemptive(resources=rcpsp_model.resources,
...
def evaluate_constraints(solution: Union[RCPSPSolution, RCPSPSolutionPreemptive],
...
def compute_constraints_details(solution: Union[RCPSPSolution, RCPSPSolutionPreemptive],
                                constraints: SpecialConstraintsDescription):
    if "rcpsp_schedule_feasible" in solution.__dict__.keys() and not solution.rcpsp_schedule_feasible:
        return []
...
def check_solution(problem: Union[RCPSPModelSpecialConstraints, RCPSPModelSpecialConstraintsPreemptive],
                   solution: Union[RCPSPSolutionSpecial,
    ...
    for (t1, t2, off) in start_at_end_plus_offset:
        ...
        if not b:
            ...
            print(solution.get_start_time(t2), ' >= ', solution.get_end_time(t1), '+', off)
            return False
    for (t1, t2, off) in start_after_nunit:
    ...
    for t in problem.special_constraints.start_times_window:
        if problem.special_constraints.start_times_window[t][0] is not None:
            if solution.get_start_time(t) < problem.special_constraints.start_times_window[t][0]:
                print("start time 0, ", t, solution.get_start_time(t),
        ...
        if problem.special_constraints.start_times_window[t][1] is not None:
            if solution.get_start_time(t) > problem.special_constraints.start_times_window[t][1]:
                print("start time 1, ", t, solution.get_start_time(t),
    ...
    for t in problem.special_constraints.end_times_window:
        if problem.special_constraints.end_times_window[t][0] is not None:
            if solution.get_end_time(t) < problem.special_constraints.end_times_window[t][0]:
                print("end time 0, ", t, solution.get_end_time(t),
        ...
        if problem.special_constraints.end_times_window[t][1] is not None:
            if solution.get_end_time(t) > problem.special_constraints.end_times_window[t][1]:
                print("end time 1, ", t, solution.get_end_time(t),
                      problem.special_constraints.end_times_window[t][1])
                return False
    return True


def generate_schedule_from_permutation_serial_sgs_preemptive(solution,
                                                             rcpsp_problem: RCPSPModelSpecialConstraintsPreemptive):
    activity_end_times = {}
    ...
    for res in rcpsp_problem.resources_list:
        if rcpsp_problem.is_varying_resource():
            resource_avail_in_time[res] = np.copy(rcpsp_problem.resources[res][:new_horizon + 1])
        else:
            resource_avail_in_time[res] = np.full(new_horizon, rcpsp_problem.resources[res],
                                                  dtype=int).tolist()
    ...
    for act in rcpsp_problem.tasks_list:
        ...
        if rcpsp_problem.do_special_constraints:
            if act in rcpsp_problem.special_constraints.start_times_window:
                minimum_starting_time[act] = rcpsp_problem.special_constraints.start_times_window[act][0] \
                    if rcpsp_problem.special_constraints.start_times_window[act][0] is not None else 0

    perm_extended = [rcpsp_problem.tasks_list_non_dummy[x] for x in solution.rcpsp_permutation]
    ...
    for k in modes_dict:
        if modes_dict[k] not in rcpsp_problem.mode_details[k]:
            modes_dict[k] = 1
    expected_durations_task = {k: rcpsp_problem.mode_details[k][modes_dict[k]]["duration"]
                               for k in modes_dict}
    schedules = {}
        ...
        for task_id in perm:
            ...
            for pred in rcpsp_problem.predecessors.get(task_id, {}):
                if pred in perm_extended:
                    ...
                    break
            if not ignore_sc:
                for pred in rcpsp_problem.special_constraints.dict_start_at_end_reverse.get(task_id, {}):
                    if pred in perm_extended:
                        respected = False
                        break
                for pred in rcpsp_problem.special_constraints.dict_start_at_end_offset_reverse.get(task_id, {}):
                    if pred in perm_extended:
                        respected = False
                        break
                for pred in rcpsp_problem.special_constraints.dict_start_after_nunit_reverse.get(task_id, {}):
                    if pred in perm_extended:
                        respected = False
                        break
            task_to_start_too = set()
            if respected:
                task_to_start_too = rcpsp_problem.special_constraints.dict_start_together.get(task_id, set())
                if not ignore_sc:
                    if len(task_to_start_too) > 0:
                        if not all(s not in perm_extended
                                   for t in task_to_start_too
                                   for s in rcpsp_problem.predecessors[t]):
                            respected = False
                        if not all(s not in perm_extended
                                   for t in task_to_start_too
                                   for s in rcpsp_problem.special_constraints.dict_start_at_end_reverse.get(t, {})):
                            respected = False
                        if not all(s not in perm_extended
                                   for t in task_to_start_too
                                   for s in rcpsp_problem.special_constraints.dict_start_at_end_offset_reverse.get(t, {})):
                            respected = False
                        if not all(s not in perm_extended
                                   for t in task_to_start_too
                                   for s in rcpsp_problem.special_constraints.dict_start_after_nunit_reverse.get(t, {})):
                            respected = False
            if respected:
    ...
    while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
        ...
        if len(act_ids) == 0: # The constraints in the model are not necessarly trustable, leading to this problem.
        ...
        valid = False
        first_step = False  # we force the starting of all act_id to be the same current time
        while not valid:
            reached_t = None
            if all(expected_durations_task[act_id] for act_id in act_ids) == 0:
                ...
                reached_end = True
                if not first_step:
                    current_min_time = next((t for t in range(current_min_time, new_horizon)
                                            if all(resource_avail_in_time[res][t] >=
                                                   sum([ressource_consumption(res=res,
                                                                              task=ac,
                                                                              mode=modes_dict[ac],
                ...
                for ac in act_ids:
                    reached_t = None
                    for t in range(current_min_time_dict[ac],
                                   current_min_time_dict[ac]
                                   + expected_durations_task[ac]-cur_duration[ac]):
                        if t >= new_horizon:
                            reached_end = False
                        ...
                        for res in rcpsp_problem.resources_list:
                            for t in range(starts[ac][-1], ends[ac][-1]):
                                ...
                                if resource_avail_in_time[res][t] < 0:
                                    print("problem")
                    if reached_end and reached_dict[ac] is not None and not rcpsp_problem.can_be_preempted(ac):
                        starts[ac] += [current_min_time_dict[ac]]
                        ends[ac] += [reached_dict[ac]+1]
                        cur_duration[ac] += ends[ac][-1] - starts[ac][-1]
                        for res in rcpsp_problem.resources_list:
                            for t in range(starts[ac][-1], ends[ac][-1]):
                                ...
                                if resource_avail_in_time[res][t] < 0:
                                    print("problem")
                                if res in rcpsp_problem.non_renewable_resources and t == ends[ac][-1] - 1:
                                    for tt in range(t+1, new_horizon):
                                        resource_avail_in_time[res][tt] -= \
                                            rcpsp_problem.mode_details[ac][modes_dict[ac]].get(res, 0)
                                        if resource_avail_in_time[res][tt] < 0:
                                            unfeasible_non_renewable_resources = True
            valid = all(cur_duration[ac] == expected_durations_task[ac] for ac in act_ids)
            if not valid:
                current_min_time_dict = {ac: next((t for t in range(reached_dict[ac]+2
                                                                    if reached_dict[ac] is not None
                                                                    else current_min_time_dict[ac]+1,
                                                                    new_horizon)
                                         if all(resource_avail_in_time[res][t] >=
                                                sum([ressource_consumption(res=res, task=ac, mode=modes_dict[ac],
                                                                           duration=cur_duration[ac]+1)])
                                                for res in rcpsp_problem.resources_list)),
                                                  None) for ac in act_ids}
                if any(current_min_time_dict[ac] is None for ac in current_min_time_dict):
                    unfeasible = True
                    break
        if not unfeasible_non_renewable_resources and not unfeasible:
            for ac in starts:
                ...
                for s in rcpsp_problem.successors[ac]:
                    minimum_starting_time[s] = max(minimum_starting_time[s], activity_end_times[ac])
                for s in rcpsp_problem.special_constraints.dict_start_at_end.get(ac, {}):
                    minimum_starting_time[s] = max(minimum_starting_time[s], activity_end_times[ac])
                for s in rcpsp_problem.special_constraints.dict_start_after_nunit.get(ac, {}):
                    minimum_starting_time[s] = max(starts[ac][0] \
                                               + rcpsp_problem.special_constraints.dict_start_after_nunit[ac][s],
                                                   minimum_starting_time[s])
                for s in rcpsp_problem.special_constraints.dict_start_at_end_offset.get(ac, {}):
                    minimum_starting_time[s] = max(activity_end_times[ac] \
                                               + rcpsp_problem.special_constraints.dict_start_at_end_offset[ac][s],
                                                   minimum_starting_time[s])
        else:
            break
    rcpsp_schedule = {}
    ...
    if unfeasible_non_renewable_resources or unfeasible:
        print('unfeasible: ', unfeasible, 'unfeasible_non_renewable_resources: ', unfeasible_non_renewable_resources)
...
def generate_schedule_from_permutation_serial_sgs_partial_schedule_preempptive(solution,
                                                                               rcpsp_problem,
    ...
    for res in rcpsp_problem.resources_list:
        if rcpsp_problem.is_varying_resource():
            resource_avail_in_time[res] = list(rcpsp_problem.resources[res][:new_horizon + 1])
        else:
            resource_avail_in_time[res] = np.full(new_horizon,
                                                  rcpsp_problem.resources[res], dtype=int).tolist()
    # 3
    minimum_starting_time = {}
    for act in rcpsp_problem.tasks_list:
        ...
        if rcpsp_problem.do_special_constraints:
            if act in rcpsp_problem.special_constraints.start_times_window:
                minimum_starting_time[act] = max(rcpsp_problem.special_constraints.start_times_window[act][0],
                                                 minimum_starting_time[act]) \
                    if rcpsp_problem.special_constraints.start_times_window[act][0] is not None \
    ...
    for task in partial_schedule:
        ...
        done_duration_task[task] = sum([ends[i]-starts[i] for i in range(len(starts))])
        end_t = ends[-1]
        for s, e in zip(starts, ends):
            for t in range(s, e):
                for res in resource_avail_in_time:
                    resource_avail_in_time[res][t] -= rcpsp_problem.mode_details[task][modes_dict[task]].get(res, 0)
                    if res in rcpsp_problem.non_renewable_resources and t == end_t - 1:
                        for tt in range(end_t, new_horizon):
                            resource_avail_in_time[res][tt] -= \
                                rcpsp_problem.mode_details[task][modes_dict[task]].get(res, 0)
                            if resource_avail_in_time[res][tt] < 0:
                                unfeasible_non_renewable_resources = True
        if done_duration_task[task] == expected_durations_task[task]:
            ...
            for s in rcpsp_problem.successors[task]:
                minimum_starting_time[s] = max(minimum_starting_time[s], activity_end_times[task])
        else:
            ...
            if not valid:
                current_min_time_dict = {ac: next(t for t in range(reached_dict[ac]+2
                                         ...
                                         if all(resource_avail_in_time[res][t] >=
                                                sum([ressource_consumption(res=res, task=ac, mode=modes_dict[ac],
                                                                           duration=cur_duration[ac]+1)])
                                                for res in rcpsp_problem.resources_list)) for ac in act_ids}
...
def generate_schedule_from_permutation_serial_sgs(solution: RCPSPSolution,
    ...
    for res in rcpsp_problem.resources_list:
        if rcpsp_problem.is_varying_resource():
            resource_avail_in_time[res] = rcpsp_problem.resources[res][:new_horizon + 1]
        else:
            resource_avail_in_time[res] = np.full(new_horizon, rcpsp_problem.resources[res],
                                                  dtype=int).tolist()
    minimum_starting_time = {}
    ...
    while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
        act_ids = look_for_task(list([k for k in rcpsp_problem.special_constraints.dict_start_at_end_reverse
                                      if k in perm_extended]))
        act_ids = []
        if len(act_ids) == 0:
            act_ids = look_for_task(perm_extended)
        if len(act_ids) == 0:
            act_ids = look_for_task(perm_extended, ignore_sc=True)
        current_min_time = max([minimum_starting_time[act_id] for act_id in act_ids])
        max_duration = max([rcpsp_problem.mode_details[act_id][modes_dict[act_id]]['duration']
                            for act_id in act_ids])
        valid = False
        while not valid:
            valid = True
            for t in range(current_min_time,
                           ...
                           + max_duration):
                for res in rcpsp_problem.resources_list:
                    r = sum([ressource_consumption(res=res,
                                                   task=task,
                                                   duration=t-current_min_time,
                                                   mode=modes_dict[task]) for task in act_ids])
                    if r == 0:
                        continue
                    if t < new_horizon:
        ...
        if not unfeasible_non_renewable_resources:
            ...
            for act_id in act_ids:
                ...
                for s in rcpsp_problem.special_constraints.dict_start_at_end.get(act_id, {}):
                    minimum_starting_time[s] = max(minimum_starting_time[s], activity_end_times[act_id])
                for s in rcpsp_problem.special_constraints.dict_start_after_nunit.get(act_id, {}):
                    minimum_starting_time[s] = max(minimum_starting_time[s], current_min_time \
                                               +rcpsp_problem.special_constraints.dict_start_after_nunit[act_id][s])
                for s in rcpsp_problem.special_constraints.dict_start_at_end_offset.get(act_id, {}):
                    minimum_starting_time[s] = max(minimum_starting_time[s], activity_end_times[act_id] \
                                               +rcpsp_problem.special_constraints.dict_start_at_end_offset[act_id][s])
    rcpsp_schedule = {}
    for act_id in activity_end_times:
        rcpsp_schedule[act_id] = {}
...
def generate_schedule_from_permutation_serial_sgs_partial_schedule(solution: RCPSPSolution,
                                                                   rcpsp_problem: RCPSPModelSpecialConstraints,
                                                                   current_t,
                                                                   completed_tasks,
                                                                   scheduled_tasks_start_times):
    ...
    for act in rcpsp_problem.tasks_list:
        ...
        if rcpsp_problem.do_special_constraints:
            if act in rcpsp_problem.special_constraints.start_times_window:
                minimum_starting_time[act] = max(rcpsp_problem.special_constraints.start_times_window[act][0],
                                                 minimum_starting_time[act]) \
                    if rcpsp_problem.special_constraints.start_times_window[act][0] is not None else minimum_starting_time[act]
    perm_extended = [rcpsp_problem.tasks_list_non_dummy[x] for x in solution.rcpsp_permutation]
    perm_extended.insert(0, rcpsp_problem.source_task)
    perm_extended.append(rcpsp_problem.sink_task)
    modes_dict = rcpsp_problem.build_mode_dict(solution.rcpsp_modes)
    ...
    for act_id in scheduled_tasks_start_times:
        ...
        end_t = current_min_time + rcpsp_problem.mode_details[act_id][modes_dict[act_id]]['duration']
        for t in range(current_min_time,
                       end_t):
            for res in resource_avail_in_time:
                resource_avail_in_time[res][t] -= rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(res, 0)
                if res in rcpsp_problem.non_renewable_resources and t == end_t - 1:
                    for tt in range(end_t, new_horizon):
                        resource_avail_in_time[res][tt] -= rcpsp_problem.mode_details[act_id][modes_dict[act_id]].get(
                            res, 0)
                        if resource_avail_in_time[res][tt] < 0:
                            unfeasible_non_renewable_resources = True
    ...
    while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
        ...
        for task_id in perm_extended:
            ...
            task_to_start_too = set()
            if respected:
                task_to_start_too = [k for k in rcpsp_problem.special_constraints.dict_start_together.get(task_id,
                                                                                                          set())
                                     if k in perm_extended]
                if len(task_to_start_too) > 0:
                    if not all(s not in perm_extended
        ...
        if not unfeasible_non_renewable_resources:
            ...
            for act_id in act_ids:
                ...
                for s in rcpsp_problem.special_constraints.dict_start_after_nunit.get(act_id, {}):
                    minimum_starting_time[s] = current_min_time \
                                               + rcpsp_problem.special_constraints.dict_start_after_nunit[act_id][s]
                for s in rcpsp_problem.special_constraints.dict_start_at_end_offset.get(act_id, {}):
                    minimum_starting_time[s] = activity_end_times[act_id] \
                                               + rcpsp_problem.special_constraints.dict_start_at_end_offset[act_id][s]
...
def create_np_data_and_jit_functions(rcpsp_problem: Union[RCPSPModelSpecialConstraintsPreemptive]):
    ...
    duration_array = np.zeros((rcpsp_problem.n_jobs, rcpsp_problem.max_number_of_mode), dtype=np.int)
    ...
    for k in range(len(rcpsp_problem.resources_list)):
        if rcpsp_problem.is_varying_resource():
            ressource_available[k, :] = \
                rcpsp_problem.resources[rcpsp_problem.resources_list[k]][:ressource_available.shape[1]]
        else:
            ressource_available[k, :] = np.full(ressource_available.shape[1],
    ...
    if not rcpsp_problem.is_duration_minimum_preemption():
        func_sgs = partial(sgs_fast_preemptive_some_special_constraints,
        ...
        func_sgs = partial(sgs_fast_preemptive_minduration,
                           consumption_array=consumption_array,
                           preemptive_tag=preemptive_tag,
                           duration_array=duration_array,
                           predecessors=predecessors,
        ...
        func_sgs_2 = partial(sgs_fast_partial_schedule_preemptive_minduration,
                             ...
                             min_duration_preemptive=min_duration_preemptive,
                             min_duration_preemptive_bool=min_duration_preemptive_bool)
    func_compute_mean_resource = partial(compute_mean_ressource,
                                         consumption_array=consumption_array,
                                         ressource_available=ressource_available,
                                         ressource_renewable=ressource_renewable)
