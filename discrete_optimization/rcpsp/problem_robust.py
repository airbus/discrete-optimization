...
class AggregRcpspProblem(RobustProblem, RcpspProblem):
    ...
    def get_unique_rcpsp_problem(self) -> RcpspProblem:
...
def create_poisson_laws_duration(
    rcpsp_problem: RcpspProblem, range_around_mean: int = 3
    ...
    sink = rcpsp_problem.sink_task
    for job in rcpsp_problem.mode_details:
        poisson_dict[job] = {}
        for mode in rcpsp_problem.mode_details[job]:
            poisson_dict[job][mode] = {}
            duration = rcpsp_problem.mode_details[job][mode]["duration"]
...
def create_poisson_laws_resource(
    rcpsp_problem: RcpspProblem, range_around_mean: int = 1
) -> dict[Hashable, dict[int, dict[str, tuple[int, int, int]]]]:
    poisson_dict: dict[Hashable, dict[int, dict[str, tuple[int, int, int]]]] = {}
    source = rcpsp_problem.source_task
    sink = rcpsp_problem.sink_task
    limit_resource = rcpsp_problem.resources
    resources_non_renewable = rcpsp_problem.non_renewable_resources
    for job in rcpsp_problem.mode_details:
        poisson_dict[job] = {}
        for mode in rcpsp_problem.mode_details[job]:
            poisson_dict[job][mode] = {}
            for resource in rcpsp_problem.mode_details[job][mode]:
                if resource == "duration":
                    continue
                if resource in resources_non_renewable:
                    continue
                resource_consumption = rcpsp_problem.mode_details[job][mode][resource]
                if job in {source, sink}:
                    ...
                    max_rc = min(
                        ...
                        rcpsp_problem.get_max_resource_capacity(resource),
                    )
                    poisson_dict[job][mode][resource] = (
                        min_rc,
                        resource_consumption,
...
class UncertainRcpspProblem:
    def __init__(
        ...
        self.base_rcpsp_problem = base_rcpsp_problem
    ...
    def create_rcpsp_problem(
        ...
        model = self.base_rcpsp_problem.copy()
        for activity in self.probas:
            if activity in {
                self.base_rcpsp_problem.source_task,
                self.base_rcpsp_problem.sink_task,
            }:
                continue
