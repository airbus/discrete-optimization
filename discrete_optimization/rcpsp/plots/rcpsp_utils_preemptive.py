...
def compute_resource_consumption(rcpsp_model: RCPSPModelPreemptive,
    ...
    for act_id in rcpsp_model.get_tasks_list():
        for ir in range(len(list_resources)):
            ...
            for s, e in zip(rcpsp_sol.get_start_times_list(act_id),
...
def compute_nice_resource_consumption(rcpsp_model: RCPSPModelPreemptive,
                                      rcpsp_sol: RCPSPSolutionPreemptive,
                                      list_resources: List[Union[int, str]] = None):
    if list_resources is None:
        list_resources = rcpsp_model.resources_list
...
def plot_ressource_view(rcpsp_model: RCPSPModelPreemptive,
                        rcpsp_sol: RCPSPSolutionPreemptive,
    ...
    sorted_activities = sorted(rcpsp_model.get_tasks_list(),
    ...
    for j in sorted_activities:
        ...
        for time_start, time_end in zip(time_starts, time_ends):
            for i in range(len(list_resource)):
                ...
                if cons == 0:
                    continue
                bound = rcpsp_model.get_resource_available(list_resource[i], 0) if not with_calendar \
                        else max(rcpsp_model.get_resource_availability_array(list_resource[i]))
    ...
    merged_times, merged_cons = compute_nice_resource_consumption(rcpsp_model,
                                                                  rcpsp_sol,
    ...
    for i in range(len(list_resource)):
        ...
        if not with_calendar:
            ax[i].axhline(y=rcpsp_model.get_resource_available(list_resource[i], 0),
                          linestyle="--",
...
def plot_task_gantt(rcpsp_model: RCPSPModelPreemptive,
                    rcpsp_sol: RCPSPSolutionPreemptive,
                    fig=None,
                    ax=None,
                    x_lim=None,
    ...
    sorted_task_by_start = sorted(rcpsp_sol.rcpsp_schedule,
                                  key=lambda x:
                                  100000 * rcpsp_sol.rcpsp_schedule[x]["starts"][0] + rcpsp_model.index_task[x])
    sorted_task_by_end = sorted(rcpsp_sol.rcpsp_schedule,
                                key=lambda x: 100000 * rcpsp_sol.rcpsp_schedule[x]["ends"][-1] + rcpsp_model.index_task[x])
    ...
    for j in range(nb_task):
        ...
        for start, end in zip(rcpsp_sol.rcpsp_schedule[tasks[j]]["starts"],
        ...
        ax.annotate(tasks[j],
                    xy=((rcpsp_sol.rcpsp_schedule[tasks[j]]["starts"][0]
                         + rcpsp_sol.rcpsp_schedule[tasks[j]]["ends"][0])/2, j),
                    # textcoords="offset points",
    ...
    ax.set_yticklabels(tuple([str(tasks[j]) for j in range(nb_task)]),
                       fontdict={"size": 5})
...
def compute_schedule_per_resource_individual(rcpsp_model: RCPSPModelPreemptive,
                                             rcpsp_sol: RCPSPSolutionPreemptive,
                                             resource_types_to_consider: List[str]=None,
                                             verbose=False):
    nb_ressources = len(rcpsp_model.resources_list)
    modes_dict = rcpsp_model.build_mode_dict(rcpsp_modes_from_solution=rcpsp_sol.rcpsp_modes)
    if resource_types_to_consider is None:
        resources = rcpsp_model.resources_list
    else:
        resources = resource_types_to_consider
    sorted_task_by_start = sorted(rcpsp_sol.rcpsp_schedule,
                                  key=lambda x: 100000*rcpsp_sol.rcpsp_schedule[x]["starts"][0]+rcpsp_model.index_task[x])
    sorted_task_by_end = sorted(rcpsp_sol.rcpsp_schedule,
                                key=lambda x: 100000*rcpsp_sol.rcpsp_schedule[x]["ends"][-1]+rcpsp_model.index_task[x])
    max_time = rcpsp_sol.rcpsp_schedule[sorted_task_by_end[-1]]["ends"][-1]
    min_time = rcpsp_sol.rcpsp_schedule[sorted_task_by_start[0]]["starts"][0]
                ...
                while rneeded > 0:
                    ...
                    if len(availables_people_r) > 0:
                        ...
                        array_ressource_usage[r]["activity_last_n_hours"][:, resource] = np.convolve(array_ressource_usage[r]["binary_activity"][:, resource],
                                                                                                     ...
                                                                                                     mode="same")
                        array_ressource_usage[r]["boxes_time"] += [[(resource-0.25, start_time+0.01,
                                                                     rcpsp_model.index_task[activity]),
                                                                    (resource-0.25, end_time-0.01,
                                                                     rcpsp_model.index_task[activity]),
                                                                    (resource+0.25, end_time-0.01,
                                                                     rcpsp_model.index_task[activity]),
                                                                    (resource+0.25, start_time+0.01,
                                                                     rcpsp_model.index_task[activity]),
                                                                    (resource-0.25, start_time+0.01,
                                                                     rcpsp_model.index_task[activity])]]
                        # for plot purposes.
...
def plot_resource_individual_gantt(rcpsp_model: RCPSPModelPreemptive,
                                   rcpsp_sol: RCPSPSolutionPreemptive,
                                   resource_types_to_consider: List[str]=None,
                                   title_figure="",
                                   x_lim=None,
