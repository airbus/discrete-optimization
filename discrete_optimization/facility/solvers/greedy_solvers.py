...
class GreedySolverDistanceBased(SolverDO):
    def __init__(self, facility_problem: FacilityProblem,
        ...
        self.aggreg_sol, self.aggreg_dict, self.params_objective_function = \
            build_aggreg_function_and_params_objective(problem=self.facility_problem,
                                                       params_objective_function=params_objective_function)
        self.matrix_cost = np.zeros((self.facility_problem.customer_count, self.facility_problem.facility_count))
        for k in range(self.facility_problem.customer_count):
            for j in range(self.facility_problem.facility_count):
                self.matrix_cost[k, j] = \
                    self.facility_problem.evaluate_customer_facility(facility=
                                                                     self.facility_problem.facilities[j],
                                                                     customer=
                                                                     self.facility_problem.customers[k])
        ...
        self.available_demands = np.array([self.facility_problem.facilities[k].capacity
    ...
    def solve(self, **kwargs):
        ...
        for customer in self.sorted_customers:
            prior_customers = kwargs.get("prio", {}).get(customer, self.sorted_distance[customer, :])
            if any(x % 1 != x for x in prior_customers):
                prior_customers = self.sorted_distance[customer, :]
            for f in prior_customers:
                f = int(f)
                if capacity_remaining[f] >= self.facility_problem.customers[customer].demand:
                    solution[customer] = f
                    capacity_remaining[f] -= self.facility_problem.customers[customer].demand
                    break
        sol = FacilitySolution(problem=self.facility_problem,
                               facility_for_customers=solution)
        fit = self.aggreg_sol(sol)
