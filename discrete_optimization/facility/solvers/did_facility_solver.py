...
class DidFacilitySolver(DidSolver, SolverFacility):
            ...
            for f in range(nb_facilities):
                transition = dp.Transition(
                    ...
                    effects=[
                        ...
                        (current_facility, f),
                        # (nb_facility_used, nb_facility_used+1),
                        (current_capacity, self.problem.customers[i].demand),
                    ],
                    preconditions=[
                        unallocated.contains(i),
    ...
    def init_model_mix(self, **kwargs):
        model = dp.Model()
        nb_facilities = self.problem.facility_count
        nb_customers = self.problem.customer_count
        capacities_facilities = [int(f.capacity) for f in self.problem.facilities]
        setup_cost = [10 * int(f.setup_cost) for f in self.problem.facilities]
        matrix = compute_matrix_distance_facility_problem(self.problem)
        matrix = [
            [int(10 * matrix[i, j]) for j in range(matrix.shape[1])]
        ...
        current_facility = model.add_element_var(object_type=factories, target=0)
        current_capacity = model.add_int_resource_var(target=0)
        closed_facility = model.add_set_var(object_type=factories, target=set())
        unallocated = model.add_set_var(
            object_type=customer, target=range(nb_customers)
        )
        mode = model.add_int_var(target=0)
        just_switch = model.add_int_var(target=0)
        counter_jump = model.add_int_var(target=0)
        counter_mode_0 = model.add_int_var(target=0)
        counter_mode_1 = model.add_int_var(target=0)

        for f in range(nb_facilities):
            new_cost = (used_facilities.contains(f)).if_then_else(0, setup_cost[f])
            alloc = dp.Transition(
                name=f"alloc_current_customer_to_{f}",
                cost=new_cost
                + distance_matrix[current_customer, f]
                + dp.IntExpr.state_cost(),
                effects=[
                    ...
                    (current_facility, f),
                    (current_capacity, used_capacities[f]),
                    (unallocated, unallocated.remove(current_customer)),
                    (current_customer, current_customer + 1),
                    (just_switch, 0),
                    (counter_jump, 0),
                    (counter_mode_0, counter_mode_0 + 1),
                ],
                preconditions=[
                    current_customer <= self.problem.customer_count - 1,
                    used_capacities[f] + demands[current_customer] <= capacities[f],
                    mode == 0,
                    ~(closed_facility.contains(f)),
                ],
            )
            model.add_transition(alloc)
        switch_1 = dp.Transition(
            name="switch_to_customer",
            cost=20 + dp.IntExpr.state_cost(),
            effects=[(mode, 1), (just_switch, 1), (counter_mode_0, 0)],
            preconditions=[
                mode == 0,
                just_switch == 0,
                # counter_mode_0 >= 1
        ...
        switch_2 = dp.Transition(
            name="switch_to_factory",
            cost=20 + dp.IntExpr.state_cost(),
            effects=[
                (mode, 0),
                (just_switch, 1),
                (closed_facility, closed_facility.add(current_facility)),
                (counter_mode_1, 0),
            ],
            preconditions=[
                mode == 1,
        ...
        model.add_transition(switch_1)
        model.add_transition(switch_2)
        jump = dp.Transition(
            name=f"jump_customer",
        ...
        for i in range(nb_customers):
            transition = dp.Transition(
                ...
                effects=[
                    (unallocated, unallocated.remove(i)),
                    (current_capacity, current_capacity + demands[i]),
                    (just_switch, 0),
                    (counter_jump, 0),
                    (counter_mode_1, counter_mode_1 + 1),
                ],
                preconditions=[
                    unallocated.contains(i),
                    ~used_facilities.is_empty(),
                    current_capacity + demands[i] <= capacities[current_facility],
                    mode == 1,
                ],
            )
            model.add_transition(transition)
            # for f in range(nb_facilities):
            #     new_cost = (used_facilities.contains(f)).if_then_else(0, setup_cost[f])
            #     transition = dp.Transition(name=f"alloc_via_{f}_{i}",
            #                                cost=setup_cost[f] + distance_matrix[i, f] + dp.IntExpr.state_cost(),
            #                                effects=[(unallocated, unallocated.remove(i)),
            #                                         (used_facilities, used_facilities.add(f)),
            #                                         (current_facility, f),
            #                                         (used_capacities[f], demands[i])],
            #                                preconditions=[unallocated.contains(i),
            #                                               ~(used_facilities.contains(f)),
            #                                               demands[i] <= capacities[f],
            #                                               mode == 1])
            #     model.add_transition(transition)
