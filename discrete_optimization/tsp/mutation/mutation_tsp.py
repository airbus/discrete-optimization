...
import numpy as np
from discrete_optimization.tsp.tsp_model import Point, Point2D, SolutionTSP, TSPModel, \
...
class SwapTSPMove(LocalMove):
    ...
    def apply_local_move(self, solution: SolutionTSP) -> SolutionTSP:
        ...
        #           -sum(previous))
        solution.length = solution.length+solution.lengths[i1]+solution.lengths[i1+1] + \
                          solution.lengths[i2]+solution.lengths[i2+1] - sum(previous)
