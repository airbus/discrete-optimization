...
class CPSatAllocSchedulingSolver(
    ...
    def solve(
        ...
        **kwargs: Any,
        ...
        if kwargs["run_multistep"]:
            return self.solve_multistep(
                callbacks=callbacks,
                parameters_cp=parameters_cp,
                time_limit=time_limit,
                **kwargs,
            )
        if kwargs["run_lexicographic"]:
            ...
            return OrtoolsCpSatSolver.solve(
                self,
                callbacks=callbacks,
                parameters_cp=parameters_cp,
                time_limit=time_limit,
    ...
    def solve_lexicographic(
        ...
        **kwargs,
        ...
        list_solution_fits = []
        self.init_model(objectives=[objectives[0]], **kwargs)
        for i_obj in range(1, len(objectives) + 1):
            ...
            sol, fit = ortools_callback.res.get_best_solution_fit()
            logger.info(f"Optimizing pass n°{i_obj-1}, status = {status}")
            logger.info(f"Evaluation : {self.problem.evaluate(sol)}")
            list_solution_fits += ortools_callback.res.list_solution_fits
            current_variables = {
                "starts_var": {
                    i: solver.Value(self.variables["starts_var"][i])
                    for i in self.variables["starts_var"]
                },
                "ends_var": {
                    i: solver.Value(self.variables["ends_var"][i])
                    for i in self.variables["ends_var"]
                },
                "is_present_var": {
                    i: {
                        j: solver.Value(self.variables["is_present_var"][i][j])
                        for j in self.variables["is_present_var"][i]
                    }
                    for i in self.variables["is_present_var"]
                },
                "objs": [solver.Value(x) for x in self.variables["objs"]],
            }
            if i_obj == len(objectives):
                break
            self.init_model(objectives=objectives[: i_obj + 1], **kwargs)
            # Lexicographic constraints
            for i in range(len(current_variables["objs"])):
                if objectives[i] in {
                    ...
                    self.cp_model.Add(
                        self.variables["objs"][i] <= current_variables["objs"][i]
                    )
                if objectives[i] == ObjectivesEnum.MIN_WORKLOAD:
                    self.cp_model.Add(
                        self.variables["objs"][i] <= current_variables["objs"][i]
                    )
            if kwargs["hint_lexicographic"]:
                # Warm start
                for key in ["starts_var", "ends_var"]:
                    for i in self.variables[key]:
                        self.cp_model.AddHint(
                            self.variables[key][i], current_variables[key][i]
                        )
                for key in self.variables["is_present_var"]:
                    for team in self.variables["is_present_var"][key]:
                        self.cp_model.AddHint(
                            self.variables["is_present_var"][key][team],
                            current_variables["is_present_var"][key][team],
                        )
    ...
    def solve_multistep(
        ...
        **kwargs,
        ...
        if kwargs["optional_activities"]:
            ...
            objs = [ObjectivesEnum.NB_TEAMS, ObjectivesEnum.MIN_WORKLOAD]
        self.init_model(objectives=objs, **kwargs)
        res = super().solve(
            callbacks=callbacks,
            parameters_cp=parameters_cp,
            time_limit=time_limit,
        ...
        nb_teams = self.problem.evaluate(sol)["nb_teams"]
        sol: AllocSchedulingSolution
        if kwargs["optional_activities"]:
            objs = [
                ObjectivesEnum.NB_DONE_AC,
        ...
        current_variables = {
            "starts_var": {i: sol.schedule[i, 0] for i in self.variables["starts_var"]},
            "ends_var": {i: sol.schedule[i, 1] for i in self.variables["ends_var"]},
            "is_present_var": {
                i: {
                    j: 1 if sol.allocation[i] == j else 0
                    for j in self.variables["is_present_var"][i]
                }
                for i in self.variables["is_present_var"]
            },
        ...
        self.cp_model.Add(self.variables["objs"][0] <= current_variables["objs"][0])
        if kwargs["hint_lexicographic"]:
            # Warm start
            for key in ["starts_var", "ends_var"]:
                for i in self.variables[key]:
        ...
        return ResultStorage(
            list_solution_fits=res.list_solution_fits + res_sp.list_solution_fits,
            mode_optim=self.params_objective_function.sense_function,
        )
