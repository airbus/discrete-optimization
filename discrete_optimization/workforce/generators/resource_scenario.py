...
from discrete_optimization.workforce.scheduling.problem import AllocSchedulingProblem, AllocSchedulingSolution
...
class ParamsRandomness:
    def __init__(self, seed: int = 42,
        ...
        if duration_discrete_distribution is None:
            self.duration_discrete_distribution = ([15, 30, 60, 120], [0.25, 0.25, 0.25, 0.25])
    ...
    def generate_list(self, solution: TeamAllocationSolution):
        self.nb_disruption = self.random_state.get_discrete_truncated_uniform_sample(
            lower=self.lower_nb_disruption,
            upper=self.upper_nb_disruption
        )
        self.nb_teams_per_disruption = [self.random_state.get_discrete_truncated_uniform_sample(lower=self.lower_nb_teams,
                                                                                                upper=self.upper_nb_teams)
                                        for _ in range(self.nb_disruption)]
        self.starts = [self.random_state.get_discrete_truncated_uniform_sample(lower=self.lower_time,
                                                                               upper=self.upper_time)
                       for _ in range(self.nb_disruption)]
        self.lengths = [self.random_state.get_random_element_prop(list=self.duration_discrete_distribution[0],
        ...
        self.nb_teams_per_disruption = [self.random_state.get_discrete_truncated_uniform_sample(
            lower=self.lower_nb_teams,
            upper=self.upper_nb_teams)
                                        for _ in range(self.nb_disruption)]
        self.starts = [self.random_state.get_discrete_truncated_uniform_sample(lower=self.lower_time,
                                                                               upper=self.upper_time)
        ...
        used_teams = list({i for i in set(solution.allocation) if int(i) in problem.index_to_teams_name})
        disruptions = []
        for j in range(self.nb_disruption):
            sub_teams = random.sample(used_teams,
                                      min(self.nb_teams_per_disruption[j],
                                          len(used_teams)))
            for k in range(len(sub_teams)):
                disruptions.append((self.starts[j], self.starts[j]+self.lengths[j], sub_teams[k]))
        return disruptions
...
def compute_available_teams_per_activities_alloc_problem(problem: TeamAllocationProblem,
                                                         starts: np.ndarray,
                                                         ends: np.ndarray,
...
def update_allocation_constraint(starts: np.ndarray,
                                 ends: np.ndarray,
                                 problem: TeamAllocationProblem,
                                 calendars_team: Dict[Hashable, np.ndarray]):
    available_team_per_activity = compute_available_teams_per_activities_alloc_problem(problem=problem,
                                                                                       starts=starts,
                                                                                       ends=ends,
                                                                                       calendars_team=calendars_team)
    ...
    problem.graph_allocation = GraphBipartite(nodes=graph.nodes,
                                              edges=edges,
...
def create_scheduling_problem_several_resource_dropping(allocation_problem: TeamAllocationProblem,
        ...
        for i in range(scheduling_problem.number_tasks):
            sched[i, 0] = scheduling_problem.original_start[scheduling_problem.index_to_task[i]]
            sched[i, 1] = scheduling_problem.original_end[scheduling_problem.index_to_task[i]]
            team_index = base_solution.allocation[allocation_problem.index_activities_name[
        ...
        sol_sched = AllocSchedulingSolution(problem=scheduling_problem,
                                            schedule=sched,
        ...
        #         used_teams[x] = False
        additional_cp_constraints = (AdditionalCPConstraints(nb_teams_bounds=(None, nb_team_used), #(nb_team_used - 1, nb_team_used),
                                                             team_used_constraint=used_teams_dict,
                                                             set_tasks_ignore_reallocation=set(),
                                                             forced_allocation={}))
        return {"additional_constraints": additional_cp_constraints,
                "base_solution": sol_sched}


def generate_resource_disruption_scenario_from(original_allocation_problem: TeamAllocationProblem,
                                               ...
                                               delta_minutes_flexibility_for_release_deadline: int = 30,
                                               add_same_allocation_constraint: bool = True):
    """
    ...
    for team in original_scheduling_problem.team_names:
        ...
        for slot in original_scheduling_problem.calendar_team[team]:
            calendars_array[team][slot[0]:min(slot[1], calendars_array[team].shape[0])] = 1
    new_alloc_problem = deepcopy(original_allocation_problem)
    update_allocation_constraint(starts, ends, problem=new_alloc_problem, calendars_team=calendars_array)
    available_teams_in_this_scenario = (
        set([original_allocation_problem.teams_name[x] for x in original_solution.allocation]))
    new_alloc_problem = cut_number_of_team(new_alloc_problem,
                                           subset_teams_keep=available_teams_in_this_scenario)
    return {"original_allocation_problem": original_scheduling_problem,
