...
class RandomState:
    ...
    def get_random_element_prop(self, list: list[T], probs: list[float]) -> T:
        ...
        sum_props = sum(probs)
        assert 1 - epsilon <= sum_props <= 1 + epsilon, f'Sum of probabilities must be 1 +-{epsilon} but is {sum(probs)}'
    ...
    def get_discrete_truncated_normal_sample(self, center: int, sigma: int, lower: int, upper: int) -> int:
        if lower == upper:
            return lower
