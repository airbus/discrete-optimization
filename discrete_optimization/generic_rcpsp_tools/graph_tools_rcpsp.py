...
class GraphRCPSP:
    def __init__(self, problem: Union[RCPSPModel,
        ...
        descendants = {n: nx.algorithms.descendants(self.graph_nx, n)
        ...
        for k in descendants:
            self.descendants_map[k] = {"succs": descendants[k],
                                       "nb": len(descendants[k])}
        ancestors = {n: nx.algorithms.ancestors(self.graph_nx, n)
                     for n in self.graph_nx.nodes()}
        for k in ancestors:
            self.ancestors_map[k] = {"succs": ancestors[k],
                                     "nb": len(ancestors[k])}
        self.graph_without_source_sink = nx.subgraph(self.graph_nx, [n for n in self.graph_nx
                                                                     if n not in {self.problem.sink_task,
    ...
    def compute_component_in_non_dummy_graph(self):
        return [c for c in sorted(nx.weakly_connected_components(self.graph_without_source_sink),
                                  key=len, reverse=True)]


class GraphRCPSPSpecialConstraints(GraphRCPSP):
    def __init__(self, problem: Union[RCPSPModelSpecialConstraints,
                                      RCPSPModelSpecialConstraintsPreemptive]):
        super().__init__(problem)
        self.special_constraints: SpecialConstraintsDescription = problem.special_constraints
        ...
        for K in self.special_constraints.start_together+self.special_constraints.start_at_end\
                + self.special_constraints.start_at_end_plus_offset+self.special_constraints.start_after_nunit\
        ...
        self.components_graph_constraints = [c for c in nx.strongly_connected_components(self.graph_constraints)]
...
def build_unrelated_task(graph: GraphRCPSP):
    ...
    unrel = {n: all_tasks.difference(set(ancestors[n]["succs"]).union(set(descendants[n]["succs"])).union({n}))
             for n in all_tasks}
    set_pairs = set()
    for task in unrel:
        for other_task in unrel[task]:
            if (task, other_task) not in set_pairs and (other_task, task) not in set_pairs:
                set_pairs.add((task, other_task))
    return unrel, set_pairs


def build_graph_rcpsp_object(rcpsp_problem):
    if isinstance(rcpsp_problem, (RCPSPModelSpecialConstraints,
                                  RCPSPModelSpecialConstraintsPreemptive)):
        return GraphRCPSPSpecialConstraints(problem=rcpsp_problem)
    else:
        if "special_constraints" in rcpsp_problem.__dict__.keys() \
                and isinstance(getattr(rcpsp_problem, "special_constraints"), SpecialConstraintsDescription):
            return GraphRCPSPSpecialConstraints(problem=rcpsp_problem)
