...
from discrete_optimization.generic_rcpsp_tools.neighbor_tools_rcpsp import ParamsConstraintBuilder, \
    NeighborBuilder, NeighborRandom, NeighborBuilderSubPart, NeighborConstraintBreaks, \
    NeighborRandomAndNeighborGraph, BasicConstraintBuilder, NeighborBuilderTimeWindow, RCPSPModel, ANY_RCPSP, \
    build_graph_rcpsp_object, ConstraintHandler, ConstraintHandlerScheduling, GraphRCPSP, NeighborBuilderMix, \
    ANY_SOLUTION_MSRCPSP, ANY_MSRCPSP, MS_RCPSPModel, MS_RCPSPModel_Variant, ObjectiveSubproblem, \
...
def return_random_basic_constraint_handler(rcpsp_model: ANY_RCPSP,
                                           graph=None,
                                           fraction_to_fix=0.9,
                                           minus_delta=100,
                                           plus_delta=100,
                                           minus_delta_2=4000,
                                           plus_delta_2=4000):
    return BasicConstraintBuilder(params_constraint_builder=ParamsConstraintBuilder(minus_delta_primary=minus_delta,
                                                                                    plus_delta_primary=plus_delta,
                                                                                    minus_delta_secondary=minus_delta_2,
                                                                                    plus_delta_secondary=plus_delta_2),
                                  neighbor_builder=NeighborRandom(problem=rcpsp_model,
                                                                  graph=graph,
                                                                  fraction_subproblem=fraction_to_fix))
...
def build_neighbor_random(option_neighbor: OptionNeighborRandom, rcpsp_model: ANY_RCPSP) -> ConstraintHandler:
    graph = build_graph_rcpsp_object(rcpsp_problem=rcpsp_model)
    params_use_case = [ConstraintHandlerScheduling(problem=rcpsp_model,
                                                   basic_constraint_builder=
                                          BasicConstraintBuilder(params_constraint_builder=
                                                                 ParamsConstraintBuilder(minus_delta_primary=100,
                                                                                         plus_delta_primary=100,
                                                                                         minus_delta_secondary=4000,
                                                                                         plus_delta_secondary=4000),
                                                                 neighbor_builder=NeighborRandom(problem=rcpsp_model,
                                                                                                 graph=graph,
                                                                                                 fraction_subproblem=0.75)),
                                                   params_list=[
                                              ParamsConstraintBuilder(minus_delta_primary=100,
                                                                      plus_delta_primary=100,
                                                                      minus_delta_secondary=4000,
    ...
    params_all = [ConstraintHandlerScheduling(problem=rcpsp_model,
                                              basic_constraint_builder=return_random_basic_constraint_handler(rcpsp_model, graph,
                                                                                                     fraction_to_fix=p.fraction_to_fix,
                                                                                                     minus_delta=p.minus_delta,
                                                                                                     plus_delta=p.plus_delta,
                                                                                                     minus_delta_2=4000,
                                                                                                     plus_delta_2=4000),
                                              params_list=[ParamsConstraintBuilder(p.minus_delta, p.plus_delta, 4000, 4000,
    ...
    params_fast = [ConstraintHandlerScheduling(problem=rcpsp_model,
                                               basic_constraint_builder=return_random_basic_constraint_handler(rcpsp_model, graph,
                                                                                                      fraction_to_fix=p.fraction_to_fix,
                                                                                                      minus_delta=p.minus_delta,
                                                                                                      plus_delta=p.plus_delta,
    ...
    params_debug = [ConstraintHandlerScheduling(problem=rcpsp_model,
                                                basic_constraint_builder=
                                       return_random_basic_constraint_handler(rcpsp_model, graph,
                                                                              fraction_to_fix=p.fraction_to_fix,
                                                                              minus_delta=p.minus_delta,
                                                                              plus_delta=p.plus_delta,
                                                                              minus_delta_2=4000,
                                                ...
                                                params_list=[ParamsConstraintBuilder(p.minus_delta, p.plus_delta, 4000, 4000,
                                                                            constraint_max_time_to_current_solution=False)],
                                                use_makespan_of_subtasks=False)
                    for p in [Params(fraction_to_fix=1.,
                                     minus_delta=0,
                                     plus_delta=0)]
                    ]
    params_large = [ConstraintHandlerScheduling(problem=rcpsp_model,
                                                basic_constraint_builder=
                                       return_random_basic_constraint_handler(rcpsp_model, graph,
                                                                              fraction_to_fix=p.fraction_to_fix,
                                                                              minus_delta=p.minus_delta,
                                                                              ...
                                                                              minus_delta_2=4000,
                                                                              plus_delta_2=4000),
                                                params_list=[ParamsConstraintBuilder(minus_delta_primary=p.minus_delta,
    ...
    params_no_constraint = [ConstraintHandlerScheduling(problem=rcpsp_model,
                                                        basic_constraint_builder=
                                               return_random_basic_constraint_handler(rcpsp_model, graph,
                                                                                      fraction_to_fix=p.fraction_to_fix,
                                                                                      minus_delta=p.minus_delta,
                                                                                      ...
                                                                                      plus_delta_2=10000),
                                                        params_list=[
                                                   ParamsConstraintBuilder(p.minus_delta, p.plus_delta, 4000, 4000,
                                                                           constraint_max_time_to_current_solution=False)],
                                                        use_makespan_of_subtasks=False)
                            for p in [Params(0., 4000, 4000)]]
...
def build_constraint_handler_cut_part(rcpsp_problem: ANY_RCPSP, graph=None, **kwargs):
    n1 = NeighborBuilderSubPart(problem=rcpsp_problem,
                                graph=graph,
                                nb_cut_part=
                                kwargs.get("nb_cut_part", 10))
    basic_constraint_builder = \
        BasicConstraintBuilder(neighbor_builder=n1,
    ...
    params_list = kwargs.get("params_list", [
        ...
        ParamsConstraintBuilder(minus_delta_primary=6000,
                                ...
                                constraint_max_time_to_current_solution=False)])
    constraint_handler = ConstraintHandlerScheduling(problem=rcpsp_problem,
                                                     basic_constraint_builder=basic_constraint_builder,
                                                     params_list=params_list,
                                                     use_makespan_of_subtasks=kwargs.get("use_makespan_of_subtasks",
                                                                                         False),
                                                     objective_subproblem=kwargs.get("objective_subproblem", ObjectiveSubproblem.GLOBAL_MAKESPAN),
                                                     verbose=kwargs.get("verbose", False))
    return constraint_handler


def build_basic_random_constraint_handler(rcpsp_problem: ANY_RCPSP, graph, **kwargs):
    return ConstraintHandlerScheduling(problem=rcpsp_problem,
                                       basic_constraint_builder=
                                       BasicConstraintBuilder(
                                           neighbor_builder=
                                           NeighborRandom(problem=rcpsp_problem,
                                                          graph=graph,
                                                          fraction_subproblem=kwargs.get("fraction_subproblem", 0.1),
                                                          delta_abs_time_from_makespan_to_not_fix=kwargs.get("delta_abs_time_from_makespan_to_not_fix", 5),
                                                          delta_rel_time_from_makespan_to_not_fix=kwargs.get("delta_rel_time_from_makespan_to_not_fix", 0.1)),
                                           multiskill=kwargs.get("multiskill", False),
                                           preemptive=kwargs.get("preemptive", False)),
                                       params_list=kwargs.get("params_list", [
                                           ParamsConstraintBuilder(minus_delta_primary=100,
                                       ...
                                       objective_subproblem=kwargs.get("objective_subproblem",
                                                                       ObjectiveSubproblem.GLOBAL_MAKESPAN))


def build_basic_time_window_constraint_handler(rcpsp_problem: ANY_RCPSP, graph, **kwargs):
    return ConstraintHandlerScheduling(problem=rcpsp_problem,
                                       basic_constraint_builder=
                                       BasicConstraintBuilder(
                                           neighbor_builder=
                                           NeighborBuilderTimeWindow(problem=rcpsp_problem,
                                                                     graph=graph,
...
def build_basic_random_and_neighbor(rcpsp_problem: ANY_RCPSP, graph, **kwargs):
    return ConstraintHandlerScheduling(problem=rcpsp_problem,
                                       ...
                                       BasicConstraintBuilder(
                                           neighbor_builder=
                                           NeighborRandomAndNeighborGraph(problem=rcpsp_problem,
                                                                          graph=graph,
                                                                          fraction_subproblem=kwargs.get("fraction_subproblem",
                                                                                                         0.1)),
                                           multiskill=kwargs.get("multiskill", False),
                                           preemptive=kwargs.get("preemptive", False)),
                                       params_list=kwargs.get("params_list", [
...
def build_neighbor_mixing_methods(rcpsp_model: ANY_RCPSP,
                                  graph: GraphRCPSP = None, **kwargs):
    if graph is None:
        graph = build_graph_rcpsp_object(rcpsp_problem=rcpsp_model)
    n1 = NeighborBuilderSubPart(problem=rcpsp_model,
                                graph=graph,
                                nb_cut_part=kwargs.get("cut_part", 10))  # 2
    n2 = NeighborRandomAndNeighborGraph(problem=rcpsp_model,
                                        graph=graph,
                                        fraction_subproblem=kwargs.get("fraction_subproblem", 0.05))
    n3 = NeighborConstraintBreaks(problem=rcpsp_model,
                                  graph=graph,
    ...
    basic_constraint_builder = BasicConstraintBuilder(params_constraint_builder=
                                                      ...
                                                      multiskill=isinstance(rcpsp_model,
                                                                            (MS_RCPSPModel_Variant, MS_RCPSPModel)))
    params_list = kwargs.get("params_list", [# ParamsConstraintBuilder(plus_delta=6000,
                                             #                         minus_delta=6000,
                                             #                         plus_delta_2=400,
                                             #                         minus_delta_2=400,
                                             #                         constraint_max_time=False),
        ParamsConstraintBuilder(minus_delta_primary=6000, plus_delta_primary=6000, minus_delta_secondary=10,
                                plus_delta_secondary=10, constraint_max_time_to_current_solution=False)])
    constraint_handler = ConstraintHandlerScheduling(problem=rcpsp_model,
                                                     basic_constraint_builder=basic_constraint_builder,
                                                     params_list=params_list)
    return constraint_handler


def build_neighbor_mixing_cut_parts(rcpsp_model: ANY_RCPSP,
                                    graph: GraphRCPSP = None, **kwargs):
    if graph is None:
        graph = build_graph_rcpsp_object(rcpsp_problem=rcpsp_model)

    n1_s = [NeighborBuilderSubPart(problem=rcpsp_model,
                                   graph=graph,
                                   nb_cut_part=c_part)
            for c_part in [4, 5, 6, 7]]  # 2
    n2_s = [NeighborRandomAndNeighborGraph(problem=rcpsp_model,
                                           graph=graph,
                                           fraction_subproblem=fs)
            for fs in [0.3, 0.4, 0.5, 0.6]]
    n2_s = []
    n_mix = NeighborBuilderMix(list_neighbor=n1_s +n2_s,
...
def mix_both(rcpsp_model: ANY_RCPSP,
             option_neighbor_random: OptionNeighborRandom,
             graph: GraphRCPSP = None, **kwargs):
    a_s = [build_neighbor_random(option_neighbor=option_neighbor_random,
    ...
    a_s = [build_neighbor_random(option_neighbor=option_neighbor_random, rcpsp_model=rcpsp_model)]
    b = build_neighbor_mixing_methods(rcpsp_model, graph=graph, **kwargs)
    list_c = a_s + [b]
    return ConstraintHandlerMix(problem=rcpsp_model,
...
def random_neigh(rcpsp_model, fraction_subproblem: float=0.35, **kwargs):
    return build_basic_random_constraint_handler(
        ...
        graph=build_graph_rcpsp_object(rcpsp_problem=rcpsp_model),
        params_list=
        kwargs.get("params_list",
                   [ParamsConstraintBuilder(minus_delta_primary=10,
...
def time_window_neigh(rcpsp_model, **kwargs):
    return build_basic_time_window_constraint_handler(rcpsp_problem=rcpsp_model,
        graph=build_graph_rcpsp_object(rcpsp_problem=rcpsp_model),
        params_list=
        kwargs.get("params_list",
                   [ParamsConstraintBuilder(minus_delta_primary=10,
...
def constraint_neigh(rcpsp_model,
                     fraction_subproblem: float = 0.35, **kwargs):
    gr = build_graph_rcpsp_object(rcpsp_problem=rcpsp_model)
    n1 = NeighborBuilderSubPart(problem=rcpsp_model,
                                graph=build_graph_rcpsp_object(rcpsp_problem=rcpsp_model),
                                nb_cut_part=kwargs.get("nb_cut_part", 3))
    n2 = NeighborConstraintBreaks(problem=rcpsp_model,
                                  graph=gr,
                                  fraction_subproblem=fraction_subproblem,  # 0.25
                                  other_constraint_handler=n1)
    return ConstraintHandlerScheduling(problem=rcpsp_model,
                                       basic_constraint_builder=
                                       BasicConstraintBuilder(
                                           neighbor_builder=n2,
                                           preemptive=kwargs.get("preemptive", rcpsp_model.is_preemptive()),
...
def mix(rcpsp_model,
    ...
    return ConstraintHandlerMix(problem=rcpsp_model,
                                ...
                                list_proba=[0.5, 0.5],
                                update_proba=True,
                                tag_constraint_handler=["cut_parts",
                                                        "random"])


def mix_lot(rcpsp_model,
            nb_cut_parts,
            fraction_subproblems,
            **kwargs):
    c1 = [cut_parts(rcpsp_model,
                    nb_cut_part,
                    **kwargs) for nb_cut_part in nb_cut_parts]
    c2 = [random_neigh(rcpsp_model,
                       fraction_subproblem=fraction_subproblem,
                       **kwargs)
    ...
    if "generalized_precedence_constraint" in kwargs and kwargs["generalized_precedence_constraint"]:
        c3 = [constraint_neigh(rcpsp_model,
                               fraction_subproblem=0.25,
                               **kwargs)]
    ...
    if "time_windows" in kwargs and kwargs["time_windows"]:
        ...
        tags += ["time_windows"]
    from discrete_optimization.generic_rcpsp_tools.neighbor_tools_rcpsp import EquilibrateMultiskillAllocation, \
        EquilibrateMultiskillAllocationNonPreemptive
    c5 = []
    if kwargs.get("equilibrate_multiskill", False):
        if rcpsp_model.is_preemptive():
            c5 = [EquilibrateMultiskillAllocation(problem=rcpsp_model)
                  for i in range(max(1, int((len(c1)+len(c2)+len(c3))/2)))]
            tags += ["equilibrate_"+str(i) for i in range(len(c5))]
        else:
            c5 = [EquilibrateMultiskillAllocationNonPreemptive(problem=rcpsp_model)
                  for i in range(max(1, int((len(c1) + len(c2) + len(c3)) / 2)))]
            tags += ["equilibrate_" + str(i) for i in range(len(c5))]
    if kwargs.get("equilibrate_multiskill_v2", False):
        c5 = [ConstraintHandlerMultiskillAllocation(problem=rcpsp_model)
              for i in range(max(1, int((len(c1) + len(c2) + len(c3)) / 2)))]
        tags += ["equilibrate_" + str(i) for i in range(len(c5))]

    return ConstraintHandlerMix(problem=rcpsp_model,
                                list_constraints_handler=c1+c2+c3+c4+c5,
                                list_proba=[1/(len(c1)+len(c2)+len(c3)+len(c4)+len(c5))]*(len(c1)+len(c2)+len(c3)+len(c4)+len(c5)),
                                update_proba=False,
