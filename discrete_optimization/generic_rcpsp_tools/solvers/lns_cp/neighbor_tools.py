...
def constraints_strings(
    current_solution: Union[RcpspSolution, MultiskillRcpspSolution],
...
def constraints_strings_multiskill(
    current_solution: MultiskillRcpspSolution,
    cp_solver: ANY_CP_SOLVER,
    tasks_primary: set[Hashable],
    tasks_secondary: set[Hashable],
    params_constraints: ParamsConstraintBuilder,
...
def constraint_number_of_change_in_worker_allocation(
    current_solution: MultiskillRcpspSolution, nb_moves: int
):
    ref_unit_used = [
...
def constraint_number_of_change_in_worker_allocation_preemptive(
    current_solution: PreemptiveMultiskillRcpspSolution,
    nb_preemptive: int,
    nb_moves: int,
):
    ref_unit_used = [
...
def constraints_exchange_worker(
    current_solution: MultiskillRcpspSolution, nb_moves: int, cp_solver: ANY_CP_SOLVER
):
    ref_unit_used = [
        [False for i in range(current_solution.problem.nb_tasks)]
        for j in range(current_solution.problem.nb_employees)
...
def constraints_start_on_end(
    current_solution: MultiskillRcpspSolution, cp_solver: ANY_CP_SOLVER, frac=0.5
):
    end_times = {
    ...
    for i1, i2 in selected:
        if i1 not in selected_task or i2 not in selected_task:
            continue
        if isinstance(cp_solver, CpMspspMznMultiskillRcpspSolver):
            st = (
                "constraint start["
...
def constraint_unit_used_to_tasks(
    ...
    current_solution: MultiskillRcpspSolution,
    cp_solver: Union[CpMultiskillRcpspSolver],
):
...
def constraint_unit_used_to_tasks_preemptive(
    tasks_set: set[Hashable],
    current_solution: PreemptiveMultiskillRcpspSolution,
    cp_solver: Union[CpPreemptiveMultiskillRcpspSolver],
    exceptions=None,
):
...
class NeighborBuilderSubPart(NeighborBuilder):
    ...
    def __init__(
        self, problem: RcpspProblem, graph: GraphRcpsp = None, nb_cut_part: int = 10
    ):
    ...
    def find_subtasks(
        ...
        while len_subtask < self.nb_jobs_subproblem:
            ...
            for k in set(task_intersect):
                ...
                if isinstance(self.graph, GraphSpecialConstraintsRcpsp):
...
class NeighborConstraintBreaks(NeighborBuilder):
    def __init__(
        self,
        problem: Union[SpecialConstraintsPreemptiveRcpspProblem, RcpspProblem],
        graph: GraphRcpsp = None,
        fraction_subproblem: float = 0.1,
