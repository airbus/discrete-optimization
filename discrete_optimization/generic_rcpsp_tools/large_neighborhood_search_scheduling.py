...
from discrete_optimization.generic_rcpsp_tools.neighbor_builder import build_neighbor_random, OptionNeighborRandom
...
from discrete_optimization.rcpsp_multiskill.solvers.lns_post_process_rcpsp import PostProMSRCPSP, PostProMSRCPSPPreemptive
from discrete_optimization.generic_rcpsp_tools.neighbor_tools_rcpsp import ParamsConstraintBuilder, \
    BasicConstraintBuilder, NeighborBuilderMix, NeighborConstraintBreaks, NeighborRandomAndNeighborGraph, \
    NeighborBuilderSubPart, ConstraintHandlerScheduling, ANY_RCPSP
...
from discrete_optimization.rcpsp.solver.rcpsp_lp_lns_solver import InitialMethodRCPSP, InitialSolutionRCPSP
...
from discrete_optimization.rcpsp_multiskill.solvers.cp_solvers import CP_MS_MRCPSP_MZN, CP_MS_MRCPSP_MZN_PREEMPTIVE
...
def build_default_cp_model(rcpsp_problem: ANY_RCPSP, partial_solution=None, **kwargs):
    if rcpsp_problem.is_preemptive():
        if rcpsp_problem.is_multiskill():
            solver = CP_MS_MRCPSP_MZN_PREEMPTIVE(rcpsp_model=rcpsp_problem,
            ...
            solver.init_model(output_type=True,
                              ...
                              possibly_preemptive=[rcpsp_problem.preemptive_indicator[t]
                              ...
                              add_partial_solution_hard_constraint=kwargs.get("add_partial_solution_hard_constraint",
                                                                              False),
                              **{x: kwargs[x] for x in kwargs if x not in ["nb_preemptive",
                                                                           "max_preempted",
                                                                           "fake_tasks",
    ...
    if rcpsp_problem.is_preemptive():
        if not rcpsp_problem.is_multiskill():
            if rcpsp_problem.is_rcpsp_multimode():
                solver = CP_MRCPSP_MZN_PREEMMPTIVE(rcpsp_model=rcpsp_problem,
                                                   cp_solver_name=kwargs.get("cp_solver_name", CPSolverName.CHUFFED))
                solver.init_model(output_type=True,
                                  ...
                                  partial_solution=partial_solution,
                                  possibly_preemptive=[rcpsp_problem.preemptive_indicator[t] for t in
                                                       rcpsp_problem.tasks_list],
                                  add_partial_solution_hard_constraint=
                                  kwargs.get("add_partial_solution_hard_constraint", False),
                                  **{x: kwargs[x] for x in kwargs if x not in ["nb_preemptive",
                                                                               "max_preempted",
                                                                               "fake_tasks",
                                                                               "add_partial_solution_hard_constraint"]})
            ...
            if not rcpsp_problem.is_rcpsp_multimode():
                solver = CP_RCPSP_MZN_PREEMMPTIVE(rcpsp_model=rcpsp_problem,
                                                  cp_solver_name=kwargs.get("cp_solver_name", CPSolverName.CHUFFED))
                solver.init_model(output_type=True, model_type="single-preemptive",
                                  nb_preemptive=kwargs.get("nb_preemptive", 13),
                                  max_preempted=kwargs.get("max_preempted", 100),
                                  fake_tasks=kwargs.get("fake_tasks", True),
                                  partial_solution=partial_solution,
    ...
    if not rcpsp_problem.is_preemptive():
        if rcpsp_problem.is_multiskill():
            solver = CP_MS_MRCPSP_MZN(rcpsp_model=rcpsp_problem,
                                      cp_solver_name=kwargs.get("cp_solver_name", CPSolverName.CHUFFED),
                                      one_ressource_per_task=kwargs.get("one_ressource_per_task",
                                                                        rcpsp_problem.one_unit_per_task_max))
            solver.init_model(output_type=True,
                              exact_skills_need=False,
                              partial_solution=partial_solution,
                              add_partial_solution_hard_constraint=kwargs.get("add_partial_solution_hard_constraint",
                                                                              False),
                              **{x: kwargs[x] for x in kwargs if x not in ["add_partial_solution_hard_constraint"]})
            return solver
        if not rcpsp_problem.is_multiskill():
            if rcpsp_problem.is_rcpsp_multimode():
                solver = CP_MRCPSP_MZN(rcpsp_model=rcpsp_problem,
                                       cp_solver_name=kwargs.get("cp_solver_name", CPSolverName.CHUFFED))
                solver.init_model(output_type=True,
                                  partial_solution=partial_solution,
                                  add_partial_solution_hard_constraint=
                ...
                return solver
            if not rcpsp_problem.is_rcpsp_multimode():
                solver = CP_RCPSP_MZN(rcpsp_model=rcpsp_problem,
                                      cp_solver_name=kwargs.get("cp_solver_name", CPSolverName.CHUFFED))
                solver.init_model(output_type=True,
                                  partial_solution=partial_solution,
                                  add_partial_solution_hard_constraint=
...
def build_default_postpro(rcpsp_problem: ANY_RCPSP, partial_solution=None, **kwargs):
    if not rcpsp_problem.is_multiskill():
        if rcpsp_problem.is_preemptive():
            from discrete_optimization.rcpsp.solver.cp_lns_methods_preemptive import PostProLeftShift
            post_process_solution = PostProLeftShift(problem=rcpsp_problem,
        ...
        if not rcpsp_problem.is_preemptive():
            ...
            post_process_solution = rcpsp_lns.PostProcessLeftShift(rcpsp_problem=rcpsp_problem,
                                                                   partial_solution=partial_solution)
            return post_process_solution
    if rcpsp_problem.is_multiskill():
        if rcpsp_problem.is_preemptive():
            ...
            post_process_solution = PostProMSRCPSP(problem=rcpsp_problem,
                                                   params_objective_function=None)
            return post_process_solution
...
def build_default_initial_solution(rcpsp_problem: ANY_RCPSP, **kwargs):
    if rcpsp_problem.is_multiskill():
        ...
        initial_solution_provider = InitialSolutionFromSolver(
            LS_RCPSP_Solver(model=rcpsp_problem,
...
def build_constraint_handler(rcpsp_problem: ANY_RCPSP, graph, **kwargs):
    constraint_handler_type = kwargs.get("constraint_handler_type", ConstraintHandlerType.MIX_SUBPROBLEMS)
    ...
    if constraint_handler_type == ConstraintHandlerType.MIX_SUBPROBLEMS:
        ...
        n2 = NeighborRandomAndNeighborGraph(problem=rcpsp_problem,
                                            graph=graph,
                                            fraction_subproblem=
                                            kwargs.get("fraction_subproblem", 0.05))
        n3 = NeighborConstraintBreaks(problem=rcpsp_problem,
                                      graph=graph,
                                      fraction_subproblem=
                                      kwargs.get("fraction_subproblem", 0.05),
                                      other_constraint_handler=n1)
        n_mix = NeighborBuilderMix(list_neighbor=[n1, n2, n3],
                                   weight_neighbor=[0.2, 0.5, 0.3])
        basic_constraint_builder = \
            BasicConstraintBuilder(neighbor_builder=n_mix,
        ...
        params_list = kwargs.get("params_list", [
            ParamsConstraintBuilder(minus_delta_primary=6000,
                                    ...
                                    plus_delta_secondary=400,
                                    constraint_max_time_to_current_solution=False),
            ParamsConstraintBuilder(minus_delta_primary=5000,
                                    plus_delta_primary=5000,
                                    minus_delta_secondary=2,
                                    plus_delta_secondary=2,
                                    constraint_max_time_to_current_solution=False)])
        constraint_handler = NeighborRepairProblems(problem=rcpsp_problem,
                                                    params_list=params_list)
...
def build_constraint_handler_helper(rcpsp_problem: ANY_RCPSP, graph, **kwargs):
    ...
    constraint_handler = None
    if option == 0:
        constraint_handler = build_neighbor_random(option_neighbor=kwargs.get("option_neighbor_random",
                                                                              OptionNeighborRandom.MIX_ALL),
                                                   rcpsp_model=rcpsp_problem)
    if option == 1:
        from discrete_optimization.generic_rcpsp_tools.neighbor_builder import build_neighbor_mixing_methods, \
            mix_both
        constraint_handler = mix_both(rcpsp_model=rcpsp_problem,
                                      option_neighbor_random=kwargs.get("option_neighbor_random",
                                                                        OptionNeighborRandom.MIX_ALL),
                                      graph=graph,
    ...
    if option == 2:
        from discrete_optimization.generic_rcpsp_tools.neighbor_builder import build_neighbor_mixing_methods
        constraint_handler = build_neighbor_mixing_methods(rcpsp_model=rcpsp_problem,
                                                           graph=graph,
                                                           fraction_subproblem=kwargs.get("fraction_subproblem",
                                                                                          0.05),
                                                           cut_part=kwargs.get("cut_part", 10),
                                                           ...
                                                           kwargs.get("params_list",
                                                                      [ParamsConstraintBuilder(minus_delta_primary=6000,
                                                                       ...
                                                                       ParamsConstraintBuilder(minus_delta_primary=6000,
                                                                                               ...
                                                                                               constraint_max_time_to_current_solution=False)]))
    if option == 3:
        from discrete_optimization.generic_rcpsp_tools.neighbor_builder import build_neighbor_mixing_cut_parts
        constraint_handler = build_neighbor_mixing_cut_parts(rcpsp_model=rcpsp_problem,
                                                             graph=graph,
                                                             params_list=
                                                             kwargs.get("params_list",
...
class LargeNeighborhoodSearchScheduling(SolverDO):
    def __init__(self,
        ...
        if solver is None:
            solver = build_default_cp_model(rcpsp_problem=rcpsp_problem,
                                            partial_solution=partial_solution,
                                            **kwargs)
        self.cp_solver = solver
        params_objective_function = get_default_objective_setup(problem=self.rcpsp_problem)
        ...
        if self.constraint_handler is None:
            self.constraint_handler = build_constraint_handler(rcpsp_problem=self.rcpsp_problem,
                                                               graph=graph,
        ...
        if self.post_pro is None:
            self.post_pro = build_default_postpro(rcpsp_problem=self.rcpsp_problem, partial_solution=partial_solution,
                                                  **kwargs)
        self.initial_solution_provider = kwargs.get("initial_solution_provider", None)
        if self.initial_solution_provider is None:
            self.initial_solution_provider = build_default_initial_solution(rcpsp_problem=self.rcpsp_problem,
                                                                            **kwargs)
        self.lns_solver = LNS_CP(problem=self.rcpsp_problem,
    ...
    def solve(self,
              ...
              **args) -> ResultStorage:
        return self.lns_solver.solve_lns(parameters_cp=parameters_cp,
