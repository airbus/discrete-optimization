...
from discrete_optimization.rcpsp_multiskill.rcpsp_multiskill import MS_RCPSPModel, MS_RCPSPModel_Variant, \
    MS_RCPSPSolution, MS_RCPSPSolution_Preemptive, MS_RCPSPSolution_Variant, MS_RCPSPSolution_Preemptive_Variant, \
    employee_usage
from discrete_optimization.generic_rcpsp_tools.graph_tools_rcpsp import GraphRCPSP, \
...
import random
import numpy as np
from typing import Set, Hashable, Union
from discrete_optimization.generic_tools.cp_tools import SignEnum, CPSolver
...
from discrete_optimization.rcpsp.solver.cp_solvers import \
    CP_RCPSP_MZN_PREEMMPTIVE, CP_MRCPSP_MZN_PREEMMPTIVE, CP_RCPSP_MZN, CP_MRCPSP_MZN
from discrete_optimization.rcpsp_multiskill.solvers.cp_solver_mspsp_instlib import CP_MSPSP_MZN
from discrete_optimization.rcpsp_multiskill.solvers.cp_solvers import CP_MS_MRCPSP_MZN, \
    CP_MS_MRCPSP_MZN_PREEMPTIVE, CP_MS_MRCPSP_MZN_PARTIAL_PREEMPTIVE
...
ANY_MSRCPSP = Union[MS_RCPSPModel_Variant,
                    MS_RCPSPModel]
ANY_SOLUTION = Union[RCPSPSolutionPreemptive,
                     RCPSPSolution,
                     MS_RCPSPSolution,
                     MS_RCPSPSolution_Variant,
                     MS_RCPSPSolution_Preemptive,
                     MS_RCPSPSolution_Preemptive_Variant]
ANY_SOLUTION_UNPREEMPTIVE = Union[RCPSPSolution, MS_RCPSPSolution, MS_RCPSPSolution_Variant]
...
ANY_SOLUTION_MSRCPSP = Union[MS_RCPSPSolution,
                             MS_RCPSPSolution_Variant,
                             MS_RCPSPSolution_Preemptive,
                             MS_RCPSPSolution_Preemptive_Variant]
ANY_CP_SOLVER = Union[CP_RCPSP_MZN_PREEMMPTIVE,
                      CP_RCPSP_MZN,
                      CP_MRCPSP_MZN,
...
class ParamsConstraintBuilder:
    def __init__(self,
        ...
        self.constraint_max_time_to_current_solution = constraint_max_time_to_current_solution
        self.fraction_of_task_assigned_multiskill = fraction_of_task_assigned_multiskill
        self.except_assigned_multiskill_primary_set = except_assigned_multiskill_primary_set
    ...
    @staticmethod
    def default():
        return ParamsConstraintBuilder(minus_delta_primary=100, plus_delta_primary=100,
                                       minus_delta_secondary=0, plus_delta_secondary=0,
                                       constraint_max_time_to_current_solution=False,
                                       fraction_of_task_assigned_multiskill=0.6)


def constraints_strings(current_solution: Union[RCPSPSolution, MS_RCPSPSolution],
    ...
    for job in tasks_primary:
        ...
        string_start_1 = cp_solver \
            .constraint_start_time_string(task=job,
                                          start_time=max(0,
                                                         start_time_j -
                                                         params_constraints.minus_delta_primary),
                                          sign=SignEnum.UEQ)
        string_start_2 = cp_solver \
            .constraint_start_time_string(task=job,
                                          start_time=min(max_time,
                                                         start_time_j +
                                                         params_constraints.plus_delta_primary)
                                          if
                                          params_constraints.constraint_max_time_to_current_solution
                                          else start_time_j + params_constraints.plus_delta_primary,
                                          sign=SignEnum.LEQ)
        list_strings += [string_start_1,
                         string_start_2]
    for job in tasks_secondary:
        if job in tasks_primary:
            continue
        start_time_j = current_solution.get_start_time(job)
        if params_constraints.minus_delta_secondary == 0 and params_constraints.plus_delta_secondary == 0:
            string_start = cp_solver.constraint_start_time_string(task=job,
            ...
            string_start_1 = cp_solver\
                .constraint_start_time_string(task=job,
                                              start_time=max(0,
                                                             start_time_j - params_constraints.minus_delta_secondary),
                                              sign=SignEnum.UEQ)
            string_start_2 = cp_solver \
                .constraint_start_time_string(task=job,
                                              start_time=min(max_time,
            ...
            list_strings += [string_start_1,
                             string_start_2]
    if params_constraints.constraint_max_time_to_current_solution:
        string = cp_solver.constraint_start_time_string(current_solution.problem.sink_task,
                                                        start_time=max_time,
                                                        sign=SignEnum.LEQ)
        list_strings += [string]
    return list_strings
...
def constraints_strings_preemptive(current_solution: ANY_SOLUTION_PREEMPTIVE,
                                   cp_solver: ANY_CP_SOLVER,
                                   tasks_primary: Set[Hashable],
                                   tasks_secondary: Set[Hashable],
                                   params_constraints: ParamsConstraintBuilder):
    ...
    for job in tasks_primary:
        ...
        for j in range(len(list_starts)):
            ...
            string1_start = cp_solver \
                .constraint_start_time_string_preemptive_i(task=job,
                                                           start_time=max(0,
                                                                          start_time_j
                                                                          - params_constraints.minus_delta_primary),
                                                           sign=SignEnum.UEQ,
                                                           part_id=j + 1)
            string2_start = cp_solver \
                .constraint_start_time_string_preemptive_i(task=job,
                                                           start_time=min(max_time,
                                                                          start_time_j
                                                                          + params_constraints.plus_delta_primary)
            ...
            string1_dur = cp_solver.constraint_duration_string_preemptive_i(task=job,
                                                                            duration=max(duration_j -
                                                                                         params_constraints
                                                                                         .minus_delta_primary_duration,
                                                                                         0),
                                                                            sign=SignEnum.UEQ,
                                                                            part_id=j + 1)
    ...
    for job in tasks_secondary:
        ...
        for j in range(len(list_starts)):
            ...
            end_time_j = list_ends[j]
            duration_j = end_time_j - start_time_j
            if params_constraints.minus_delta_secondary == 0 and \
                    params_constraints.plus_delta_secondary == 0:
                ...
                string1_dur = cp_solver.constraint_duration_string_preemptive_i(task=job,
                                                                                duration=max(duration_j -
                                                                                             params_constraints.minus_delta_secondary_duration,
                                                                                             0),
                                                                                sign=SignEnum.UEQ,
                                                                                part_id=j + 1)
                string2_dur = cp_solver.constraint_duration_string_preemptive_i(task=job,
...
def constraints_strings_multiskill(current_solution: MS_RCPSPSolution,
                                   cp_solver: ANY_CP_SOLVER,
                                   tasks_primary: Set[Hashable],
                                   tasks_secondary: Set[Hashable],
                                   params_constraints: ParamsConstraintBuilder):
    list_strings = constraints_strings(current_solution=current_solution,
    ...
    if params_constraints.first_method_multiskill:
        if True:
            if not params_constraints.except_assigned_multiskill_primary_set:
                list_strings += constraint_unit_used_to_tasks(tasks_set= # tasks_secondary,
                                                              set(random.sample(current_solution.problem.tasks_list,
                                                                  min(int(params_constraints.
                                                                          fraction_of_task_assigned_multiskill
        ...
        if params_constraints.additional_methods:
            # those are WIP
            list_strings += constraints_start_on_end(current_solution,
                                                     cp_solver=cp_solver,
                                                     frac=0.25)
            ...
            nb_usage_per_employee = [sum([e in current_solution.employee_usage[t] for t in current_solution.employee_usage])
            ...
            for i in range(len(nb_usage_per_employee)):
                list_strings += ["constraint sum(i in Act)(unit_used["+str(i+1)+",i])<="+str(nb_usage_per_employee[i]+3)+";\n"]
                list_strings += ["constraint sum(i in Act)(unit_used["+str(i+1)+",i])>="+str(nb_usage_per_employee[i]-3)+";\n"]
        ...
        list_strings += constraint_unit_used_subset_employees(employees_set=set_employees_to_fix,
                                                              current_solution=current_solution,
                                                              cp_solver=cp_solver,
...
def constraint_number_of_change_in_worker_allocation(current_solution: MS_RCPSPSolution, nb_moves: int):
    ref_unit_used = [[False for i in range(current_solution.problem.nb_tasks)]
    ...
    for i in range(len(current_solution.problem.tasks_list)):
        ...
        for j in range(current_solution.problem.nb_employees):
            if current_solution.problem.employees_list[j] in current_solution.employee_usage.get(t, {}):
                ref_unit_used[j][i] = True
    stringit = "[" + ",".join(['false' if ref_unit_used[j][i] else 'true'
                               for j in range(len(ref_unit_used)) for i in range(len(ref_unit_used[j]))]) + "]"
    s = """
        int: nb_move=""" + str(nb_moves) + """;\n
        ...
        array[Units, Act] of bool: ref_unit_used_not=array2d(Units, Act, """ + stringit + """);\n
...
def constraint_number_of_change_in_worker_allocation_preemptive(current_solution: MS_RCPSPSolution_Preemptive,
                                                                nb_preemptive: int,
                                                                nb_moves: int):
    ref_unit_used = [[[False for k in range(nb_preemptive)] for i in range(current_solution.problem.nb_tasks)]
                     for j in range(current_solution.problem.nb_employees)]
    for i in range(len(current_solution.problem.tasks_list)):
        t = current_solution.problem.tasks_list[i]
    ...
    stringit = "[" + ",".join(['false' if ref_unit_used[j][i][l] else 'true'
                               for j in range(len(ref_unit_used)) for i in range(len(ref_unit_used[j]))
    ...
    s = """
        ...
        array[Units, Tasks, PREEMPTIVE] of bool: ref_unit_used_not=array3d(Units, Tasks, PREEMPTIVE, """ + stringit + """);\n
...
def constraints_exchange_worker(current_solution: MS_RCPSPSolution,
                                nb_moves: int,
                                cp_solver: ANY_CP_SOLVER):
    ref_unit_used = [[False for i in range(current_solution.problem.nb_tasks)]
                     for j in range(current_solution.problem.nb_employees)]
    for i in range(len(current_solution.problem.tasks_list)):
        ...
        for j in range(current_solution.problem.nb_employees):
            if current_solution.problem.employees_list[j] in current_solution.employee_usage.get(t, {}):
                ref_unit_used[j][i] = True
    stringit = "["+",".join(['true' if ref_unit_used[j][i] else 'false'
                             for j in range(len(ref_unit_used)) for i in range(len(ref_unit_used[j]))])+"]"
    ...
    s = """
        ...
        array[Units, Act] of bool: ref_unit_used==array2d(Units, Act, """+stringit+""")\n;
...
def constraints_start_on_end(current_solution: MS_RCPSPSolution,
                             cp_solver: ANY_CP_SOLVER,
                             frac=0.5):
    end_times = {t: current_solution.get_end_time(t) for t in current_solution.problem.tasks_list}
    data = {}
    listed = []
    ...
    selected_task = random.sample(cp_solver.rcpsp_model.tasks_list,
                                  k=int(frac*cp_solver.rcpsp_model.nb_tasks))
    s = []
    cnt = 0
    for i1, i2 in selected:
        ...
        if isinstance(cp_solver, CP_MSPSP_MZN):
            st = "constraint start["+str(cp_solver.index_in_minizinc[i1]) + \
             "]==start["+str(cp_solver.index_in_minizinc[i2])+"]+dur["\
...
def constraints_start_on_end_preemptive(current_solution: ANY_SOLUTION_PREEMPTIVE,
                                        cp_solver: ANY_CP_SOLVER,
                                        frac=0.5):
    end_times = {t: current_solution.get_end_time(t) for t in current_solution.problem.tasks_list}
    data = {}
    ...
    for i1, i2 in selected:
        if i1 not in selected_task or i2 not in selected_task:
            continue
        st = "constraint s_preemptive["+str(cp_solver.index_in_minizinc[i1]) + \
             ", 1]==s_preemptive["+str(cp_solver.index_in_minizinc[i2])+",nb_preemptive];\n"
        s += [st]
        cnt += 1
    print(cnt, " constraint added start on end")
    return s
...
def constraints_strings_multiskill_preemptive(current_solution: ANY_SOLUTION,
                                              cp_solver: ANY_CP_SOLVER,
                                              tasks_primary: Set[Hashable],
                                              tasks_secondary: Set[Hashable],
                                              params_constraints: ParamsConstraintBuilder):
    list_strings = constraints_strings_preemptive(current_solution=current_solution,
                                                  tasks_primary=tasks_primary,
                                                  tasks_secondary=tasks_secondary,
                                                  cp_solver=cp_solver,
                                                  params_constraints=params_constraints)
    if random.random() < 0.99 and params_constraints.first_method_multiskill:
        from discrete_optimization.rcpsp_multiskill.rcpsp_multiskill import start_together_problem_description
        constraint_description = \
            start_together_problem_description(solution=current_solution,
        ...
        if params_constraints.additional_methods:
            # those are WIP
            list_strings += constraints_start_on_end_preemptive(current_solution,
                                                                cp_solver=cp_solver,
                                                                frac=0.3)
            list_strings += [constraint_number_of_change_in_worker_allocation_preemptive(current_solution=
                                                                                         current_solution,
                                                                                         nb_moves=4,
                                                                                         nb_preemptive=
                                                                                         cp_solver.nb_preemptive)]
...
def constraint_unit_used_to_tasks(tasks_set: Set[Hashable],
    ...
    for task in tasks_set:
        if task in current_solution.employee_usage:
            employees = set([e for e in current_solution.employee_usage[task]
                             if len(current_solution.employee_usage[task][e]) > 0])
            for e in all_employees:
                strings += cp_solver.constraint_used_employee(task=task,
                                                              employee=e,
                                                              indicator=e in employees)
        else:
            for e in all_employees:
                strings += cp_solver.constraint_used_employee(task=task,
                                                              employee=e,
...
def constraint_unit_used_subset_employees(employees_set: Set[Hashable],
                                          current_solution: MS_RCPSPSolution,
                                          cp_solver: Union[CP_MS_MRCPSP_MZN],
                                          employees_usage_dict=None):
    if employees_usage_dict is None:
        employee_usage_matrix, sum_usage, employees_usage_dict = \
            employee_usage(current_solution, current_solution.problem)
    strings = []
    for e in employees_set:
        ...
        for t in current_solution.problem.tasks_list:
            if (t, 0) not in employees_usage_dict[e]:
                strings += cp_solver.constraint_used_employee(task=t,
                                                              employee=e,
                                                              indicator=False)
    return strings


def constraint_unit_used_to_tasks_preemptive(tasks_set: Set[Hashable],
        ...
        if task in current_solution.employee_usage:
            ...
            for i in range(len_emp_usage):
                if (task, i) in exceptions:
                    continue
                employees = set([e for e in current_solution.employee_usage[task][i]
...
def constraint_unit_used_subset_employees_preemptive(employees_set: Set[Hashable],
                                                     current_solution: MS_RCPSPSolution_Preemptive,
                                                     cp_solver: Union[CP_MS_MRCPSP_MZN_PREEMPTIVE],
                                                     employees_usage_dict=None,
                                                     exceptions=None):
...
class NeighborBuilder:
    @abstractmethod
    def find_subtasks(self, current_solution: ANY_SOLUTION,
                      subtasks: Optional[Set[Hashable]] = None) -> Tuple[Set[Hashable], Set[Hashable]]:
        """
...
class NeighborBuilderSubPart(NeighborBuilder):
    ...
    def find_subtasks(self, current_solution: RCPSPSolution,
                      subtasks: Optional[Set[Hashable]] = None) -> Tuple[Set[Hashable], Set[Hashable]]:
        nb_job_sub = math.ceil(self.problem.n_jobs / self.nb_cut_part)
        task_of_interest = sorted(self.problem.tasks_list,
                                  key=lambda x: current_solution.get_end_time(x))
        task_of_interest = task_of_interest[self.current_sub_part * nb_job_sub:
                                            (self.current_sub_part + 1) * nb_job_sub]
        if subtasks is None:
...
class NeighborRandom(NeighborBuilder):
    def __init__(self, problem: ANY_RCPSP,
                 graph: GraphRCPSP = None,
                 fraction_subproblem: float = 0.9,
                 delta_abs_time_from_makespan_to_not_fix: int = 5,
                 delta_rel_time_from_makespan_to_not_fix: float = 0.):
        self.problem = problem
        self.graph = graph
        if self.graph is None:
            self.graph = build_graph_rcpsp_object(rcpsp_problem=problem)
        self.fraction_subproblem = fraction_subproblem
        self.delta_abs_time_from_makespan_to_not_fix = delta_abs_time_from_makespan_to_not_fix
        self.delta_rel_time_from_makespan_to_not_fix = delta_rel_time_from_makespan_to_not_fix
        self.set_tasks = set(self.problem.tasks_list)
    ...
    def find_subtasks(self, current_solution: ANY_SOLUTION,
        ...
        last_jobs = [x for x in self.problem.tasks_list
                     if max_time-self.delta_abs_time_from_makespan_to_not_fix <= current_solution.get_end_time(x)
                     <= max_time or
                     (1-self.delta_rel_time_from_makespan_to_not_fix)*max_time <= current_solution.get_end_time(x)
                     <= max_time]
        nb_jobs = self.problem.n_jobs
        tasks_subproblem = set(random.sample(self.problem.tasks_list,
                                             int(self.fraction_subproblem * nb_jobs)))
        for lj in last_jobs:
...
class NeighborRandomAndNeighborGraph(NeighborBuilder):
    def __init__(self, problem: ANY_RCPSP,
        ...
        self.fraction_subproblem = fraction_subproblem
        self.nb_jobs_subproblem = math.ceil(self.problem.n_jobs*self.fraction_subproblem)
        self.set_tasks = set(self.problem.tasks_list)
    ...
    def find_subtasks(self, current_solution: RCPSPSolution,
        ...
        while len_subtask < self.nb_jobs_subproblem:
            ...
            interval = (current_solution.get_start_time(random_pick),
                        current_solution.get_end_time(random_pick))
            task_intersect = [t for t in self.problem.tasks_list if
                              intersect(interval,
                                        (current_solution.get_start_time(t),
                                         current_solution.get_end_time(t))) is not None]
            for k in set(task_intersect):
                task_intersect += list(self.graph.get_pred_activities(k)) + \
                                  list(self.graph.get_next_activities(k))
...
class NeighborConstraintBreaks(NeighborBuilder):
    def __init__(self, problem: Union[RCPSPModelSpecialConstraintsPreemptive, RCPSPModelSpecialConstraints],
                 graph: GraphRCPSP = None,
                 fraction_subproblem: float = 0.1,
        ...
        if self.other is None:
            self.other = NeighborBuilderSubPart(problem=self.problem,
                                                graph=self.graph,
    ...
    def find_subtasks(self, current_solution: RCPSPSolution,
        ...
        if "special_constraints" in self.problem.__dict__.keys():
            details_constraints = compute_constraints_details(solution=current_solution,
                                                              constraints=self.problem.special_constraints)
            ...
            if len(sorted_constraints) == 1:
                ...
                st1, end1 = current_solution.get_start_time(t1), current_solution.get_end_time(t1)
                st2, end2 = current_solution.get_start_time(t2), current_solution.get_end_time(t2)
                st_min = min(st1, st2)
                st_max = max(end1, end2)
                tasks = [t for t in self.problem.tasks_list
                         if st_min-5 <= current_solution.get_start_time(t)
                         <= st_max + 5]
                subtasks = set(tasks)
            ...
            len_subtasks = 0
            j = 0
            while j <= len(sorted_constraints) - 1 and len_subtasks < 2 * self.nb_jobs_subproblem:
                t1, t2 = sorted_constraints[j][1], sorted_constraints[j][2]
                subtasks.add(t1)
                subtasks.add(t2)
                if t1 in self.graph.index_components:
                    subtasks.update(self.graph.components_graph_constraints[self.graph.index_components[t1]])
                    for c in set(self.graph.components_graph_constraints[self.graph.index_components[t1]]):
            ...
            if len_subtasks < self.nb_jobs_subproblem:
                subtasks, _ = self.other.find_subtasks(current_solution=current_solution, subtasks=subtasks)
            len_subtasks = len(subtasks)
            if len_subtasks > self.nb_jobs_subproblem:
            ...
            return self.other.find_subtasks(current_solution=current_solution, subtasks=subtasks)


class NeighborBuilderMix(NeighborBuilder):
    def __init__(self, list_neighbor: List[NeighborBuilder],
                 weight_neighbor: Union[List[float], np.array], verbose: bool = False):
    ...
    def find_subtasks(self, current_solution: RCPSPSolution,
        ...
        choice = np.random.choice(self.index_np,
                                  size=1,
                                  p=self.weight_neighbor)[0]
        if self.verbose:
            print(self.__class__.__name__)
            print(self.list_neighbor[choice])
        return self.list_neighbor[choice].find_subtasks(current_solution=current_solution,
                                                        subtasks=subtasks)


class NeighborBuilderTimeWindow(NeighborBuilder):
    def find_subtasks(self, current_solution: ANY_SOLUTION, subtasks: Optional[Set[Hashable]] = None) \
        ...
        tasks_of_interest = [t for t in self.problem.tasks_list
                             if any(current_solution.get_start_time(t) <= x <= current_solution.get_end_time(t)
        ...
        self.current_time_window = [self.current_time_window[0]+self.time_window_length,
...
class BasicConstraintBuilder:
    def __init__(self,
        ...
        self.matrix_func = {True: {True: constraints_strings_multiskill_preemptive,
                                   False: constraints_strings_multiskill},
                            False: {True: constraints_strings_preemptive,
                                    False: constraints_strings}}
    ...
    def return_constraints(self,
                           ...
                           cp_solver: ANY_CP_SOLVER,
                           params_constraint_builder: ParamsConstraintBuilder = None):
        p = params_constraint_builder if params_constraint_builder is not None else self.params_constraint_builder
        subtasks_1, subtasks_2 = self.neighbor_builder.find_subtasks(current_solution=current_solution)
        if self.verbose:
            print(self.__class__.__name__)
            print(len(subtasks_1), " in first set ", len(subtasks_2), " in second set ")
        list_strings = self.func(current_solution=current_solution,
                                 cp_solver=cp_solver,
...
class ConstraintHandlerScheduling(ConstraintHandler):
    def __init__(self, problem: ANY_RCPSP,
        ...
        if params_list is None:
            self.params_list = [
                ParamsConstraintBuilder(minus_delta_primary=6000,
    ...
    def adding_constraint_from_results_store(self, cp_solver: ANY_CP_SOLVER,
                                             child_instance,
        ...
        list_strings, subtasks_1, subtasks_2 = \
            self.basic_constraint_builder.return_constraints(current_solution=current_solution,
                                                             cp_solver=cp_solver,
        ...
        if eval.get("constraint_penalty", 0) > 0:
            ...
            string = cp_solver.constraint_start_time_string(task=self.problem.sink_task,
                                                            ...
                                                            .get_start_time(self.problem.sink_task)+20,
                                                            sign=SignEnum.LEQ)
            child_instance.add_string(string)
        if eval.get("constraint_penalty", 0) > 0:
            child_instance.add_string("constraint sec_objective<="+str(int(1.01*100*eval.get("constraint_penalty", 0))
                                                                       + 1000)
                                      + ";\n")
        ...
        if True:
            ...
            if eval.get("constraint_penalty", 0) > 0:
                ...
                try:
                    strings = cp_solver.add_hard_special_constraints(self.problem.special_constraints)
                    for s in strings:
                        child_instance.add_string(s)
                    child_instance.add_string("constraint sec_objective==0;\n")
                except Exception as e:
                    print("Hard constraint failed no method add_hard_special_constraints inside your cpsolver class,"
                ...
                if self.objective_subproblem == ObjectiveSubproblem.MAKESPAN_SUBTASKS:
                    ...
                    sum_end = sum([(10 if t == self.problem.sink_task else 1)*current_solution.get_start_time(t)
                                   for t in subtasks_1])
                    ...
                    sum_end = sum([(10 if t == self.problem.sink_task else 1) * current_solution.get_end_time(t)
                                   for t in subtasks_1])
                    strings += ["constraint objective<="+str(sum_end)+";\n"]
            for s in strings:
                child_instance.add_string(s)
...
class ConstraintHandlerMultiskillAllocation(ConstraintHandler):
    def __init__(self, problem: ANY_MSRCPSP,
                 params_list: List[ParamsConstraintBuilder] = None,
                 verbose: bool = False):
        self.problem = problem

    def adding_constraint_from_results_store(self, cp_solver: ANY_CP_SOLVER, child_instance, result_storage: ResultStorage,
                                             last_result_store: ResultStorage = None) -> Iterable[Any]:
        if last_result_store is not None:
            current_solution, fit = next((last_result_store.list_solution_fits[j]
                                          for j in range(len(last_result_store.list_solution_fits))
        ...
        list_strings = constraints_strings(
                    ...
                    tasks_secondary=set(),
                    params_constraints=ParamsConstraintBuilder(minus_delta_primary=0,
                                                               plus_delta_primary=0,
                                                               minus_delta_secondary=0,
                                                               plus_delta_secondary=0,
                                                               constraint_max_time_to_current_solution=True))
        list_strings += constraint_unit_used_to_tasks(tasks_set=set(random.sample(current_solution.problem.
                                                                                  get_tasks_list(),
                                                                                  min(int(0.8 * current_solution.
                                                                                      problem.n_jobs),
...
class EquilibrateMultiskillAllocationNonPreemptive(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: ANY_CP_SOLVER, child_instance,
                                             result_storage: ResultStorage,
                                             last_result_store: ResultStorage = None) -> Iterable[Any]:
        if last_result_store is not None:
            current_solution, fit = next((last_result_store.list_solution_fits[j]
        ...
        list_strings += cp_solver.constraint_objective_max_time_set_of_jobs([self.problem.sink_task])
        list_strings += ["constraint objective="+str(current_solution.get_end_time(self.problem.sink_task))+";\n"]
        ...
        if random.random() < 0.999:
            if eval.get("constraint_penalty", 0) == 0:
                ...
                child_instance.add_string("constraint sec_objective>=100*sum(overskill)+10*sum(res_load)+max(res_load)-min(res_load);\n")
        else:
            child_instance.add_string("constraint sec_objective>=min(res_load)-max(res_load);\n")
        return []
...
class EquilibrateMultiskillAllocation(ConstraintHandler):
    ...
    def adding_constraint_from_results_store(self, cp_solver: ANY_CP_SOLVER, child_instance,
        ...
        current_solution = current_solution
        eval = current_solution.problem.evaluate(current_solution)
        overskill = compute_overskill(problem=current_solution.problem,
        ...
        if random.random() < 0.999:
            if eval.get("constraint_penalty", 0) == 0:
                ...
                child_instance.add_string("constraint sec_objective>=100*sum(overskill)+10*sum(res_load)+max(res_load);\n")
        else:
            child_instance.add_string("constraint sec_objective>=min(res_load)-max(res_load);\n")
        return []
