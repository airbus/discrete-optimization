...
from typing import Dict, List, Hashable, Set, Tuple

from discrete_optimization.generic_tools.do_problem import Problem, ParamsObjectiveFunction,\
    ObjectiveRegister, TypeObjective, \
    TypeAttribute, MethodAggregating
from discrete_optimization.generic_tools.do_problem import EncodingRegister, Solution
from discrete_optimization.generic_tools.graph_api import Graph


class GPDPSolution(Solution):
    def copy(self):
        return GPDPSolution(problem=self.problem,
...
class GPDP(Problem):
    def __init__(self, number_vehicle,
                 ...
                 list_pickup_deliverable_per_cluster: List[Tuple[List[Hashable], List[Hashable]]] = None,
        ...
        for v in self.origin_vehicle:
            self.all_nodes_dict[self.origin_vehicle[v]] = {"type": "origin", "vehicle": v,
                                                           "time_delta_node":
                                                               self.time_delta_node[self.origin_vehicle[v]]}
        for v in self.target_vehicle:
            self.all_nodes_dict[self.target_vehicle[v]] = {"type": "target", "vehicle": v,
                                                           "time_delta_node":
                                                               self.time_delta_node[self.target_vehicle[v]]}
        for n in self.nodes_transportation:
            self.all_nodes_dict[n] = {"type": "customer", "time_delta_node": self.time_delta_node[n]}
        ...
        for node in self.distance_delta:
            ...
            for node1 in self.distance_delta[node]:
                self.edges_dict[node][node1] = {"distance": self.distance_delta[node][node1],
        ...
        print("compute graph")
        if compute_graph:
            self.graph = Graph(nodes=[(n, self.all_nodes_dict[n]) for n in self.all_nodes_dict],
                               edges=[(node1, node2, self.edges_dict[node1][node2])
                                      for node1 in self.edges_dict for node2 in self.edges_dict[node1]],
                               undirected=False,
        ...
        self.nodes_to_index = {i: self.list_nodes[i] for i in range(len(self.list_nodes))}
        ...
        self.any_grouping = any(len(self.group_identical_vehicles[k]) > 1 for k in self.group_identical_vehicles)
        self.vehicles_representative = [self.group_identical_vehicles[k][0] for k in self.group_identical_vehicles]
        self.slack_time_bound_per_node = slack_time_bound_per_node
        if self.slack_time_bound_per_node is None:
            self.slack_time_bound_per_node = {node: (0, self.time_delta_node[node])
                                              for node in self.time_delta_node}
...
def build_pruned_problem(problem: GPDP, undirected=True):
    ...
    for node in problem.graph.neighbors_dict:
        ...
        distance = [((node, n), problem.graph.edges_infos_dict[(node, n)]["distance"])
    ...
    connected_components = [(len(c), c) for c in sorted(nx.weakly_connected_components(graph_nx),
    ...
    if len(connected_components) > 1:
        ...
        for j in range(len(connected_components)):
            ...
            for k in range(j+1, len(connected_components)):
                ...
                distance = [((n1, n2),  problem.graph.edges_infos_dict[(n1, n2)]["distance"])
                            for n1 in nodes_j for n2 in nodes_k]
                if len(distance) > 0:
                    sorted_nodes = sorted(distance, key=lambda x: x[1])
        ...
        for edge in added_edges:
            ...
            if edge[1] not in graph_nx:
                graph_nx.add_node(edge[1])
            graph_nx.add_edge(edge[0], edge[1])
        connected_components = [(len(c), c) for c in sorted(nx.strongly_connected_components(graph_nx),
                                                            key=len, reverse=True)]
        #print("Connected components after post process= ", len(connected_components))

    return GPDP(number_vehicle=problem.number_vehicle,
...
def max_distance(problem: GPDP):
    return max([problem.distance_delta[i][j] for i in problem.distance_delta
               for j in problem.distance_delta[i]])


def max_time(problem: GPDP):
    return max([problem.time_delta[i][j] for i in problem.time_delta
               for j in problem.time_delta[i]]+[problem.time_delta_node[j]
                                                for j in problem.time_delta_node])


def build_matrix_distance(problem: GPDP):
    matrix_distance = 100000*np.ones((len(problem.all_nodes_dict), len(problem.all_nodes_dict)))
    for j in problem.distance_delta:
        for k in problem.distance_delta[j]:
            matrix_distance[problem.index_nodes[j], problem.index_nodes[k]] = problem.distance_delta[j][k]
    return matrix_distance


def build_matrix_time(problem: GPDP):
    matrix_time = 10000*np.ones((len(problem.all_nodes_dict), len(problem.all_nodes_dict)))
    for j in problem.time_delta:
        for k in problem.time_delta[j]:
            matrix_time[problem.index_nodes[j], problem.index_nodes[k]] = problem.time_delta[j][k]
    return matrix_time
...
class ProxyClass:
    ...
    def from_vrp_model_to_gpdp(vrp_model: VrpProblem):
        ...
        clients_node = [(i, vrp_model.customers[i])
                        for i in range(nb_customers) if i not in all_start_index and i not in all_end_index]
        virtual_debut_node = [len(clients_node)+k for k in range(nb_vehicle)]
        virtual_end_node = [len(clients_node)+nb_vehicle+k for k in range(nb_vehicle)]
        virtual_to_initial = {virtual_debut_node[i]: vrp_model.start_indexes[i]
                              for i in range(len(virtual_debut_node))}
        virtual_to_end = {virtual_end_node[i]: vrp_model.end_indexes[i]
                          for i in range(len(virtual_debut_node))}
        real_client_to_initial = {j: clients_node[j][0] for j in range(len(clients_node))}
        ...
        clients_gpdp = [j for j in range(len(clients_node))]+virtual_debut_node+virtual_end_node
        ...
        for node1 in clients_gpdp:
            ...
            for node2 in clients_gpdp:
                ...
                dictionnary_distance[node1][node2] = vrp_model.evaluate_function_indexes(index_1=prev_node1,
                                                                                         index_2=prev_node2)
        ...
        for node in clients_gpdp:
            ...
            coordinates[node] = (vrp_model.customers[prev_node].x,
                                 vrp_model.customers[prev_node].y)
            resources_flow_node[node] = {"demand": -int(vrp_model.customers[prev_node].demand)}
        for v in range(nb_vehicle):
            resources_flow_node[origin_vehicle[v]]["demand"] = int(vrp_model.vehicle_capacities[v])
        ...
        # resources_set = set()
        capacities = {i: {"demand": (0, vrp_model.vehicle_capacities[i])}
                      for i in range(nb_vehicle)}
        # {Vehicle:{resource: (min_capacity, max_capacity)}}
        resources_flow_node = resources_flow_node
        resources_flow_edges = {(x, y): {"demand": 0} for x in dictionnary_distance
        ...
        return GPDP(number_vehicle=nb_vehicle,
    ...
    def from_tsp_model_gpdp(tsp_model: TSPModel):
        ...
        clients_node = [(i, tsp_model.list_points[i])
                        for i in range(nb_customers) if i not in all_start_index and i not in all_end_index]
        virtual_debut_node = [len(clients_node) + k for k in range(nb_vehicle)]
        virtual_end_node = [len(clients_node) + nb_vehicle + k for k in range(nb_vehicle)]
        virtual_to_initial = {virtual_debut_node[i]: tsp_model.start_index
                              for i in range(len(virtual_debut_node))}
        virtual_to_end = {virtual_end_node[i]: tsp_model.end_index
                          for i in range(len(virtual_debut_node))}
        real_client_to_initial = {j: clients_node[j][0] for j in range(len(clients_node))}
        origin_vehicle = {i: virtual_debut_node[i] for i in range(nb_vehicle)}
        target_vehicle = {i: virtual_end_node[i] for i in range(nb_vehicle)}
        ...
        for node1 in clients_gpdp:
            ...
            for node2 in clients_gpdp:
                ...
                if node2 in real_client_to_initial:
                    ...
                    prev_node2 = virtual_to_end[node2]
                dictionnary_distance[node1][node2] = tsp_model.evaluate_function_indexes(index_1=prev_node1,
                                                                                         index_2=prev_node2)
        ...
        for node in clients_gpdp:
            ...
            if node in real_client_to_initial:
                ...
                prev_node = virtual_to_end[node]
            coordinates[node] = (tsp_model.list_points[prev_node].x, tsp_model.list_points[prev_node].y)
