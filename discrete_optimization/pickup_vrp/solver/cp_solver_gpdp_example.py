...
from discrete_optimization.generic_tools.ls.simulated_annealing import SimulatedAnnealing, TemperatureSchedulingFactor

from discrete_optimization.generic_tools.mutations.mixed_mutation import BasicPortfolioMutation

from discrete_optimization.tsp.mutation.mutation_tsp import MutationSwapTSP, Mutation2Opt, Mutation2OptIntersection

from discrete_optimization.generic_tools.ls.local_search import RestartHandlerLimit, ModeMutation

from discrete_optimization.generic_tools.mutations.mutation_catalog import get_available_mutations
...
files_mzn = {"gpdp": os.path.join(this_path, "../minizinc/gpdp.mzn"),
...
def find_intersection(trajectory,
                      points,
                      test_all=False,
                      nb_tests=10):
    perm = trajectory
...
def script_example():
    ...
    coordinates = np.random.randint(-20, 20, size=(number_of_nodes_transportation+2*n_vehicles,
                                                   2))
    ...
    instance["distance_delta"] = [[int(distance_delta[i, j])
                                   for j in range(distance_delta.shape[1])]
                                  for i in range(distance_delta.shape[0])]
    instance["time_delta"] = [[int(time_delta[i, j])
                               for j in range(time_delta.shape[1])]
    ...
    for i in range(len(results)):
        ...
        for vehicle in range(len(traj)):
            ...
            for j in range(len(traj[vehicle])-1):
                ax.plot([coordinates[traj[vehicle][j]-1, 0], coordinates[traj[vehicle][j+1]-1, 0]],
                        [coordinates[traj[vehicle][j]-1, 1], coordinates[traj[vehicle][j+1]-1, 1]],
                        color=color)
...
def script_example_lns():
    ...
    instance["time_delta"] = [[int(time_delta[i, j])
                              ...
                              for i in range(time_delta.shape[0])]
    results = []
    indexes = [(v, index) for v in range(n_vehicles)
               for index in range(number_of_nodes_transportation+2)]
    dummy_solution = [[number_of_nodes_transportation+n_vehicles+v+1]*(number_of_nodes_transportation+2)
            ...
            if random.random() <= 0.5:
                ...
                for v, index in indexes_fix:
                    if trajo[v][index] != number_of_nodes_transportation+n_vehicles+v+1:
                        child.add_string(
                            "constraint trajectories["+str(v+1)+","+str(index+1)+"]=="+str(trajo[v][index])+";\n")
            else:
                print("METHOD 2")
                vehicles = random.choice(range(n_vehicles))
                for vehicle in [vehicles]:
                    traj = trajo[vehicle]
                    nodes = [n for n in traj if n != number_of_nodes_transportation+vehicle+1
                             and n != number_of_nodes_transportation+n_vehicles+vehicle+1]
                    for n in nodes:
                        child.add_string("constraint "+str(n)+" in {trajectories["+str(vehicle+1)+
                                         ", i]|i in TRAJECTORY_INDEX};\n")
                    child.add_string("array[TRAJECTORY_INDEX] of ALL_NODES: ref="+str(traj)+";\n")
                    # child.add_string("constraint sum(i in TRAJECTORY_INDEX)(trajectories["+str(vehicle+1)+",i]!=ref[i])<=2;\n")
                    ...
                    child.add_string("constraint forall(i in TRAJECTORY_INDEX)"
                                     "(do_2_opt /\ opt_index[1]<=i /\ i<=opt_index[2]->trajectories["
                                     + str(vehicle+1)+",i]==ref[opt_index[2]-i+opt_index[1]]);\n")
                    child.add_string("constraint forall(i in TRAJECTORY_INDEX)"
                                     "(do_2_opt=false ->trajectories["
                                     + str(vehicle + 1) + ",i]==ref[i]);\n")
                if True:
                    for v in range(n_vehicles):
                        if v in [vehicles]:
                            continue
                        for index in range(number_of_nodes_transportation+2):
                            child.add_string("constraint trajectories["+str(v+1)+","+str(index+1)+"]=="
                                             + str(trajo[v][index])+";\n")
            if i > 0:
                child.add_string("constraint distance<="+str(int(last_result[1]))+";\n")
                child.add_string("constraint final_duration<="+str(int(last_result[2]))+";\n")

            result = child.solve(timeout=timedelta(seconds=20),
                                 intermediate_solutions=True,
                                 free_search=False)
            for ir in range(len(result)):
                results += [(result[ir].dict["trajectories"],
    ...
    for i in range(len(results)):
        ...
        for vehicle in path:
            ...
            for j in range(len(path_v)):
                ...
                ax.plot([coordinates[origin, 0], coordinates[destination, 0]],
...
def init_model_resources():
    ...
    coordinates = np.random.randint(-20, 20, size=(total_node,
                                                   2))
    ...
    instance["resource_flow_edges"] = np.zeros((total_node, total_node, instance["number_resource"]))
    instance["max_capacity_resource"] = 200*np.ones((n_vehicles, instance["number_resource"]), dtype=int)
    instance["min_capacity_resource"] = 10*np.zeros((n_vehicles, instance["number_resource"]), dtype=int)
    instance["max_capacity_resource"] = [[int(instance["max_capacity_resource"][i, j])
                                          for j in range(instance["max_capacity_resource"].shape[1])]
                                         for i in range(instance["max_capacity_resource"].shape[0])]
    instance["min_capacity_resource"] = [[int(instance["min_capacity_resource"][i, j])
                                          for j in range(instance["min_capacity_resource"].shape[1])]
                                         for i in range(instance["min_capacity_resource"].shape[0])]
    for v in range(n_vehicles):
        instance["resource_flow_node"][index_start[v], 0] = 80  # all vehicles loaded with 10
        instance["resource_flow_node"][index_start[v], 1] = 30
    instance["resource_flow_edges"][:, :, 0] = np.maximum(-distance_delta/2, -5)  # negative flow for resource 0
    carburant_node = random.sample(range(number_of_nodes_transportation), n_vehicles)
    instance["cut_to_max"] = [True, False]
    instance["resource_flow_node"][0:number_of_nodes_transportation, 1] = np.random.randint(-5, 5,
                                                                                            size=
                                                                                            (number_of_nodes_transportation)) # random depot/delivery.
    for i in carburant_node:
        instance["resource_flow_node"][i, 0] = 20
    instance["resource_flow_node"] = [[int(instance["resource_flow_node"][i, j])
                                       for j in range(instance["resource_flow_node"].shape[1])]
                                      for i in range(instance["resource_flow_node"].shape[0])]
    instance["resource_flow_edges"] = [[[int(instance["resource_flow_edges"][i, j, k])
    ...
    for k in range(nb_request_transportation):
        prec = set(random.sample([n for n in nodes_transportation if n not in chosen], 1))
        chosen.update(prec)
        succ = set(random.sample([n for n in nodes_transportation if n not in chosen], 1))
        chosen.update(succ)
...
def init_model_ortools():
    ...
    instance["time_delta"] = [[int(dmatrix[i][j])
    ...
    instance["resource_flow_edges"] = [[[int(instance["resource_flow_edges"][i, j, k])
                                        ...
                                        for j in range(instance["resource_flow_edges"].shape[1])]
                                       for i in range(instance["resource_flow_edges"].shape[0])]
    instance["precedence_pickup"] = [[{data["pickups_deliveries"][i][0]}, {data["pickups_deliveries"][i][1]}]
    ...
    coordinates = np.array([[-2, 4], [4, 4], [-4, 3], [-3, 3], [1, 2], [3, 2], [-1, 1],
                            [2, 1], [1, -1], [4, -1], [-3, -2], [-2, -2], [-1, -3], [2, -3], [-4, -4], [3, -4],
...
def init_model_ortools_tsp():
    ...
    distance = [[distance[i][j] for j in sorted(distance[i])]
...
def do_lns(child, last_result, n_vehicles, number_of_nodes_transportation, proportion_fix=0.5):
    indexes = [(v, index) for v in range(n_vehicles)
               for index in range(number_of_nodes_transportation + 2)]
    ...
    if random.random() <= 0.9:
        ...
        for v, index in indexes_fix:
            if trajo[v][index] != number_of_nodes_transportation + n_vehicles + v + 1:
                child.add_string(
                    "constraint trajectories[" + str(v + 1) + "," + str(index + 1) + "]==" + str(
                        trajo[v][index]) + ";\n")
    else:
        print("METHOD 2")
        vehicles = random.choice(range(n_vehicles))
        ...
        if True:
            for v in range(n_vehicles):
                ...
                for index in range(number_of_nodes_transportation + 2):
                    child.add_string("constraint trajectories[" + str(v + 1) + "," + str(index + 1) + "]=="
                                     + str(trajo[v][index]) + ";\n")
    #child.add_string("constraint distance<=" + str(int(last_result["distance"])) + ";\n")
    #child.add_string("constraint final_duration<=" + str(int(last_result["final_duration"])) + ";\n")
...
def run_resource(version: Example = Example.TSP):
    ...
    def get_dummy():
        ...
        dummy_solution = [[dict_instance["number_of_nodes_transportation"] + dict_instance["number_vehicle"] + v + 1] *
        ...
        dist = sum([dict_instance["distance_delta"][t1 - 1][t2 - 1]
                    for t1, t2 in zip(dummy_solution[v][:-1], dummy_solution[v][1:])
                    for v in range(len(dummy_solution))])
        time = max([sum([dict_instance["time_delta"][t1 - 1][t2 - 1]
                    for t1, t2 in zip(dummy_solution[v][:-1], dummy_solution[v][1:])])
                    for v in range(len(dummy_solution))])
        return [{"trajectories": dummy_solution, "final_duration": time, "distance": dist}]
    ...
    for i in range(100):
        with instance.branch() as child:
            do_lns(child=child,
                   ...
                   number_of_nodes_transportation=dict_instance["number_of_nodes_transportation"],
                   proportion_fix=0.85)
            res = child.solve(timeout=timedelta(seconds=100),
                              intermediate_solutions=True,
                              free_search=False, optimisation_level=2)
            print("Iter n°", i)
            print(res.status)
            ...
            try:
                ...
                for v in range(len(trajo)):
                    print("Vehicle ", v)
                    print("->".join([str(trajo[v][j]) for j in range(len(trajo[v]))
                                     if trajo[v][j] != v+1+dict_instance["number_of_nodes_transportation"] +
    ...
    for i in range(len(results)):
        ...
        for vehicle in range(len(traj)):
            color = colors(vehicle)
            for j in range(len(traj[vehicle])-1):
                ax[0].plot([coordinates[traj[vehicle][j]-1, 0], coordinates[traj[vehicle][j+1]-1, 0]],
                           [coordinates[traj[vehicle][j]-1, 1], coordinates[traj[vehicle][j+1]-1, 1]],
                           color=color)
            for resource in range(len(dict_instance["max_capacity_resource"][0])):
                ax[1].plot(results[i]["resource_leaving_node"][vehicle][resource][:], color=color,
                           label="load resource:"+str(i)+" vehicle: "+str(vehicle))
                ax[1].axhline(dict_instance["max_capacity_resource"][vehicle][resource],
                              label="max limit resource "+str(i))
                ax[1].axhline(dict_instance["min_capacity_resource"][vehicle][resource],
                              linestyle="--", label="min limit resource "+str(i))
...
from discrete_optimization.tsp.tsp_model import TSPModel2D, TSPModel, Point2D, TSPModelDistanceMatrix
from discrete_optimization.tsp.plots.plot_tsp import plot_tsp_solution


def run_tsp():
    dict_instance, coordinates = init_model_ortools_tsp()
    tsp_model = TSPModelDistanceMatrix(list_points=[Point2D(x=x[0], y=x[1]) for x in coordinates],
    ...
    print(list_mutation)
    list_mutation = [mutate[0].build(tsp_model, solution,
                                     attribute="permutation",
                                     **mutate[1]) for mutate in list_mutation
    ...
    sa = SimulatedAnnealing(evaluator=tsp_model,
                            mutator=mutate_portfolio,
                            restart_handler=res,
                            temperature_handler=TemperatureSchedulingFactor(temperature=10,
                                                                            restart_handler=res,
                                                                            coefficient=0.99999),
