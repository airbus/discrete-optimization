...
def retrieve_solutions_vehicle_types(model: mip.Model, variable_decisions: Dict[str, Any]):
    nSolutions = model.num_solutions
    range_solutions = range(nSolutions)
    list_results = []
    for s in range_solutions:
...
class LinearFlowSolver(SolverDO):
        ...
        for r in resources_variable_coming:
            for node in resources_variable_coming[r]:
                for vehicle, edge in edges_in_all_vehicles[node]:
                    if edge[0] == edge[1]:
                        continue
                    model.add_constr(resources_variable_coming[r][node] >=
                                     resources_variable_leaving[r][edge[0]]+self.problem.resources_flow_edges[edge][r]
                                     - 100000 * (1 - variables_edges[vehicle][edge])) # TODO Check if it works
                if node not in all_origin:
                    model.add_constr(resources_variable_leaving[r][node] ==
    ...
    def simple_capacity_constraint(self, model: mip.Model, variables_edges, edges_in_all_vehicles,
        ...
        consumption_per_vehicle = {v: {r: model.add_var(var_type=mip.CONTINUOUS,
        ...
        for v in consumption_per_vehicle:
            for r in consumption_per_vehicle[v]:
                model.add_constr(consumption_per_vehicle[v][r]
                                 == mip.quicksum(variables_edges[v][e]
    ...
    def time_evolution(self, model: mip.Model,
                       variables_edges,
                       edges_in_all_vehicles,
                       edges_out_all_vehicles):
        time_coming = {node: model.add_var(var_type=mip.CONTINUOUS,
                                           name="time_coming_" + str(node))
                       for node in edges_in_all_vehicles}
        time_leaving = {node: model.add_var(var_type=mip.CONTINUOUS,
                                            name="time_leaving_" + str(node))
                        for node in edges_in_all_vehicles}
        for v in self.problem.origin_vehicle:
            model.add_constr(time_coming[self.problem.origin_vehicle[v]]
                             == 0)
        index = 0
        all_origin = set(self.problem.origin_vehicle.values())
        all_target = set(self.problem.target_vehicle.values())
        for node in time_leaving:
            for vehicle, edge in edges_in_all_vehicles[node]:
                if edge[0] == edge[1]:
                    continue
                model.add_constr(time_coming[node] >=
                                 time_leaving[edge[0]]+self.problem.time_delta[edge[0]][edge[1]]
                                 - 100000*(1-variables_edges[vehicle][edge]))
                # Big M : check if it works
            if node not in all_origin:
                model.add_constr(time_leaving[node] - time_coming[node] - self.problem.time_delta_node[node] >= 0)
        return {"time_coming": time_coming,
                "time_leaving": time_leaving}

    def init_model(self, **kwargs):
        model: mip.Model = mip.Model("GPDP-flow", sense=mip.MINIMIZE, solver_name=mip.CBC)
        ...
        if self.problem.any_grouping:
            ...
            for k in self.problem.group_identical_vehicles:
                ...
                variables_edges[j] = {e: model.add_var(var_type=mip.BINARY,
        ...
        for vehicle in variables_edges:
            for e in variables_edges[vehicle]:
                if e[0] == e[1]:
                    constraint_loop[(vehicle, e)] = model.add_constr(variables_edges[vehicle][e] == 0,
        ...
        for vehicle in range(nb_vehicle):
            ...
            constraints_out_flow[(vehicle, node_origin)] = \
                model.add_constr(mip.quicksum([variables_edges[vehicle][edge]
            ...
            constraints_out_flow[(vehicle, node_target)] = \
                model.add_constr(mip.quicksum([variables_edges[vehicle][edge]
                                               ...
                                               if edge[1] != node_target or same_node]) <= 0,
        ...
        for vehicle in range(nb_vehicle):
            ...
            model.add_constr(mip.quicksum([variables_edges[v][edge]
            ...
            for node in edges_in_per_vehicles[vehicle]:
                if same_node or node not in {node_origin, node_target}:
                    constraints_flow_conservation[(vehicle, node)] = \
                        model.add_constr(mip.quicksum([variables_edges[vehicle][e]
                ...
                for edge in variables_edges[vehicle]:
                    if (edge[1], edge[0]) in variables_edges[vehicle]:
                        ...
                        if edge[0] == node_target or edge[1] == node_target:
                            continue
                        constraint_tour_2length[cnt_tour] = model.add_constr(variables_edges[vehicle][edge]
        ...
        if include_subtour:
            variables_order = {node: model.add_var(var_type=mip.CONTINUOUS,
                                                   lb=0, ub=len(self.problem.all_nodes)+1,
                                                   name="order_"+str(node))
                               for node in edges_in_all_vehicles}
            constraints_order = {}
            for vehicle in range(nb_vehicle):
                node_origin = self.problem.origin_vehicle[name_vehicles[vehicle]]
                constraints_order[node_origin] = model.add_constr(variables_order[node_origin] == 0,
            ...
            for node in variables_order:
                if node not in constraints_order:
                    for vehicle, edge in edges_in_all_vehicles[node]:
                        if use_big_m:
                            constraints_order[node] = model.add_constr(variables_order[node] >=
                                                                       variables_order[edge[0]]
                                                                       + 1 - 1000*(1-variables_edges[vehicle][edge]),
                                                                       name="order_"+str(node))
                            # Big M constraint, TODO
                        if use_indicator:
                            raise MipModelException("indicator constraint dont exist in pymip")
            ...
            for node in [self.problem.origin_vehicle[name_vehicles[vehicle]]]:
                if node in edges_in_all_vehicles:
                    constraints_in_flow[node] = model.add_constr(
                        mip.quicksum([variables_edges[x[0]][x[1]] for x in edges_in_all_vehicles[node]]) == 0,
...
class LinearFlowSolverVehicleType(SolverDO):
    ...
    def one_visit_per_node(self, model: mip.Model,
                           nodes_of_interest, variables_edges,
                           edges_in_all_vehicles, edges_out_all_vehicles):
        constraint_one_visit = {}
        for node in nodes_of_interest:
            constraint_one_visit[node] = model.add_constr(
        ...
        for cluster in self.problem.clusters_to_node:
            constraint_cluster[cluster] = model.add_constr(
                mip.quicksum([variables_edges[x[0]][x[1]]
                              for node in self.problem.clusters_to_node[cluster]
                              for x in edges_in_all_vehicles[node]
                              if node in nodes_of_interest]) >= 1,
                name="visit_" + str(cluster))
            constraint_cluster[(cluster, "out")] = model.add_constr(
                mip.quicksum([variables_edges[x[0]][x[1]]
                              for node in self.problem.clusters_to_node[cluster]
    ...
    def simple_capacity_constraint(self, model: mip.Model, variables_edges, edges_in_all_vehicles,
        ...
        # corresponding to negative flow values in the problem definition.
        consumption_per_vehicle = {v: {r: model.add_var(var_type=mip.CONTINUOUS,
                                                        lb=0, ub=len(self.problem.group_identical_vehicles[v]) *
                                             self.problem.capacities[self.problem.group_identical_vehicles[v][0]][r][1],
                                                        name="consumption_v_"+str(v)+'_'+str(r))
                                       for r in self.problem.resources_set}
                ...
                model.add_constr(consumption_per_vehicle[v][r]
                                 ...
                                 mip.quicksum(variables_edges[v][e]
                                              * (- self.problem.resources_flow_node[e[1]][r]
                                              ...
                                              if e[1] !=
                                              self.problem.origin_vehicle[self.problem.group_identical_vehicles[v][0]]))
                # redundant : d
                # model.add_constr(consumption_per_vehicle[v][r]<=self.problem.capacities[v][r][1],
                #                 name="constraint_capa_"+str(v)+"_"+str(r))
        return {"consumption_per_vehicle": consumption_per_vehicle}
    ...
    def init_model(self, **kwargs):
        ...
        for k in self.problem.group_identical_vehicles:
            j = self.problem.group_identical_vehicles[k][0]
            variables_edges[k] = {e: model.add_var(var_type=mip.BINARY,
        ...
        for group_vehicle in self.problem.group_identical_vehicles:
            ...
            constraints_out_flow[(group_vehicle, node_origin)] = \
                model.add_constr(mip.quicksum([variables_edges[group_vehicle][edge]
            ...
            for node in edges_in_per_vehicles[group_vehicle]:
                if same_node or node not in {node_origin, node_target}:
                    constraints_flow_conservation[(group_vehicle, node)] = \
                        model.add_constr(mip.quicksum([variables_edges[group_vehicle][e]
...
class ConstraintHandlerOrWarmStart:
    ...
    def adding_constraint(self, rebuilt_dict):
        ...
        vehicle_to_not_constraints = set(random.sample(range(self.problem.number_vehicle),
                                                       min(2, self.problem.number_vehicle)))
        for v in range(self.problem.number_vehicle):
            if v not in vehicle_to_not_constraints:
                edges_to_constraint[v].update(set(random.sample(set(self.linear_solver.variable_decisions
                                                                    ["variables_edges"][v]),
                                                                int(0.9*len(self.linear_solver.variable_decisions
                                                                        ["variables_edges"][v])))))
            else:
                edges_to_constraint[v].update(set(random.sample(set(self.linear_solver.variable_decisions
                                                                    ["variables_edges"][v]),
...
def reevaluate_result(temporary_result: TemporaryResult, problem: GPDP):
    ...
    for v in temporary_result.graph_vehicle:
        graph_of_interest = nx.subgraph(problem.graph.graph_nx,
        ...
        rebuilt_dict[v] = rebuild_routine(sorted_connected_component=
                                          ...
                                          evaluate_function_indexes=
                                          problem.evaluate_function_node)
    ...
    if len_component_global > 1:
        ...
        for s in components_global:
            ...
            if not any(problem.clusters_dict[problem.target_vehicle[v]]
                ...
                list_constraints += [lp_solver.model.add_constr(mip.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
...
def update_model(problem: GPDP,
    ...
    if len_component_global > 1:
        ...
        try:
            ...
            variable_order = {node: lp_solver.model.add_var(var_type=mip.CONTINUOUS,
                                                            name="order_" + str(node))
                              for node in edges_in_all_vehicles}
            for vehicle in range(lp_solver.problem.number_vehicle):
                node_origin = lp_solver.problem.origin_vehicle[vehicle]
                constraints_order[node_origin] = lp_solver.model.add_constr(variable_order[node_origin] == 0,
                                                                            name="order_" + str(node_origin))
                    ...
                    for vehicle, edge in edges_in_all_vehicles[node]:
                        ...
                        if use_big_m:
                            constraints_order[node] = lp_solver.model.add_constr(
                                ...
                                + 1 - 1000 * (1 - lp_solver.variable_decisions["variables_edges"][vehicle][edge]),
    ...
    if len_component_global > 1:
        ...
        for s in components_global:
            ...
            edge_out_of_interest = [e for n in s[0]
                                    ...
                                    and e[1][1] not in s[0]]
            if not any(problem.target_vehicle[v] in s[0] for v in problem.origin_vehicle):
                list_constraints += [lp_solver.model.add_lazy_constr(mip.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                                   for e in edge_out_of_interest]) >= 1)]
            if not any(problem.origin_vehicle[v] in s[0] for v in problem.origin_vehicle):
                list_constraints += [lp_solver.model.add_lazy_constr(mip.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                                   for e in edge_in_of_interest]) >= 1)]
