...
from discrete_optimization.generic_tools.do_problem import Problem, Solution, ParamsObjectiveFunction,\
...
class TemporaryResult:
    def __init__(self, flow_solution: Dict[int, Dict],
...
def retrieve_solutions_vehicle_types(model: grb.Model, variable_decisions: Dict[str, Any]):
    nSolutions = model.SolCount
    range_solutions = range(nSolutions)
    list_results = []
    for s in range_solutions:
...
class LinearFlowSolver(SolverDO):
    ...
    def one_visit_per_node(self, model, nodes_of_interest, variables_edges,
                           edges_in_all_vehicles, edges_out_all_vehicles):
        constraint_one_visit = {}
        for node in nodes_of_interest:
            constraint_one_visit[node] = model.addConstr(
                lhs=grb.quicksum([variables_edges[x[0]][x[1]] for x in edges_in_all_vehicles[node]]),
                sense=grb.GRB.GREATER_EQUAL,
                rhs=1, name="visit_" + str(node))
            constraint_one_visit[(node, "out")] = model.addConstr(
                lhs=grb.quicksum([variables_edges[x[0]][x[1]] for x in edges_out_all_vehicles[node]]),
                sense=grb.GRB.GREATER_EQUAL,
                rhs=1,
                name="visitout_" + str(node))
        model.update()

    def custom_constraint(self, model,
                          nodes_to_visit,
                          nodes_to_validate,
                          edges_validating,
                          variables_edges,
        ...
        for validate in edges_validating:
            if len(edges_validating[validate]["index_validate"]) > 0:
                for vehicle in range(len(edges_validating[validate]["index_validate"])):
                    ...
                    constraint_one_visit[validate] = \
                        model.addConstr(grb.quicksum([variables_edges[vehicle][e]
                                                      for e in edges_validating[validate]["edges"]]
                                                     + [variables_edges[vehicle][e]
                                                        for e in edges_in_per_vehicle[vehicle][node]]) >= 1,
                                        name="visit_alternative_"+str(node))
        model.update()

    def one_visit_per_clusters(self, model, nodes_of_interest, variables_edges,
                               edges_in_all_vehicles, edges_out_all_vehicles):
        constraint_cluster = {}
        for cluster in self.problem.clusters_to_node:
            if all(node in nodes_of_interest for node in self.problem.clusters_to_node[cluster]):
                constraint_cluster[cluster] = model.addConstr(
                    lhs=grb.quicksum([variables_edges[x[0]][x[1]]
                ...
                constraint_cluster[(cluster, "out")] = model.addConstr(
                    lhs=grb.quicksum([variables_edges[x[0]][x[1]]
                                      ...
                                      and x[0] in self.problem.vehicles_representative]),
                    sense=grb.GRB.EQUAL,
                    rhs=1, name="visitout_" + str(cluster))
        model.update()

    def resources_constraint(self, model: grb.Model, variables_edges, edges_in_all_vehicles, edges_out_all_vehicles):
        resources = self.problem.resources_set
        resources_variable_coming = {r: {node: model.addVar(vtype=grb.GRB.CONTINUOUS,
                                                            name="res_coming_"+str(r)+"_" + str(node))
                                     for node in edges_in_all_vehicles} for r in resources}
        resources_variable_leaving = {r: {node: model.addVar(vtype=grb.GRB.CONTINUOUS,
                                                             name="res_leaving_" + str(r) + "_" + str(node))
                                          for node in edges_in_all_vehicles} for r in resources}
        for v in self.problem.origin_vehicle:
            for r in resources_variable_coming:
                model.addConstr(resources_variable_coming[r][self.problem.origin_vehicle[v]]
                                == self.problem.resources_flow_node[self.problem.origin_vehicle[v]][r])
                model.addConstr(resources_variable_leaving[r][self.problem.origin_vehicle[v]]
                                == self.problem.resources_flow_node[self.problem.origin_vehicle[v]][r])
    ...
    def simple_capacity_constraint(self, model: grb.Model, variables_edges, edges_in_all_vehicles,
                                   edges_out_all_vehicles):
        ...
        consumption_per_vehicle = {v: {r: model.addVar(vtype=grb.GRB.CONTINUOUS,
                                                       lb=0, ub=self.problem.capacities[v][r][1],
        ...
        for v in consumption_per_vehicle:
            for r in consumption_per_vehicle[v]:
                model.addConstr(consumption_per_vehicle[v][r]
                                == grb.quicksum(variables_edges[v][e]
                                                * (- self.problem.resources_flow_node[e[1]][r]
                                                   - self.problem.resources_flow_edges[e][r])
                                                for e in variables_edges[v] if e[1] != self.problem.origin_vehicle[v]))
                # redundant :
                model.addConstr(lhs=consumption_per_vehicle[v][r],
    ...
    def time_evolution(self, model: grb.Model,
                       variables_edges,
                       edges_in_all_vehicles,
                       edges_out_all_vehicles):
        time_coming = {node: model.addVar(vtype=grb.GRB.CONTINUOUS,
                                          name="time_coming_" + str(node))
                       for node in edges_in_all_vehicles}
        time_leaving = {node: model.addVar(vtype=grb.GRB.CONTINUOUS,
                                           name="time_leaving_" + str(node))
                        for node in edges_in_all_vehicles}
        for v in self.problem.origin_vehicle:
        ...
        for node in time_leaving:
            for vehicle, edge in edges_in_all_vehicles[node]:
                ...
                index += 1
            if node not in all_origin:
                model.addConstr(time_leaving[node] >=
                                time_coming[node] + self.problem.time_delta_node[node])
        return {"time_coming": time_coming,
    ...
    def init_model(self, **kwargs):
        ...
        nodes_of_interest = [n for n in graph.get_nodes()
                             if n not in self.problem.nodes_origin
                             and n not in self.problem.nodes_target]
        if self.problem.any_grouping:
            ...
            for k in self.problem.group_identical_vehicles:
                ...
                variables_edges[j] = {e: model.addVar(vtype=grb.GRB.BINARY,
                                                      name="flow_"+str(j)+"_"+str(e),
                                                      obj=graph.edges_infos_dict[e]
                                                      ["distance"])
        ...
        if optimize_span:
            # model.update()
            self.spans = {j: model.addVar(vtype=grb.GRB.INTEGER, name="span_"+str(j))
                          for j in range(nb_vehicle)}
            self.objective = model.addVar(vtype=grb.GRB.INTEGER, obj=100)
            for j in self.spans:
                model.addConstr(self.spans[j] == grb.quicksum([
        ...
        for vehicle in variables_edges:
            for e in variables_edges[vehicle]:
                if e[0] == e[1]:
                    constraint_loop[(vehicle, e)] = model.addConstr(variables_edges[vehicle][e] == 0,
        ...
        for vehicle in variables_edges:
            ...
            for edge in variables_edges[vehicle]:
                ...
                if out_ in self.problem.clusters_dict \
                        and self.problem.clusters_dict[out_] not in edges_out_per_vehicles_cluster[vehicle]:
                    edges_out_per_vehicles_cluster[vehicle][self.problem.clusters_dict[out_]] = set()
                if in_ in self.problem.clusters_dict \
                        and self.problem.clusters_dict[in_] not in edges_in_per_vehicles_cluster[vehicle]:
                    edges_in_per_vehicles_cluster[vehicle][self.problem.clusters_dict[in_]] = set()
                if out_ in self.problem.clusters_dict \
                        and self.problem.clusters_dict[out_] not in edges_out_all_vehicles_cluster:
                    edges_out_all_vehicles_cluster[self.problem.clusters_dict[out_]] = set()
                if in_ in self.problem.clusters_dict \
                        and self.problem.clusters_dict[in_] not in edges_in_all_vehicles_cluster:
                    edges_in_all_vehicles_cluster[self.problem.clusters_dict[in_]] = set()
                ...
                if out_ in self.problem.clusters_dict:
                    edges_out_all_vehicles_cluster[self.problem.clusters_dict[out_]].add((vehicle, edge))
                    edges_out_per_vehicles_cluster[vehicle][self.problem.clusters_dict[out_]].add(edge)
                if in_ in self.problem.clusters_dict:
                    edges_in_all_vehicles_cluster[self.problem.clusters_dict[in_]].add((vehicle, edge))
                    edges_in_per_vehicles_cluster[vehicle][self.problem.clusters_dict[in_]].add(edge)
        ...
        for vehicle in range(nb_vehicle):
            ...
            constraints_out_flow[(vehicle, node_origin)] = \
                model.addConstr(grb.quicksum([variables_edges[vehicle][edge]
                                              for edge in edges_out_per_vehicles[vehicle].get(node_origin, set())
                                              if edge[1] != node_origin or same_node]) == count_origin[node_origin],
                                name="outflow_"+str((vehicle, node_origin)))  # Avoid loop
            constraints_in_flow[(vehicle, node_target)] = \
                model.addConstr(grb.quicksum([variables_edges[vehicle][edge]
                                              for edge in edges_in_per_vehicles[vehicle].get(node_target, set())
                                              if edge[0] != node_target or same_node]) == count_target[node_target],
                                name="inflow_" + str((vehicle, node_target)))  # Avoid loop

            constraints_out_flow[(vehicle, node_target)] = \
                model.addConstr(lhs=grb.quicksum([variables_edges[vehicle][edge]
                ...
                for edge in variables_edges[vehicle]:
                    if (edge[1], edge[0]) in variables_edges[vehicle]:
                        ...
                        if edge[0] == node_target or edge[1] == node_target:
                            continue
                        constraint_tour_2length[cnt_tour] = model.addConstr(variables_edges[vehicle][edge]
                                                                            + variables_edges[vehicle][(edge[1],
                                                                                                        edge[0])] <= 1,
        ...
        if include_triangle:
            ...
            for node in graph.graph_nx.nodes():
                ...
                neigh_2 = {nn: neigh.intersection([n for n in nx.neighbors(graph.graph_nx, nn)]) for nn in neigh}
                        ...
                        for node_neigh_neigh in neigh_2[node_neigh]:
                            for vehicle in range(nb_vehicle):
                                constraint_triangle[cnt_triangle] = model.addConstr(
                                    ...
                                    + variables_edges[vehicle][(node_neigh, node_neigh_neigh)]
                                    + variables_edges[vehicle][(node_neigh_neigh, node)] <= 2,
                                    name="triangle_" + str((cnt_triangle))
                                )
                                cnt_triangle += 1

        if include_subtour:
            variables_order = {node: model.addVar(vtype=grb.GRB.CONTINUOUS,
                                                  name="order_"+str(node))
                               for node in edges_in_all_vehicles}
            constraints_order = {}
            for vehicle in range(nb_vehicle):
                node_origin = self.problem.origin_vehicle[name_vehicles[vehicle]]
                constraints_order[node_origin] = model.addConstr(lhs=variables_order[node_origin],
                                                                 sense=grb.GRB.EQUAL,
                                                                 rhs=0,
            ...
            for node in variables_order:
                if node not in constraints_order:
                    for vehicle, edge in edges_in_all_vehicles[node]:
                        if use_big_m:
                            constraints_order[node] = model.addConstr(variables_order[node] >=
                                                                      variables_order[edge[0]]+1
                                                                      - 2*nb_nodes*(1-variables_edges[vehicle][edge]),
                                                                      name="order_"+str(node)) # Big M constraint, TODO
                        if use_indicator:
                            constraints_order[node] = model.addGenConstrIndicator(variables_edges[vehicle][edge], 1,
            ...
            for node in [self.problem.origin_vehicle[name_vehicles[vehicle]]]:
                if node in edges_in_all_vehicles:
                    constraints_in_flow[node] = model.addConstr(
                        grb.quicksum([variables_edges[x[0]][x[1]] for x in edges_in_all_vehicles[node]])==0,
                        name="no_inflow_start_" + str(node))

        if one_visit_per_node:
            self.one_visit_per_node(model=model,
        ...
        if edges_validating_constraint:
            self.custom_constraint(model=model,
                                   nodes_to_visit=set([n for n in nodes_of_interest
                                                       if n >= 366]),
        ...
        if one_visit_per_cluster:
            self.one_visit_per_clusters(model=model,
                                        nodes_of_interest=nodes_of_interest,
                                        variables_edges=variables_edges,
                                        edges_in_all_vehicles=edges_in_all_vehicles,
                                        edges_out_all_vehicles=edges_out_all_vehicles)
        if include_capacity:
            vars = self.simple_capacity_constraint(model=model,
                                                   variables_edges=variables_edges,
                                                   edges_in_all_vehicles=edges_in_all_vehicles,
                                                   edges_out_all_vehicles=edges_out_all_vehicles)
        ...
        if include_time_evolution:
            vars = self.time_evolution(model=model,
                                       variables_edges=variables_edges,
                                       edges_in_all_vehicles=edges_in_all_vehicles,
                                       edges_out_all_vehicles=edges_out_all_vehicles)
            all_variables.update(vars)
        ...
        self.edges_info = {"edges_in_all_vehicles": edges_in_all_vehicles,
                           "edges_out_all_vehicles": edges_out_all_vehicles,
    ...
    def retrieve_solutions(self, range_solutions: Iterable[int]):
        solutions = retrieve_solutions(model=self.model, variable_decisions=self.variable_decisions)
    ...
    def solve(self, parameters_milp: ParametersMilp = None, **kwargs) -> List[TemporaryResult]:
    ...
    def solve_iterative(self, parameters_milp: ParametersMilp = None, **kwargs):
        ...
        while not finished:
            ...
            if all(rebuilt_dict[v] is not None for v in rebuilt_dict):
                json.dump(rebuilt_dict, open(os.path.join(folder, "res_"+str(nb_iteration)
                                                          + "_" +str(time.time_ns())+".json"), "w"), indent=4)
            c = ConstraintHandlerOrWarmStart(linear_solver=self, problem=self.problem, do_lns=do_lns)
            c.adding_constraint(rebuilt_dict)
            if warm_start is not None and all(rebuilt_dict[v] is None for v in rebuilt_dict):
                c.adding_constraint(warm_start)
            self.model.update()
            solutions: List[TemporaryResult] = self.solve(parameters_milp=parameters_milp, **kwargs)
            all_solutions += solutions
...
class LinearFlowSolverVehicleType(SolverDO):
    ...
    def init_model(self, **kwargs):
        ...
        for k in self.problem.group_identical_vehicles:
            j = self.problem.group_identical_vehicles[k][0]
            variables_edges[k] = {e: model.addVar(vtype=grb.GRB.BINARY,
        ...
        for group_vehicle in variables_edges:
            for e in variables_edges[group_vehicle]:
                if e[0] == e[1]:
                    constraint_loop[(group_vehicle, e)] = model.addConstr(variables_edges[group_vehicle][e] == 0,
        ...
        for group_vehicle in self.problem.group_identical_vehicles:
            ...
            node_origin = self.problem.origin_vehicle[name_vehicles[representative_vehicle]]
            node_target = self.problem.target_vehicle[name_vehicles[representative_vehicle]]
            same_node = (node_origin == node_target)
            # neighbors_origin = graph.get_neighbors(self.problem.origin_vehicle[name_vehicles[vehicle]])
            constraints_out_flow[(group_vehicle, node_origin)] = \
                model.addConstr(lhs=grb.quicksum([variables_edges[group_vehicle][edge]
                                                  for edge in edges_out_per_vehicles[group_vehicle].get(node_origin, set())
                                                  if edge[1] != node_origin or same_node]),
                                rhs=count_origin[node_origin],
                                sense=grb.GRB.LESS_EQUAL,
                                name="outflow_"+str((group_vehicle, node_origin))) # Avoid loop
            constraints_in_flow[(group_vehicle, node_target)] = \
                model.addConstr(lhs=grb.quicksum([variables_edges[group_vehicle][edge]
                                                  for edge in edges_in_per_vehicles[group_vehicle].get(node_target, set())
                                                  if edge[0] != node_target or same_node]),
                                rhs=count_target[node_target],
                                sense=grb.GRB.LESS_EQUAL,
                                name="inflow_" + str((group_vehicle, node_target)))  # Avoid loop

            constraints_out_flow[(group_vehicle, node_target)] = \
                model.addConstr(lhs=grb.quicksum([variables_edges[group_vehicle][edge]
                                                  for edge in edges_out_per_vehicles[group_vehicle].get(node_target, set())
                                                  if edge[1] != node_target or same_node]),
                                rhs=0,
                                sense=grb.GRB.LESS_EQUAL,
        ...
        for group_vehicle in self.problem.group_identical_vehicles:
            ...
            for node in edges_in_per_vehicles[group_vehicle]:
                if same_node or node not in {node_origin, node_target}:
                    constraints_flow_conservation[(group_vehicle, node)] = \
                        model.addConstr(lhs=grb.quicksum([variables_edges[group_vehicle][e]
                                                          for e in edges_in_per_vehicles[group_vehicle].get(node, set())
                                                          if e[1] != e[0]]
                                                         +[-variables_edges[group_vehicle][e]
                                                           for e in edges_out_per_vehicles[group_vehicle].get(node, set())
                                                           if e[1] != e[0]]),
                                        sense=grb.GRB.EQUAL,
                                        rhs=0,
                                        name="convflow_"+str((group_vehicle, node)))

                    constraints_flow_conservation[(group_vehicle, node, "in")] = \
                        model.addConstr(lhs=grb.quicksum([variables_edges[group_vehicle][e]
                                                          for e in edges_in_per_vehicles[group_vehicle].get(node, set())
                                                          if e[1] != e[0]]),
                                        sense=grb.GRB.LESS_EQUAL,
    ...
    def retrieve_solutions(self, range_solutions: Iterable[int]):
        ...
        for sol in solutions:
            path_dict = build_path_from_vehicle_type_flow(result_from_retrieve=sol[0],
...
def build_path_from_vehicle_type_flow(result_from_retrieve, problem: GPDP, solver: LinearFlowSolverVehicleType):
    ...
    for group_vehicle in vehicle_per_type:
        ...
        origin = problem.origin_vehicle[problem.group_identical_vehicles[group_vehicle][0]]
...
class LinearFlowSolverLazyConstraint(LinearFlowSolver):
    ...
    def solve(self, parameters_milp: ParametersMilp = None, **kwargs) -> List[TemporaryResult]:
        ...
        if "warm_start" in kwargs and kwargs.get("no_warm_start", False) == False:
            c = ConstraintHandlerOrWarmStart(linear_solver=self,
                                             problem=self.problem,
                                             do_lns=False)
            c.adding_constraint(kwargs.get("warm_start"))
        print("optimizing...")
        indexes_edges = [(v, e) for v in self.variable_decisions["variables_edges"]
                         for e in self.variable_decisions["variables_edges"][v]]
        # subtour = SubtourAddingConstraint(problem=self.problem,
        #                                   linear_solver=self,
        #                                   lazy=True)
        ...
        def callback(model, where):
            if where == grb.GRB.Callback.MIPSOL:
                solution = model.cbGetSolution([self.variable_decisions["variables_edges"][x[0]][x[1]]
                ...
                for k in range(len(solution)):
                    if solution[k] > 0.5:
                        ...
                        cost[indexes_edges[k][0]] += self.problem.graph.edges_infos_dict[indexes_edges[k][1]]["distance"]
                list_temporary_results = build_graph_solutions(solutions=[({"variables_edges": flow_solution}, 0)],
                                                               lp_solver=self)
                ...
                connected_components = {v: [(cyc, len(cyc))
                                            for cyc in nx.simple_cycles(temporary_result.graph_vehicle[v])]
                                        for v in vehicle_keys}
                print(cost)
                print("Cycles : ", [len(connected_components[v]) for v in connected_components])
                sorted_connected_component = {v: sorted(connected_components[v], key=lambda x: x[1], reverse=False)
                                              for v in connected_components}
                temporary_result.connected_components_per_vehicle = sorted_connected_component
                            ...
                            for p_ind in temporary_result.paths_component[v]:
                                ...
                                if self.problem.target_vehicle[v] not in pp:
                                    for vv in temporary_result.paths_component:
                                        if all((e0, e1) in self.variable_decisions["variables_edges"][vv]
                                               for e0, e1 in zip(pp[:-1], pp[1:])):
                                            keys = [(e0, e1) for e0, e1 in zip(pp[:-1], pp[1:])]
                                            model.cbLazy(
                                                grb.quicksum([self.variable_decisions["variables_edges"][vv][key]
                                                              for key in keys]) <= len(pp) - 2)
                for v in sorted_connected_component:
                    if len(sorted_connected_component[v]) > 1:
                        for s, l in sorted_connected_component[v]:
                            keys = [(e0, e1) for e0, e1 in zip(s[:-1], s[1:])]+[(s[-1], s[0])]
                            model.cbLazy(grb.quicksum([self.variable_decisions["variables_edges"][v][key]
                ...
                if not self.clusters_version:
                    subtour = SubtourAddingConstraint(problem=self.problem,
                                                      linear_solver=self,
                                                      lazy=True)
                    constraints_added = subtour.adding_component_constraints([temporary_result])
                else:
                    subtour = SubtourAddingConstraintCluster(problem=self.problem,
                                                             linear_solver=self,
                                                             lazy=True)
                    constraints_added = subtour.adding_component_constraints([temporary_result])
                # for v in sorted_connected_component:
    ...
    def solve_iterative(self, parameters_milp: ParametersMilp = None, **kwargs):
        ...
        if warm_start is not None:
            c.adding_constraint(warm_start)
        solutions: List[TemporaryResult] = self.solve(parameters_milp=parameters_milp, no_warm_start=True, **kwargs)
        # if self.clusters_version:
        #     subtour = SubtourAddingConstraintCluster(problem=self.problem, linear_solver=self)
        # else:
        #     subtour = SubtourAddingConstraint(problem=self.problem,
        ...
        while not finished:
            ...
            if all(rebuilt_dict[v] is not None for v in rebuilt_dict):
                json.dump(rebuilt_dict, open(os.path.join(folder, "res_" + str(nb_iteration)
                                                          + "_" + str(time.time_ns()) + ".json"), "w"),
                          indent=4, default=int)
            c.adding_constraint(rebuilt_dict)
            if warm_start is not None and all(rebuilt_dict[v] is None for v in rebuilt_dict):
                c.adding_constraint(warm_start)
            self.model.update()
            solutions: List[TemporaryResult] = self.solve(parameters_milp=parameters_milp,
                                                          no_warm_start=True, **kwargs)
            all_solutions += solutions
            if self.clusters_version:
                subtour = SubtourAddingConstraintCluster(problem=self.problem, linear_solver=self)
            else:
    ...
    def adding_component_constraints(self, list_solution: List[TemporaryResult]):
        ...
        for l in list_solution:
            for v in l.connected_components_per_vehicle:
                if self.lazy:
                    c += update_model_lazy(self.problem,
                                           self.linear_solver,
                    ...
                    if len(l.connected_components_per_vehicle[v]) > 1:
                        ...
                        for p_ind in l.paths_component[v]:
                            pp = l.paths_component[v][p_ind]+[l.paths_component[v][p_ind][0]]
                            for vv in self.linear_solver.variable_decisions["variables_edges"]:
                                if all((e0, e1) in self.linear_solver.variable_decisions["variables_edges"][vv]
                                       for e0, e1 in zip(pp[:-1], pp[1:])):
                                    print("YES")
                                    keys = [(e0, e1) for e0, e1 in zip(pp[:-1], pp[1:])]
                                    c += [self.linear_solver.model.addConstr(
                                        grb.quicksum([self.linear_solver.variable_decisions["variables_edges"][vv][key]
                                                      for key in keys]) <= len(pp)-2)
                                         ]

                            ind += 1
                    if True:
                        c += update_model(self.problem,
                                          self.linear_solver,
                                          l.connected_components_per_vehicle[v],
                                          self.linear_solver.edges_info["edges_in_all_vehicles"],
                                          self.linear_solver.edges_info["edges_out_all_vehicles"],
...
class SubtourAddingConstraintCluster:
    def __init__(self, problem: GPDP,
                 linear_solver: LinearFlowSolver, lazy=False):
        self.problem = problem
        self.linear_solver = linear_solver
        self.constraints_storage = {}
        self.lazy = lazy
        ...
        for l in list_solution:
            for v in l.connected_components_per_vehicle:
                ...
                for x in connected_components:
                    s = set([self.problem.clusters_dict[k]
                             for k in x[0]])
                    conn += [(s, len(s))]
                c += update_model_cluster_tsp(self.problem,
                                              self.linear_solver,
                                              conn,
...
class ConstraintHandlerOrWarmStart:
    def __init__(self, linear_solver: LinearFlowSolver,
                 problem: GPDP,
                 do_lns: bool = True):
        self.linear_solver = linear_solver
        self.problem = problem
    ...
    def adding_constraint(self, rebuilt_dict):
        ...
        for v in rebuilt_dict:
            if rebuilt_dict[v] is None:
                continue
            edges_to_add[v].update({(e0, e1) for e0, e1 in zip(rebuilt_dict[v][:-1], rebuilt_dict[v][1:])})
            # print("len rebuilt : ", len(rebuilt_dict[v]))
            print("edges to add , ", edges_to_add)
            edges_missing = {(v, e) for e in edges_to_add[v]
        ...
        if self.do_lns:
            try:
                self.linear_solver.model.remove([self.linear_solver.constraint_on_edge[iedge]
                                                 for iedge in self.linear_solver.constraint_on_edge])
                self.linear_solver.model.update()
            except:
                pass
        self.linear_solver.constraint_on_edge = {}
        edges_to_constraint: Dict[int, set] = {v: set() for v in range(self.problem.number_vehicle)}
        vehicle_to_not_constraints = set(random.sample(range(self.problem.number_vehicle),
                                                       min(3, self.problem.number_vehicle)))
        for v in range(self.problem.number_vehicle):
            path = [(e0, e1) for e0, e1 in zip(rebuilt_dict[v][:-1], rebuilt_dict[v][1:])]
            if v not in vehicle_to_not_constraints:
                edges_to_constraint[v].update(set(random.sample(set(self.linear_solver.variable_decisions
                                                                    ["variables_edges"][v]),
                                                                int(0.5*len(self.linear_solver.variable_decisions
                                                                            ["variables_edges"][v])))))
                for p in path:
                ...
                edges_to_constraint[v].update(set(random.sample(set(self.linear_solver.variable_decisions
                                                                    ["variables_edges"][v]),
                                                                int(0.4 *
                                                                    len(self.linear_solver.variable_decisions
                                                                        ["variables_edges"][v])))))
                ...
                if True:
                    ...
                    for p in path[j:min(len(path), j+350)]:
                        for sets in [self.linear_solver.edges_info["edges_in_per_vehicles"][v].get(p[0], set()),
                                     self.linear_solver.edges_info["edges_in_per_vehicles"][v].get(p[1], set()),
                                     self.linear_solver.edges_info["edges_out_per_vehicles"][v].get(p[0], set()),
                                     self.linear_solver.edges_info["edges_out_per_vehicles"][v].get(p[1], set())]:
                            for set_ in sets:
                ...
                if False:
                    for kk in path[:j]+path[min(len(path), j+400):]:
                        if kk not in edges_to_constraint[v]:
                            edges_to_constraint[v].add(kk)
                        for sets in [self.linear_solver.edges_info["edges_in_per_vehicles"][v][kk[0]],
                                     self.linear_solver.edges_info["edges_in_per_vehicles"][v][kk[1]],
                                     self.linear_solver.edges_info["edges_out_per_vehicles"][v][kk[0]],
                                     self.linear_solver.edges_info["edges_out_per_vehicles"][v][kk[1]]]:
                            for set_ in sets:
                                if set_ not in edges_to_constraint[v]:
                                    edges_to_constraint[v].add(set_)

        print(sum([len(edges_to_constraint[v])
                   for v in edges_to_constraint]),
              " edges constraint over ", sum([len(self.linear_solver.variable_decisions["variables_edges"][v])
            ...
            if rebuilt_dict[v] is not None:
                for e in self.linear_solver.variable_decisions["variables_edges"][v]:
                    ...
                    if v in edges_to_add and e in edges_to_add[v]:
                        self.linear_solver.variable_decisions["variables_edges"][v][e].start = 1
                        self.linear_solver.variable_decisions["variables_edges"][v][e].varhintval = 1
                        val = 1
                    else:
                        self.linear_solver.variable_decisions["variables_edges"][v][e].start = 0
                        self.linear_solver.variable_decisions["variables_edges"][v][e].varhintval = 0
                    if self.do_lns:
                        if rebuilt_dict[v] is not None and v in edges_to_constraint \
                                and e in edges_to_constraint[v]:
                            self.linear_solver.constraint_on_edge[iedge] = self.linear_solver.model.addConstr(
                                self.linear_solver.variable_decisions["variables_edges"][v][e] == val,
                                name="c_" + str(v) + "_" + str(e) + "_" + str(val))
                            iedge += 1
        self.linear_solver.model.update()


def build_the_cycles(flow_solution,
                     component,
                     start_index,
                     end_index):
    edge_of_interest = {e for e in flow_solution if e[1] in component and e[0] in component}
...
def rebuild_routine(sorted_connected_component,
                    paths_component,
                    node_to_component,
                    indexes,
                    graph,
                    ...
                    end_index,
                    verbose=False):
    rebuilded_path = list(paths_component[node_to_component[start_index]])  # Initial path
        ...
        if len(component_reconnected) == len(sorted_connected_component)-1 and \
            ...
            for e in edge_out_of_interest:
                ...
                if (next_node_component_e1, next_node_1) in edge_in_of_interest\
                        and next_node_1 in graph[e[0]]:
                    cost = graph[e[0]][e[1]]["distance"] + graph[next_node_component_e1][next_node_1]["distance"] - \
...
def rebuild_routine_variant(sorted_connected_component,
                            paths_component,
                            node_to_component,
                            indexes,
                            graph,
...
def reevaluate_result(temporary_result: TemporaryResult,
                      problem: GPDP,
        ...
        for v in connected_components:
            ...
            if len(connected_components[v]) > 1:
                for c, l in connected_components[v]:
                    graph_of_interest_i: nx.DiGraph = temporary_result.graph_vehicle[v].subgraph(c)
                    cycles = [(set(cyc), len(cyc)) for cyc in nx.simple_cycles(graph_of_interest_i)]
                    if len(cycles) >= 2:
                        ...
                        print(cycles)
                        # TODO ...
                        temporary_result.rebuilt_dict = {v: None for v in connected_components}
    ...
    for v in temporary_result.graph_vehicle:
        graph_of_interest: nx.DiGraph = nx.subgraph(problem.graph.graph_nx,
        ...
        for i in range(nb):
            ...
            paths_component[v][i], indexes_component[v][i] = build_the_cycles(flow_solution=
                                                                              temporary_result.flow_solution[v],
                                                                              component=s[0],
        ...
        rebuilt_dict[v] = rout(sorted_connected_component=
        ...
        if rebuilt_dict[v] is not None:
            ...
            print("distance :",
                  sum([f(e, e1) for e, e1 in zip(rebuilt_dict[v][:-1],
                                                 rebuilt_dict[v][1:])]))
...
def build_graph_solutions(solutions: List[Tuple[Dict, float]],
                          lp_solver: LinearFlowSolver) -> List[TemporaryResult]:
    ...
    for s in range(n_solutions):
        ...
        for vehicle in current_solution:
            for e in current_solution[vehicle]:
                ...
                g_vehicle[vehicle].add_edge(clients[0],
                                            clients[1],
                                            weight=lp_solver.problem.graph.edges_infos_dict[clients]["distance"])
                g_merge.add_edge(clients[0],
                                 clients[1],
                                 weight=lp_solver.problem.graph.edges_infos_dict[clients]["distance"])
        transformed_solutions += [TemporaryResult(**{"graph_merge": g_merge.copy(),
...
def update_model_cluster_tsp(problem: GPDP,
                             lp_solver: LinearFlowSolver,
                             components_global,
                             edges_in_all_vehicles,
                             edges_out_all_vehicles, lazy=False):
    ...
    if len_component_global > 1:
        ...
        for s in components_global:
            ...
            if not any(problem.clusters_dict[problem.target_vehicle[v]]
                       in s[0] for v in problem.origin_vehicle):
                if lazy:
                    list_constraints += [lp_solver.model.cbLazy(
                        grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                      for e in edge_out_of_interest]) >= 1)]
                else:
                    list_constraints += [lp_solver.model.addConstr(grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                             for e in edge_out_of_interest]) >= 1,
                                                               name="component_out_" + str(s)[:10])]
            if not any(problem.clusters_dict[problem.origin_vehicle[v]]
                       in s[0] for v in problem.origin_vehicle):
                if lazy:
                    list_constraints += [lp_solver.model.cbLazy(
                        grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                      for e in edge_in_of_interest]) >= 1)]
                else:
                    list_constraints += [lp_solver.model.addConstr(grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                                 for e in edge_in_of_interest]) >= 1,
    ...
    if len_component_global > 1:
        ...
        for s in components_global[:1]:
            ...
            edge_out_of_interest = [e for n in s[0]
                                    ...
                                    if e[1][0] in s[0]
                                    and e[1][1] not in s[0]]
            if not any(problem.target_vehicle[v] in s[0] for v in problem.origin_vehicle):
                list_constraints += [lp_solver.model.addConstr(grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                             for e in edge_out_of_interest]) >= 1,
                                                               name="component_out_" + str(s)[:10])]
            if not any(problem.origin_vehicle[v] in s[0] for v in problem.origin_vehicle):
                list_constraints += [lp_solver.model.addConstr(grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                             for e in edge_in_of_interest]) >= 1,
                                                               name="component_in_"+str(s)[:10])]
    if do_order:
        if len_component_global > 1:
            ...
            try:
                ...
                variable_order = {node: lp_solver.model.addVar(vtype=grb.GRB.CONTINUOUS,
                                                               name="order_" + str(node))
                                  for node in edges_in_all_vehicles}
                for vehicle in range(lp_solver.problem.number_vehicle):
                    node_origin = lp_solver.problem.origin_vehicle[vehicle]
                    constraints_order[node_origin] = lp_solver.model.addConstr(lhs=variable_order[node_origin],
                                                                               sense=grb.GRB.EQUAL,
                                                                               rhs=0,
                                                                               name="order_" + str(node_origin))
                        ...
                        for vehicle, edge in edges_in_all_vehicles[node]:
                            ...
                            if use_big_m:
                                constraints_order[node] = lp_solver.model.addConstr(
                                    ...
                                    + 1 - 2*nb_nodes * (1 - lp_solver.variable_decisions["variables_edges"][vehicle][edge]),
                                    name="order_" + str(node))  # Big M constraint, TODO
                            if use_indicator:
...
def update_model_lazy(problem: GPDP,
                      lp_solver: LinearFlowSolver,
                      components_global,
                      edges_in_all_vehicles,
                      edges_out_all_vehicles, do_order=False):
    len_component_global = len(components_global)
    list_constraints = []
    if len_component_global > 1:
        print("Nb component : ", len_component_global)
        for s in components_global[:1]:
            ...
            edge_out_of_interest = [e for n in s[0]
                                    ...
                                    and e[1][1] not in s[0]]
            if not any(problem.target_vehicle[v] in s[0] for v in problem.origin_vehicle):
                list_constraints += [lp_solver.model.cbLazy(grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                          for e in edge_out_of_interest]) >= 1)]
            if not any(problem.origin_vehicle[v] in s[0] for v in problem.origin_vehicle):
                list_constraints += [lp_solver.model.cbLazy(grb.quicksum([lp_solver.variable_decisions["variables_edges"][e[0]][e[1]]
                                                                          for e in edge_in_of_interest]) >= 1)]
                    ...
                    for vehicle, edge in edges_in_all_vehicles[node]:
                        ...
                        if use_big_m:
                            constraints_order[node] = lp_solver.model.cbLazy(
                                variable_order[node] >=
                                variable_order[edge[0]]
                                + 1 - 1000 * (1 - lp_solver.variable_decisions["variables_edges"][vehicle][edge]))  # Big M constraint, TODO
                        if use_indicator:
                            constraints_order[node] = lp_solver.model.addGenConstrIndicator(
                                lp_solver.variable_decisions["variables_edges"][vehicle][edge], 1,
                                variable_order[node] == variable_order[edge[0]] + 1,
...
def plot_solution(temporary_result: TemporaryResult, problem: GPDP):
    ...
    for node in problem.clusters_dict:
        ax[0].scatter([problem.coordinates_2d[node][0]],
    ...
    for v in temporary_result.rebuilt_dict:
        color = colors(v)
        ax[1].plot([problem.coordinates_2d[n][0] for n in temporary_result.rebuilt_dict[v]],
