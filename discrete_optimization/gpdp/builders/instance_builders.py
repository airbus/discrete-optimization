...
from discrete_optimization.gpdp.problem import Edge, GpdpProblem, Node, ProxyClass
...
def load_vrp_and_transform(index_in_files_available: int = 1) -> GpdpProblem:
    file_path = vrp_parser.get_data_available()[index_in_files_available]
    vrp_problem = vrp_parser.parse_file(file_path)
    gpdp = ProxyClass.from_vrp_to_gpdp(vrp_problem=vrp_problem)
    return gpdp


def load_tsp_and_transform(index_in_files_available: int = 1) -> GpdpProblem:
    ...
    gpdp = ProxyClass.from_tsp_to_gpdp(tsp_model=tsp_model)
    return gpdp


def create_ortools_example() -> GpdpProblem:
